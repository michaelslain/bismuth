...
// Spawn the bundled `bismuth-core` sidecar on the standard port (4321 — the frontend's
// default backend base). Stores the child so it's killed on exit. Best-effort: any
// failure (no vault chosen, sidecar missing, port already serving) leaves the app
// running against whatever is on :4321, or shows the frontend's "disconnected" state.
fn start_backend(app: &tauri::AppHandle) {
    let Some((vault, memory)) = resolve_vault_memory(app) else {
        eprintln!("bismuth: no vault selected — backend not started");
        return;
    };
    let sidecar = match app.shell().sidecar("bismuth-core") {
        Ok(c) => c,
        Err(e) => { eprintln!("bismuth: sidecar resolve failed: {e}"); return; }
    };
    ...
    match cmd.spawn() {
        Ok((mut rx, child)) => {
            ...
            eprintln!("bismuth: core sidecar started (vault={vault}, :4321)");
        }
        Err(e) => eprintln!("bismuth: failed to spawn core sidecar: {e}"),
    }
}
...
pub fn run() {
    let app = tauri::Builder::default()
        ...
        .setup(|app| {
            // Bundled builds spawn their own core server; in dev (`bun run dev`) the
            // concurrently-launched core already owns :4321, so don't double-spawn.
