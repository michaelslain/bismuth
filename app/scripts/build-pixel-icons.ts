...
//
// RETIRED — DO NOT RUN, AND DO NOT RE-WIRE IT TO A package.json SCRIPT.
//
// This generated `app/src/icons/pixelPaths.ts`, the pixel half of the FIRST of three icon systems.
// That output file is deleted and nothing imports it: `icons/registry.ts` resolves names through
// `icons/iconMap.ts` against Phosphor SVG art now (`bun run icons:svg` →
// app/scripts/build-icon-svgs.ts). Running this script would re-create a dead 30KB module that no
...
// That entry has been removed; the only live icon-generating script name now is `icons:svg`
// (Phosphor art, current) — `icons:font` (the Nerd Font subset) and the whole retired subsystem
// behind it (`icons/nerdGlyphs.ts`, `build-icon-font.ts`, `styles/icons.css`, the woff2) were
// deleted in ds-conformance Task 8, once nothing read the CSS custom property that named the font.
//
// This script is kept, rather than deleted, as the record of how the pixel set was produced. See
// docs/overview/third-party-notices.md.
//
// ── Original header ────────────────────────────────────────────────────────────────────────────
...
//
// (Historic instruction, kept for context: this was run via `bun run icons:build` after changing
// PIXEL_MAP below. That script entry no longer exists — see the RETIRED note above.)
//
...
// This generator is RETIRED and its `icons:build` script entry was removed — see the header of
// app/scripts/build-pixel-icons.ts. Nothing imports this file.
//
