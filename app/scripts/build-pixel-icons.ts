...
const SVG_DIR = join(HERE, "..", "node_modules", "@hackernoon", "pixel-icon-library", "icons", "SVG", "regular");
...
const polygonToPath = (points: string): string => {
  ...
  if (n.length < 6 || n.length % 2 !== 0) throw new Error(`bad polygon points: ${points}`);
  let [cx, cy] = [n[0], n[1]];
  let d = `M${cx} ${cy}`;
...
const rectToPath = (attrs: string): string => {
  const num = (k: string) => Number(new RegExp(`\\b${k}="([^"]+)"`).exec(attrs)?.[1] ?? 0);
...
function extractPath(file: string): string {
  ...
  if (!/viewBox="0 0 24 24"/.test(svg)) throw new Error(`${file}.svg: not on the 24x24 grid`);
  if (/transform=/.test(svg)) throw new Error(`${file}.svg: has a transform (flattening unsafe)`);
  ...
  for (const [el] of svg.matchAll(/<(?:path|polygon|rect)\b[^>]*>/g)) {
    ...
    if (el.startsWith("<path")) out.push(/\sd="([^"]+)"/.exec(el)![1].replace(/^\s*m/, "M"));
    else if (el.startsWith("<polygon")) out.push(polygonToPath(/\spoints="([^"]+)"/.exec(el)![1]));
    else out.push(rectToPath(el));
  }
  if (out.length === 0) throw new Error(`${file}.svg: no drawable shapes found`);
  return out.join("");
}

const missing = Object.values(PIXEL_MAP).filter((f) => !existsSync(join(SVG_DIR, `${f}.svg`)));
if (missing.length) {
