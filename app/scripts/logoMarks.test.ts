...
describe('logo marks', () => {
    ...
    it('every mark is a self-contained svg string', () => {
        for (const name of MARK_NAMES) {
            ...
            expect(svg.startsWith('<svg'), `${name} starts with <svg`).toBe(
                true,
            )
            expect(svg.trimEnd().endsWith('</svg>'), `${name} closes`).toBe(
                true,
            )
            expect(svg).toContain('viewBox="0 0 100 100"')
            const refs = [...svg.matchAll(/url\(#([a-z0-9-]+)\)/gi)].map(
                m => m[1],
            )
            for (const id of refs) {
                expect(svg, `${name} inlines gradient #${id}`).toContain(
                    `id="${id}"`,
                )
            }
        }
    ...
    it("matches the schema's appearance.icon enum", async () => {
        const { SETTINGS_SCHEMA } =
            await import('../../core/src/schema/settingsSchema')
        const appearance = (
            SETTINGS_SCHEMA.appearance.type as {
                fields: Record<string, { type: unknown; default?: unknown }>
            }
        ).fields
        const iconType = appearance.icon.type as {
            kind: string
            values: string[]
        }
