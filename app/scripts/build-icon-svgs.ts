...
const LIBRARY_FILE = 'icon-library.json'
...
// ── The full library ────────────────────────────────────────────────────────────────────────────
// Every icon the set's own metadata package lists, in the set's own PascalCase names (`Books`,
// `HouseLine`, `MapTrifold`), each with a lowercase search string: its kebab slug, any canonical
// names that map to it (so searching "bot" still finds Robot), and the set's own tags + categories
// (so "library" finds Books). Same refuse-to-write discipline as the manifest: a listed icon with
// no art in SOURCE_PACKAGE is a version skew between the two packages, not something to skip.
type LibraryMeta = {
    name: string
    pascal_name: string
    tags: string[]
...
const libMeta = (await import(LIBRARY_METADATA_PACKAGE)) as { icons: LibraryMeta[] }

const canonicalBySlug = new Map<string, string[]>()
for (const [name, entry] of Object.entries(ICON_MAP)) {
    if (entry.kind !== 'slug') continue
    const list = canonicalBySlug.get(entry.slug) ?? []
    list.push(name.toLowerCase())
    canonicalBySlug.set(entry.slug, list)
}

const library: [string, string, string][] = []
const missingArt: string[] = []
for (const meta of libMeta.icons) {
    const icon = PH.icons[meta.name]
    if (!icon) {
        missingArt.push(meta.name)
        continue
    }
    const terms = new Set<string>([
        meta.name,
        ...(canonicalBySlug.get(meta.name) ?? []),
        // `*new*` / `*updated*` are release markers, not meanings.
        ...meta.tags.filter(t => !t.startsWith('*')).map(t => t.toLowerCase()),
        ...meta.categories.map(c => c.toLowerCase()),
    ])
    library.push([meta.pascal_name, icon.body, [...terms].join(' ')])
}
if (missingArt.length) {
    console.error(
        `\nREFUSING TO WRITE ${LIBRARY_FILE}: ${missingArt.length} icon(s) listed by ` +
            `${LIBRARY_METADATA_PACKAGE} have no art in ${SOURCE_PACKAGE}: ${missingArt.join(', ')}`,
    )
    process.exit(1)
}
library.sort((a, b) => a[0].localeCompare(b[0]))

writeFileSync(
    join(OUT_DIR, LIBRARY_FILE),
    `${JSON.stringify({
        $comment:
            'GENERATED by app/scripts/build-icon-svgs.ts — do not edit. Every Phosphor Regular ' +
            'icon as [PascalName, svg body, search terms]; loaded lazily by icons/iconLibrary.ts.',
        source: {
            package: SOURCE_PACKAGE,
            version: (phMeta as { version: string }).version,
            metadata: LIBRARY_METADATA_PACKAGE,
            nativeGrid: `${SRC_WIDTH}x${SRC_HEIGHT}`,
        },
        icons: library,
    })}\n`,
)
console.log(`wrote src/assets/icons/${LIBRARY_FILE}: ${library.length} icon(s)`)
