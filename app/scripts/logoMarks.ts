...
function nodeB(): string {
    ...
    let nodes = ''
    N.forEach((n, i) => {
        nodes += circle(n[0], n[1], 4, IRID[i % IRID.length])
    })
...
function grid(a: number, b: number): { pts: number[][]; edges: number[][][] } {
    ...
    const pts: number[][] = []
    for (let r = 0; r < n; r++)
        for (let c = 0; c < n; c++) pts.push([a + c * step, a + r * step, r, c])
    const edges: number[][][] = []
    pts.forEach(([x, y, r, c]) => {
        if (c < n - 1)
            edges.push([
...
function latticeBody(a: number, b: number): string {
    ...
    let nodes = ''
    pts.forEach(([x, y, r, c]) => {
        nodes += circle(x, y, 3, IRID[(r + c) % IRID.length])
    })
    return g + nodes
}
function lattice(): string {
    return wrap(latticeBody(20, 80))
}
...
function radialGraph(): string {
    ...
    outer.forEach((p, k) => {
        edges.push([p, inner[Math.floor(k / 2)]])
    })
    ...
    let nodes = ''
    outer.forEach((p, i) => {
        nodes += circle(p[0], p[1], 2.6, IRID[i % IRID.length])
    })
    inner.forEach((p, i) => {
        nodes += circle(p[0], p[1], 3.2, IRID[i % IRID.length])
    })
