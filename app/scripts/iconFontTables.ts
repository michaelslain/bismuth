...
function readUIntBase128(b: Uint8Array, pos: number): [value: number, next: number] {
  let value = 0;
  for (let i = 0; i < 5; i++) {
    const byte = b[pos++];
    if (byte === undefined) throw new Error("UIntBase128 ran past end of buffer");
...
function readSfnt(b: Uint8Array): FontTables {
  ...
  for (let i = 0; i < numTables; i++) {
    const rec = 12 + i * 16;
    tables.set(tag4(b, rec), b.subarray(be32(b, rec + 8), be32(b, rec + 8) + be32(b, rec + 12)));
  }
...
function readWoff2(b: Uint8Array): FontTables {
  ...
  for (let i = 0; i < numTables; i++) {
    ...
    let tag: string;
    if (idx === 0x3f) { tag = tag4(b, p); p += 4; } else {
      const known = KNOWN_TAGS[idx];
      if (known === undefined) throw new Error(`woff2 table directory: unknown tag index ${idx}`);
      tag = known;
    }
    ...
    p = next;
    if (transformed) { const [tLen, n2] = readUIntBase128(b, p); length = tLen; p = n2; }
    dir.push({ tag, length, transformed });
  }

  const stream = brotliDecompressSync(b.subarray(p, p + totalCompressed));
  const data = new Uint8Array(stream.buffer, stream.byteOffset, stream.byteLength);
  ...
  for (const e of dir) {
    if (!e.transformed) tables.set(e.tag, data.subarray(at, at + e.length));
      ...
      throw new Error(`woff2: table '${e.tag}' is transformed; this reader would misread it`);
    }
    at += e.length;
  }
  if (at !== data.length) {
    throw new Error(`woff2: table data is ${data.length} bytes but the directory accounts for ${at}`);
  }
...
export function readFontTables(bytes: Uint8Array): FontTables {
  ...
  if (sig === "OTTO" || sig === "true" || sig === "ttcf" || be32(bytes, 0) === 0x00010000) {
...
export function readCmap(tables: Map<string, Uint8Array>): Map<number, number> {
  ...
  if (out.size === 0) throw new Error("cmap parsed to zero mappings (formats 4/12 not found?)");
  return out;
}
...
export function readAdvanceWidths(tables: Map<string, Uint8Array>): number[] {
  ...
  if (!hhea || !hmtx || !maxp) throw new Error("font is missing hhea, hmtx or maxp");
  ...
  const widths: number[] = [];
  for (let g = 0; g < numGlyphs; g++) widths.push(be16(hmtx, Math.min(g, numHMetrics - 1) * 4));
  return widths;
}
 ...
 */
export function checkGlyphs(bytes: Uint8Array, codepoints: number[]): {
  results: GlyphCheck[];
