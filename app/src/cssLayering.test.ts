...
export const MAX_APP_CSS_CLASS_RULES = 376;
...
describe("css layering — runtime classes stay out of CSS Modules", () => {
  it("finds the modules it is supposed to be guarding", () => {
    ...
    expect(modules.map((f) => f.slice(SRC.length + 1))).toContain("bases/BaseView.module.css");
  });

  it("no runtime-emitted class is DEFINED in a CSS Module", () => {
    const offenders: string[] = [];
    for (const f of modules) {
      for (const hit of runtimeClassesDefinedIn(readFileSync(f, "utf8"))) {
  ...
  it("modules use only the parenthesized `:global(...)` form", () => {
    ...
    const bare = modules
      .filter((f) => /:global(?!\()/.test(stripComments(readFileSync(f, "utf8"))))
  ...
  it("the scanner catches an offender and forgives the sanctioned form", () => {
    ...
    expect(runtimeClassesDefinedIn(".card :global(.bismuth-task-box) { color: red; }")).toEqual([]);
    expect(runtimeClassesDefinedIn(".card :global(li:has(> .cm-tag)) { color: red; }")).toEqual([]);
    expect(runtimeClassesDefinedIn("/* styles .bismuth-tag like the editor */\n.card { color: red; }")).toEqual([]);
    expect(runtimeClassesDefinedIn(".bismuth-tag { color: red; }")).toEqual([".bismuth-tag"]);
    expect(runtimeClassesDefinedIn(".card :global(.cm-x) { }\n.callout-note { }")).toEqual([".callout-note"]);
  });
});
...
describe("css layering — App.css is the global manifest", () => {
  it("imports the three global stylesheets before anything else", () => {
    ...
    for (const f of ["tokens", "reset", "content"]) {
      expect(appCss, `imports styles/${f}.css`).toContain(`@import "./styles/${f}.css";`);
    }
    expect(lastImport).toBeGreaterThan(-1);
    expect(firstRule, "a rule precedes the @import block").toBeGreaterThan(lastImport);
  });
