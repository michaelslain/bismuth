...
describe('classifyPickResult', () => {
    ...
    it('reports a user cancel when the dialog resolves without a path', () => {
        expect(classifyPickResult({ value: null })).toEqual({
            status: 'cancelled',
        })
    })

    it('reports a cancel when the dialog resolves to a non-string', () => {
        expect(classifyPickResult({ value: undefined })).toEqual({
            status: 'cancelled',
        })
    })

    it('reports an ERROR — distinct from a cancel — when the dialog throws', () => {
        const r = classifyPickResult({
            thrown: new Error('dialog.open not allowed'),
        })
        expect(r.status).toBe('error')
        expect(r.status === 'error' && r.message).toContain(
    ...
    it('reports a cancel when there is no Tauri dialog to open', () => {
        expect(classifyPickResult({ unavailable: true })).toEqual({
            status: 'cancelled',
        })
    })
})
