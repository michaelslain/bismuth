...
let milkdownModule: Promise<{
    createBlockEditor: typeof CreateBlockEditorFn
}> | null = null
function loadMilkdown(): Promise<{
    createBlockEditor: typeof CreateBlockEditorFn
}> {
...
export function BlockEditor(props: {
    ...
    const replaceBlocks = (next: Block[]): void =>
        setBlocks(reconcile(next, { key: 'id' }))
    ...
    const docText = (): string =>
        serializeBlocksToMarkdown(frontmatter(), blocks)
    ...
    const scheduleSave = (): void => {
        ...
        saveTimer = setTimeout(async () => {
            ...
            if (serializeBlocksToMarkdown(frontmatter(), blocks) === text)
                pendingSave = false
    ...
    createEffect(async () => {
        ...
        if (restore != null && restore > 0) {
            ...
            const repin = (): void => {
                ...
                if (Math.abs(host.scrollTop - restore) > 1)
                    host.scrollTop = restore
    ...
    const splitBlock = (id: string, caret: number): void => {
        ...
        if (
            ...
            created = regenerateRaw({
                ...created,
                indent: cur.indent ?? '',
                marker: cur.marker ?? created.marker,
                ordered: cur.ordered,
                text: after,
            })
        } else {
    ...
    const [builder, setBuilder] = createSignal<{
        blockId: string
        ...
        () => {
            ...
            if (a.kind === 'wikilink') {
                ...
                const ranked = notes
                    .map(n => ({
                        n,
                        score: matchScore(n.label.toLowerCase(), q),
                    }))
                    .filter(r => r.score >= 0)
                    .sort(
                        (r1, r2) =>
                            r1.score - r2.score ||
                            r1.n.label.localeCompare(r2.n.label),
                    )
                    .slice(0, 50)
                return ranked.map(r => ({
                    label: r.n.label,
            ...
            const ranked = tags
                ...
                .filter(r => r.score >= 0)
                .sort(
                    (r1, r2) => r1.score - r2.score || r1.t.localeCompare(r2.t),
                )
                .slice(0, 50)
    ...
    const autoRows = createMemo<PopoverRow[]>(() =>
        autoCandidates().map(c => ({
            label: c.label,
            icon:
                auto()?.kind === 'tag' || auto()?.kind === 'heading'
                    ? 'Hash'
                    : 'FileText',
            detail: c.detail,
        })),
    ...
    const onTextBlur = (id: string): void => {
        ...
        if (reconciled.length === 1 && reconciled[0].type === blocks[i].type)
            return // unchanged
        const next = blocks.slice()
    ...
    const focusSibling = (id: string, dir: 1 | -1): void => {
        ...
        if (i === -1) return
        let j = i + dir
        while (j >= 0 && j < blocks.length && !isTextNavTarget(blocks[j]))
            j += dir
    ...
    onMount(() => {
        const onChipClick = (e: MouseEvent): void => {
            ...
            const chipEl = (e.target as HTMLElement | null)?.closest?.(
                'span.bismuth-wikilink[data-href]',
            ) as HTMLElement | null
            if (!chipEl) return
            ...
            window.dispatchEvent(
                new CustomEvent('bismuth-open', {
                    detail: {
                        path: wikilinkOpenPath(target, resolved),
                        heading,
                    },
                }),
            )
    ...
    createEffect(() => {
        if (props.path) clearPendingAnchor(props.path)
    })
    ...
    createEffect(() => {
        if (props.path) clearPendingCursor(props.path)
    })
    onMount(() => {
    ...
    return (
        <div class="block-editor" ref={host}>
            ...
            <Show when={slash()}>
                {s => (
                    <div
                        ...
                        onMouseDown={
                            e => e.preventDefault() /* keep textarea focus */
                        }
                    >
                        <Show
            ...
            <Show when={auto()}>
                {a => (
                    <div
                        ...
                        onMouseDown={
                            e =>
                                e.preventDefault() /* keep the surface focused */
                        }
                    >
                        <Show
            ...
            <Show when={format()}>{f => <FormatBar state={f()} />}</Show>
        ...
        return (
            <div class={wrapClass()}>
                ...
                <Show
                    ...
                    fallback={<CodeBlock block={p.block} />}
                >
    ...
    function CalloutBlockView(p: { block: Block }) {
        const meta = createMemo(() =>
            calloutMeta(p.block.calloutType ?? 'note'),
        )
    ...
    function RichTextBlock(p: { block: Block }) {
                ...
                return createBlockEditor({
                    ...
                    onSlash: (query, rect) => {
                        ...
                        if (
                            ...
                            p.block.type !== 'bulletItem' &&
                            p.block.type !== 'orderedItem'
                        )
                            return
                        const open = slash()
                        setSlash({
        ...
        createEffect(() => {
            ...
            else if (handle() && !root.contains(document.activeElement))
                unmount()
        })
        ...
        return (
            <div
                ref={el => {
                    ...
                    // eslint-disable-next-line solid/no-innerhtml -- sanitized by renderNoteBody (DOMPurify)
                    if (!handle())
                        el.innerHTML = p.block.text
                            ? renderNoteBody(p.block.text)
                            : ''
                }}
                class={cls()}
                    ...
                    <div class="block-query-edit">
                        <IconButton
                            ...
                            onClick={() =>
                                setBuilder({
                                    blockId: p.block.id,
                                    initial: parseQueryBlockBody(body()),
                                })
                            }
                        />
                    </div>
...
function propValues(value: unknown): string[] {
    if (Array.isArray(value))
        return value.map(v => scalarText(v)).filter(s => s.length > 0)
