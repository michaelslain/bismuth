...
export function canDismissBoot(s: BootGateSignals): boolean {
  ...
  if (s.graphMounts && !s.graphPainted && !s.hidden && !s.paintWaitExpired) return false;
  return true;
}
...
export function createBootGate(opts: { graphMounts: boolean; onDismiss: () => void }): BootGate {
  ...
  return {
    ...
    setGraphPainted(v) { signals.graphPainted = v && signals.dataReady; maybeDismiss(); },
    setHidden(v) { signals.hidden = v; maybeDismiss(); },
    setPaintWaitExpired(v) { signals.paintWaitExpired = v; maybeDismiss(); },
