...
export function ChatView(props: {
    ...
    const onComposerInput = (value: string) => {
        if (pendingHistoryText !== null && value === pendingHistoryText)
    ...
    const [gateRefusal, setGateRefusal] = createSignal<{
        binary: string
        message: string
    } | null>(null)
    ...
    const [providerChoice, setProviderChoice] =
        createSignal<ChatProviderChoice | null>(
            readProviderChoice(props.chatId),
        )
    ...
    // chats. NOTE this is only the display warm-up + the default applied to FRESH sessions — a RESUMED
    // conversation's model comes from the session itself (the server's per-session store + the CLI's
    // own restore, Bug #89) and is adopted into these keys, never overridden by them.
    const [lastModel, setLastModel] = createSignal(
        readLastModel(provider(), props.chatId),
    )
    ...
    const rememberModel = (model: string, opts?: { global?: boolean }) => {
        ...
        try {
            ...
            if (opts?.global !== false) localStorage.setItem(keys.global, model) // global fallback for brand-new chats
        } catch {
    ...
    const [queuedTurns, setQueuedTurns] = createSignal<
        { id: string; wire: string; text: string; images: Attachment[] }[]
    >([])
    ...
    const addImageFiles = async (files: File[]) => {
        for (const f of files) {
            if (!f.type.startsWith('image/')) {
                ...
                setTurnError(
                    `"${f.name || 'attachment'}" isn't an image — it can't be attached.`,
                )
                continue
            }
            if (f.size > MAX_IMAGE_BYTES) {
                setTurnError(
                    `Image "${f.name || 'attachment'}" is too large (max 10 MB).`,
                )
                continue
            }
            ...
            if (att) setAttachments(a => [...a, att])
                ...
                setTurnError(
                    `Unsupported image type ${f.type || '(unknown)'} — use PNG, JPEG, GIF, or WebP.`,
                )
        }
    }
    ...
    const toggleComputerUse = () => {
        ...
        const { next, note } = computeChromeToggle(
        ...
        if (cmd.kind === 'rename') {
            ...
            window.dispatchEvent(
                new CustomEvent('bismuth-chat-rename', {
                    detail: { chatId: props.chatId, name: cmd.name },
                }),
            )
            setTurnError(null)
            return true
    ...
    const send = () => {
        ...
        if (
            atts.reduce((n, a) => n + a.data.length, 0) > MAX_TOTAL_IMAGE_BYTES
        ) {
            setTurnError(
        ...
        const preamble = text.startsWith('/')
            ? ''
            : buildEditorContext(hiddenPaths(), getChatReferences(props.chatId))
    ...
    const slashMatches = createMemo<string[]>(() => {
        ...
        const cmds = withClientSlashCommands(
            manifest()?.slashCommands ?? [],
        ).filter(c => providerCan(provider(), 'computerUse') || c !== 'chrome')
    ...
    const slashRows = createMemo<PopoverRow[]>(() =>
        ...
        slashMatches().map(c => ({
            label: `/${c}`,
            icon: 'ChevronRight',
            detail: manifest()?.commandDetails?.[c] ?? SLASH_COMMAND_DETAILS[c],
        })),
    )
        ...
        const items: MenuItem[] = [
            {
                label: 'Reply',
                icon: 'Reply',
                onSelect: () => replyToMessage(text),
            },
            {
                label: 'Copy',
                icon: 'Copy',
                onSelect: () => copyMessage(selected || text),
            },
        ]
    ...
    onMount(() => {
        const onMention = (e: Event) => {
            ...
            // Chat mentions write the bare basename: no note-id list is threaded into ChatView, so
            // `wikilinkFor` never sees a sibling with the same name to disambiguate against. Only
            // the editor drop-to-link path in App.tsx path-qualifies duplicate names.
    ...
    return (
        <div
            ...
            onDrop={onHostDrop}
        >
            {/* The bar itself is chat/ChatHeader.tsx — props-only and story-covered. The two
            ...
            <ChatHeader
                ...
                displayModelValue={displayModelValue()}
                onSwitchModel={switchModel}
                effortOptions={effortOptions()}
                effortValue={effortDisplay()}
                ...
                computerUse={chatComputerUse(props.chatId)}
                onToggleComputerUse={toggleComputerUse}
                authProviders={authProviders()}
                authOpen={authOpen()}
                onToggleAuth={() => setAuthOpen(!authOpen())}
                authPanel={<AuthPanel />}
    ...
    function HistoryPanel() {
        ...
        const scopeOptions: SegmentedOption<ChatScope>[] = [
            ...
            },
            {
                id: 'all',
                label: 'All',
                title: "Both, with the daemon's marked",
            },
        ]
    ...
    function QuestionCard(p: { part: QuestionPart }) {
        ...
        const isPicked = (qi: number, label: string) =>
            sel.picks[qi].includes(label)
            ...
            >
                ...
                >
                    <div class={turnStyles['chat-question-actions']}>
                        ...
                        <TextButton
                            size="sm"
                            onClick={() => answerQuestion(p.part.id, null)}
                        >
                            SKIP
                        </TextButton>
                    </div>
                ...
                >
                    <div class={turnStyles['chat-permission-actions']}>
                        <TextButton
                            ...
                            size="sm"
                            onClick={() =>
                                answerPermission(p.part.id, 'allow', false)
                            }
                        >
                            ALLOW
                        </TextButton>
                        <TextButton
                            size="sm"
                            onClick={() =>
                                answerPermission(p.part.id, 'allow', true)
                            }
                        >
                            ALLOW ALWAYS
                        </TextButton>
                        <TextButton
                            ...
                            size="sm"
                            onClick={() =>
                                answerPermission(p.part.id, 'deny', false)
                            }
                        >
                            DENY
                        </TextButton>
