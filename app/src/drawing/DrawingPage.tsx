...
export function ImageMarkupPage(props: { path: string }) {
    const [sidecar] = createResource(
        ...
        async (src): Promise<string> => {
            ...
            try {
                const r = await fetch(
                    `${apiBase()}/file?path=${encodeURIComponent(sc)}`,
                )
                ...
                if (text.trim()) {
                    try {
                        ...
                        /* corrupt but present → leave it, don't clobber */
                    }
            ...
            if (missing) {
                ...
                try {
                    await api.saveDrawing(sc, await seedMarkupDoc(src))
                } catch (e) {
                    pushToast(
                        `Couldn't open for markup: ${(e as Error).message}`,
                    )
                }
            }
            return sc
    ...
    const loadingLabel = () =>
        PDF_NAME_RE.test(props.path) ? 'Rasterizing PDF…' : 'Loading image…'
    return (
        <Show
            when={sidecar()}
            keyed
            fallback={<Loading>{loadingLabel()}</Loading>}
        >
...
function seedMarkupDoc(srcPath: string): Promise<DrawingDoc> {
    return PDF_NAME_RE.test(srcPath)
        ? seedPdfDoc(srcPath)
        : seedImageDoc(srcPath)
}
...
async function seedImageDoc(imagePath: string): Promise<DrawingDoc> {
    ...
    doc.paper.bg = 'blank'
    try {
        const src = await blobToDataUrl(
            await (await fetch(api.assetUrl(imagePath))).blob(),
        )
    ...
    try {
        ...
        if (rasters.length) {
            doc.pages = await Promise.all(
                rasters.map(async (src): Promise<Page> => ({
                    strokes: [],
                    images: [await imageElFromSrc(src)],
                })),
            )
        }
    } catch (e) {
