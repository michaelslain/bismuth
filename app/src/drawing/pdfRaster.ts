...
const DEFAULTS: Required<RasterizeOpts> = {
    targetWidth: 1600,
    maxDim: 4000,
    quality: 0.85,
    maxPages: 100,
...
export async function rasterizePdf(
    ...
    if (pdf.numPages > maxPages) {
        pushToast(
            `PDF has ${pdf.numPages} pages — only the first ${maxPages} are opened for markup.`,
        )
    }
    try {
        for (let n = 1; n <= lastPage; n++) {
            try {
                urls.push(
                    await renderPage(pdf, n, targetWidth, maxDim, quality),
                )
            } catch (e) {
                pushToast(
                    `Couldn't render PDF page ${n}: ${(e as Error).message}`,
                )
            }
        }
    } finally {
