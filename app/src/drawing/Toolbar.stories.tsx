    ...
    return (
      <div class="draw-app" style={{ height: "220px" }}>
        <Toolbar
          ...
          onRedo={() => {}}
          zoom={zoom}
          onZoomIn={() => setZoom((z) => Math.min(4, Math.round((z + 0.05) * 100) / 100))}
          onZoomOut={() => setZoom((z) => Math.max(0.25, Math.round((z - 0.05) * 100) / 100))}
  ...
  render: () => {
    ...
    return (
      <div class="draw-app" style={{ height: "160px" }}>
        <Toolbar tools={tools} setTools={setTools} onUndo={() => {}} onRedo={() => {}} />
      </div>
    );
  },
};
