...
function Slider(props: { value: number; min: number; max: number; step: number; onChange: (v: number) => void }) {
  return (
    <>
      ...
      />
      <span style={{ width: "52px", "text-align": "right", "font-variant-numeric": "tabular-nums", opacity: 0.7 }}>{props.value}</span>
    </>
  );
}
  ...
  return (
    <div style={{ "max-width": "620px", margin: "0 auto", padding: "28px 28px 60px" }}>
      ...
      <Section title="Appearance">
        ...
        <Row label="Theme">
          <select style={selectStyle} value={settings.appearance.theme}
        ...
        <Row label="Editor font">
          <select style={selectStyle} value={settings.appearance.editorFont}
        ...
        <Row label="Editor font size">
          <Slider value={settings.appearance.editorFontSize} min={11} max={28} step={1}
      ...
      <Section title="Graph">
        ...
        <Row label="Spin speed">
          <Slider value={settings.graph.spinSpeed} min={0} max={0.01} step={0.0005}
            onChange={(v) => set("graph", "spinSpeed", v)} />
        </Row>
        <Row label="Node palette">
          <select style={selectStyle} value={settings.graph.palette}
        ...
        <Row label="Repulsion" hint="More negative = nodes push apart harder">
          <Slider value={settings.graph.repulsion} min={-40} max={-1} step={1}
            onChange={(v) => set("graph", "repulsion", v)} />
        </Row>
        <Row label="Link distance">
          <Slider value={settings.graph.linkDistance} min={1} max={40} step={1}
            onChange={(v) => set("graph", "linkDistance", v)} />
        </Row>
        <Row label="Centering" hint="Pull toward the center — higher = denser ball">
          <Slider value={settings.graph.centering} min={0} max={0.5} step={0.01}
      ...
      <Section title="Editor">
        ...
        <Row label="Auto-save delay" hint="Milliseconds of idle before saving">
          <Slider value={settings.editor.autoSaveDelay} min={200} max={3000} step={100}
      ...
      <Section title="Vault & backup">
        <Row label="Vault path" hint="Set at launch — change requires restarting core">
          <code style={{ "font-size": "11px", opacity: 0.7, "max-width": "260px", overflow: "hidden", "text-overflow": "ellipsis", "white-space": "nowrap" }}>{vault()}</code>
        </Row>
        ...
        <Row label="Manual backup">
          ...
          <button style={selectStyle} onClick={backupNow}>Backup now</button>
        </Row>
      </Section>
    </div>
  );
