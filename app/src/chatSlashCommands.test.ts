...
describe('parseChatSlashCommand', () => {
    it('parses `/rename <name>`', () => {
        expect(parseChatSlashCommand('/rename Planning')).toEqual({
            kind: 'rename',
            name: 'Planning',
        })
    })
    it('keeps a multi-word rename name', () => {
        expect(parseChatSlashCommand('/rename Q3 Planning Sync')).toEqual({
            kind: 'rename',
            name: 'Q3 Planning Sync',
        })
    })
    it('`/rename` with no arg → empty name (caller reverts to auto label)', () => {
        expect(parseChatSlashCommand('/rename')).toEqual({
            kind: 'rename',
            name: '',
        })
    })
    it('parses `/color <token>` keeping the raw arg (caller resolves it)', () => {
        expect(parseChatSlashCommand('/color blue')).toEqual({
            kind: 'color',
            arg: 'blue',
        })
        expect(parseChatSlashCommand('/color #ffcc00')).toEqual({
            kind: 'color',
            arg: '#ffcc00',
        })
    })
    it('`/colour` (British spelling) also parses as color', () => {
        expect(parseChatSlashCommand('/colour green')).toEqual({
            kind: 'color',
            arg: 'green',
        })
    })
    it('is case-insensitive on the command word', () => {
        expect(parseChatSlashCommand('/RENAME Foo')).toEqual({
            kind: 'rename',
            name: 'Foo',
        })
    })
    ...
    it('parses a bare `/chrome` with an empty arg', () => {
        expect(parseChatSlashCommand('/chrome')).toEqual({
            kind: 'chrome',
            arg: '',
        })
    })
    ...
    it('captures the `/chrome` argument so on/off can be asked for explicitly', () => {
        expect(parseChatSlashCommand('/chrome on')).toEqual({
            kind: 'chrome',
            arg: 'on',
        })
        expect(parseChatSlashCommand('/chrome off')).toEqual({
            kind: 'chrome',
            arg: 'off',
        })
        expect(parseChatSlashCommand('  /chrome   OFF  ')).toEqual({
        ...
        expect(parseChatSlashCommand('/chrome foo')).toEqual({
            kind: 'chrome',
            arg: 'foo',
        })
    })
...
describe('CLIENT_SLASH_COMMANDS / withClientSlashCommands (BUG #87 autocomplete fix)', () => {
    ...
    it('appends the client commands to an empty (pre-manifest) list, so /chrome shows before any session exists', () => {
        expect(withClientSlashCommands([])).toEqual([
            'rename',
            'color',
            'chrome',
        ])
    ...
    it("appends after the backend's own commands, preserving their order", () => {
        expect(withClientSlashCommands(['compact', 'mcp'])).toEqual([
            'compact',
            'mcp',
            'rename',
            'color',
    ...
    it("dedupes: a backend command sharing a client command's name isn't duplicated", () => {
        expect(withClientSlashCommands(['chrome'])).toEqual([
            'chrome',
            'rename',
            'color',
        ])
...
describe('chromeToggleNote (BUG #87 visible feedback)', () => {
    it("enabled → states enabled + 'for this chat' + 'next message'", () => {
        expect(chromeToggleNote(true)).toBe(
            'Browser (--chrome) enabled for this chat — takes effect from your next message.',
        )
    })
    it("disabled → states disabled + 'for this chat' + 'next message'", () => {
        expect(chromeToggleNote(false)).toBe(
            'Browser (--chrome) disabled for this chat — takes effect from your next message.',
        )
    })
...
describe('computeChromeCommand (BUG #87 bounce 3: /chrome is an ENABLE verb, not a toggle)', () => {
    ...
    it("INVARIANT: only an explicit off/disable can report 'disabled'", () => {
        for (const current of [true, false]) {
            expect(computeChromeCommand(current, 'off')!.note).toContain(
                'disabled',
            )
            expect(computeChromeCommand(current, 'disable')!.note).toContain(
                'disabled',
            )
        }
    })
