...
export function createViewDrag(
    ...
    function resolveTarget(x: number, y: number): DropTarget | null {
        ...
        if (strip) {
            ...
            const chips = [...strip.querySelectorAll('[data-tab-chip]')].map(
                c => {
    ...
    function onUp(): void {
        const s = state()
        if (s.active) {
            if (s.descriptor && s.target)
                onDrop(s.descriptor, s.target, { x: s.x, y: s.y })
            swallowNextClick()
        } else {
    ...
    return {
        ...
        startTab(e, tabId, label, onTap, path) {
            ...
            arm(
                e,
                { kind: 'tab', tabId, label, width: rect.width, path },
                rect,
                onTap,
            )
        },
        startPane(e, tabId, leafId, label, path) {
            ...
            arm(
                e,
                { kind: 'pane', tabId, leafId, label, width: rect.width, path },
                rect,
            )
        },
