 ...
 */
export async function createBlockEditor(
    opts: CreateBlockEditorOptions,
): Promise<BlockEditorHandle> {
    ...
    const markdownCaretOffset = (v: EditorView): number => {
        ...
        const before = editor.action(ctx =>
            ctx.get(serializerCtx)(v.state.doc.cut(0, head)),
        )
    ...
    const structuralKeymap = keymap({
        ...
        Escape: blockCmd(
            () => autocompleteConsumed('Escape') || slashConsumed('Escape'),
        ),
    ...
    return {
        ...
        applyAutocomplete: (from, text, caretAfter) => {
            ...
            editor.action(ctx => {
                ...
                const inline = doc.firstChild
                    ? doc.firstChild.content
                    : doc.content
                const tr = v.state.tr.replaceWith(fromPos, toPos, inline)
                const caretPos =
                    fromPos +
                    (typeof caretAfter === 'number' ? caretAfter : inline.size)
                try {
                    tr.setSelection(
                        TextSelection.near(
                            tr.doc.resolve(
                                Math.min(caretPos, tr.doc.content.size),
                            ),
                        ),
                    )
                } catch {
