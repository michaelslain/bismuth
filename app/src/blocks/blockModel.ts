...
export function renderBlockToMarkdown(block: Block): string {
    switch (block.type) {
        ...
        case 'callout': {
            ...
            const head =
                block.calloutHeaderRaw ?? `> [!${block.calloutType ?? 'note'}]`
...
function splitTrailingBlanks(raw: string): {
    content: string
    trailing: string
} {
...
export const SLASH_ITEM_BLOCK_TYPES: Record<string, BlockType> =
    Object.fromEntries(
