...
function maybeGc(): void {
    ...
    if (
        typeof (Bun as unknown as { gc?: (sync: boolean) => void }).gc ===
        'function'
    ) {
...
test('per-block Milkdown init scaling (50/100/150/200)', async () => {
    ...
    lines.push('')
    lines.push(
        '=== Per-block Milkdown init benchmark (happy-dom, JS-construction lower bound) ===',
    )
    lines.push('')
    lines.push(
        ['  N', '  total(ms)', '  per(ms)', '  rssΔ(MB)', ' rss/blk(MB)'].join(
            ' | ',
        ),
    )
