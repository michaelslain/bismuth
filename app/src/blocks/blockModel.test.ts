...
describe('edit round-trip', () => {
    test("change a paragraph's text", () => {
        ...
        const { reparsed } = assertSurgicalEdit(md, pIdx, b =>
            setBlockText(b, 'new paragraph text.'),
        )
    ...
    test('toggle a task', () => {
        ...
        const { reparsed } = assertSurgicalEdit(md, tIdx, b =>
            toggleTaskChecked(b),
        )
    ...
    test('toggle a nested task preserves indent', () => {
        ...
        const childIdx = blocks.findIndex(
            b => b.type === 'task' && (b.indent ?? '').length > 0,
        )
        const { reparsed } = assertSurgicalEdit(md, childIdx, b =>
            toggleTaskChecked(b),
        )
    ...
    test('change heading level', () => {
        const md = '# top\n\n## section\n\nbody\n'
        const { blocks } = parseMarkdownToBlocks(md)
        const hIdx = blocks.findIndex(
            b => b.type === 'heading' && b.level === 2,
        )
        const { reparsed } = assertSurgicalEdit(md, hIdx, b =>
            setHeadingLevel(b, 4),
        )
    ...
    test('edit a code block body', () => {
        ...
        const { reparsed } = assertSurgicalEdit(md, cIdx, b =>
            setBlockText(b, 'next();\nmore();'),
        )
    ...
    test('edit a list item text', () => {
        const md = '- alpha\n- beta\n- gamma\n'
        const { blocks } = parseMarkdownToBlocks(md)
        const { reparsed } = assertSurgicalEdit(md, 1, b =>
            setBlockText(b, 'BETA'),
        )
...
describe('callouts', () => {
    ...
    test('fold markers are captured', () => {
        expect(
            parseMarkdownToBlocks('> [!warning]+ e\n> b\n').blocks[0],
        ).toMatchObject({ foldable: true, collapsed: false })
        expect(
            parseMarkdownToBlocks('> [!warning]- c\n> b\n').blocks[0],
        ).toMatchObject({ foldable: true, collapsed: true })
        expect(
            parseMarkdownToBlocks('> [!warning] n\n> b\n').blocks[0],
        ).toMatchObject({ foldable: false })
    })

    test('a plain blockquote is still a `quote` block', () => {
        expect(
            parseMarkdownToBlocks('> just a quote\n> line two\n').blocks[0]
                .type,
        ).toBe('quote')
    })
    ...
    test('editing a callout body re-emits a callout of the same type (surgical edit)', () => {
        ...
        const { blocks } = parseMarkdownToBlocks(md)
        const { reparsed } = assertSurgicalEdit(md, 0, b =>
            setBlockText(b, 'new body'),
        )
        ...
        expect(
            reparsed.blocks[0].raw.startsWith('> [!warning]+ Heads up\n'),
        ).toBe(true)
    })
...
describe('edit reconciliation', () => {
    ...
    test('code block containing a ``` line stays ONE code block (fence lengthened)', () => {
        ...
        const reparsed = parseMarkdownToBlocks(
            r.map(b => b.raw).join(''),
        ).blocks
