...
function makeAtom(def: AtomDef): MilkdownPlugin[] {
    ...
    const nodePlugin = $node(id, () => ({
        ...
        parseDOM: [
            {
                ...
                getAttrs: (dom: unknown) => {
                    ...
                    return {
                        raw:
                            el.getAttribute('data-bismuth-raw') ??
                            el.textContent ??
                            '',
                        attrs: safeParseAttrs(
                            el.getAttribute('data-bismuth-attrs'),
                        ),
                    }
                },
            },
        ...
        toDOM: node => {
            ...
            const span = def.dom(
                m ?? ([raw] as unknown as RegExpExecArray),
                extra,
            )
        ...
        if (spec && spec.kind === 'image' && spec.src) {
            ...
            img.addEventListener('error', () => {
                span.replaceChildren(
                    document.createTextNode('▦ ' + (spec.alt || inner)),
                )
...
function scheduleMathUpgrade(): void {
    ...
    onMathReady(() => {
        ...
        for (const el of document.querySelectorAll<HTMLElement>(
            '.bismuth-block-milkdown span.bismuth-math[data-math]',
        )) {
