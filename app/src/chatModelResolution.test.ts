...
describe('splitModelSuffix', () => {
    test('splits the [1m] context-variant suffix off', () => {
        expect(splitModelSuffix('opus[1m]')).toEqual({
            base: 'opus',
            oneM: true,
        })
        expect(splitModelSuffix('claude-opus-4-8[1m]')).toEqual({
            base: 'claude-opus-4-8',
            oneM: true,
        })
    })
    test('plain values have no suffix', () => {
        expect(splitModelSuffix('haiku')).toEqual({
            base: 'haiku',
            oneM: false,
        })
...
describe('modelsCorrespond (alias ↔ resolved-id tolerance)', () => {
    test('exact values correspond', () => {
        ...
        expect(
            modelsCorrespond('claude-opus-4-8[1m]', 'claude-opus-4-8[1m]'),
        ).toBe(true)
    })
    test('an alias corresponds to its resolved id (the live-CLI shape)', () => {
        ...
        expect(modelsCorrespond('haiku', 'claude-haiku-4-5-20251001')).toBe(
            true,
        )
        expect(modelsCorrespond('claude-haiku-4-5-20251001', 'haiku')).toBe(
            true,
        )
        expect(modelsCorrespond('sonnet', 'claude-sonnet-4-5-20250929')).toBe(
            true,
        )
    ...
    test('different families / versions never correspond', () => {
        expect(modelsCorrespond('haiku', 'claude-sonnet-4-5-20250929')).toBe(
            false,
        )
        expect(modelsCorrespond('claude-opus-4-5', 'claude-opus-4-8')).toBe(
            false,
        )
    ...
    test("'default' corresponds to anything (it aliases the CLI's own resolution)", () => {
        expect(modelsCorrespond('default', 'claude-sonnet-4-5-20250929')).toBe(
            true,
        )
...
describe('modelOptionFor (resolved id → picker option)', () => {
    ...
    test('a resolved id maps to its alias option', () => {
        expect(modelOptionFor('claude-haiku-4-5-20251001', OPTIONS)).toBe(
            'haiku',
        )
        expect(modelOptionFor('claude-opus-4-8[1m]', OPTIONS)).toBe('opus[1m]')
        expect(modelOptionFor('claude-sonnet-4-5-20250929', OPTIONS)).toBe(
            'sonnet',
        )
    })
...
describe('modelLabelFor', () => {
    ...
    test('falls back to the raw value when nothing matches', () => {
        expect(modelLabelFor('claude-nova-1-0', OPTIONS)).toBe(
            'claude-nova-1-0',
        )
...
describe("resolveInitialModel — FRESH sessions (spawn default vs the user's persisted choice)", () => {
    test("adopts the session's own model when there's no persisted choice yet", () => {
        expect(resolveInitialModel('', 'claude-sonnet-4-5', false)).toEqual({
            adopt: 'claude-sonnet-4-5',
        })
    })
    ...
    test('no-op when the persisted alias corresponds to the reported resolved id (the clobber case)', () => {
        ...
        expect(
            resolveInitialModel('opus[1m]', 'claude-opus-4-8[1m]', false),
        ).toBeNull()
        expect(
            resolveInitialModel('claude-opus-4-5', 'claude-opus-4-5', false),
        ).toBeNull()
    })
    test('enforces the persisted choice when the session spawned with a different model (the bug)', () => {
        expect(
            resolveInitialModel('claude-opus-4-5', 'claude-sonnet-4-5', false),
        ).toEqual({ enforce: 'claude-opus-4-5' })
        expect(
            resolveInitialModel(
                'opus[1m]',
    ...
    test('enforces even when the persisted value came from the GLOBAL fallback key, not a per-chat one', () => {
        ...
        expect(
            resolveInitialModel('claude-haiku-4-5', 'claude-sonnet-4-5', false),
        ).toEqual({ enforce: 'claude-haiku-4-5' })
    })
})
...
describe('resolveInitialModel — RESUMED sessions (the session owns its model)', () => {
    test("adopts the session's own saved model, never enforcing the tab/global fallback over it", () => {
        ...
        expect(resolveInitialModel('haiku', 'opus[1m]', true)).toEqual({
            adopt: 'opus[1m]',
        })
    })
    test("adopts even when there's no persisted value at all", () => {
        expect(resolveInitialModel('', 'opus[1m]', true)).toEqual({
            adopt: 'opus[1m]',
        })
    })
...
describe('reconcileManifestModel — LATER manifests (manifest late-arrival / mid-session change)', () => {
    ...
    test('the resolved form of the current choice is NOT drift (the first-turn clobber case)', () => {
        ...
        expect(
            reconcileManifestModel('opus[1m]', 'claude-opus-4-8[1m]', OPTIONS),
        ).toBeNull()
        expect(
            reconcileManifestModel(
                'haiku',
    ...
    test('a genuine mid-session model change is adopted, mapped into picker-space', () => {
        ...
        ).toEqual({ adopt: 'sonnet' })
        expect(
            reconcileManifestModel('sonnet', 'claude-opus-4-8[1m]', OPTIONS),
        ).toEqual({ adopt: 'opus[1m]' })
    })
    test("adopts the raw id when the models frame hasn't landed / the model is unknown", () => {
        expect(
            reconcileManifestModel('haiku', 'claude-sonnet-4-5-20250929', []),
        ).toEqual({ adopt: 'claude-sonnet-4-5-20250929' })
        expect(
            reconcileManifestModel('haiku', 'claude-nova-1-0', OPTIONS),
        ).toEqual({ adopt: 'claude-nova-1-0' })
    })
    test('no current choice at all adopts the reported model', () => {
        expect(
            reconcileManifestModel('', 'claude-haiku-4-5-20251001', OPTIONS),
        ).toEqual({ adopt: 'haiku' })
    })
})
