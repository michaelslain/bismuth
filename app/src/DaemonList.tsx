...
function CronRow(props: {
    ...
    const freq = () => {
        ...
        if (d?.on === 'file-change')
            return d.watch ? `on change: ${d.watch}` : 'on change'
...
export function DaemonList(props: {
    ...
    const processes = createMemo(() =>
        props.nodes.filter(n => n.kind === 'process'),
    )
    const empty = createMemo(
        () => crons().length === 0 && processes().length === 0,
    )
    ...
    async function toggleEnabled(node: GraphNode) {
        ...
        const call =
            node.kind === 'cron' ? api.setCronEnabled : api.setProcessEnabled
        ...
        if (res.ok) {
            ...
            pushToast(
                `Couldn't ${enabled ? 'disable' : 'enable'} ${node.label}`,
            )
        }
    }
    ...
    function itemsFor(node: GraphNode): MenuItem[] {
        ...
        const toggle: MenuItem = enabled
            ? {
                  label: 'Disable',
                  icon: 'PowerOff',
                  onSelect: () => void toggleEnabled(node),
              }
            : {
                  label: 'Enable',
                  icon: 'Power',
