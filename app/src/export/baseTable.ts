...
export function viewResultToTable(
    config: BaseConfig,
    vr: ViewResult,
): TableData {
    const ids = vr.columns
    const rows = vr.groups
        .flatMap(g => g.rows)
        .map((r: Row) => ids.map(id => cellText(resolveProperty(id, r))))
...
export async function baseToViewResult(
    ...
    const spec =
        config.source ??
        (rows.length ? { kind: 'base' as const } : { kind: 'notes' as const })
    const allRows =
        spec.kind === 'base' && rows.length
            ? rows
