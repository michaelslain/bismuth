...
describe('isRasterUnsafeColor', () => {
    test('flags every modern color function html2canvas chokes on', () => {
        ...
        expect(
            isRasterUnsafeColor('color-mix(in srgb, #fff 42%, transparent)'),
        ).toBe(true)
    ...
    test('flags an unsafe color embedded in a longer value (box-shadow)', () => {
        expect(isRasterUnsafeColor('0 2px 8px color(srgb 0 0 0 / 0.4)')).toBe(
            true,
        )
...
describe('colorSrgbToRgb (the exact Chrome serialization of computed color-mix)', () => {
    test('opaque color(srgb) -> rgb()', () => {
        ...
        expect(colorSrgbToRgb('color(srgb 0.05 0.055 0.086)')).toBe(
            'rgb(13, 14, 22)',
        )
    })

    test('alpha channel -> rgba()', () => {
        expect(colorSrgbToRgb('color(srgb 0.682 0.706 0.761 / 0.22)')).toBe(
            'rgba(174, 180, 194, 0.22)',
        )
        expect(colorSrgbToRgb('color(srgb 1 1 1 / 0.5)')).toBe(
            'rgba(255, 255, 255, 0.5)',
        )
    })

    test('percentage channels and alpha', () => {
        expect(colorSrgbToRgb('color(srgb 100% 0% 50% / 50%)')).toBe(
            'rgba(255, 0, 128, 0.5)',
        )
    })
    ...
    test('none channels read as 0', () => {
        ...
        expect(colorSrgbToRgb('color(srgb 1 0 0 / none)')).toBe(
            'rgba(255, 0, 0, 0)',
        )
    })
    ...
    test('whitespace-tolerant', () => {
        expect(colorSrgbToRgb('  color(srgb  0 0 0  /  0.5 )')).toBe(
            'rgba(0, 0, 0, 0.5)',
        )
    })
...
describe('normalizeCssColor', () => {
    test('safe values pass through verbatim', () => {
        ...
        expect(normalizeCssColor('rgba(1, 2, 3, 0.5)', 'rgb(0, 0, 0)')).toBe(
            'rgba(1, 2, 3, 0.5)',
        )
    })
    ...
    test('color(srgb) resolves through the pure parser', () => {
        expect(
            normalizeCssColor('color(srgb 1 0 0 / 0.25)', 'rgb(0, 0, 0)'),
    ...
    test('empty and unresolvable values fall back', () => {
        ...
        expect(normalizeCssColor('oklch(0.7 0.1 200)', 'rgb(9, 9, 9)')).toBe(
            'rgb(9, 9, 9)',
        )
    })
})
