...
function fieldsHtml(
    ...
    return cols
        ...
        .map(
            f =>
                `<div class="exp-field"><span class="exp-flabel">${escapeHtml(columnLabel(f.id, config))}</span>` +
...
function groupHeader(key: string, p: ThemePalette): string {
    return key === ''
        ? ''
        : `<div class="exp-group" style="color:${groupColorHex(key, p)}">${escapeHtml(key)}</div>`
}
...
function kanbanCols(vr: ViewResult): string[] {
    ...
    return explicit
        ? vr.columns
        : vr.columns.filter(c => c !== 'note.order' && c !== 'order')
}
...
export function kanbanHtml(
    ...
    const columns = vr.groups
        ...
        .join('')
    return {
        body: `<div class="exp-kanban">${columns}</div>`,
        css: kanbanCss(p),
    }
}
                ...
                g.rows
                    .map(row => {
                        const meta = cols
                            ...
                            .map(
                                t =>
                                    `<span class="exp-listmeta">${renderCellHtml(t)}</span>`,
                            )
                            .join('')
