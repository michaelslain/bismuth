...
export function buildEditorContextText(input: EditorContextInput): string {
    const activeFile =
        input.activeFile && !input.hiddenPaths.has(input.activeFile)
            ? input.activeFile
            : null
    const openFiles = input.openFiles.filter(
        f => !input.hiddenPaths.has(f.path),
    )
    const selectionHidden =
        !!input.selectionPath && input.hiddenPaths.has(input.selectionPath)
    ...
    if (referencedFiles.length)
        lines.push(`Referenced files: ${referencedFiles.join(', ')}`)
    if (selection) {
        lines.push(
            `Current selection${input.selectionPath ? ` (from ${input.selectionPath})` : ''}:`,
        )
