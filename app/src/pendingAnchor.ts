...
// navigation. A wikilink click (Editor.tsx) dispatches `bismuth-open`
...
// heading — once. Mirrors scrollMemory.ts's transient, in-session, per-path design, and shares
// its own implementation with pendingCursor.ts via createOneShotPathChannel.
//
...
import { createOneShotPathChannel } from './oneShotPathChannel'

const anchorChannel = createOneShotPathChannel<string>()
...
export function setPendingAnchor(path: string, heading: string): void {
    anchorChannel.set(path, heading)
}
...
export function takePendingAnchor(path: string): string | undefined {
    return anchorChannel.take(path)
}

/** Forget a buffer's pending anchor without consuming a scroll — a surface that can't honor it
 *  clears it on mount so a later open won't jump. */
export function clearPendingAnchor(path: string): void {
    anchorChannel.clear(path)
}
