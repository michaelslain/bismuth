...
describe("isHeicName / jpegNameFor", () => {
  ...
  test("renames to .jpg, preserving any directory prefix", () => {
    ...
    expect(jpegNameFor("/Users/me/Pictures/IMG_1.heif")).toBe("/Users/me/Pictures/IMG_1.jpg");
  });
...
describe("classifyIntake", () => {
  ...
  test("everything unrecognised is `other` — never silently discarded", () => {
    ...
    for (const n of ["notes.txt", "data.csv", "deck.key", "archive.zip", "Makefile", "a.svg"]) {
...
describe("filePathsFromTransfer", () => {
  const dt = (data: Record<string, string>) => ({ getData: (f: string) => data[f] ?? "" });

  test("reads a single Finder file URL", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "file:///Users/me/a.png" })))
      .toEqual(["/Users/me/a.png"]);
  });

  test("percent-decodes spaces and unicode", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "file:///Users/me/My%20Photo.heic" })))
    ...
    expect(filePathsFromTransfer(dt({ "text/uri-list": "file:///Users/me/re%CC%81sume%CC%81.pdf" })))
      .toEqual(["/Users/me/résumé.pdf"]);
  });

  test("reads a multi-file selection, CRLF-separated per RFC 2483", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "file:///a/1.png\r\nfile:///a/2.pdf\r\n" })))
      .toEqual(["/a/1.png", "/a/2.pdf"]);
  });

  test("skips comments and blank lines", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "# comment\n\nfile:///a/1.png\n" })))
      .toEqual(["/a/1.png"]);
  });

  test("skips non-file URLs — a link dragged from a browser is not a file", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "https://example.com/a.png" }))).toEqual([]);
    expect(filePathsFromTransfer(dt({ "text/uri-list": "https://example.com/x\nfile:///a/1.png" })))
      .toEqual(["/a/1.png"]);
  });
  ...
  test("returns empty for an absent transfer or an absent flavour", () => {
    ...
    expect(filePathsFromTransfer(dt({ "text/plain": "file:///a/1.png" }))).toEqual([]);
  });

  test("survives a getData that throws (dragover blocks it) instead of killing the drop", () => {
    expect(filePathsFromTransfer({ getData: () => { throw new Error("blocked"); } })).toEqual([]);
  });

  test("drops an unparseable entry but keeps the good ones", () => {
    expect(filePathsFromTransfer(dt({ "text/uri-list": "file:///a/%ZZ\nfile:///a/ok.png" })))
      .toEqual(["/a/ok.png"]);
  });
});
