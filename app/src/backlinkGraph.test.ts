...
const note = (id: string, label = id): GraphNode => ({ id, label, kind: "note" });
const tag = (id: string, label = id): GraphNode => ({ id, label, kind: "tag" });

describe("pathToNoteId", () => {
  it("strips a trailing .md extension", () => {
    expect(pathToNoteId("reading/quotes/borges.md")).toBe("reading/quotes/borges");
  });
...
describe("deriveBacklinks", () => {
  ...
  it("returns an empty list for an empty noteId", () => {
    const g: GraphData = { nodes: [note("a")], edges: [{ from: "a", to: "b", kind: "link" }] };
  ...
  it("sorts results by label", () => {
    ...
    expect(deriveBacklinks(g, "target").map((e) => e.label)).toEqual(["Alpha", "Mid", "Zed"]);
  });
});
