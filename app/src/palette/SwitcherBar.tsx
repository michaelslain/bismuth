...
export function SwitcherBar(props: Props) {
    ...
    const fileRows = createMemo<Match[]>(() =>
        rankItems(items(), query(), frecency),
    )
    ...
    const runContentSearch = (q: string) => {
        ...
        api.search(q, { caseSensitive: false, wholeWord: false, regex: false })
            ...
            .catch(() => {
                /* content search is best-effort; the file rows + AI path still work */
            })
    }
    ...
    const [aiState, setAiState] = createSignal<SwitcherAiState>(
        initialSwitcherAiState,
    )
    ...
    const navCount = createMemo(() => {
        if (aiPhase() === 'idle')
            return fileRows().length + contentRows().length
    ...
    const commitRow = (i: number) => {
        ...
        if (i < files.length) {
            const f = files[i]
            if (f) commitFile(f.item)
            return
        }
        const c = contentRows()[i - files.length]
        if (c) {
            recordUse(fileKey(c.path))
            openPath(c.path)
        }
    }
    ...
    const askAi = () => {
        ...
        api.searchPrompt(q)
            .then(r =>
                setAiState(s =>
                    switcherAiReducer(s, { type: 'resolved', gen, results: r }),
                ),
            )
            .catch(e =>
                setAiState(s =>
                    switcherAiReducer(s, {
                        type: 'rejected',
                        gen,
                        message: (e as Error).message || 'AI search failed',
                    }),
                ),
            )
    }
