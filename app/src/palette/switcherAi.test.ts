...
describe('isNaturalLanguageQuery', () => {
    ...
    it('is true once a query has three or more words', () => {
        expect(isNaturalLanguageQuery('where did I write about metals')).toBe(
            true,
        )
...
describe('switcherAiReducer', () => {
    it('starts idle', () => {
        expect(initialSwitcherAiState).toEqual({
            phase: 'idle',
            gen: 0,
            results: [],
            error: null,
    ...
    it('resolved for the current gen shows results (possibly empty)', () => {
        const loading = switcherAiReducer(initialSwitcherAiState, {
            type: 'ask',
        }) // gen 1
        const next = switcherAiReducer(loading, {
            type: 'resolved',
            gen: 1,
            results: [result('a.md')],
        })
    ...
    it('rejected for the current gen shows the error message verbatim', () => {
        ...
        const next = switcherAiReducer(loading, {
            type: 'rejected',
            gen: 1,
            message: 'AI search needs Claude Code installed',
        })
    ...
    describe('stale response after reset (keystroke cancels in-flight AI)', () => {
        it('ignores a resolved event carrying a superseded gen', () => {
            ...
            const lateResolve = switcherAiReducer(afterKeystroke, {
                type: 'resolved',
                gen: 1,
                results: [result('late.md')],
            })
        ...
        it('ignores a rejected event carrying a superseded gen', () => {
            ...
            const lateReject = switcherAiReducer(afterKeystroke, {
                type: 'rejected',
                gen: 1,
                message: 'boom',
            })
        ...
        it("a second ask (re-ask while showing results/error) supersedes the first request's response", () => {
            const loading1 = switcherAiReducer(initialSwitcherAiState, {
                type: 'ask',
            }) // gen 1
            const results1 = switcherAiReducer(loading1, {
                type: 'resolved',
                gen: 1,
                results: [result('a.md')],
            })
            ...
            const lateFirstResolve = switcherAiReducer(loading2, {
                type: 'resolved',
                gen: 1,
                results: [result('stale.md')],
            })
            expect(lateFirstResolve).toEqual(loading2)
            // The second request's own response DOES apply.
            const results2 = switcherAiReducer(loading2, {
                type: 'resolved',
                gen: 2,
                results: [result('fresh.md')],
            })
