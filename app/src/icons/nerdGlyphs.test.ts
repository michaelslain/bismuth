...
const PAIRS: [string, string, string][] = [
    [
        'Folder',
        'FolderOpen',
        "the file tree's collapse affordance (FileTree.tsx)",
    ],
    [
        'Square',
        'SquareCheck',
        'the task/multi-select checkbox at size 13 (ChatView.tsx)',
    ],
    ['Undo2', 'Redo2', 'adjacent in the drawing toolbar (drawing/Toolbar.tsx)'],
    [
        'Brain',
        'BrainCircuit',
        'distinct concepts that were one drawing before the migration',
    ],
    ['MessageSquare', 'MessagesSquare', 'one message vs many'],
    [
        'Settings',
        'Settings2',
        "the command vs the file tree's system-folder mark",
    ],
...
describe('nerdGlyphs — every registry name has a codepoint, and nothing else does', () => {
    it('maps EXACTLY the 140 names the registry exposes', () => {
        ...
        expect(
            names.filter(n => !mapped.has(n)),
            'registry names with NO codepoint',
        ).toEqual([])
        expect(
            [...mapped].filter(n => !names.includes(n)),
            'codepoints for names the registry does not have',
        ).toEqual([])
    })
    ...
    it('every codepoint is a plausible Nerd Font codepoint', () => {
        ...
        for (const [name, cp] of Object.entries(NERD_GLYPHS)) {
            ...
            expect(cp, `${name} is above the ASCII range`).toBeGreaterThan(
                0x2000,
            )
            expect(cp, `${name} is inside Unicode`).toBeLessThanOrEqual(
                0x10ffff,
            )
        }
    })
...
describe('nerdGlyphs — no two names share a codepoint', () => {
    it('assigns 140 names to 140 DISTINCT codepoints', () => {
        ...
        for (const [name, cp] of Object.entries(NERD_GLYPHS))
            byCode.set(cp, [...(byCode.get(cp) ?? []), name])
        const collisions = [...byCode]
            .filter(([, names]) => names.length > 1)
            .map(
                ([cp, names]) =>
                    `U+${cp.toString(16).toUpperCase()} <- ${names.sort().join(' + ')}`,
            )
    ...
    it('the collision detector actually detects a collision', () => {
        ...
        expect(
            detect({ ...NERD_GLYPHS, SquareKanban: NERD_GLYPHS.Columns3! }),
        ).toEqual(['U+F1487 <- Columns3 + SquareKanban'])
    })
})
...
describe('nerdGlyphs — the fallback is not mistakable for an icon', () => {
    it('has its own codepoint, shared with nothing', () => {
        const clashes = Object.entries(NERD_GLYPHS)
            .filter(([, cp]) => cp === FALLBACK_CODEPOINT)
            .map(([n]) => n)
...
describe('nerdGlyphs — every codepoint is really in the subset font', () => {
    ...
    const all = [
        ...Object.entries(NERD_GLYPHS),
        ['FALLBACK_CODEPOINT', FALLBACK_CODEPOINT] as const,
    ]
    const { results } = checkGlyphs(
        fontBytes,
        all.map(([, cp]) => cp),
    )
    ...
    it('all 141 declared codepoints resolve to a real glyph', () => {
        const missing = results
            ...
            .map(
                ({ name, r }) =>
                    `${name} (U+${r.codepoint.toString(16).toUpperCase()})`,
            )
        expect(
            missing,
            'declared here but absent from the font — run `bun run icons:font`',
        ).toEqual([])
