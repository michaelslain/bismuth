...
const NEVER_SUBSET = {
    'md-bell (unused upstream glyph)': 0xf0f3,
    'the letter A': 0x41,
    'an emoji': 0x1f600,
}
...
describe('icon font — the artifact exists and is the real thing', () => {
    ...
    it('contains EXACTLY the codepoints nerdGlyphs.ts declares — no more, no fewer', () => {
        ...
        const declared = new Set([
            ...Object.values(NERD_GLYPHS),
            FALLBACK_CODEPOINT,
        ])
        const inFont = new Set(codepoints)
        const hexes = (s: Set<number>) =>
            [...s]
                .sort((a, b) => a - b)
                .map(c => `U+${c.toString(16).toUpperCase()}`)
        expect(
            hexes(new Set([...declared].filter(c => !inFont.has(c)))),
            'declared but NOT in the font',
        ).toEqual([])
        expect(
            hexes(new Set([...inFont].filter(c => !declared.has(c)))),
            'in the font but not declared',
        ).toEqual([])
        expect(codepoints.length).toBe(141) // 140 registry names + the fallback
        expect(
            new Set(codepoints).size,
            'no duplicate codepoints in the manifest',
        ).toBe(codepoints.length)
    })
})

describe('icon font — no glyph is tofu', () => {
    const { results, notdefAdvance, unitsPerEm, mappedCodepoints, container } =
        checkGlyphs(fontBytes, codepoints)
    ...
    it('every requested codepoint resolves to a real glyph, never .notdef', () => {
        const tofu = results
            .filter(r => r.tofu)
            .map(r => `U+${r.codepoint.toString(16).toUpperCase()}`)
        expect(
            tofu,
            `${tofu.length} of ${results.length} codepoints render as tofu`,
        ).toEqual([])
    })
    ...
    it('every one of the named anchor glyphs is present', () => {
        ...
        const missing = Object.entries(ANCHORS).filter(
            ([, cp]) => checkGlyphs(fontBytes, [cp]).results[0]!.tofu,
        )
        expect(missing.map(([name]) => name)).toEqual([])
        for (const cp of Object.values(ANCHORS))
...
describe('icon font — the tofu check can actually fail', () => {
    ...
    it('a real glyph and an absent one are distinguished by glyph id and NOT by advance width', () => {
        ...
        const absent = checkGlyphs(fontBytes, [
            NEVER_SUBSET['md-bell (unused upstream glyph)'],
        ]).results[0]!
        ...
        expect(
            widths[real.glyphId],
            'a real glyph and .notdef advance identically',
        ).toBe(widths[0])
    })
})
...
describe('icon font — registry coverage is complete', () => {
    ...
    it('no registry name is unmapped — neither as an alias nor outright', () => {
        expect({ aliasOfMapped, noMapping }).toEqual({
            aliasOfMapped: [],
            noMapping: [],
        })
    })
...
describe('icon font — the CSS that loads it points at the file that exists', () => {
    ...
    const tokensCss = readFileSync(
        join(APP_SRC, 'styles', 'tokens.css'),
        'utf8',
    )
    ...
    it('--icon-font-stack names the family FIRST and still reaches a text font', () => {
        ...
        expect(
            decl,
            '--icon-font-stack is defined in styles/tokens.css',
        ).toBeTruthy()
