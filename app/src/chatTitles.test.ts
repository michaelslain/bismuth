...
test('a rename override wins over the session title and the fallback', () => {
    expect(resolveChatHeaderTitle('My chat', 'Session summary', 'Chat')).toBe(
        'My chat',
    )
})

test('falls back to the session title when there is no rename override', () => {
    expect(resolveChatHeaderTitle(undefined, 'Session summary', 'Chat')).toBe(
        'Session summary',
    )
    expect(resolveChatHeaderTitle('', 'Session summary', 'Chat')).toBe(
        'Session summary',
    )
})
...
test('trims a real override/title before showing it', () => {
    expect(resolveChatHeaderTitle('  Deploy plan  ', 'x', 'Chat')).toBe(
        'Deploy plan',
    )
    expect(resolveChatHeaderTitle(undefined, '  Weekly review ', 'Chat')).toBe(
        'Weekly review',
    )
})
