...
function mountBlock(
    source = SOURCE,
    dim: '2d' | '3d' = '2d',
    cfgOverride?: Partial<GraphConfig>,
): Mounted {
    ...
    r.render(data)
    r.setConfig({
        ...embeddedGraphConfig(DEFAULTS.graph, THEMES.ink, dim),
        ...cfgOverride,
    })
    ctx.fills.length = 0
    ctx.strokes = 0
...
const nodeIds = (source = SOURCE) =>
    parseGraphBlock(source).spec.nodes.map(n => n.id)
...
describe('EmbeddedGraph — the ```graph block on the unified renderer', () => {
    ...
    it('shows no cluster machinery — a hand-authored block has no communities', () => {
        ...
        const stray = ctx.fills.filter(
            f => !f.text.startsWith('[[') && /[A-Z]/.test(f.text),
        )
    ...
    it('still emits a live atmosphere with no communities to emit from', () => {
        ...
        const stats = (
            r as unknown as {
                computeStats(): { bloomPoints: number; bloomWeight: number }
            }
        ).computeStats()
