...
const idx = (w: number, cx: number, cy: number, dx: number, dy: number) =>
    (cy + dy) * w + (cx + dx)
...
test('buildBloom really blurs, at the right resolution and orientation', () => {
    ...
    const cx = Math.floor(0.5 * FIELD_W),
        cy = Math.floor(0.5 * FIELD_H)
...
test('three-pass blur falls off monotonically from the peak, along both an axis and a diagonal', () => {
    ...
    for (const ray of [axisRay, diagRay]) {
        for (let i = 1; i < ray.length; i++)
            expect(ray[i]).toBeLessThan(ray[i - 1])
    }
})
...
test('cloudGrid samples DENSER as the cloud grows — the spacing, not the count, is what is fixed', () => {
    ...
    const ladder = [0.002, 0.01, 0.03, 0.06, 0.1, 0.2, 0.5, 2].map(sd =>
        cloudSampleCount(sd, sd),
    )
    ...
    const arcGap = (sd: number) =>
        (2 * Math.PI * 2 * sd * FIELD_W) / cloudGrid(sd, 0).perRing
    ...
    for (const sd of [0, 0.001, 0.01, 0.1, 1])
        expect(cloudGrid(sd, sd).perRing).toBeGreaterThanOrEqual(3)
})
...
test('a non-finite or negative spread degrades to that same point, and NEVER to silence', () => {
    ...
    for (const [sx, sy] of [
        ...
        expect(
            out.every(p => Number.isFinite(p.x) && Number.isFinite(p.y)),
        ).toBe(true)
...
test('a spread cloud does NOT out-peak the individual points it stands for', () => {
    ...
    for (let i = 0; i < 60; i++) {
        leaves.push({
            x: 0.5 + ((i % 10) - 4.5) * 0.07,
            y: 0.5 + (Math.floor(i / 10) - 2.5) * 0.11,
            weight: 1,
        })
    }
    ...
    const peak = (pts: BloomPoint[]) =>
        Math.max(
            ...blur(accumulate(pts, FIELD_W, FIELD_H), FIELD_W, FIELD_H, 6),
        )
    ...
    const lit = (pts: BloomPoint[]) =>
        buildBloom(pts).filter(v => v > 0.376).length
...
test('TWO CLUSTERS WITH DIFFERENT SLOTS PRODUCE DIFFERENT HUES — the whole point of the change', () => {
    ...
    for (let i = 0; i < 20; i++)
        pts.push({ x: 0.2 + (i % 5) * 0.005, y: 0.5, weight: 1, rgb: RED })
    for (let i = 0; i < 20; i++)
        pts.push({ x: 0.8 + (i % 5) * 0.005, y: 0.5, weight: 1, rgb: BLUE })
    const f = buildBloom(pts)
    expect(f.rgb).toBeDefined()

    const left = hueAt(f, 0.2, 0.5),
        right = hueAt(f, 0.8, 0.5)
...
test('carrying colour cannot change how BRIGHT the atmosphere is', () => {
    ...
    const geometry: BloomPoint[] = [
        { x: 0.25, y: 0.5, weight: 2 },
        { x: 0.75, y: 0.5, weight: 1 },
        { x: 0.4, y: 0.2, weight: 3 },
    ]
    const mono = buildBloom(geometry)
    const col = buildBloom(
        geometry.map((p, i) => ({ ...p, rgb: i === 0 ? RED : BLUE })),
    )
...
test('THE HALO BUG — a magnified pair stays ONE lit body; a fit-sized kernel leaves a dark gap', () => {
    ...
    const gapAlpha = (zoom: number, radius: number) => {
        ...
        let lo = 1
        for (
            let x = Math.round((0.5 - sep / 2) * FIELD_W);
            x <= Math.round((0.5 + sep / 2) * FIELD_W);
            x++
        ) {
    ...
    for (const zoom of [3, 4, 6]) {
        ...
        expect(gapAlpha(zoom, blurRadiusForZoom(zoom))).toBeGreaterThan(
            gapAlpha(zoom, BASE_BLUR_RADIUS),
        )
    }
})
...
test('scaleField dims a field by the ratio, clamps out of range, and leaves 1 untouched', () => {
    ...
    for (let i = 0; i < ref.length; i++)
        expect(half[i]).toBeCloseTo(ref[i] * 0.5, 6)
