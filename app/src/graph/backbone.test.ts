...
describe('buildLevelEdges', () => {
    ...
    it('breaks a hub tie by the LOWER id, regardless of which node is seen first', () => {
        const tiedHigherFirst: PathNode[] = [
            { id: 'z2', path: [9], deg: 7 },
            { id: 'z1', path: [9], deg: 7 },
        ]
        const tiedLowerFirst: PathNode[] = [
            { id: 'z1', path: [9], deg: 7 },
            { id: 'z2', path: [9], deg: 7 },
        ]
        expect(
            buildLevelEdges(tiedHigherFirst, [], 1).levelHubs[0].get(9),
        ).toEqual({ id: 'z1', deg: 7 })
        expect(
            buildLevelEdges(tiedLowerFirst, [], 1).levelHubs[0].get(9),
        ).toEqual({ id: 'z1', deg: 7 })
    })
    ...
    it("caps each level's pairs at maxPairs, keeping the heaviest", () => {
        const { levelPairs } = buildLevelEdges(
            fixtureNodes(),
            fixtureEdges,
            2,
            /* maxPairs */ 2,
    ...
    it('drops an edge whose endpoint has no community at all', () => {
        ...
        expect(levelPairs[0]).toEqual([
            { a: { id: 'a', deg: 1 }, b: { id: 'b', deg: 1 }, count: 1 },
        ])
    })

    it('returns empty structures for levelCount 0', () => {
        const { levelHubs, levelPairs, truncated } = buildLevelEdges(
            fixtureNodes(),
            fixtureEdges,
            0,
        )
    ...
    it('reports how many pairs were dropped by the cap at each level', () => {
        const { truncated } = buildLevelEdges(
            fixtureNodes(),
            fixtureEdges,
            2,
            /* maxPairs */ 2,
...
describe('computeEdgeLevelWeights', () => {
    ...
    it('behaves the same shape at FILE_LABEL_REVEAL_T (0.75) as it does at the ported Canvas default (0.62)', () => {
        ...
        expect(w[2]).toBeCloseTo(1, 10)
        expect(w[3]).toBeCloseTo(0, 10)
        const wPast = computeEdgeLevelWeights(
            FILE_LABEL_REVEAL_T + 0.2,
            3,
            FILE_LABEL_REVEAL_T,
        )
    ...
    it("crosses level boundaries on the SAME FRAME as clusterLevelAlphas only because the same revealT is passed to both — the comment's claim is conditional, not automatic", () => {
        ...
        for (const t of [0.1, 0.3, 0.5]) {
            ...
            const levelAlphas = clusterLevelAlphas(
                t,
                levelCount,
                FILE_LABEL_REVEAL_T,
            )
            ...
            for (let L = 0; L < levelCount - 1; L++)
                expect(w[L]).toBeCloseTo(levelAlphas[L], 10)
        }
        ...
        const levelAlphasAtFileReveal = clusterLevelAlphas(
            t,
            4,
            FILE_LABEL_REVEAL_T,
        )
        ...
        expect(
            Math.abs(wDefault[2] - levelAlphasAtFileReveal[2]),
...
describe('bandsForT', () => {
    it('collapses to pure member territory when the graph has no community hierarchy at all', () => {
        for (const t of [0, 0.2, 0.5, 0.8, 1]) {
            expect(bandsForT(t, 0)).toEqual({
                massAlpha: 0,
                backboneAlpha: 0,
                memberAlpha: 1,
            })
            expect(bandsForT(t, -1)).toEqual({
                massAlpha: 0,
                backboneAlpha: 0,
                memberAlpha: 1,
            })
    ...
    it('the three alphas sum to 1 at every t (necessary, not sufficient — see tests below)', () => {
        ...
        for (let i = 0; i <= 20; i++) {
            ...
            expect(b.massAlpha + b.backboneAlpha + b.memberAlpha).toBeCloseTo(
                1,
                10,
            )
        }
    ...
    it('massAlpha owns the field up to BACKBONE_START_T and is fully gone by the end of that crossfade', () => {
        ...
        expect(
            bandsForT(BACKBONE_START_T + BACKBONE_FADE_SPAN, N).massAlpha,
        ).toBeCloseTo(0, 10)
    ...
    it('backboneAlpha is 0 outside its band and full strength across its plateau', () => {
        ...
        expect(
            bandsForT(BACKBONE_START_T + BACKBONE_FADE_SPAN, N).backboneAlpha,
        ).toBeCloseTo(1, 10)
        const plateauMid =
            (BACKBONE_START_T + BACKBONE_FADE_SPAN + MEMBER_START_T) / 2
        ...
        expect(
            bandsForT(MEMBER_START_T + MEMBER_FADE_SPAN, N).backboneAlpha,
        ).toBeCloseTo(0, 10)
    ...
    it('memberAlpha is 0 until MEMBER_START_T and reaches full strength by the end of that crossfade', () => {
        ...
        expect(
            bandsForT(MEMBER_START_T + MEMBER_FADE_SPAN, N).memberAlpha,
        ).toBeCloseTo(1, 10)
    ...
    it('memberAlpha crosses exactly half strength at FILE_LABEL_REVEAL_T — the real reason MEMBER_START_T is 0.68', () => {
        expect(bandsForT(FILE_LABEL_REVEAL_T, 4).memberAlpha).toBeCloseTo(
            0.5,
            10,
        )
    })
