...
function graph(overrides?: {
    ...
    return {
        nodes: overrides?.nodes ?? [
            {
                id: 'A',
                position: p ? [1, 2, 3] : [0, 0, 0],
                position2d: p ? [1, 2] : [0, 0],
            },
            {
                id: 'B',
                position: p ? [4, 5, 6] : [0, 0, 0],
                position2d: p ? [4, 5] : [0, 0],
            },
            {
                id: 'C',
                position: p ? [7, 8, 9] : [0, 0, 0],
                position2d: p ? [7, 8] : [0, 0],
            },
        ],
...
describe('structuralGraphSig — real structural changes DO change the signature', () => {
    ...
    test('retargeting an edge (same node set + count) changes it', () => {
        expect(
            structuralGraphSig(graph({ edges: [{ from: 'A', to: 'C' }] })),
        ).not.toBe(base)
    })
    ...
    test('removing a node changes it', () => {
        expect(
            structuralGraphSig(graph({ nodes: [{ id: 'A' }, { id: 'B' }] })),
        ).not.toBe(base)
    })

    test('a daemon cron flipping to running changes it', () => {
        const idle = structuralGraphSig(
            graph({
                nodes: [
        ...
        const running = structuralGraphSig(
            graph({
                nodes: [
...
describe('shouldResetView — camera reset only on a genuinely new graph', () => {
    ...
    test('identical node set does NOT reset (a same-structure re-fetch)', () => {
        expect(shouldResetView(ids('A', 'B', 'C'), nodes('A', 'B', 'C'))).toBe(
            false,
        )
    })
    ...
    test('an incremental add (open a tab / add a note) does NOT reset — camera preserved', () => {
        expect(
            shouldResetView(ids('A', 'B', 'C'), nodes('A', 'B', 'C', 'D')),
        ).toBe(false)
    })

    test('an incremental remove does NOT reset', () => {
        expect(
            shouldResetView(ids('A', 'B', 'C', 'D'), nodes('A', 'B', 'C')),
        ).toBe(false)
    })
