...
const EDGE_BUDGET_2D = 6000,
    EDGE_FLOOR_2D = 0.06
const EDGE_BUDGET_3D = 6000,
    EDGE_FLOOR_3D = 0.45
...
const MESH_W_BASE = 0.3,
    MESH_W_MIN = 0.12,
    MESH_W_MAX = 1.1 // CanvasGraphRenderer.ts:1298
 ...
 */
export function deriveEdgeBaseAlpha(
    neutralCss: string,
    bgCss: string,
    edgeCss: string,
): number {
...
interface EntityView {
    ...
    count: number
    wx: number
    wy: number // members' 2D world centroid (same space as NodeView.p2)
    ...
    p3: Vec3
    sdx: number
    sdy: number // members' per-axis world spread about it (lod.ts LodCluster.sdx/sdy) —
    ...
    name: string
    rowR: number
    colR: number // uncapped mass radii in cells (sqrt scaling — lod.ts massRadii)
    memberIds: string[]
    ...
    onGrid: boolean
    drawnRowR: number
    drawnColR: number // grid-capped radii for this frame
}
interface LabelDraw {
...
function nodePathLabels(n: GraphNode): string[] | undefined {
    if (n.communityPathLabels && n.communityPathLabels.length)
        return n.communityPathLabels
    return n.community != null
        ? [n.communityLabel ?? `cluster ${n.community}`]
        : undefined
}
...
export class AsciiGraphRenderer implements GraphRenderer {
    ...
    private p3SpacingCache: SpacingCache<RespaceVec3[]> =
        createSpacingCache(cloneVec3Array)
    private p2SpacingCache: SpacingCache<RespaceVec3[]> =
        createSpacingCache(cloneVec3Array)
    private radius3 = 1
    private radius2 = 1
    ...
    private groupBatches = new Map<
        string,
        { alpha: number; width: number; pts: number[] }
    >()
    ...
    mount(
        ...
        if (
            ...
            ).__asciiGraphStats = () => this.computeStats()
    ...
    setPaintCallback(cb: (nodeCount: number) => void) {
        this.onPaint = cb
    }
    setZoomCallback(cb: (pct: number) => void) {
        this.onZoom = cb
    }
    setBloomCallback(cb: ((field: DensityField) => void) | undefined) {
        this.onBloom = cb
    }
    ...
    render(g: GraphData) {
        ...
        if (struct === this.sig && this.nodes.length) {
            this.dirty = true
            return
        }
    ...
    private build(g: GraphData, resetCamera: boolean) {
        ...
        for (const e of g.edges) {
            ...
            this.link(e.from, e.to)
            this.link(e.to, e.from)
        }
        ...
        const intentionalLayout = g.nodes.some(
            n =>
                n.kind === 'agent' ||
                n.kind === 'daemon' ||
                n.kind === 'cron' ||
        ...
        const spaced3 = this.p3SpacingCache.getOrCompute(this.sig, () =>
            scaleToSpacing(raw3, targetSpacing),
        )
        const spaced2 = this.p2SpacingCache.getOrCompute(this.sig, () =>
            scaleToSpacing(raw2, targetSpacing),
        )
        ...
        for (const nv of this.nodes) {
            ...
            if (path && path.length > this.levelCount)
                this.levelCount = path.length
        }
        ...
        this.communityNamesByLevel = Array.from(
        ...
        this.clusterHubByLevel = Array.from(
            { length: this.levelCount },
            () => new Map<number, string>(),
        )
        if (this.levelCount > 0) {
            const membersByLevel: Map<
                number,
        ...
        if (this.levelCount > 0) {
            ...
            this.levelPairs = built.levelPairs.map(pairs => {
                ...
                for (const p of pairs) {
                    const a = this.byId.get(p.a.id),
                        b = this.byId.get(p.b.id)
        ...
        this.entityFlat = []
        this.entityLevels = this.lodLevels.map((lv, L) =>
            lv.clusters.map(c => {
                const { rowR, colR } = massRadii(
                    c.count,
                    this.cellW,
                    this.cellH,
                )
                const ev: EntityView = {
                    flat: this.entityFlat.length,
                    level: L,
                    ...
                    name:
                        this.communityNamesByLevel[L]?.get(c.community) ??
                        `cluster ${c.community}`,
                    rowR,
                    colR,
        ...
        this.namableByLevel = Array.from(
            ...
            (_, L) =>
                new Set(
                    (this.lodLevels[L]?.clusters ?? []).map(c => c.community),
                ),
        )
        ...
        this.alwaysOn = computeAlwaysOnSet(
            ...
            this.activeFile,
            this.cfg.graphLabelHubCount ?? 10,
        )
        this.restyle()
    ...
    setConfig(cfg: GraphConfig) {
        ...
        if (cfg.viewMode !== prevMode) {
            ...
            if (noViewYet) {
                ...
                this.modeMorph = {
                    fromFlatten: this.morphFlatten,
                    toFlatten: cfg.viewMode === '2d' ? 1 : 0,
                    fromRx: this.rx,
                    fromRy: this.ry,
                    toRx: cfg.viewMode === '2d' ? 0 : -0.5,
                    toRy: 0,
                    startT: null,
                }
            }
    ...
    private applyGround() {
        ...
        this.viewport.style.background = this.cfg.transparent
            ? 'transparent'
            : ''
    }
    ...
    private readTokens() {
        ...
        this.groundColor =
            gb && !gb.includes('gradient') ? gb : read('--bg', '#0b0c11')
        ...
        this.edgeBaseAlpha = deriveEdgeBaseAlpha(
            this.colors[C_MUTED],
            this.groundColor,
            this.colors[C_EDGE],
        )
    ...
    private rebuildCommunityColors() {
        ...
        const sizesByLevel: Map<number, number>[] = Array.from(
            { length: this.levelCount },
            () => new Map(),
        )
        for (const nv of this.nodes) {
    ...
    private rebuildEntityColors() {
        for (let L = 0; L < this.entityLevels.length; L++) {
            ...
            for (const ev of this.entityLevels[L])
                ev.color = slotMap?.get(ev.community) ?? C_FAINT
        }
    }
    ...
    private colorLevelsFor(n: GraphNode): number[] {
        switch (n.kind) {
            ...
            case 'process': {
                ...
                return [
                    vs.fill === 'palette' || vs.border === 'palette'
                        ? RAMP[hashKey(n.id) % RAMP.length]
                        : C_FAINT,
                ]
            }
            default: {
                ...
                if (path && path.length) {
                    return path.map(
                        (c, L) =>
                            this.communitySlotByLevel[L]?.get(c) ?? C_FAINT,
                    )
                }
                ...
                if (n.kind === 'tag')
                    return [RAMP[hashKey('tag:' + n.label) % RAMP.length]]
                const key =
                    n.kind === 'note'
    ...
    private measure() {
        ...
        if (!isUsableBox(r.width, r.height)) {
            if (!this.boxReady) this.dirty = true
            return
        }
        this.boxReady = true
    ...
    private syncSize() {
        ...
        if (
            this.boxReady &&
            Math.abs(r.width - this.W) < 0.5 &&
            Math.abs(r.height - this.H) < 0.5
        )
            return
        this.measure()
        this.fit()
    ...
    private start() {
        if (this.running || !this.visible || !this.host) return
        this.running = true
    ...
    private tick = (t: number) => {
        try {
            ...
            if (!this.frameErrorLogged) {
                ...
                console.error(
                    '[graph] frame failed; the render loop continues',
                    err,
                )
            }
        } finally {
    ...
    private frame = (t: number) => {
        ...
        // not this clamp).
        const dt = this.lastFrameT
            ? Math.max(1, Math.min(100, t - this.lastFrameT))
            : 16.67
        if (this.lastFrameT) {
            this.fpsAccum += t - this.lastFrameT
            this.fpsFrames++
            if (this.fpsAccum >= 500) {
                this.onFps?.(
                    Math.round((this.fpsFrames * 1000) / this.fpsAccum),
                )
                this.fpsAccum = 0
                this.fpsFrames = 0
            }
        ...
        if (
            !is2d &&
            this.cfg.spin &&
            this.nodes.length <= 350 &&
            !this.userTook &&
            !this.dragging
        ) {
            this.ry += this.cfg.spinSpeed ?? 0.0015
            this.dirty = true
        }
        ...
        if (Math.abs(this.goalRes - this.res) > RES_EPS) {
            this.res += (this.goalRes - this.res) * glide
            this.dirty = true
        } else if (this.res !== this.goalRes) {
            this.res = this.goalRes
            this.dirty = true
        }
        if (
            Math.hypot(
                this.goalTarget[0] - this.target[0],
                this.goalTarget[1] - this.target[1],
                this.goalTarget[2] - this.target[2],
            ) > 0.3
        ) {
            for (let i = 0; i < 3; i++)
                this.target[i] += (this.goalTarget[i] - this.target[i]) * glide
            this.dirty = true
        }
    ...
    private emitBloom() {
        ...
        if (weight > 0) {
            ...
            this.bloomSdxFrame = 0
            this.bloomSdyFrame = 0
        }
        ...
        const decayed =
            raw >= this.bloomPeakRef
                ? raw
                : this.bloomPeakRef +
                  (raw - this.bloomPeakRef) * BLOOM_PEAK_DECAY
        ...
        this.onBloom(
            this.bloomPeakRef > 0
                ? scaleField(field, raw / this.bloomPeakRef)
                : field,
        )
    }
     ...
     */
    private nodeBloomRgb(
        nv: NodeView,
    ): readonly [number, number, number] | undefined {
        const cbl = nv.colorByLevel
        const slot =
            cbl.length > 1
                ? cbl[Math.min(this.frameColorL0, cbl.length - 1)]
                : nv.color
    ...
    private zoomStepAnchored(pct: number, ax: number, ay: number) {
        ...
        const sB = this.pxPerWorld * before,
            sA = this.pxPerWorld * after
        if (!(sB > 0) || !(sA > 0)) return
    ...
    private rasterize(is2d: boolean) {
        ...
        this.panXQ = qx.whole
        this.panYQ = qy.whole
        this.panXFrac = qx.frac
        this.panYFrac = qy.frac
        ...
        if (clusterColorsOn && this.levelCount > 1) {
            const picked = this.activeColorLevels(
                clusterLevelAlphas(t, this.levelCount),
            )
            colorL0 = picked.L0
            colorL1 = picked.L1
            colorW1 = picked.w1
        ...
        if (glyphA > LOD_ALPHA_EPS) {
            ...
            if (lodOn) {
                for (const nv of this.nodes) {
                    ...
                    const slot =
                        nv.colorByLevel[
                            Math.min(colorL0, nv.colorByLevel.length - 1)
                        ]
            ...
            const keepFrac =
                this.edges.length > budget
                    ? Math.max(floor, budget / this.edges.length)
                    : 1
            for (const e of this.edges) {
                ...
                const ca =
                    a.colorByLevel[Math.min(colorL0, a.colorByLevel.length - 1)]
                const cb =
                    b.colorByLevel[Math.min(colorL0, b.colorByLevel.length - 1)]
                ...
                const incident =
                    hov != null && (a.node.id === hov || b.node.id === hov)
                const focused =
                    focus != null &&
                    (focus.has(a.node.id) || focus.has(b.node.id))
                ...
                if (
                    ...
                    !(hov == null && focused) &&
                    !(
                        this.inViewClusters.has(ca) &&
                        this.inViewClusters.has(cb)
                    )
                ) {
                ...
                if (ca === cb) this.edgesIntraVisibleFrame++
                else this.edgesCrossVisibleFrame++

                if (incident) {
                    this.edgeAccent.push(e)
                    continue
                }
                if (hov != null) {
                    this.edgeDim.push(e)
        ...
        if (
            mix &&
            glyphA > LOD_ALPHA_EPS &&
            mix.backboneAlpha > LOD_ALPHA_EPS &&
            this.levelPairs.length
        ) {
            ...
            const w = computeEdgeLevelWeights(
                t,
                this.levelCount,
                FILE_LABEL_REVEAL_T,
            )
    ...
    private activeColorLevels(levelAlphas: number[]): {
        L0: number
        L1: number
        w1: number
    } {
    ...
    private nodeColorSlotForFrame(
        nv: NodeView,
        clusterColorsOn: boolean,
        L0: number,
        L1: number,
        w1: number,
    ): number {
    ...
    private projectEntities(level: number) {
        ...
        for (const ev of this.entityLevels[level]) {
            ...
            ev.onGrid =
                projValid &&
                ev.col >= -ev.drawnColR &&
                ev.col < m.cols + ev.drawnColR &&
                ev.row >= -ev.drawnRowR &&
                ev.row < m.rows + ev.drawnRowR
        }
    }
    ...
    private groupBatch(
        ...
        const aq = Math.max(
            1,
            Math.min(GROUP_ALPHA_STEPS, Math.round(alpha * GROUP_ALPHA_STEPS)),
        )
        const key = `${aq}|${width}`
        let b = this.groupBatches.get(key)
        if (!b) {
            b = { alpha: aq / GROUP_ALPHA_STEPS, width, pts: [] }
            this.groupBatches.set(key, b)
        }
        return b
    }
    ...
    private cellCX(col: number) {
        return this.m.padX + col * this.m.cellW + this.m.cellW / 2
    }
    private cellCY(row: number) {
        return this.m.padY + row * this.m.cellH + this.m.cellH / 2
    }
    ...
    private drawAggregateEdges(level: number, levelAlpha: number) {
        ...
        for (const e of lv.edges) {
            ...
            const alpha =
                base * (AGG_EDGE_ALPHA_MIN + (1 - AGG_EDGE_ALPHA_MIN) * e.w)
            if (alpha <= 0.004) continue
            const width =
                (e.w >= AGG_EDGE_DOUBLE_W ? GROUP_W_BASE * 2 : GROUP_W_BASE) *
                wScale
            this.pushGroupSeg(
                this.groupBatch(alpha, width),
                a.col,
                a.row,
                b.col,
                b.row,
    ...
    private queueBackbone(level: number, weight: number) {
        ...
        for (let wb = 0; wb < EDGE_WEIGHT_BUCKETS; wb++) {
            const alpha =
                base *
                (GROUP_EDGE_ALPHA_MIN +
                    (1 - GROUP_EDGE_ALPHA_MIN) *
                        ((wb + 0.5) / EDGE_WEIGHT_BUCKETS))
            if (alpha <= 0.004) continue
            const width = Math.max(
                GROUP_W_MIN,
                Math.min(
                    GROUP_W_MAX,
                    (GROUP_W_BASE + wb * GROUP_W_STEP) * wScale,
                ),
            )
    ...
    private drawEntityMasses(level: number, levelAlpha: number) {
        ...
        for (const ev of this.entityLevels[level]) {
            ...
            const rowR = ev.drawnRowR,
                colR = ev.drawnColR
            const invR2 = 1 / (rowR * rowR),
                invC2 = 1 / (colR * colR)
            for (let dy = -rowR; dy <= rowR; dy++) {
                ...
                for (let dx = -colR; dx <= colR; dx++) {
                    ...
                    if (
                        ...
                        continue // a real note owns the cell
    ...
    private cameraFrame(): {
        ...
        const s2d = this.fitPxPerWorldFor(true) * this.res,
            s3d = this.fitPxPerWorldFor(false)
    ...
    private layoutLabels(is2d: boolean) {
        ...
        if (cAlpha > 0.01 && this.levelCount > 0) {
            ...
            for (let L = 0; L < levelAlphas.length; L++) {
                ...
                if (a > 0.01) {
                    if (this.lodOn) this.layoutEntityNames(L, a)
                    else this.layoutClusterNames(L, a)
                }
            }
        }
        ...
        for (const nv of this.nodes) {
            if (
                nv.projValid &&
                inViewport(nv.sx, nv.sy, this.W, this.H, VIEWPORT_LABEL_PAD)
            )
                ordered.push(nv)
        }
        ...
        const forced = (nv: NodeView) => {
            ...
            return (
                ...
                nv.node.kind === 'self' ||
                this.searchMatches.has(id) ||
                (this.highlightSet?.has(id) ?? false) ||
                (this.hoveredId != null &&
                    (this.adjacency.get(this.hoveredId)?.has(id) ?? false))
            )
        }
        const rank = (nv: NodeView) =>
            forced(nv)
                ? 1e9
                : this.alwaysOn.has(nv.node.id)
                  ? 1e6 + nv.deg
                  : nv.deg + nv.dr
        ...
        for (const nv of ordered) {
            ...
            if (!free && everyNode) {
                ...
                if (alt >= 0 && alt + len <= m.cols && !taken(alt, len, row)) {
                    col = alt
                    free = true
                }
            }
            ...
            const accent =
                nv.node.id === this.activeFile ||
                nv.node.id === this.hoveredId ||
                this.searchMatches.has(nv.node.id)
            const colorSlot = accent
                ? C_ACCENT
                : is2d
                  ? C_MUTED
                  : nv.dr > 0.55
    ...
    private layoutClusterNames(level: number, alpha: number) {
        ...
        for (const [community, members] of items) {
            ...
            if (row < 0 || row >= m.rows) continue
            const text = trimDanglingWord(
                clusterLabelText(
                    names.get(community) ?? `cluster ${community}`,
                ),
            )
            const len = text.length
            ...
            const wCells = eyebrowWidthCells(
                len,
                CLUSTER_LABEL_TRACKING_EM,
                this.fontPx,
                this.cellW,
            )
            ...
            const color =
                this.communityColorsByLevel[level]?.get(community) ?? '#888'
            this.labels.push({
                text,
                col,
                row,
    ...
    private layoutEntityNames(level: number, alpha: number) {
        ...
        for (const ev of evs) {
            ...
            this.labels.push({
                ...
                col,
                row,
                color: this.resolveFillColor(ev.color),
                accent: false,
                alpha,
                eyebrow: true,
                widthCells: wCells,
    ...
    private resolveFillColor(slot: number): string {
        if (slot >= COMM_BLEND_BASE)
            return this.blendColors[slot - COMM_BLEND_BASE] ?? '#888'
        if (slot >= BLEND_BASE)
            return this.commColors[slot - BLEND_BASE] ?? '#888'
    ...
    private strokeEdges() {
        ...
        if (this.intraOn && meshA > 0.004) {
            ...
            const meshW = Math.max(
                MESH_W_MIN,
                Math.min(MESH_W_MAX, MESH_W_BASE * this.lineWidthScale()),
            )
        ...
        ctx.lineWidth = Math.max(
            EDGE_W_MIN,
            Math.min(EDGE_W_MAX, EDGE_W_GAIN + (EDGE_W_MAX - EDGE_W_GAIN) * t),
        )
    ...
    private paint() {
        ...
        ctx.setTransform(
            ...
            0,
            this.dpr,
            this.panXFrac * this.dpr,
            this.panYFrac * this.dpr,
        )
        ...
        ctx.globalAlpha = 1
        const ctxLS = ctx as CanvasRenderingContext2D & {
        ...
        for (const l of this.labels) {
            ...
            if (this.letterSpacingSupported)
                ctxLS.letterSpacing = l.eyebrow
                    ? eyebrowLS
                    : this.pinnedLetterSpacing
    ...
    private setSelectionSuppressed(on: boolean): void {
        ...
        if (on) {
            if (this.prevUserSelect === null)
                this.prevUserSelect = body.style.userSelect
    ...
    private onPointerLeave = () => {
        if (this.hoveredId || this.hoverEntityIdx >= 0)
            this.applyHover(null, -1)
        this.dirty = true
    }
    ...
    private pick(clientX: number, clientY: number): NodeView | null {
        ...
        const { col, row } = pxToCell(
            clientX - r.left - this.panXFrac,
            clientY - r.top - this.panYFrac,
            this.m,
        )
        const idx = nearestCellNode(
            col,
            row,
            this.m,
            this.cellNode,
    ...
    private onPointerMove = (e: PointerEvent) => {
        if (!this.dragging) {
            ...
            this.applyHover(
                nv,
                nv ? -1 : this.pickEntityIdx(e.clientX, e.clientY),
            )
        }
        ...
        if (this.cfg.viewMode === '2d') {
            ...
            this.rx = Math.max(
                -Math.PI / 2 + 0.05,
                Math.min(Math.PI / 2 - 0.05, this.rx),
            )
        }
        this.dirty = true
    }
    ...
    frameSubset(ids: string[]) {
        ...
        if (!is2d)
            t = Math.max(
                t,
                zoomT(this.P * (1 - 1 / Math.max(1, wantMag)), this.P),
            )
        this.goalTarget = c
    ...
    private clickEntity(evIdx: number) {
        ...
        const levelT = isLeaf
            ? FILE_LABEL_REVEAL_T + FILE_LABEL_FADE_SPAN * 0.5
            : bounds[childLevel]
        ...
        const targetT = Math.min(
            levelT,
            this.clusterFitCeilingT(ext.hx, ext.hy),
        )
        const targetPct = snapZoomPercent(
            resolutionPercent(resFromT(targetT, this.maxRes), this.maxRes),
        )
     ...
     */
    private clusterExtent2d(ev: EntityView): {
        cx: number
        cy: number
        hx: number
        hy: number
        ...
        if (!Number.isFinite(minX) || !Number.isFinite(minY))
            return { cx: ev.wx, cy: ev.wy, hx: 1e-6, hy: 1e-6 }
        const cx = (minX + maxX) / 2,
            cy = (minY + maxY) / 2
        return {
            cx,
            cy,
            hx: Math.max(1e-6, maxX - cx),
            hy: Math.max(1e-6, maxY - cy),
        }
    }
    ...
    private clusterFitCeilingT(hx: number, hy: number): number {
        ...
        const maxScale = fitScaleForBox(
            ...
            hx,
            hy,
            CLUSTER_CLICK_MAX_COVER,
        )
        return resolutionT(
            maxScale / Math.max(1e-9, this.fitPxPerWorldFor(true)),
            this.maxRes,
        )
    }
    ...
    getNodesForUI(): NodeForUI[] {
        return this.nodes
            .filter(n => n.node.kind !== 'self')
            .map(n => ({
                id: n.node.id,
                label: n.node.label,
                folder: n.node.folder,
                community: n.node.community,
                communityLabel: n.node.communityLabel,
            }))
    }
