...
const fullGraph = (): GraphData => ({
  ...
  nodes: [
    { id: "center", label: "center", kind: "note", folder: "", community: 1, communityPath: [0, 1] },
    { id: "sibling", label: "sibling", kind: "note", folder: "", community: 1, communityPath: [0, 1] },
    { id: "bridge", label: "bridge", kind: "note", folder: "", community: 2, communityPath: [0, 2] },
    { id: "unrelated", label: "unrelated", kind: "note", folder: "", community: 3, communityPath: [1, 3] },
  ],
  edges: [],
});
...
describe("localLayoutInput", () => {
  it("without a source, produces the pre-Task-5 community-less shape (id-only)", () => {
    ...
    expect(input.edges).toEqual([{ from: "center", to: "sibling" }, { from: "center", to: "bridge" }]);
  });
  ...
  it("a local id missing from the source (e.g. a memory-only node the full graph doesn't carry) falls back to undefined, not a crash", () => {
    const g = local();
    const partialSource: GraphData = { nodes: fullGraph().nodes.filter((n) => n.id !== "bridge"), edges: [] };
  ...
  it("end-to-end: feeding the built input through computeLayout actually changes the settle (community gravity engages)", () => {
    ...
    const g: GraphData = {
      nodes: [
        ...Array.from({ length: 5 }, (_, i) => ({ id: `c${i}`, label: `c${i}`, kind: "note" as const, folder: "" })),
        ...Array.from({ length: 5 }, (_, i) => ({ id: `o${i}`, label: `o${i}`, kind: "note" as const, folder: "" })),
      ],
      edges: [
        ...Array.from({ length: 4 }, (_, i) => ({ from: "c0", to: `c${i + 1}`, kind: "link" as const })),
        ...Array.from({ length: 4 }, (_, i) => ({ from: "o0", to: `o${i + 1}`, kind: "link" as const })),
        { from: "c0", to: "o0", kind: "link" as const }, // the one cross-community bridge
      ],
    };
    const source: GraphData = {
      nodes: [
        ...g.nodes.slice(0, 5).map((n) => ({ ...n, community: 0, communityPath: [0] })),
        ...g.nodes.slice(5).map((n) => ({ ...n, community: 1, communityPath: [1] })),
      ],
      edges: [],
    };
