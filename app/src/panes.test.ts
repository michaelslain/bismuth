...
import { equalize, setContent, findLeafByContent, leafCount } from './panes'
...
test('movePane changes split direction when dropped on a perpendicular edge', () => {
    ...
    const out = movePane(r2, (r2 as Split).a.id, (r2 as Split).b.id, 'down')!
        .root as Split
...
test('focusNeighbor picks the straight-across pane in a 2x2 grid', () => {
    ...
    const rightCol = splitLeaf(
        left.root,
        ((left.root as Split).b as Leaf).id,
        'col',
    ) // right: C / D
...
test('setTabPinned unpinning re-sorts the tab behind the remaining pinned block', () => {
    ...
    expect(
        next.find(t => (t.root as Leaf).content === 'a')!.pinned,
    ).toBeUndefined()
})
...
test('serialize/deserialize round-trips the pinned flag', () => {
    ...
    expect(
        out.find(t => (t.root as Leaf).content === 'a')!.pinned,
    ).toBeUndefined()
})
        ...
        tabs: [
            {
                id: 'u',
                focusId: 'u',
                root: { kind: 'leaf', id: 'u', content: 'a' },
            },
            {
                id: 'p',
                focusId: 'p',
                pinned: true,
                root: { kind: 'leaf', id: 'p', content: 'b' },
            },
        ],
        activeTabId: 'u',
    })
