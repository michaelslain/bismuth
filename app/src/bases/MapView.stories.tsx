...
function placeRow(name: string, note: Record<string, unknown>): Partial<Row> {
  return {
    file: { name, basename: name, path: `places/${name}.md`, folder: "places", ext: "md", size: 512, ctime: 0, mtime: 0, tags: [], links: [] },
    note,
  };
}
  ...
  render: () => {
    ...
    return (
      <div style={{ height: "480px" }}>
        <MapView result={sampleViewResult(PLACES, { views })} config={sampleBaseConfig({ views })} onOpen={() => {}} />
      </div>
    );
  },
};
...
export const CustomFieldsFixedFraming: Story = {
  render: () => {
    ...
    const rows: Partial<Row>[] = [
      ...
      placeRow("Washington DC", { latitude: 38.9072, longitude: -77.0369 }),
    ];
    return (
      <div style={{ height: "480px" }}>
        <MapView result={sampleViewResult(rows, { views })} config={sampleBaseConfig({ views })} onOpen={() => {}} />
      </div>
    );
  },
};
