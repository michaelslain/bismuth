    ...
    onMathReady(() => {
        ...
        for (const el of document.querySelectorAll<HTMLElement>(
            ...
            el.innerHTML = sanitizeHtml(
                renderMath(el.dataset.math ?? '', el.dataset.display === '1'),
            )
        }
    })
}
...
const mathBlockExt: TokenizerAndRendererExtension = {
    ...
    renderer(token) {
        return mathHtml(String(token.text), true)
    },
}
...
const mathInlineExt: TokenizerAndRendererExtension = {
    ...
    tokenizer(src) {
        ...
        if (!m) return undefined
        if (
            (m[1].match(INLINE_MATH_NEWLINE_RE)?.length ?? 0) >
            MAX_INLINE_MATH_NEWLINES
        )
            return undefined
        return { type: 'bismuthMathInline', raw: m[0], text: m[1] }
    },
    renderer(token) {
        return mathHtml(String(token.text), false)
    },
}
...
marked.use({
    renderer: {
        tablecell(token) {
            const listHtml = renderCellListHtml(
                token.text,
                item => marked.parseInline(item, { async: false }) as string,
            )
...
export function renderMarkdown(src: string): string {
    return sanitizeHtml(
        marked.parse(
            iridescentBismuth(imagesOutsideCode(pageBreaksToDivs(src ?? ''))),
            { async: false },
        ) as string,
    )
}
...
function tagsToSpans(src: string): string {
    return src.replace(
        ...
        (_m, pre: string, tag: string) =>
            `${pre}<span class="bismuth-tag">#${escapeHtml(tag)}</span>`,
    )
}
...
function linkifyOutsideCode(src: string): string {
    ...
    return unmaskCode(
        tagsToSpans(memoryRefsToAnchors(wikilinksToAnchors(masked))),
        codes,
    )
}
    ...
    return sanitizeHtml(
        marked.parseInline(
            iridescentBismuth(
                tagsToSpans(
                    memoryRefsToAnchors(
                        wikilinksToAnchors(imagesOutsideCode(src ?? '')),
                    ),
                ),
            ),
            {
