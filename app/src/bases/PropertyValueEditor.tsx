...
export function PropertyValueEditor(props: {
    ...
    const multiKind = () =>
        props.kind.kind === 'multiselect' ? props.kind : null
...
function MultiSelectEditor(props: {
    ...
    const available = createMemo(() =>
        multiselectAvailable(props.options, selected()),
    )
    ...
    const add = (v: string) => {
        if (v && !selected().includes(v)) write([...selected(), v])
    }
