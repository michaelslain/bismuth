...
export const Default: Story = {
  render: () => <ListView result={sampleViewResult()} config={sampleBaseConfig()} />,
};

/** Grouped by `status` — a colored group heading + row count per distinct value. */
export const Grouped: Story = {
  render: () => {
    const views = [{ type: "list" as const, name: "List", groupBy: { property: "status" } }];
    return <ListView result={sampleViewResult(undefined, { views })} config={sampleBaseConfig({ views })} />;
  },
};
