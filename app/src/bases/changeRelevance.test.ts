...
import {
    leafIsFileStructuralOnly,
    hasPropertyFilters,
    changeAffectsView,
    type ViewDeps,
} from './changeRelevance'
...
describe('leafIsFileStructuralOnly', () => {
    ...
    test("string literals aren't mistaken for property identifiers", () => {
        ...
        expect(
            leafIsFileStructuralOnly('file.name == "note.status thing"'),
        ).toBe(true)
    })
...
describe('changeAffectsView', () => {
    ...
    test('DAEMON.md-style content-only edit to an unrelated file is skipped for a file-only base', () => {
        expect(
            changeAffectsView(
                change(['DAEMON.md'], { graph: false, tree: false }),
                fileOnlyDeps(['tasks/a.md']),
            ),
        ).toBe(false)
    })
    test("content-only edit to one of the view's own rows revalidates", () => {
        expect(
            changeAffectsView(
                change(['tasks/a.md'], { graph: false, tree: false }),
                fileOnlyDeps(['tasks/a.md']),
            ),
        ).toBe(true)
    })
    test('structural (tree) and tag/link (graph) changes always revalidate', () => {
        expect(
            changeAffectsView(
                change(['new.md'], { graph: false, tree: true }),
                fileOnlyDeps(['tasks/a.md']),
            ),
        ).toBe(true)
        expect(
            changeAffectsView(
                change(['x.md'], { graph: true, tree: false }),
                fileOnlyDeps(['tasks/a.md']),
            ),
        ).toBe(true)
    })
    test('poll catch-up (no dirty) and not-yet-resolved (null deps) revalidate', () => {
        ...
        expect(
            changeAffectsView(
                change(['x.md'], { graph: false, tree: false }),
                null,
            ),
        ).toBe(true)
    })
    ...
    test('scoped (from:) and composed (base ref:) sources revalidate on content-only edits', () => {
        ...
        expect(
            changeAffectsView(
                change(['other.md'], { graph: false, tree: false }),
                scoped,
            ),
        ).toBe(true)
