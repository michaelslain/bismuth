...
export type PropType =
    'string' | 'number' | 'date' | 'boolean' | 'tag' | 'list' | 'link'
...
export function compileTaskLeaves(tf: TaskFilters): string[] {
    ...
    if (tf.priority && tf.priority !== 'any')
        leaves.push(`priority is ${tf.priority}`)
...
function taskSortLine(tf: TaskFilters): string {
    return tf.sortKey
        ? `sort by ${tf.sortKey}${tf.sortReverse ? ' reverse' : ''}`
        : ''
}
...
export function leafToRow(expr: Expr): NotesRow | null {
    ...
    if (expr.type === 'unary' && expr.op === '!') {
        ...
        if (
            inner.type === 'call' &&
            inner.callee.type === 'member' &&
            inner.callee.name === 'hasTag'
        ) {
        ...
        if (inner.type === 'ident')
            return mk(inner.name, 'is_empty', '', 'string')
    ...
    if (expr.type === 'call' && expr.callee.type === 'member') {
        ...
        if (method === 'hasTag' && arg != null)
            return mk('tags', 'has_tag', arg, 'tag')
        if (method === 'inFolder' && arg != null)
            return mk('file.folder', 'in_folder', arg, 'string')
        if (arg == null) return null
        const prop = obj.type === 'ident' ? obj.name : memberPath(obj)
        if (prop == null) return null
        if (method === 'contains') return mk(prop, 'contains', arg, 'string')
        if (method === 'startsWith')
            return mk(prop, 'starts_with', arg, 'string')
    ...
    if (expr.type === 'binary') {
        ...
        if (
            left.type === 'call' &&
            left.callee.type === 'ident' &&
            left.callee.name === 'date'
        ) {
            ...
            if (prop) {
                ...
                if (op === '<' && rhs != null)
                    return mk(prop, 'date_before', rhs, 'date')
                if (op === '>=' && rhs != null)
                    return mk(prop, 'date_after', rhs, 'date')
            }
        }
        const lProp =
            left.type === 'ident'
...
function dateRhsToVal(e: Expr): string | null {
    ...
    if (e.type === 'binary' && (e.op === '+' || e.op === '-')) {
        if (
            e.left.type === 'call' &&
            e.left.callee.type === 'ident' &&
            e.left.callee.name === 'today'
        ) {
            const amt =
                e.right.type === 'str' ? e.right.value.replace(/d$/, '') : null
...
function reverseWhere(where: string): BuilderState['notes'] {
    ...
    return {
        connective,
        rows: connective === 'and' ? coalesceDateWithin(rows) : rows,
    }
}
...
export function isBuilderRepresentable(body: string): boolean {
    ...
    if (v)
        for (const k of Object.keys(v))
            if (!['type', 'name', 'sort', 'groupBy', 'limit'].includes(k))
                return false
    return true
}
