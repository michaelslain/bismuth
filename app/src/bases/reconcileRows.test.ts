...
function fm(path: string, extra: Record<string, unknown> = {}) {
    ...
    const name = (slash >= 0 ? path.slice(slash + 1) : path).replace(
        /\.md$/,
        '',
    )
    return {
...
function result(groups: ResultGroup[]): ViewResult {
    return {
        view: { type: 'list', name: 'List' } as ViewResult['view'],
        columns: ['file.name'],
        groups,
        summaries: {},
    }
...
describe('rowKey', () => {
    test('task rows key by path+description (line-independent), note rows by path', () => {
        ...
        expect(rowKey(taskRow('a.md', 3, 'todo', ' ', 'Clean fridge'))).toBe(
            'a.md Clean fridge',
        )
        expect(rowKey(taskRow('a.md', 9, 'todo', ' ', 'Clean fridge'))).toBe(
            'a.md Clean fridge',
        )
...
describe('rowsEqual', () => {
    ...
    test('a line-only change does NOT break equality (line is volatile — sinking a done task renumbers siblings)', () => {
        ...
        expect(
            rowsEqual(
                taskRow('a.md', 3, 'todo', ' ', 'Clean fridge'),
                taskRow('a.md', 2, 'todo', ' ', 'Clean fridge'),
            ),
        ).toBe(true)
    })
    test('an mtime-only change does NOT break equality (volatile stat; body freshness is separate)', () => {
        ...
        expect(
            rowsEqual(noteRow('a.md'), {
                file: fm('a.md', { mtime: 5, size: 9 }),
                note: {},
                formula: {},
            }),
        ).toBe(true)
...
describe('reconcileRows', () => {
    ...
    test('completing+sinking a task renumbers survivors but they KEEP identity (the reload-on-check bug)', () => {
        ...
        const prev = [
            taskRow('B.md', 2, 'todo', ' ', 'Time block'),
            clean,
            santaroga,
        ]
        const next = [
            taskRow('B.md', 2, 'todo', ' ', 'Clean fridge'),
            taskRow('B.md', 3, 'todo', ' ', 'Santaroga'),
        ]
        const out = reconcileRows(prev, next)
    ...
    test('duplicate descriptions in one note map to DISTINCT prior objects (no shared identity)', () => {
        ...
        const next = [
            taskRow('a.md', 1, 'todo', ' ', 'dup'),
            taskRow('a.md', 2, 'todo', ' ', 'dup'),
            taskRow('a.md', 3, 'todo', ' ', 'other!'),
        ]
        const out = reconcileRows(prev, next)
