...
function Pane(props: { w: string; h?: string; children: any }) {
  return <div style={{ width: props.w, height: props.h ?? "620px", position: "relative", overflow: "hidden" }}>{props.children}</div>;
}
...
export const CramMode: Story = {
  render: () => (
    <Pane w="1100px">
      <FlashcardsView rows={DECK} config={config} basePath={CRAM_BASE_PATH} onReviewed={() => {}} />
    </Pane>
  ),
};
