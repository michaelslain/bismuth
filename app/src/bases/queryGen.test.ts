...
const row = (
    ...
    op: NotesOp,
    val = '',
    type: NotesRow['type'] = 'string',
): NotesRow => ({ prop, op, val, type })
...
describe('compileNotesRow — each operator -> expr', () => {
    test('string ops', () => {
        expect(compileNotesRow(row('status', 'equals', 'done'))).toBe(
            `status == "done"`,
        )
        expect(compileNotesRow(row('status', 'not_equals', 'done'))).toBe(
            `status != "done"`,
        )
        expect(compileNotesRow(row('title', 'contains', 'rust'))).toBe(
            `title.contains("rust")`,
        )
        expect(compileNotesRow(row('title', 'starts_with', 'A'))).toBe(
            `title.startsWith("A")`,
        )
        expect(compileNotesRow(row('title', 'ends_with', 'z'))).toBe(
            `title.endsWith("z")`,
        )
        expect(compileNotesRow(row('title', 'matches', '^a.*'))).toBe(
            `title.matches("^a.*")`,
        )
    })
    test('number ops emit bare numeric literals', () => {
        expect(compileNotesRow(row('rating', 'gte', '4', 'number'))).toBe(
            'rating >= 4',
        )
        expect(compileNotesRow(row('rating', 'gt', '4', 'number'))).toBe(
            'rating > 4',
        )
        expect(compileNotesRow(row('rating', 'lt', '2', 'number'))).toBe(
            'rating < 2',
        )
        expect(compileNotesRow(row('rating', 'lte', '2', 'number'))).toBe(
            'rating <= 2',
        )
        expect(compileNotesRow(row('rating', 'equals', '5', 'number'))).toBe(
            'rating == 5',
        )
    })
    test('tag + folder ops', () => {
        expect(compileNotesRow(row('tags', 'has_tag', 'book', 'tag'))).toBe(
            `file.hasTag("book")`,
        )
        expect(compileNotesRow(row('tags', 'not_tag', 'book', 'tag'))).toBe(
            `!file.hasTag("book")`,
        )
        expect(
            compileNotesRow(row('file.folder', 'in_folder', 'reading')),
        ).toBe(`file.inFolder("reading")`)
        expect(
            compileNotesRow(row('file.folder', 'folder_is', 'reading')),
        ).toBe(`file.folder == "reading"`)
    })
    test('date ops', () => {
        expect(
            compileNotesRow(row('due', 'date_before', 'today', 'date')),
        ).toBe('date(due) < today()')
        expect(compileNotesRow(row('due', 'date_after', 'today', 'date'))).toBe(
            'date(due) >= today()',
        )
        expect(
            compileNotesRow(row('due', 'date_before', '2026-01-01', 'date')),
        ).toBe(`date(due) < date("2026-01-01")`)
    })
    ...
    test('raw op passes through verbatim', () => {
        expect(compileNotesRow(row('', 'raw', 'rating * 2 > 8'))).toBe(
            'rating * 2 > 8',
        )
    })
})
...
describe('compileNotesWhere — multiple rows joined', () => {
    test('single row is unwrapped', () => {
        expect(
            compileNotesWhere({
                connective: 'and',
                rows: [row('tags', 'has_tag', 'book', 'tag')],
            }),
        ).toBe(`file.hasTag("book")`)
    })
    test('AND of two rows', () => {
        const w = compileNotesWhere({
            connective: 'and',
            rows: [
                row('tags', 'has_tag', 'book', 'tag'),
                row('rating', 'gte', '4', 'number'),
            ],
        })
...
describe('compileTaskLeaves — presets', () => {
    ...
    test('done + overdue + has', () => {
        ...
        ).toEqual(['done', 'due before today'])
        expect(
            compileTaskLeaves({ ...defaultTaskFilters(), due: 'has' }),
        ).toEqual(['due after 1900-01-01'])
        expect(
            compileTaskLeaves({ ...defaultTaskFilters(), due: 'today' }),
        ).toEqual(['due today'])
    })
...
describe('buildQueryBlockBody — tasks -> FLAT', () => {
    test('filters joined with AND, optional from/view/group/limit', () => {
        ...
        expect(body).toBe(
            [
                'tasks: not done AND priority is high',
                'from: [[Google Keep]]',
                'view: cards',
                'group: due',
                'limit: 5',
    ...
    test('list view omits the view: line', () => {
        const state: BuilderState = {
            ...
            source: 'tasks',
            view: 'list',
            tasks: { ...defaultTaskFilters(), status: 'open' },
        }
...
describe('buildQueryBlockBody — base -> FLAT', () => {
    test('of + where + view + limit', () => {
        ...
        expect(buildQueryBlockBody(state)).toBe(
            [
                'of: [[Books]]',
                'where: rating >= 4',
                'view: cards',
...
describe('parseQueryBlockBody — flat tasks/base', () => {
    ...
    test('tasks presets are reversed; unknown leaves go to rawWhere', () => {
        const s = parseQueryBlockBody(
            'tasks: not done AND priority is high AND tag includes foo\nfrom: [[Keep]]\nview: cards',
        )
...
describe('parseQueryBlockBody — notes config', () => {
    ...
    test('un-reversible expression fails open to rawWhere', () => {
        const s = parseQueryBlockBody(
            `source: notes where items.filter(x => x > 2).length > 0\nviews:\n  - type: table`,
        )
        expect(s.source).toBe('notes')
        expect(s.notes.rows).toEqual([])
    ...
    test('mixed AND/OR fails open to rawWhere', () => {
        const s = parseQueryBlockBody(
            `source: notes where a == 1 && b == 2 || c == 3\nviews:\n  - type: table`,
        )
        expect(s.notes.rows).toEqual([])
...
describe('build -> parse -> build idempotence (supported subset)', () => {
    ...
    test('date filters', () => {
        const s = notesState([row('due', 'date_before', 'today', 'date')], {
            view: 'table',
        })
        const once = rebuild(s)
        const twice = rebuild(parseQueryBlockBody(once))
...
describe('review fixes — task sort, date_within, builder-representable', () => {
    ...
    test('date_within round-trips as a single row', () => {
        ...
        expect(back.notes.rows[0]).toMatchObject({
            prop: 'due',
            op: 'date_within',
            val: '7',
        })
    ...
    test("hand-edited 'due before in 30 days' round-trips unchanged (not collapsed to 7)", () => {
        const s = parseQueryBlockBody(
            'tasks: not done AND due before in 30 days',
        )
    ...
    test("hand-edited 'due after 2030-01-01' round-trips unchanged (not collapsed to the has-sentinel)", () => {
        const s = parseQueryBlockBody(
            'tasks: not done AND due after 2030-01-01',
        )
        expect(s.tasks.due).toBe('any')
    ...
    test('isBuilderRepresentable gates rich configs', () => {
        expect(
            isBuilderRepresentable(
                buildQueryBlockBody(
                    notesState([row('status', 'equals', 'open')]),
                ),
            ),
        ).toBe(true)
