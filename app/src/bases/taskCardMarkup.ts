...
function pruneEmptyHeadings(lines: string[]): string[] {
    ...
    for (let i = 0; i < lines.length; i++) {
        if (HEADING_RE.test(lines[i])) {
            ...
            for (
                let j = i + 1;
                j < lines.length && !HEADING_RE.test(lines[j]);
                j++
            ) {
                if (lines[j].trim() !== '') {
                    hasContent = true
 ...
 */
export function buildTaskCardParts(
    content: string,
    mode?: 'body' | 'tasks',
): TaskCardParts {
    ...
    const toMarkup = (e: Entry): string => {
        ...
        return `${indent}- ${markerFor(status, e.line)}${wrapFields(body)}`
    }

    const isResolved = (e: Entry): boolean =>
        e.status !== null && isResolvedStatus(e.status)
    const open = entries.filter(e => !isResolved(e))
    const done = entries.filter(isResolved)

    return {
        openHtml: renderNoteBody(
            pruneEmptyHeadings(open.map(toMarkup)).join('\n'),
        ),
