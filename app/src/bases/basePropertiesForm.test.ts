...
test('parsePropertyOptions: comma, newline, and mixed, trims + dedupes, drops empties', () => {
    expect(parsePropertyOptions('todo, doing, done')).toEqual([
        'todo',
        'doing',
        'done',
    ])
...
test('seedPropertyRows: reads declared list-form properties in order, with type carriers', () => {
    const config: BaseConfig = {
        ...
        properties: {
            status: {
                type: { kind: 'select', options: ['todo', 'done'] },
                hidden: true,
            },
            price: {
                type: { kind: 'number', number: 'currency', unit: 'USD' },
                default: 9.99,
            },
        },
    }
...
test('buildPropertiesYaml: hidden + boolean default coercion', () => {
    ...
    expect(out).toEqual([
        { name: 'done', type: 'boolean', hidden: true, default: false },
    ])
})
