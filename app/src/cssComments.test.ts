...
function commentClosers(css: string): { line: number; before: string; excerpt: string }[] {
...
describe("CSS comments never close early", () => {
  ...
  for (const rel of cssFiles) {
    it(`${rel} closes every comment on whitespace or '*'`, () => {
      ...
      const bad = closers.filter((c) => !/\s/.test(c.before) && c.before !== "*");
      expect(
        bad.map((c) => `${rel}:${c.line} — comment ends mid-prose at ...${c.excerpt}`),
      ).toEqual([]);
    });
  }
});
...
.chat-host { display: flex; height: 100%; --chat-surface-2: var(--surface-2); }`;
  ...
  it("flags the --r-*/ closer", () => {
    const bad = commentClosers(REGRESSION).filter((c) => !/\s/.test(c.before) && c.before !== "*");
  ...
  it("accepts the fixed text", () => {
    const fixed = REGRESSION.replace("--r-*/--rule-*/--shadow-*", "--r-*, --rule-* and --shadow-*");
    const bad = commentClosers(fixed).filter((c) => !/\s/.test(c.before) && c.before !== "*");
    expect(bad).toEqual([]);
  });
});
