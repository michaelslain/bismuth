...
export const duePages = createMemo<DaemonPage[]>(() =>
    dueSorted(pages(), Date.now()),
)
...
export const anyWorking = createMemo<boolean>(() =>
    pages().some(p => p.status === 'working'),
)
...
export async function refreshDaemonPages(onReview?: () => void): Promise<void> {
    ...
    pushToast(
        message,
        onReview ? { label: 'Review', onClick: onReview } : undefined,
    )
}
