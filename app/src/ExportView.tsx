...
const THEME_LABEL: Record<ExportTheme, string> = {
    dark: 'Dark',
    light: 'Light',
}
...
const MODE_ICON: Record<RenderMode, string> = {
    visual: 'LayoutGrid',
    data: 'Table',
}
const SPANS: CalSpan[] = ['month', 'week', '3day', 'day']
const SPAN_LABEL: Record<CalSpan, string> = {
    month: 'Month',
    week: 'Week',
    '3day': '3-day',
    day: 'Day',
...
export function ExportView(props: { path: string }) {
    ...
    const [calSpan, setCalSpan] = createSignal<CalSpan>(
        (loadLs(SPAN_KEY) as CalSpan) || 'month',
    )
    ...
    const [vaultRoot] = createResource(() =>
        api
            .terminalInfo()
            .then(i => i.vault)
            .catch(() => ''),
    ...
    const browseSource = async () => {
        if (!isTauri()) {
            pushToast(
                'Browsing for a file needs the desktop app — type a vault path here in the browser',
            )
            return
        }
    ...
    const browseDest = async () => {
        if (!isTauri()) {
            pushToast(
                'Choosing a folder needs the desktop app — browser exports go to Downloads',
            )
            return
        }
        ...
        try {
            ...
            if (dest && isTauri()) {
                ...
                if (first.via === 'tauri') {
                    ...
                    pushToast(
                        files.length > 1
                            ? `Exported ${files.length} pages → ${dir}`
                            : `Exported → ${first.path}`,
                    )
                } else {
                    pushToast(
