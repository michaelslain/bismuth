...
import {
    insertEmbedsInTableCell,
    tableCellDropTargetAtPoint,
    tableFindHighlight,
    tableSelectionGuard,
...
const editorTheme = EditorView.theme({
    ...
    '&': {
        ...
        color: 'color-mix(in srgb, var(--fg) 88%, var(--text-muted))',
        height: '100%',
    },
...
const yamlHighlight = HighlightStyle.define([
    ...
    {
        tag: [t.separator, t.punctuation, t.bracket, t.brace],
        color: 'var(--faint)',
    },
])
...
function templatePaths(): string[] {
    if (!templatesFetched) {
        ...
        void api
            .templates()
            .then(ts => {
                cachedTemplatePaths = ts.map(t => t.path)
            })
            .catch(() => {})
    }
...
function vaultPaths(): VaultPath[] {
    if (!vaultPathsFetched) {
        ...
        void api
            .tree()
            .then(entries => {
                cachedVaultPaths = entries.map(e => ({
                    path: e.path,
                    kind: e.kind,
                }))
            })
...
const extFromMime = (mime: string): string =>
    MIME_EXT[mime] ?? (mime.split('/')[1] || 'bin')
...
function caretAfterReorder(
    state: EditorState,
    head: number,
    caretLine: Line,
    ch: { from: number; to: number; insert: string },
): number {
    ...
    let occ = 0
    for (let i = 0; i <= caretIdx && i < origLines.length; i++)
        if (origLines[i] === caretLine.text) occ++
    const newLines = ch.insert.split('\n')
    let seen = 0,
        newIdx = -1
    for (let i = 0; i < newLines.length; i++)
        if (newLines[i] === caretLine.text && ++seen === occ) {
            newIdx = i
            break
        }
...
async function dropFilesIntoCell(
    ...
    if (
        !insertEmbedsInTableCell(view, target.from, target.r, target.c, embeds)
    ) {
...
async function embedNativePaths(
    view: EditorView,
    paths: string[],
    notePath: string | null,
): Promise<void> {
...
function revealHeading(view: EditorView, heading: string): boolean {
    const idx = findHeadingLineIndex(
...
export function Editor(props: {
    ...
    const setDraw = (on: boolean): void => {
        ...
        if (on) {
            ...
            if (ae === document.body || wrapper.contains(ae))
                v.contentDOM.focus()
        }
    }
    const onDrawKey = (e: KeyboardEvent): void => {
        ...
        if (!matchesKeybinding(e, settings.keybindings['toggle-draw-mode']))
            return
        e.preventDefault()
        e.stopPropagation()
        setDraw(!drawMode())
    ...
    const save = async (path: string, text: string) => {
        ...
        if (conflicted) {
            pushToast(
                'This note changed elsewhere while you were editing — your edits were kept, but check nearby content for an overwritten external change.',
            )
        }
        diskBase = finalText
    ...
    const onFindKey = (e: KeyboardEvent): void => {
        ...
        if (searchPanelOpen(view.state)) {
            const inp = view.dom.querySelector<HTMLInputElement>(
                '.bismuth-find-input',
            )
            inp?.focus()
            inp?.select()
    ...
    onMount(() => {
        const onTableDrop = (e: Event): void => {
            ...
            const reference =
                d.altKey || settings.attachments.onDrop === 'reference'
            void dropFilesIntoCell(
                view,
                d.files,
                activePath,
                reference,
        ...
        onCleanup(() =>
            window.removeEventListener('bismuth-table-drop', onTableDrop),
        )
    })
    ...
    onMount(() => {
        ...
        onCleanup(() =>
            window.removeEventListener('bismuth-reveal-heading', onReveal),
        )
    })
    ...
    onMount(() => {
        const handleNativeDrop = async (d: NativeDragDetail): Promise<void> => {
            ...
            if (cellTarget) {
                await embedNativePathsIntoCell(
                    v,
                    embeddable,
                    activePath,
    ...
    createEffect(async () => {
        ...
        const autosave = EditorView.updateListener.of(u => {
            ...
            if (view && !isConfigBuffer(path)) {
                ...
                reorderTimer = setTimeout(() => {
                    ...
                    const caretLine = state.doc.lineAt(
                        state.selection.main.head,
                    )
                    const spec = reorderAroundLine(state, caretLine.number) // null when already sorted
                    if (!spec) return
                    const ch = spec.changes as {
                        from: number
                        to: number
                        insert: string
                    }
                    const newHead = caretAfterReorder(
                        state,
                        state.selection.main.head,
                        caretLine,
                        ch,
            ...
            saveTimer = setTimeout(
                async () => {
                    ...
                    if (isMd && view) {
                        ...
                        if (normalized !== cur) {
                            view.dispatch(
                                externalReconcileSpec(cur, normalized),
                            )
                        }
                    }
                    const text = view
                ...
                lastSavedText === undefined
                    ? Math.min(settings.editor.autoSaveDelay, 150)
        ...
        const undoRedoScrollGuard = EditorView.updateListener.of(u => {
            ...
            const repin = () => {
                ...
                v.requestMeasure({
                    ...
                    write: () => {
                        if (view !== v) return
                        if (v.scrollDOM.scrollTop !== keep)
                            v.scrollDOM.scrollTop = keep
                        if (++cycles < 6) repin()
                    },
                })
            }
        ...
        const extensions = isYaml
            ...
            : [
                  ...
                  yamlSchema({
                      ...
                      resolveLink: target =>
                          props.noteNames().some(n => n.label === target),
                  }),
                  ...
                  ...(ed.livePreview
                      ? [
                            taskFold(),
                            foldBlocks(() => path, 'markdown', {
                                hasGutter: ed.lineNumbers,
                            }),
                        ]
                      : []),
                  ...
                  ...(ed.spellcheck || ed.grammarCheck
                      ? [
                            harperSpellcheck({
                                getBodyRange: frontmatterBodyRange,
                                spelling: ed.spellcheck,
                                grammar: ed.grammarCheck,
                            }),
                        ]
                      : []),
              ]
        ...
        const initialSelection =
            pendingCursor != null
                ? { anchor: Math.min(pendingCursor, text.length) }
                : undefined
                ...
                extensions: [
                    ...
                    EditorView.domEventHandlers({
                        ...
                        paste: (e, view) => {
                            ...
                            const paths = filePathsFromTransfer(
                                ce.clipboardData,
                            )
                            if (paths.length) {
                                e.preventDefault()
                            ...
                            for (const f of files) {
                                ...
                                const name =
                                    f.name ||
                                    pastedImageName(extFromMime(f.type))
                        ...
                        mousedown: (e, view) => {
                            ...
                            if (!tgt?.closest?.('.cm-link, .cm-wikilink'))
                                return false
                            ...
                            for (const m of line.text.matchAll(
                                ...
                                const s = line.from + (m.index ?? 0),
                                    en = s + m[0].length
                                if (pos >= s && pos <= en) {
                                    const { target, heading } = parseWikilink(
                                        m[1],
                                    )
                            ...
                            for (const m of line.text.matchAll(
                                new RegExp(MEMORY_REF_RE.source, 'g'),
                            )) {
                                const s2 =
                                    line.from + (m.index ?? 0) + m[1].length
                                const en = s2 + 2 + m[2].length
                                if (pos >= s2 && pos <= en) {
                                    const resolved = resolveMemorySlug(
                                        m[2],
                                        props.memoryNames(),
                                    )
                                    window.dispatchEvent(
                            ...
                            for (const { start, end, url } of findBareUrls(
                                line.text,
                            )) {
                                if (
                                    pos >= line.from + start &&
        ...
        if (restore != null && restore > 0) {
            ...
            const repin = () => {
                ...
                v.requestMeasure({
                    ...
                    write: () => {
                        if (view !== v) return
                        if (Math.abs(v.scrollDOM.scrollTop - restore) > 1)
                            v.scrollDOM.scrollTop = restore
                        if (++cycles < 6) repin()
                    },
                })
