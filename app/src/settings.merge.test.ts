...
describe('mergeServerSettings', () => {
    ...
    it("ignores wrong-typed values (editorFontSize: 'big' falls back to default)", () => {
        const out = mergeServerSettings({
            appearance: { editorFontSize: 'big' },
        })
        expect(out.appearance.editorFontSize).toBe(
            DEFAULTS.appearance.editorFontSize,
        )
    })
    ...
    it('keeps the default toolbar when the server value is missing or not an array', () => {
        ...
        expect(mergeServerSettings({ toolbar: 'nope' }).toolbar).toEqual(
            DEFAULTS.toolbar,
        )
    })
})
