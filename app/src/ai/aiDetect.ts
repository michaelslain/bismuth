...
export function aiProb(
    results: Array<{ label: string; score: number }>,
): number {
    const ai = results.find(
        r =>
            /(^|_)1$/.test(r.label) ||
            /\b(ai|fake|machine|generat|llm)/i.test(r.label),
    )
    if (ai) return ai.score
    const human = results.find(
        r => /(^|_)0$/.test(r.label) || /human|real/i.test(r.label),
    )
...
export async function detectAiScore(
    ...
    const classify = await getPipeline(pct =>
        onProgress?.({ phase: 'load', pct }),
    )
    const probs: number[] = []
