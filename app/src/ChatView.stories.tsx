...
function installFakeChatSocket(frames: readonly ChatFrame[]): () => void {
  ...
  globalThis.WebSocket = makeFakeChatSocketClass(frames) as unknown as typeof WebSocket;
  return () => {
...
const SESSION_OPEN: ChatFrame[] = [
  { type: "manifest", manifest: MANIFEST },
  {
    type: "models",
    models: [
      { value: "opus", label: "Opus", description: "Most capable", effortLevels: ["low", "medium", "high"] },
      { value: "sonnet", label: "Sonnet", description: "Balanced", effortLevels: ["low", "medium", "high"] },
    ],
  },
];
  ...
  render: () => (
    <div style={{ height: STORY_H, width: "100%" }}>
      <FakeSocketChat
        ...
        frames={[
          ...SESSION_OPEN,
          { type: "user-message", text: "What's still open in my daily note?" },
          { type: "thinking", text: "The daily note lives under `journal/`. " },
          { type: "thinking", text: "I'll read today's file and pull out the unchecked tasks." },
          ...
          { type: "result", isError: false, numTurns: 2, costUsd: 0.0314 },
          { type: "context", percentage: 14, totalTokens: 28_400, maxTokens: 200_000 },
          { type: "done" },
        ]}
      />
    </div>
  ...
  render: () => (
    <div style={{ height: STORY_H, width: "100%" }}>
      <FakeSocketChat
        ...
        frames={[
          ...SESSION_OPEN,
          { type: "user-message", text: "Run the app tests and show me anything failing." },
          { type: "assistant-text", text: "Running the suite now." },
          { type: "tool-use", id: "t1", name: "Bash", kind: "execute", input: { command: "bun test app" } },
          { type: "tool-result", id: "t1", content: "2904 pass\n0 fail\nRan 2904 tests across 199 files.", isError: false },
          { type: "tool-use", id: "t2", name: "Read", kind: "read", input: { file_path: "app/src/does-not-exist.ts" } },
          { type: "tool-result", id: "t2", content: "ENOENT: no such file or directory", isError: true },
          { type: "assistant-text", text: "The suite is green. Let me grep for the stale import that path came from." },
          { type: "tool-use", id: "t3", name: "Grep", kind: "search", input: { pattern: "does-not-exist" } },
  ...
  render: () => (
    <div style={{ height: STORY_H, width: "100%" }}>
      <FakeSocketChat
        ...
        frames={[
          ...SESSION_OPEN,
          { type: "user-message", text: "Clean up the build output and export the vault." },
          { type: "assistant-text", text: "I'll remove the build directory first — this needs your approval." },
          { type: "permission", id: "p1", toolName: "Bash", input: { command: "rm -rf app/dist" } },
          {
            type: "question",
            id: "q1",
            questions: [
              {
                ...
                multiSelect: false,
                options: [
                  { label: "Markdown", description: "Plain .md files, wikilinks preserved" },
                  { label: "HTML", description: "Styled, self-contained pages" },
                  { label: "PDF", description: "One document per note" },
                ],
              },
              {
                ...
                multiSelect: true,
                options: [
                  { label: "journal", description: "Daily notes" },
                  { label: "reading", description: "Book notes and quotes" },
                  { label: "thoughts", description: "Everything else" },
                ],
              },
            ],
          },
  ...
  render: () => (
    <div style={{ height: STORY_H, width: "100%" }}>
      <FakeSocketChat
        ...
        frames={[
          ...SESSION_OPEN,
          { type: "user-message", text: "Summarise every note tagged #reading." },
          { type: "assistant-text", text: "Gathering the tagged notes…" },
          { type: "error", code: "error", message: "The session ended unexpectedly (exit code 1)." },
        ]}
      />
    </div>
  ),
