...
export function inProcessTransport(backend: LocalBackend): Transport {
    return {
        getJson: <T>(path: string) =>
            backend.dispatch('GET', path) as Promise<T>,
        getText: (path: string) =>
            backend.dispatch('GET', path) as Promise<string>,
        post: (path: string, body: unknown) =>
            backend.dispatch('POST', path, body).then(asResponse),
        put: (path: string, body: unknown) =>
            backend.dispatch('PUT', path, body).then(asResponse),
        postJson: <T>(path: string, body: unknown) =>
            backend.dispatch('POST', path, body) as Promise<T>,
        ...
        writeFileChecked: async (
            ...
            const current = (await backend.dispatch(
                'GET',
        ...
        convertHeic: async () => {
            throw new Error(
                'convertHeic is not supported by the in-process backend yet',
            )
        },
        stageTmpFile: async () => {
            throw new Error(
                'stageTmpFile is not supported by the in-process backend yet',
            )
        },
