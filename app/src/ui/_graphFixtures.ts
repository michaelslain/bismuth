...
import { SELF_NODE_ID, type GraphData, type GraphEdge, type GraphNode } from "../../../core/src/graph";
...
export function sampleGraphNode(overrides: Partial<GraphNode> = {}): GraphNode {
  return { id: "sample-note", label: "Sample Note", kind: "note", folder: "", ...overrides };
}
...
export function sampleGraphData(noteCount = 8): GraphData {
  ...
  const titles = Array.from({ length: n }, (_, i) => NOTE_TITLES[i % NOTE_TITLES.length]);
  const noteIds = titles.map((t, i) => `${slug(t)}-${i}`);

  const nodes: GraphNode[] = [
    { id: SELF_NODE_ID, label: "You", kind: "self" },
    ...titles.map((t, i): GraphNode => ({ id: noteIds[i], label: t, kind: "note", folder: "" })),
    ...TAG_NAMES.map((t): GraphNode => ({ id: `tag:${t}`, label: `#${t}`, kind: "tag" })),
  ];

  const edges: GraphEdge[] = [];
  for (let i = 0; i < noteIds.length - 1; i++) edges.push({ from: noteIds[i], to: noteIds[i + 1], kind: "link" });
  noteIds.forEach((id, i) => edges.push({ from: id, to: `tag:${TAG_NAMES[i % TAG_NAMES.length]}`, kind: "tag" }));
