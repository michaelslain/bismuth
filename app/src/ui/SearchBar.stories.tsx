 ...
 *  Escape closes) rather than `onEnter`. Proves the input's keyboard behaviour survives being
 *  wrapped in trailing controls, and that `inputRef`/`aria-label` reach the real `<input>`. */
export const WithTrailingControls: Story = {
    render: () => {
        ...
        return (
            <Show
                when={!closed()}
                fallback={<div data-testid="closed">closed</div>}
            >
                <div style={{ width: '320px' }}>
                    <SearchBar
                        value={query()}
