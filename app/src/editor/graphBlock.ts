...
class GraphBlockWidget extends WidgetType {
    ...
    private reveal = () => {
        ...
        if (!loc) return
        view.dispatch({
            effects: toggleGraphSource.of(loc.index),
            selection: { anchor: loc.range.bodyFrom },
        })
        view.focus()
    }
    ...
    graphRanges(state).forEach((r, i) => {
        if (revealed.has(i)) {
            ...
            for (let ln = openLine; ln <= closeLine; ln++) {
                ...
                deco.push(
                    (isBody
                        ? numberedLine('cm-graphblock-body', ln - openLine)
                        : graphBodyLine
                    ).range(line.from),
                )
            }
            ...
            deco.push(
                Decoration.replace({
                    widget: new GraphBlockWidget(r.body),
                    block: true,
                }).range(r.from, r.to),
            )
        }
...
const collapseOnClickOutside = EditorView.domEventHandlers({
    mousedown(e, view) {
        ...
        if (toClose.length)
            view.dispatch({
                effects: toClose.map(i => toggleGraphSource.of(i)),
            })
