...
test('tokenizeInline keeps plain markdown as a single md segment', () => {
    expect(tokenizeInline('**bold** and `code`')).toEqual([
        { type: 'md', raw: '**bold** and `code`' },
    ])
})
...
test('tokenizeInline splits a wikilink alias on the pipe', () => {
    expect(tokenizeInline('[[target|shown]]')).toEqual([
        { type: 'wikilink', target: 'target', alias: 'shown' },
    ])
})
...
test('tokenizeInline does NOT treat currency-ish $ as math', () => {
    ...
    expect(tokenizeInline('costs $5 today')).toEqual([
        { type: 'md', raw: 'costs $5 today' },
    ])
    // Space just inside the opener disqualifies it.
    expect(tokenizeInline('a $ x $ b')).toEqual([
        { type: 'md', raw: 'a $ x $ b' },
    ])
})
...
test('renderInlineMarkdown renders bold / italic / bold-italic', () => {
    ...
    expect(renderInlineMarkdown('***bi***')).toBe(
...
test('renderInlineMarkdown renders a markdown link', () => {
    expect(renderInlineMarkdown('[txt](https://e.com)')).toBe(
        '<a href="https://e.com">txt</a>',
    )
})
...
test('tokenizeInline supports nested + hyphenated tags', () => {
    expect(tokenizeInline('#area/health-log')).toEqual([
        { type: 'tag', name: 'area/health-log' },
    ])
})

test('renderInlineMarkdown renders a #tag as the themed .cm-tag chip', () => {
    expect(renderInlineMarkdown('a #work b')).toBe(
        'a <span class="cm-tag" data-tag="work">#work</span> b',
    )
})
...
test('renderInlineMarkdown renders a tag at the very start of a cell', () => {
    expect(renderInlineMarkdown('#todo item')).toBe(
        '<span class="cm-tag" data-tag="todo">#todo</span> item',
    )
})
...
test('iridescentBismuthCell leaves text with no whole-word bismuth untouched', () => {
    expect(iridescentBismuthCell('bismuths and embismuth')).toBe(
        'bismuths and embismuth',
    )
...
test('iridescentBismuthCell skips bismuth inside a code span', () => {
    expect(iridescentBismuthCell('run `bismuth serve` now')).toBe(
        'run `bismuth serve` now',
    )
})

test('iridescentBismuthCell skips bismuth inside a markdown link + bare URL', () => {
    expect(iridescentBismuthCell('[bismuth](https://bismuth.io)')).toBe(
        '[bismuth](https://bismuth.io)',
    )
    expect(iridescentBismuthCell('see https://x/bismuth here')).toBe(
        'see https://x/bismuth here',
    )
})

test('iridescentBismuthCell skips bismuth inside a #tag', () => {
    expect(iridescentBismuthCell('tagged #bismuth today')).toBe(
        'tagged #bismuth today',
    )
})
...
test('iridescentBismuthCell does not corrupt numbers or stray punctuation around a mask', () => {
    ...
    expect(iridescentBismuthCell('costs 5 for `bismuth`')).toBe(
        'costs 5 for `bismuth`',
    )
})

test('renderInlineMarkdown renders bismuth in a cell as the shared gradient span', () => {
    expect(renderInlineMarkdown('love bismuth')).toBe(
        'love <span class="bismuth-word">bismuth</span>',
    )
})
...
test('tokenizeInline detects a markdown image embed but leaves a plain link alone', () => {
    expect(tokenizeInline('![alt](cat.png)')).toEqual([
        { type: 'embed', wiki: false, target: 'cat.png', alt: 'alt' },
    ])
    // A bare `[text](url)` link is NOT an embed (no `!`) — stays an md run for `marked`.
    expect(tokenizeInline('[text](https://e.com)')).toEqual([
        { type: 'md', raw: '[text](https://e.com)' },
    ])
})
...
test('renderInlineMarkdown renders a pdf embed as an iframe off the asset URL', () => {
    const html = renderInlineMarkdown('![[doc.pdf#page=2]]', {
        assetUrl: asset,
    })
...
test('#58 control: plain emphasis (no spanned segment) still renders via marked, unchanged', () => {
    ...
    expect(renderInlineMarkdown('***bi***')).toContain(
        '<em><strong>bi</strong></em>',
    )
