...
function buildHtmlBlocks(state: EditorState): DecorationSet {
    ...
    for (const b of scanHtmlBlocks(doc)) {
        ...
        deco.push(
            Decoration.replace({
                widget: new HtmlBlockWidget(html, b.from),
                block: true,
            }).range(b.from, b.to),
        )
    }
...
export function groupInlineHtml(
    tags: InlineTag[],
...
export function inlineHtmlTags(
    state: EditorState,
    from: number,
    to: number,
): InlineTag[] {
...
export function pushInlineHtml(
    ...
    for (const s of spans) {
        ...
        deco.push(
            Decoration.replace({
                widget: new HtmlInlineWidget(
                    state.doc.sliceString(s.from, s.to),
                ),
            }).range(s.from, s.to),
        )
    }
