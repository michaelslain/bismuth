...
describe('findBismuthWords', () => {
    ...
    test('punctuation / hyphens are boundaries, so an adjacent word still matches', () => {
        expect(findBismuthWords('bismuth-crystal')).toEqual([
            { from: 0, to: 7 },
        ])
...
describe('wrapBismuthWords', () => {
    ...
    test('leaves partial-word matches untouched', () => {
        expect(wrapBismuthWords('bismuths embismuth', w => `<b>${w}</b>`)).toBe(
            'bismuths embismuth',
        )
    })
})
...
describe('bismuthWrapSource', () => {
    ...
    test('wraps a whole-word bismuth in bare prose', () => {
        expect(bismuthWrapSource('a bismuth b', PROTECT, wrap)).toBe(
            'a <x>bismuth</x> b',
        )
    })
    ...
    test('never touches a protected region (code span / URL)', () => {
        ...
        expect(bismuthWrapSource('https://bismuth.io', PROTECT, wrap)).toBe(
            'https://bismuth.io',
        )
    })

    test('wraps prose but not a protected copy in the same string', () => {
        expect(bismuthWrapSource('bismuth vs `bismuth`', PROTECT, wrap)).toBe(
            '<x>bismuth</x> vs `bismuth`',
        )
    })

    test('returns the input untouched when there is no whole-word match (cheap gate)', () => {
        expect(bismuthWrapSource('bismuths and prose', PROTECT, wrap)).toBe(
            'bismuths and prose',
        )
    })

    test('restoration never corrupts a bare number sitting next to a masked span', () => {
        expect(bismuthWrapSource('costs 5 for `bismuth`', PROTECT, wrap)).toBe(
            'costs 5 for `bismuth`',
        )
    })
})
