...
class EmbedWidget extends WidgetType {
    ...
    toDOM(): HTMLElement {
        ...
        if (s.kind === 'image' && s.src) {
            ...
            img.addEventListener('error', () =>
                this.fail(wrap, `can't load image: ${s.alt ?? ''}`),
            )
            ...
            if (this.resizable) {
                ...
                this.makeAspectResizable(wrap, () =>
                    img.naturalWidth ? img.naturalWidth / img.naturalHeight : 0,
                )
                ...
                if (this.inline && !s.width && !s.height)
            ...
            v.src = s.src
            wrap.appendChild(v)
            if (this.resizable && s.width) {
                wrap.style.width = `${s.width}px`
                if (s.height) wrap.style.height = `${s.height}px`
            }
    ...
    private makeResizable(wrap: HTMLElement): void {
        ...
        const onUp = () => {
            ...
            if (Math.abs(w - startW) > 1 || Math.abs(h - startH) > 1)
                this.ctx.commitResize(wrap, `${w}x${h}`)
        }
    ...
    private renderNote(wrap: HTMLElement, target: string): void {
        ...
        void api
            ...
            .then(text => {
                if (!text) {
                    this.fail(wrap, `note not found: ${target}`)
                    return
                }
...
function scanEmbeds(state: EditorState): EmbedToken[] {
    ...
    while ((m = EMBED_RE.exec(scan))) {
        ...
        const spec = wiki
            ? specForWikiEmbed(m[1], api.assetUrl)
            : specForMarkdownImage(m[3], m[2], api.assetUrl)
        if (!spec) continue
        ...
        const listIndent =
            standalone && line.text.trim() !== m[0].trim() ? prefix : ''
        tokens.push({
            from,
            to,
            lineFrom: line.from,
    ...
    for (const tk of tokens) {
        if (tk.standalone) {
            ...
            if (tk.listIndent) {
                ...
                decos.push(
                    Decoration.replace({
                        widget: new EmbedWidget(tk.spec, false, resizable, ctx),
                        block: false,
                    }).range(tk.from, tk.to),
                )
            } else {
            ...
            decos.push(
                Decoration.replace({
                    widget: new EmbedWidget(tk.spec, true, false, ctx),
                    block: false,
                }).range(tk.from, tk.to),
            )
        }
...
const embedRevealOnClick = EditorView.domEventHandlers({
    mousedown(event, view) {
        ...
        const wrap = target.closest<HTMLElement>(
            '.cm-embed-block, .cm-embed-inline',
        )
...
export function embedBlock(getNotes: () => NoteCandidate[]): Extension {
    ...
    const ctx: EmbedCtx = {
        getNotes,
        commitResize: (dom, size) => {
            if (view) commitEmbedSize(view, dom, size)
        },
    }
    ...
    const field = StateField.define<{
        ...
        create(state) {
            ...
            return {
                tokens,
                deco: decorationsFor(tokens, state.selection.main.head, ctx),
            }
        },
        update(value, tr) {
            if (tr.docChanged) {
