...
test('Properties is offered on line 1 but not on a later line', () => {
    ...
    const doc = 'x\ny\n/prop'
    expect(run(doc, doc.length)!.options.map(o => o.label)).not.toContain(
        'Properties',
    )
})
...
test('apply inserts the snippet text and places the caret at $0', () => {
    ...
    ;(h1.apply as (v: unknown, c: unknown, f: number, t: number) => void)(
        fakeView,
        h1,
        0,
        1,
...
test('apply for a re-trigger item inserts the [[]] skeleton with caret inside', () => {
    ...
    const fakeView = {
        dispatch: (t: unknown) => {
            calls.push(t as { changes?: unknown })
        },
    }
    try {
        ;(link.apply as (v: unknown, c: unknown, f: number, t: number) => void)(
            fakeView,
            link,
            0,
            1,
        ...
        /* startCompletion on a fake view — irrelevant to the insert assertion */
    }
