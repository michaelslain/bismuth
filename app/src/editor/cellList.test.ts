...
test('parseCellList detects a <br>-separated unordered list (- and *)', () => {
    expect(parseCellList('- a<br>- b<br>- c')).toEqual({
        ordered: false,
        items: ['a', 'b', 'c'],
    })
    expect(parseCellList('* one<br>* two')).toEqual({
        ordered: false,
        items: ['one', 'two'],
    })
})
...
test('parseCellList detects an ordered list (1. and 2))', () => {
    expect(parseCellList('1. a<br>2. b')).toEqual({
        ordered: true,
        items: ['a', 'b'],
    })
    expect(parseCellList('1) a<br>2) b<br>3) c')).toEqual({
        ordered: true,
        items: ['a', 'b', 'c'],
    })
})
...
test('parseCellList accepts <br/> and <br /> and is case-insensitive', () => {
    expect(parseCellList('- a<br/>- b')).toEqual({
        ordered: false,
        items: ['a', 'b'],
    })
    expect(parseCellList('- a<BR />- b')).toEqual({
...
test('parseCellList detects a COLLAPSED unordered list (dropped <br> — the real stored shape)', () => {
    expect(parseCellList('- a- b- c')).toEqual({
        ordered: false,
        items: ['a', 'b', 'c'],
    })
    expect(parseCellList('- milk- eggs- bread')).toEqual({
        ordered: false,
        items: ['milk', 'eggs', 'bread'],
    })
    expect(parseCellList('- cost-benefit- risk')).toEqual({
        ordered: false,
        items: ['cost-benefit', 'risk'],
    })
})
...
test('parseCellList does NOT false-split emphasis (`* ` collides with a bullet, so glued split is `-`-only)', () => {
    ...
    expect(parseCellList('- **bold** here- next')).toEqual({
        ordered: false,
        items: ['**bold** here', 'next'],
    })
...
test('parseCellList detects a COLLAPSED ordered list (dropped <br>)', () => {
    expect(parseCellList('1. a2. b3. c')).toEqual({
        ordered: true,
        items: ['a', 'b', 'c'],
    })
    expect(parseCellList('1) a2) b')).toEqual({
        ordered: true,
        items: ['a', 'b'],
    })
})
...
test('parseCellList ignores blank segments (trailing / doubled <br>)', () => {
    expect(parseCellList('- a<br>- b<br>')).toEqual({
        ordered: false,
        items: ['a', 'b'],
    })
    expect(parseCellList('- a<br><br>- b')).toEqual({
        ordered: false,
        items: ['a', 'b'],
    })
})
...
test('parseCellList allows an empty item (bare marker)', () => {
    expect(parseCellList('- a<br>-<br>- c')).toEqual({
        ordered: false,
        items: ['a', '', 'c'],
    })
})
...
test('parseCellList keeps inline markdown in item text (rendered later)', () => {
    expect(parseCellList('- **bold**<br>- [[Note]]')).toEqual({
        ordered: false,
        items: ['**bold**', '[[Note]]'],
    })
})
