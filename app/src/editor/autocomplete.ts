...
function inCode(context: CompletionContext): boolean {
    ...
    >
    for (
        let n: ParseNode | null = syntaxTree(context.state).resolveInner(
            context.pos,
            -1,
        );
        n;
        ...
        if (
            ['InlineCode', 'FencedCode', 'CodeBlock', 'CodeText'].includes(
                n.name,
            )
        )
            return true
...
function wikilinkHeadingSource(
    ...
    const headingsCache = new Map<
        string,
        Promise<ReturnType<typeof parseHeadings>>
    >()
...
export function memoryRefSource(
...
function atMentionSource(
    ...
    return (context: CompletionContext): CompletionResult | null => {
        ...
        const options: Completion[] = rankFileCandidates(
            getFiles(),
            match.query,
        )
            .slice(0, 50)
...
export function matchPropertyKeyPrefix(
    textBefore: string,
...
export function matchTagListItem(
    textBefore: string,
): { from: number; query: string } | null {
...
function propertyKeySource(
    ...
    return (context: CompletionContext): CompletionResult | null => {
        ...
        if (!match) return null
        const from = line.from + match.from
        const options: Completion[] = keySuggestions(
            getSchema(),
            match.query,
        ).map(name => ({
            label: name,
...
function enumValueSource(
    ...
    return (context: CompletionContext): CompletionResult | null => {
        ...
        const options: Completion[] = valueSuggestions(type, m[2].trim()).map(
            v => ({ label: v }),
        )
        if (options.length === 0) return null
...
const wikilinkAutoTrigger = EditorView.updateListener.of(update => {
    ...
    if (
        matchWikilinkPrefix(textBefore) ||
        matchAtMentionPrefix(textBefore) ||
        matchMemoryRefPrefix(textBefore)
    ) {
    ...
    return [
        ...
        autocompletion({
            ...
            override: [
                ...
                ...(opts.getFiles
                    ? [atMentionSource(opts.getFiles, opts.onFileMention)]
                    : []),
