...
describe('parseDateValue', () => {
    test('date only', () => {
        expect(parseDateValue('2026-06-16')).toEqual({
            date: '2026-06-16',
            time: '',
        })
    })
    test('datetime with T separator', () => {
        expect(parseDateValue('2026-06-16T14:30')).toEqual({
            date: '2026-06-16',
            time: '14:30',
        })
    })
    test('datetime with space separator', () => {
        expect(parseDateValue('2026-06-16 09:05')).toEqual({
            date: '2026-06-16',
            time: '09:05',
        })
    })
...
describe('composeDateValue', () => {
    test('date kind → bare date (time ignored)', () => {
        expect(composeDateValue('date', '2026-06-16', '14:30')).toBe(
            '2026-06-16',
        )
    })
    test('datetime kind with a time → joined with T', () => {
        expect(composeDateValue('datetime', '2026-06-16', '14:30')).toBe(
            '2026-06-16T14:30',
        )
    })
    test('datetime kind without a time → bare date', () => {
        expect(composeDateValue('datetime', '2026-06-16', '')).toBe(
            '2026-06-16',
        )
    })
