...
test('wikilinkVisibleRange: a path reveals only the basename', () => {
    ...
    expect(wikilinkVisibleRange('reading/quotes/My Note', 0)).toEqual({
        from: 17,
        to: 24,
    })
})
...
test('wikilinkVisibleRange: an alias reveals the alias text', () => {
    ...
    expect(wikilinkVisibleRange('My Note|Alias', 0)).toEqual({
        from: 10,
        to: 15,
    })
})
...
test('wikilinkVisibleRange: a heading is excluded from the revealed name', () => {
    expect(wikilinkVisibleRange('My Note#Section', 0)).toEqual({
        from: 2,
        to: 9,
    })
})
...
test('resolveNotePath: a full path target resolves to itself', () => {
    expect(resolveNotePath('reading/quotes/My Note', NOTES)).toBe(
        'reading/quotes/My Note',
    )
})
...
test('wikilinkOpenPath: a resolved note gets its id .md-suffixed', () => {
    expect(wikilinkOpenPath('My Note', 'reading/quotes/My Note')).toBe(
        'reading/quotes/My Note.md',
    )
})
...
test('wikilinkOpenPath: an unresolved image target opens as-is (no .md appended)', () => {
    ...
    ).toBe('attachments/Screenshot 2026-07-07 at 12.51.05 AM.png')
})
...
test('matches a query containing spaces', () => {
    expect(matchWikilinkPrefix('[[My Note')).toEqual({
        from: 2,
        query: 'My Note',
    })
})
...
test('buildInsert appends closing brackets when none ahead', () => {
    expect(buildInsert('Foo', false)).toEqual({
        insert: 'Foo]]',
        cursorOffset: 5,
    })
})
...
test('matchWikilinkHeadingPrefix: empty heading right after the #', () => {
    expect(matchWikilinkHeadingPrefix('[[Note#')).toEqual({
        target: 'Note',
        heading: '',
        from: 7,
    })
...
test('parseHeadings: strips trailing closing #s', () => {
    expect(parseHeadings('## Closed ##')).toEqual([
        { level: 2, text: 'Closed' },
    ])
})
