...
test('matchEmojiPrefix: closing colon is consumed but not part of the query', () => {
    ...
    expect(matchEmojiPrefix(':smile:')).toEqual({
        from: 0,
        to: 7,
        query: 'smile',
    })
...
test('matchEmojiPrefix: allows underscores (special-char shortcodes)', () => {
    expect(matchEmojiPrefix(':arrow_right')).toEqual({
        from: 0,
        to: 12,
        query: 'arrow_right',
    })
...
test('rankEmoji: empty query returns the popular entries present, in popularity order', () => {
    ...
    expect(rankEmoji(FIXTURE, '').map(e => e.name)).toEqual([
        'grinning_face',
...
test('searchEmoji: a misspelled name still surfaces the right family', () => {
    ...
    expect(
        searchEmoji('smlie')
            .slice(0, 5)
            .some(e => smiles.has(e.char)),
    ).toBe(true)
    expect(
        searchEmoji('hart').some(e => e.char === '❤️' || e.char === '♥️'),
    ).toBe(true)
    expect(
        searchEmoji('prty')
            .slice(0, 5)
