...
test('keyword match (todo → To-do, frontmatter → Properties)', () => {
    ...
    expect(filterSlashItems(SLASH_ITEMS, 'frontmatter')[0].id).toBe(
        'properties',
    )
})
...
test('prefix tier keeps declared order on ties (head → h1, h2, h3)', () => {
    expect(
        filterSlashItems(SLASH_ITEMS, 'head')
            .map(i => i.id)
            .slice(0, 3),
    ).toEqual(['h1', 'h2', 'h3'])
})
...
test('divider snippet is preceded by a blank line (avoids a setext-heading underline)', () => {
    expect(
        SLASH_ITEMS.find(i => i.id === 'divider')?.snippet.startsWith('\n'),
    ).toBe(true)
})
...
test('page break inserts a lone HTML-comment marker', () => {
    ...
    expect(parseSnippet(item!.snippet)).toEqual({
        text: '<!-- pagebreak -->\n',
        caret: 19,
    })
})
...
test('page break is findable by its keywords (page / pdf / print)', () => {
    for (const q of ['page', 'break', 'pdf', 'print']) {
        expect(
            filterSlashItems(SLASH_ITEMS, q).some(i => i.id === 'pagebreak'),
        ).toBe(true)
    }
})
