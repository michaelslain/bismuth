...
export function scanMultilineInlineMath(text: string): MathSpan[] {
    ...
    while (i < n) {
        ...
        if (close < 0) {
            i++
            continue
        } // no valid close in window → leave `$` literal
        ...
        let closeLi = li
        while (closeLi + 1 < lines.length && close >= lineStart[closeLi + 1])
            closeLi++
...
function findClose(
    ...
    for (let j = open + 1; j < text.length; j++) {
        ...
        if (c === '\n') {
            ...
            if (
                fenced[li] ||
                BLANK_LINE.test(lines[li]) ||
                MATH_FENCE.test(lines[li])
            )
                return -1
            continue
        }
        if (c === '\\') {
            j++
            continue
        } // escaped char — never a delimiter
        if (c === '$') {
...
function build(state: EditorState): {
            ...
            for (let j = i + 1; j <= doc.lines; j++) {
                ...
                if (MATH_FENCE.test(jLine.text)) {
                    ...
                    if (!cursorInside) {
                        ...
                        decos.push(
                            texDelim.range(line.from, line.to),
                            texDelim.range(jLine.from, jLine.to),
                        )
                        if (line.to > line.from)
                            decos.push(mathSrcMark.range(line.from, line.to))
                        if (jLine.to > jLine.from)
                            decos.push(mathSrcMark.range(jLine.from, jLine.to))
                        for (let k = i + 1; k < j; k++) {
                            const kl = doc.line(k)
                            if (kl.to > kl.from)
                                decos.push(mathSrcMark.range(kl.from, kl.to))
                            for (const d of latexTokenDecorations(
                                kl.from,
                                kl.text,
                            ))
                                decos.push(d)
    ...
    const spans = scanMultilineInlineMath(doc.toString()).filter(
        s => !overlapsAny(s.from, s.to, claimed),
    )
    for (const span of spans) {
        ...
        if (!cursorInside) {
            const d = Decoration.replace({
                widget: new InlineMathWidget(inner),
            ...
            decos.push(
                texDelim.range(span.from, span.from + 1),
                texDelim.range(span.to - 1, span.to),
            )
