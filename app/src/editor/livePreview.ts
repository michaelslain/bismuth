...
type ParseNode = NonNullable<
    ReturnType<ReturnType<typeof syntaxTree>['resolveInner']>['parent']
>
...
function listDepth(state: EditorState, pos: number, indent: string): number {
    let count = 0
    for (
        let n: ParseNode | null = syntaxTree(state).resolveInner(pos, 1);
        n;
        n = n.parent
    ) {
...
type CheckboxDom = HTMLElement & {
    __dispose?: () => void
    __setStatus?: Setter<TaskStatus>
}
...
class CodeHeaderWidget extends WidgetType {
    ...
    toDOM(): HTMLElement {
        ...
        wrap.__dispose = render(
            () => CodeHeader({ lang: this.lang, body: this.body }),
            wrap,
        )
        return wrap
    }
...
class MathWidget extends WidgetType {
    ...
    eq(other: MathWidget): boolean {
        return (
            other.expr === this.expr && other.displayMode === this.displayMode
        )
    }
...
function pushBismuth(
    ...
    const add = (a: number, len: number) =>
        prot.push({ from: lineFrom + a, to: lineFrom + a + len })
    ...
    for (const m of text.matchAll(TAG_RE))
        add((m.index ?? 0) + m[1].length, m[2].length)
    for (const w of words) {
        const from = lineFrom + w.from,
            to = lineFrom + w.to
...
function buildDecorations(
    ...
    const selSpans = view.hasFocus
        ? view.state.selection.ranges.map(
              r =>
                  [doc.lineAt(r.from).number, doc.lineAt(r.to).number] as const,
          )
        : []
    const isRevealed = (n: number) =>
        selSpans.some(([a, b]) => n >= a && n <= b)
    ...
    const revealsRange = (from: number, to: number) =>
    ...
    const revealsPrefix = (from: number, to: number) =>
        selRanges.some(r => r.from < to && r.to >= from)
    ...
    for (const { from, to } of view.visibleRanges) {
        ...
        while (pos <= to) {
            ...
            if (frontmatterLines.has(line.number)) {
                ...
                const isDelim =
                    line.number === frontmatterOpen ||
                    line.number === frontmatterClose
                if (isDelim) {
                    deco.push(
                        (line.number === frontmatterOpen
                            ? blockTopRule
                    ...
                    if (line.to > line.from)
                        deco.push(
                            (onCursor ? syntaxMark : fenceMark).range(
                                line.from,
                                line.to,
                            ),
                        )
            ...
            if (tableBlock) {
                if (tableBlock.startLine === activeTableOpen)
            ...
            if (hm) {
                ...
                if (!onCursor)
                    deco.push(hide.range(line.from, line.from + hm[0].length))
                // Revealed: keep the `#`s in the mono accent face (not the serif heading).
                else
                    deco.push(
                        headingMark.range(line.from, line.from + hm[1].length),
                    )
            }
            ...
            if (qm) {
                ...
                if (!onCursor)
                    deco.push(hide.range(line.from, line.from + qm[0].length))
                else
                    deco.push(
                        syntaxMark.range(line.from, line.from + qm[0].length),
                    )
            }
            ...
            if (isThematicBreak(text)) {
                if (onCursor) {
                    ...
                    if (line.to > line.from)
                        deco.push(hide.range(line.from, line.to))
                }
                pos = line.to + 1
                continue
            }
            ...
            if (taskMatch) {
                ...
                const taskDepth = listDepth(
                    view.state,
                    line.from + taskMatch[1].length,
                    taskMatch[1],
                )
                if (struck) {
                    ...
                    if (line.to > textStart)
                        deco.push(taskDoneMark.range(textStart, line.to))
                }
                ...
                if (emptyActive || revealsPrefix(line.from, prefixEnd)) {
                    ...
                    if (indentLen > 0)
                        deco.push(hide.range(line.from, line.from + indentLen))
                    deco.push(
                        listMarkerMark.range(
                            line.from + indentLen,
                    ...
                    deco.push(
                        Decoration.replace({
                            widget: new CheckboxWidget(status),
                        }).range(line.from, line.from + taskMatch[0].length),
                    )
                }
            }
            ...
            if (bulletMatch) {
                ...
                if (emptyActive || revealsPrefix(line.from, prefixEnd)) {
                    ...
                    deco.push(
                        ...
                        }).range(line.from, line.from + bulletMatch[0].length),
                    )
                }
            }
            ...
            const orderedMatch =
                isTaskLine || bulletMatch ? null : text.match(ORDERED_LINE_RE)
            if (orderedMatch) {
                const depth = listDepth(
                    view.state,
                    line.from + orderedMatch[1].length,
                    orderedMatch[1],
                )
                // Empty active item → render raw marker so the end-of-line caret stays anchored
                // (a whole-line widget replace would place it at the far left). See B2 / task branch.
                ...
                if (emptyActive || revealsPrefix(line.from, prefixEnd)) {
                    ...
                    deco.push(
                        listMarkerMark.range(
                            line.from + indentLen,
                            line.from + orderedMatch[0].length,
                        ),
                    )
                } else {
                    ...
                    deco.push(
                        Decoration.replace({
                            widget: new OrderedWidget(marker, depth),
                        }).range(line.from, line.from + orderedMatch[0].length),
                    )
                }
            }
            ...
            const htmlSpans = pushInlineHtml(
                deco,
                view.state,
                line.from,
                line.to,
            ...
            const inHtmlSpan = (s: number, e: number) =>
                htmlSpans.some(h => s < h.to && e > h.from)
            ...
            const inMathSpan = (s: number, e: number) =>
                mathSpans.some(h => s < h.to && e > h.from)
            ...
            const inMlSpan = (s: number, e: number) =>
                mlSpans.some(h => s < h.to && e > h.from)
            for (const s of mlSpans) mathSpans.push({ from: s.from, to: s.to })
            {
                ...
                for (const m of text.matchAll(BLOCK_MATH_RE)) {
                    ...
                    if (revealsRange(s, end)) {
                        ...
                        deco.push(
                            syntaxMark.range(s, s + 2),
                            syntaxMark.range(end - 2, end),
                        )
                        for (const d of latexTokenDecorations(s + 2, m[1]))
                            deco.push(d)
                    } else {
                        deco.push(
                            Decoration.replace({
                ...
                for (const m of text.matchAll(INLINE_MATH_RE)) {
                    ...
                    if (revealsRange(s, end)) {
                        ...
                        deco.push(
                            syntaxMark.range(s, s + 1),
                            syntaxMark.range(end - 1, end),
                        )
                        for (const d of latexTokenDecorations(s + 1, m[1]))
                            deco.push(d)
                    } else {
                        deco.push(
                            Decoration.replace({
            ...
            for (const m of text.matchAll(INLINE_CODE_RE)) {
                ...
                const innerStart = s + fenceLen,
                    innerEnd = end - fenceLen
...
const codeBlockRangesCache = new WeakMap<
    Text,
...
const activeCodeField = StateField.define<number | null>({
    ...
    update(value, tr) {
        ...
        for (const e of tr.effects)
            if (e.is(setActiveCodeEffect)) return e.value
...
function buildTableWidgets(state: EditorState): DecorationSet {
    ...
    for (const b of blocks) {
        ...
        deco.push(
            Decoration.replace({
                widget: new TableWidget(b.cells, b.aligns, notePath),
                block: true,
            }).range(from, to),
        )
    }
...
const tableWidgetField = StateField.define<DecorationSet>({
    ...
    update(value, tr) {
        ...
        if (tr.docChanged || tr.selection || activeChanged)
...
function calloutBlockAt(state: EditorState, lineNumber: number): number | null {
    for (const c of scanCalloutLineBlocks(state.doc)) {
        if (lineNumber >= c.fromLine && lineNumber <= c.toLine)
            return c.fromLine
    }
    return null
}
...
class CalloutWidget extends WidgetType {
    constructor(
        private readonly header: CalloutHeader,
        private readonly body: string,
    ) {
        super()
    }
    ...
    toDOM(): HTMLElement {
        ...
        wrap.innerHTML = sanitizeHtml(
            renderCalloutHtml(this.header, bodyHtml, titleHtml),
        )
        return wrap
    }
...
function buildCalloutWidgets(state: EditorState): DecorationSet {
    ...
    for (const c of scanCalloutLineBlocks(doc)) {
        ...
        deco.push(
            Decoration.replace({
                widget: new CalloutWidget(c.header, c.body),
                block: true,
            }).range(from, to),
        )
    }
...
const calloutWidgetField = StateField.define<DecorationSet>({
    ...
    update(value, tr) {
        const activeChanged =
            tr.startState.field(activeCalloutField) !==
            tr.state.field(activeCalloutField)
        if (tr.docChanged || tr.selection || activeChanged)
            return buildCalloutWidgets(tr.state)
        return value.map(tr.changes)
    },
...
const calloutDomHandlers = EditorView.domEventHandlers({
    dblclick: (e, view) => {
        ...
        view.dispatch({
            effects: setActiveCalloutEffect.of(at),
            selection: { anchor: view.state.doc.line(at).from },
        })
        view.focus()
        return false
    },
...
const calloutThemeSpec: Record<string, Record<string, string>> = {
    ...
    '.cm-callout-wrap details.callout > summary': {
        cursor: 'pointer',
        'list-style': 'none',
    },
}
...
export const livePreview = [
    ...
    EditorView.domEventHandlers({
        ...
        mousedown: (e, view) => {
            ...
            const htmlBlock = target.closest(
                '.cm-html-block',
            ) as HTMLElement | null
            ...
            view.dispatch({
                changes: { from: innerPos, to: innerPos + 1, insert: next },
            })
            ...
            const reorder = reorderAroundLine(
                view.state,
                view.state.doc.lineAt(innerPos).number,
            )
    ...
    ViewPlugin.fromClass(
        class {
            ...
            update(u: ViewUpdate) {
                const activeChanged =
                    u.startState.field(activeCodeField, false) !==
                        u.state.field(activeCodeField, false) ||
                    u.startState.field(activeTableField, false) !==
                        u.state.field(activeTableField, false) ||
                    u.startState.field(activeCalloutField, false) !==
                        u.state.field(activeCalloutField, false)
                if (
                    u.docChanged ||
                    u.viewportChanged ||
                    u.selectionSet ||
                    ...
                    if (u.docChanged) {
                        this.blockRegions = computeBlockRegions(
                            u.view.state.doc,
                        )
                    }
    ...
    EditorView.theme({
        ...
        '.cm-heading-mark, .cm-heading-mark > span': {
            'font-family': MONO_FONT,
            color: 'var(--fg)',
            'font-weight': '500',
        },
        '.cm-syntax-mark, .cm-syntax-mark > span': {
            'font-family': MONO_FONT,
            color: 'var(--fg)',
        },
        ...
        '.cm-td:focus, .cm-td:focus-within': {
            outline: 'none',
        ...
        '.cm-table-infinity .cm-table-scroll': {
            'max-width': '100%',
            'overflow-x': 'auto',
        },
        '.cm-table-infinity .cm-table-rendered': {
            width: 'max-content',
            'table-layout': 'auto',
        },
        '.cm-table-infinity .cm-td': { 'white-space': 'nowrap' },
        '.cm-table-infinity .cm-td[data-editing="1"]': {
        ...
        '.cm-col-drag svg, .cm-row-drag svg': {
            width: '100%',
            height: '100%',
            display: 'block',
        },
        '.cm-col-drag--show, .cm-row-drag--show': { opacity: '0.8' },
        '.cm-col-drag:hover, .cm-row-drag:hover': {
            opacity: '1',
            color: 'var(--accent)',
        },
        '.cm-table-reordering .cm-col-drag--show, .cm-table-reordering .cm-row-drag--show':
            { opacity: '1' },
        '.cm-table-drop-line': {
            position: 'absolute',
            background: 'var(--accent)',
        ...
        '.cm-html-inline img, .cm-html-block img': {
            'max-width': '100%',
            height: 'auto',
        },
        '.cm-html-inline a, .cm-html-block a': {
            color: 'var(--accent)',
            'text-decoration': 'none',
            'border-bottom': '1px solid var(--accent-soft)',
        },
        ...
        '.spell-mark': {
            background:
                'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="8" height="4"><path d="M0 3 Q2 1 4 3 T8 3" fill="none" stroke="%23ff9ea0" stroke-width="1.1"/></svg>\') left bottom repeat-x',
        },
        '.grammar-mark': {
            background:
                'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="8" height="4"><path d="M0 3 Q2 1 4 3 T8 3" fill="none" stroke="%238fb4ff" stroke-width="1.1"/></svg>\') left bottom repeat-x',
        },
        '.property-mark': {
            background:
                'url(\'data:image/svg+xml;utf8,<svg xmlns="http://www.w3.org/2000/svg" width="8" height="4"><path d="M0 3 Q2 1 4 3 T8 3" fill="none" stroke="%23b89cff" stroke-width="1.1"/></svg>\') left bottom repeat-x',
        },
    }),
]
