...
describe('rangeLabel', () => {
    it('renders a numeric min–max range', () => {
        expect(
            rangeLabel({ type: 'number', min: 11, max: 28 } as SchemaEntry),
        ).toBe('11–28')
    })
    it("renders enum members joined by ' | '", () => {
        expect(
            rangeLabel({
                type: { kind: 'enum', values: ['dark', 'light'] },
            } as SchemaEntry),
        ).toBe('dark | light')
    })
    ...
    it('renders a one-sided numeric bound', () => {
        ...
        expect(rangeLabel({ type: 'number', max: 10 } as SchemaEntry)).toBe(
            '≤10',
        )
    })
})
...
describe('rankPaths', () => {
    ...
    it('matches a nested path by its last segment (basename)', () => {
        expect(rankPaths(TEST_TREE, 'jour').map(e => e.path)).toEqual([
            'Templates/Journal.md',
        ])
    })

    it('matches a path containing a space across the space', () => {
        expect(rankPaths(TEST_TREE, 'daily no').map(e => e.path)).toEqual([
            'Daily Notes',
        ])
    })
