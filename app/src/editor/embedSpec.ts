...
export function specForMarkdownImage(
    url: string,
    rawAlt: string,
    assetUrl: (target: string) => string,
): EmbedSpec | null {
    ...
    if (/^(https?:|data:|blob:)/i.test(url))
        return { kind: 'image', src: url, alt, width }
...
export function computeSizeEdit(
    ...
    if (hit) {
        ...
        return {
            from: hit.from,
            to: hit.to,
            insert: `![[${beforePipe}|${size}]]`,
        }
    }
    // No wiki embed on the line → a markdown `![alt](url)` image. Persist the width in the alt.
    const width = size.split('x')[0]
    let mhit: { from: number; to: number; alt: string; url: string } | null =
        null
    for (const m of lineText.matchAll(/!\[([^\]\n]*)\]\(([^)\s]+)\)/g)) {
        const from = lineFrom + (m.index ?? 0)
        const to = from + m[0].length
        if (pos >= from && pos <= to) {
            mhit = { from, to, alt: m[1], url: m[2] }
            break
        }
    ...
    const baseAlt =
        basePipe !== -1 && /^\d+$/.test(mhit.alt.slice(basePipe + 1).trim())
            ? mhit.alt.slice(0, basePipe)
            : mhit.alt
    return {
        from: mhit.from,
        to: mhit.to,
        insert: `![${baseAlt}|${width}](${mhit.url})`,
    }
}
