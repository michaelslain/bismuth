 ...
 */
function propertyTypeCompletions(
    ctx: CompletionContext,
    typed: string,
): CompletionResult | null {
    const from = ctx.pos - typed.length
    const scalars: Completion[] = SCALAR_PROPERTY_TYPES.map(label => ({
        label,
        type: 'enum',
        detail: 'type',
    }))
    const composites: Completion[] = [
        snippetCompletion('{ enum: [${values}] }', {
            label: 'enum',
            type: 'enum',
            detail: 'one of a fixed set',
            info: 'Restrict the value to a fixed set of options.',
        }),
        snippetCompletion('{ list: ${string} }', {
            label: 'list',
            type: 'enum',
            detail: 'array of items',
            info: 'An array; the placeholder is the item type (e.g. string).',
        }),
        snippetCompletion('{ fields: { ${name}: ${string} } }', {
            label: 'object',
            type: 'enum',
            detail: 'nested fields',
            info: 'A nested object with typed sub-fields.',
        }),
    ]
    const p = typed.toLowerCase()
    const options = [...scalars, ...composites].filter(o =>
        o.label.toLowerCase().startsWith(p),
    )
...
export function dailyNoteIdsFromDoc(
    doc: string,
    ...
    for (i = i + 1; i < lines.length; i++) {
        ...
        if (m) {
            if (m[1] === 'id') cur.id = unquote(m[2])
            else cur.label = unquote(m[2])
        }
    }
    if (cur) out.push(cur)
...
function scopeAt(
    ...
    lineNumber: number,
    indent: number,
): { schema: Schema; sectionKey: string | null } {
...
function enclosingListItemType(
    ...
    for (let n = lineNumber - 1; n >= 1; n--) {
        ...
        if (header && lineIndent < itemIndent) {
            ...
            if (t && typeof t === 'object' && t.kind === 'list' && t.item)
                return t.item
            return null
        }
 ...
 */
export function keybindCompletions(
    ctx: CompletionContext,
    valueSoFar: string,
): CompletionResult | null {
    ...
    const token = (
        plusIdx >= 0 ? currentCombo.slice(plusIdx + 1) : currentCombo
    ).replace(/^\s+/, '')
    ...
    const usedFamilies = new Set(
        prior.map(modifierFamily).filter((f): f is string => !!f),
    )
    ...
    for (const mod of KEYBIND_MODIFIERS) {
        ...
        if ((fam && usedFamilies.has(fam)) || !mod.toLowerCase().startsWith(p))
            continue
        options.push({
            label: mod,
            apply: mod + '+',
    ...
    for (const k of KEYBIND_KEYS) {
        if (k.toLowerCase().startsWith(p))
            options.push({ label: k, type: 'key' })
    }
...
function launchGallery(view: EditorView, from: number, source: 'icons'): void {
    ...
    void Promise.all([
        ...
        .then(([{ openGallery }, sources]) =>
            openGallery({ source: sources.iconSource }),
        )
        .then(picked => {
            if (picked) {
                view.dispatch({
...
export function rankPaths(candidates: VaultPath[], query: string): VaultPath[] {
    ...
    const starts: VaultPath[] = [],
        baseStarts: VaultPath[] = [],
        includes: VaultPath[] = []
...
export function settingsCompletionSource(
    ...
    return (
        ...
        if (tokenMatch) {
            ...
            if (keyM) {
                ...
                if (sectionKey === 'dailyNotes' && keyM[1] === 'fileName') {
                    ...
                    const options = TEMPLATE_TOKENS.filter(t =>
                        t.token.toLowerCase().includes(q),
                    ).map(t => ({ label: t.token, type: 'enum', info: t.doc }))
                    if (options.length)
                        return {
                            from: line.from + tokenMatch.from,
                            options,
                            validFor: /^\{\{[\w+:-]*$/,
                        }
                }
        ...
        if (fullVal) {
            const { schema, sectionKey } = scopeAt(
                root,
                ctx,
                line.number,
                indent,
        ...
        if (val) {
            ...
            if (fieldType === 'icon') {
                ...
                const gallery: IconedCompletion = {
                    ...
                    apply: (
                        ...
                        _c: Completion,
                        applyFrom: number,
                    ) => launchGallery(view, applyFrom, 'icons'),
                }
            ...
            const isCommand =
                ...
                fieldType.kind === 'enum' &&
                (
                    fieldType as { allowPrefixes?: string[] }
                ).allowPrefixes?.includes('daily-note:')
            const dailyIds = isCommand
                ? dailyNoteIdsFromDoc(ctx.state.doc.toString())
                : []
            const dailyLabels = new Map<string, string>(
                dailyIds.map(d => [`daily-note:${d.id}`, d.label || d.id]),
            )

            const raw = [
                ...valueOptions(fieldType),
                ...dailyIds.map(d => `daily-note:${d.id}`),
            ]
        ...
        if (listItem) {
            const itemType = enclosingListItemType(
                root,
                ctx,
                line.number,
                listItem[1].length,
