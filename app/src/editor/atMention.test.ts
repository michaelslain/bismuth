...
describe('matchAtMentionPrefix', () => {
    ...
    it('matches `@` after whitespace, from at the `@`', () => {
        // "see @Note" — `@` at index 4.
        expect(matchAtMentionPrefix('see @Note')).toEqual({
            from: 4,
            query: 'Note',
        })
    })
    it('allows spaces in the query (file names have spaces)', () => {
        expect(matchAtMentionPrefix('@Budget report')).toEqual({
            from: 0,
            query: 'Budget report',
        })
    })
    ...
    it('stops at a second `@` — each mention is its own token', () => {
        ...
        expect(matchAtMentionPrefix('@first @sec')).toEqual({
            from: 7,
            query: 'sec',
        })
    })
...
describe('rankFileCandidates', () => {
    ...
    it('is case-insensitive on label and path', () => {
        expect(rankFileCandidates(files, 'ZEBRA').map(f => f.label)).toEqual([
            'Zebra',
        ])
    })
