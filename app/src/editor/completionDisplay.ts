...
//  - addToOptions       → prepend OUR icon (from the shared kind→icon map)
...
/** A completion that overrides its row glyph with a specific icon NAME
    ...
    addToOptions: [
        {
            ...
            render(completion: Completion): Node | null {
                // A per-row icon override (iconName) wins over the kind→icon map — that's
                ...
                const name =
                    (completion as IconedCompletion).iconName ??
                    completionIcon(completion.type)
                if (!name) return null
                // Returns immediately; the glyph fills in once the lazy chunk resolves.
...
export const completionTheme = EditorView.theme({
    ...
    '.cm-completionDetail': {
        marginLeft: 'auto',
        paddingLeft: '12px',
        opacity: 'var(--popover-detail-opacity)',
        fontStyle: 'normal',
    },
