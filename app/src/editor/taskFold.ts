...
function scanBlock(
    state: EditorState,
    startLine: number,
): { items: { resolved: boolean; headLine: number }[]; endLine: number } {
    const doc = state.doc
    ...
    while (i <= doc.lines) {
        ...
        if (isTaskLine(text) && indent === base) {
            items.push({
                resolved: isResolvedChar(statusChar(text)),
                headLine: i,
            })
            i++
 ...
 */
export function reorderAroundLine(
    state: EditorState,
    lineNo: number,
): TransactionSpec | null {
    const doc = state.doc
    if (lineNo < 1 || lineNo > doc.lines || !isTaskLine(doc.line(lineNo).text))
        return null
    let line = 1
    while (line <= doc.lines) {
        if (!isTaskLine(doc.line(line).text)) {
        ...
        if (lineNo >= line && lineNo <= endLine) {
            ...
            return reordered === original
                ? null
                : { changes: { from, to, insert: reordered } }
        }
        line = endLine + 1
    }
    return null
...
class TaskFoldWidget extends WidgetType {
    ...
    eq(other: TaskFoldWidget): boolean {
        return (
            ...
            other.anchorPos === this.anchorPos
        )
    }
    toDOM(view: EditorView): HTMLElement {
        const el = document.createElement('div')
        el.className =
            'cm-task-fold' + (this.collapsed ? ' cm-task-fold-collapsed' : '')
        ...
        if (isCollapsed && !cursorInside) {
            ranges.push({
                ...
                deco: Decoration.replace({
                    block: true,
                    widget: new TaskFoldWidget(g.count, true, g.anchorPos),
                }),
            })
        } else {
            ranges.push({
                ...
                deco: Decoration.widget({
                    block: true,
                    side: -1,
                    widget: new TaskFoldWidget(g.count, false, g.anchorPos),
                }),
            })
        }
    }
...
const taskFoldDecorations = EditorView.decorations.compute(
    ['doc', 'selection', foldedAnchors],
