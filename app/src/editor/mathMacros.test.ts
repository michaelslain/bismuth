...
describe('parseMathMacros', () => {
    ...
    test('nested + escaped braces in body are balanced correctly', () => {
        expect(parseMathMacros('\\newcommand{\\set}[1]{\\{ #1 \\}}')).toEqual({
            '\\set': '\\{ #1 \\}',
        })
        expect(
            parseMathMacros('\\newcommand{\\f}{\\frac{a}{\\frac{b}{c}}}'),
        ).toEqual({
            '\\f': '\\frac{a}{\\frac{b}{c}}',
        })
    })
    ...
    test('\\renewcommand and \\providecommand are treated as definitions', () => {
        expect(
            parseMathMacros('\\renewcommand{\\vec}[1]{\\mathbf{#1}}'),
        ).toEqual({ '\\vec': '\\mathbf{#1}' })
        expect(parseMathMacros('\\providecommand{\\e}{\\mathrm{e}}')).toEqual({
            '\\e': '\\mathrm{e}',
        })
    })

    test('\\def with and without param text', () => {
        expect(parseMathMacros('\\def\\R{\\mathbb{R}}')).toEqual({
            '\\R': '\\mathbb{R}',
        })
        expect(
            parseMathMacros('\\def\\ip#1#2{\\langle #1, #2 \\rangle}'),
        ).toEqual({
