...
describe('matchTemplateTokenPrefix', () => {
    it('matches a bare open {{ (empty query)', () => {
        expect(matchTemplateTokenPrefix('fileName: "{{')).toEqual({
            from: 11,
            query: '',
        })
    })
    it('matches a partial token name after {{', () => {
        expect(matchTemplateTokenPrefix('{{da')).toEqual({
            from: 0,
            query: 'da',
        })
    })
    it('allows offset/format chars in the query', () => {
        expect(matchTemplateTokenPrefix('{{date:')).toEqual({
            from: 0,
            query: 'date:',
        })
        expect(matchTemplateTokenPrefix('{{date+1')).toEqual({
            from: 0,
            query: 'date+1',
        })
    })
