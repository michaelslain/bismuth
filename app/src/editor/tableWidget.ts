...
function loadCellEditor(): Promise<CellEditorModule> {
    ...
    if (!cellEditorPromise)
        cellEditorPromise = import('./cellEditor').then(m => {
            cellEditorModule = m
            return m
        })
...
export function reshapeVisual(
    ...
    const carried: TableVisual = existingNew
        ? { ...existingNew }
        : { cols: [], rows: [] }
...
function readGrid(root: HTMLElement): string[][] {
    ...
    for (const tr of Array.from(root.querySelectorAll('tr'))) {
        const cells = Array.from(
            tr.querySelectorAll<HTMLElement>('[data-cell]'),
        )
        if (cells.length)
            rows.push(
                cells.map(c => {
                    ...
                    if (c.dataset.editing === '1' && cm)
                        return cmDocToCellSource(cm.state.doc.toString())
...
function currentRange(
    ...
    const after =
        containing ?? blocks.find(b => doc.line(b.startLine).from >= pos)
    if (!after) return null
    return {
        from: doc.line(after.startLine).from,
        to: doc.line(after.endLine).to,
    }
}
...
export const tableSelectionGuard = EditorState.transactionFilter.of(tr => {
    ...
    const mapped = remapCursorOffTable(
        tr.newDoc,
        head,
...
export const tableUndoSelectionGuard = EditorView.updateListener.of(u => {
    ...
    const mapped = remapCursorOffTable(
        u.state.doc,
        sel.head,
        u.startState.selection.main.head,
        active,
    )
...
function applyMergeRegionsToWrap(
    wrap: HTMLElement,
    regions: MergeRegion[],
): void {
...
function applyMergeChange(view: EditorView, ch: TableMergeChange): void {
    ...
    for (const wrap of Array.from(
        view.dom.querySelectorAll<HTMLElement>('.cm-table-wrap'),
    )) {
...
export const tableMergeUndo = [
    ...
    EditorView.updateListener.of(u => {
        for (const tr of u.transactions) {
            for (const e of tr.effects) {
                if (e.is(setTableMergesEffect))
                    applyMergeChange(u.view, e.value)
            }
        }
    }),
]
...
export function insertEmbedsInTableCell(
    ...
    const block = blocks.find(
        b =>
            doc.line(b.startLine).from <= anchorFrom &&
            anchorFrom <= doc.line(b.endLine).to,
    )
    if (!block) return false
    const cols = block.cells[0]?.length ?? 0
    if (r < 0 || r >= block.cells.length || c < 0 || c >= cols) return false
    const grid = appendToCell(
        { cells: block.cells, aligns: block.aligns },
        r,
        c,
        embeds.join('<br>'),
    )
    ...
    const md =
        surgicalTableEdit(before, block.cells, grid.cells) ?? formatTable(grid)
...
export class TableWidget extends WidgetType {
    ...
    eq(other: TableWidget): boolean {
        return (
            serializeTable(this.cells, this.aligns) ===
            serializeTable(other.cells, other.aligns)
        )
    }
    ...
    private commit(
        view: EditorView,
        root: HTMLElement,
        transform?: (g: TableGrid) => TableGrid | void,
    ): void {
        const range = currentRange(view, root)
        if (!range) return
        let grid: TableGrid = {
            cells: readGrid(root),
            aligns: this.aligns.slice(),
        }
        ...
        const md = transform
            ...
            : (surgicalTableEdit(before, this.cells, grid.cells) ??
              formatTable(grid))
        ...
        dispatchKeepScroll(view, {
            changes: { from, to, insert: delta.insert },
        })
    }
    ...
    toDOM(view: EditorView): HTMLElement {
        ...
        const renderDisplay = (cell: HTMLElement): void => {
            ...
            const hasMedia = !!cell.querySelector(
                'img, iframe, video, audio, svg, .cm-embed',
            )
            const isEmpty = !hasMedia && (cell.textContent ?? '').trim() === ''
            cell.classList.toggle('cm-td-empty', isEmpty)
            if (isEmpty)
                cell.innerHTML =
                    '<span class="cm-td-ph" aria-hidden="true"> </span>'
        }
        ...
        const enterEdit = (
            ...
            const doMount = (
                ...
                if (
                    ...
                    return // left edit / torn down / already mounted
                if (checkFocus) {
                    const ae = cell.ownerDocument
        ...
        const cellAt = (r: number, c: number): HTMLElement | undefined =>
            cellEls[r]?.[c]
            ...
            for (let c = 0; c < cols; c++) {
                ...
                cell.addEventListener('mousedown', e => {
                    ...
                    if (me.button === 2) {
                        ...
                        const onEnd = (): void => {
                            cell.ownerDocument.removeEventListener(
                                'contextmenu',
                                onEnd,
                                true,
                            )
                        ...
                        cell.ownerDocument.addEventListener(
                            'contextmenu',
                            onEnd,
                            true,
                        )
                    ...
                    if (me.button === 0) {
                        const link = (
                            me.target as HTMLElement | null
                        )?.closest?.(
                            '.cm-wikilink, .bismuth-wikilink',
                        ) as HTMLElement | null
                        if (link && cell.contains(link)) {
                            ...
                            const raw =
                                link.dataset.wikilink ??
                                (link.dataset.href ?? '').replace(/\.md$/, '')
        ...
        const syncInfinityTableWidth = (): void => {
            const sum =
                infinity && table.style.tableLayout === 'fixed'
                    ? frozenColumnsWidth(
                          colEls.map(c => parseFloat(c.style.width)),
                      )
                    : null
        ...
        const paintSelection = (): void => {
            for (const rowArr of cellEls)
                for (const el of rowArr) el?.classList.remove('cm-td-selected')
            if (selAnchor && selFocus) {
                ...
                for (let r = rect.r; r < rect.r + rect.rowSpan; r++) {
                    for (let c = rect.c; c < rect.c + rect.colSpan; c++)
                        cellAt(r, c)?.classList.add('cm-td-selected')
                }
            }
            refreshGrips()
        }
        ...
        root.appendChild(
            edgeBar('cm-table-add-col', 'Add column', () => {
                ...
                this.commit(view, root, g =>
                    insertColumn(g, g.cells[0]?.length ?? 0),
                )
            }),
        )
        root.appendChild(
        ...
        root.addEventListener(
            ...
            e => {
                const de = e as DragEvent
                const files = de.dataTransfer
                    ? Array.from(de.dataTransfer.files)
                    : []
                ...
                window.dispatchEvent(
                    new CustomEvent('bismuth-table-drop', {
                        detail: { view, files, target, altKey: de.altKey },
                    }),
                )
            },
            true,
        ...
        const mergeMenuItems = (r: number, c: number): TableMenuItem[] => {
            ...
            const sel =
                selAnchor && selFocus
                    ? rectFromCells(selAnchor, selFocus)
                    : null
            const canMerge =
                !!sel && (sel.rowSpan > 1 || sel.colSpan > 1) && sel.r >= 1
            ...
            const dispatchMerge = (next: MergeRegion[]): void => {
                view.focus()
                view.dispatch({
                    effects: setTableMergesEffect.of({
                        path: this.notePath,
                        key: sizeKey(this.cells),
                        prev: cur,
                        next,
                    }),
                })
        ...
        const persist = (): void => {
            ...
            updateVisual(this.notePath, sizeKey(this.cells), {
                cols: colEls.map(c =>
                    c.style.width ? parseFloat(c.style.width) : null,
                ),
                rows: [],
            })
        }
        ...
        const colBoundaries = (): number[] => {
            ...
            const headerCells = rowEls[0]
                ? (Array.from(rowEls[0].children) as HTMLElement[])
                : []
            if (headerCells.length === 0)
                return [table.getBoundingClientRect().left - wrapRect.left]
            const b: number[] = [
                headerCells[0].getBoundingClientRect().left - wrapRect.left,
            ]
            for (const cell of headerCells)
                b.push(cell.getBoundingClientRect().right - wrapRect.left)
            return b
        }
        ...
        const rowBoundaries = (): number[] => {
            const wrapRect = root.getBoundingClientRect()
            if (rowEls.length === 0)
                return [table.getBoundingClientRect().top - wrapRect.top]
            const b: number[] = [
                rowEls[0].getBoundingClientRect().top - wrapRect.top,
            ]
            for (const tr of rowEls)
                b.push(tr.getBoundingClientRect().bottom - wrapRect.top)
            return b
        }
        ...
        const layout = (): void => {
            ...
            const inView = (px: number): boolean =>
                px >= viewL - 0.5 && px <= viewR + 0.5
            const base = xs[0] ?? 0
            ...
            for (let c = 0; c < colHandles.length; c++) {
                ...
                h.style.display =
                    inView(edge) || straddlesRight || dragged ? '' : 'none'
            }
        ...
        const markDragged = (
            ...
            for (let r = 0; r < cellEls.length; r++) {
                for (let c = 0; c < (cellEls[r]?.length ?? 0); c++) {
                    if ((axis === 'col' ? c : r) === idx)
                        cellEls[r]?.[c]?.classList.toggle(cls, on)
                }
            }
        }
        ...
        const startReorder = (
            ...
            const pointerId =
                typeof (e as Partial<PointerEvent>).pointerId === 'number'
                    ? (e as PointerEvent).pointerId
                    : null
            if (
                grip &&
                pointerId != null &&
                typeof grip.setPointerCapture === 'function'
            ) {
                try {
                    grip.setPointerCapture(pointerId)
                } catch {
                    /* not capturable — window listeners cover it */
                }
            }
            let slot = from
                ...
                if (to !== from) {
                    if (axis === 'col') {
                        ...
                        if (cur && cur.cols.some(w => w != null)) {
                            const colCount =
                                this.cells[0]?.length ?? cur.cols.length
                            const padded = cur.cols.slice()
                            while (padded.length < colCount) padded.push(null)
                            updateVisual(this.notePath, key, {
                                cols: moveInArray(padded, from, to),
                            })
                        }
        ...
        for (let c = 0; c < cols; c++) {
            ...
            grip.addEventListener('mousedown', e =>
                startReorder('col', c, e as MouseEvent),
            )
            grip.addEventListener('pointerdown', e =>
                startReorder('col', c, e as PointerEvent),
            )
        ...
        for (let r = 1; r < rowCount; r++) {
            ...
            grip.addEventListener('mousedown', e =>
                startReorder('row', r, e as MouseEvent),
            )
            grip.addEventListener('pointerdown', e =>
                startReorder('row', r, e as PointerEvent),
            )
            overlay.appendChild(grip)
            rowGrips[r] = grip
        }
        ...
        const startColDrag = (
            ...
            const onDragMove = (me: MouseEvent): void => {
                autoScroll(me.clientX)
                drag.move(me.clientX + scrollAdj)
            }
                    ...
                    () => {
                        ...
                        if (table.style.tableLayout !== 'fixed') {
                            headerCells.forEach((cell, i) => {
                                if (colEls[i])
                                    colEls[i].style.width =
                                        `${cell.offsetWidth}px`
                            })
                            table.style.tableLayout = 'fixed'
                        }
                        return (
                            parseFloat(colEls[c].style.width) ||
                            headerCells[c]?.offsetWidth ||
                            MIN_COL
                        )
                    },
                    width => {
        ...
        if (pendingCellFocus) {
            ...
            const claim = (): void => {
                ...
                if (root.querySelector('[data-cell][data-editing="1"]')) {
                    pendingCellFocus = null
                    return
                } // user already editing
                pendingCellFocus = null
                const rows = this.cells.length
                const colCount = this.cells[0]?.length ?? 0
                if (rows === 0 || colCount === 0) return
                focusCell(
                    Math.min(Math.max(want.r, 0), rows - 1),
                    Math.min(Math.max(want.c, 0), colCount - 1),
                )
            }
            if (typeof requestAnimationFrame !== 'undefined')
    ...
    destroy(dom: HTMLElement): void {
        const wrap = (
            dom.classList.contains('cm-table-wrap')
                ? dom
                : dom.querySelector('.cm-table-wrap')
        ) as HTMLElement | null
        ;(
            wrap as unknown as { _tableRO?: ResizeObserver } | null
        )?._tableRO?.disconnect()
        // Destroy any live nested cell editor so a commit-rebuild (or view teardown) leaks no EditorView.
        for (const cell of Array.from(
            dom.querySelectorAll<HTMLElement>('[data-cell]'),
        )) {
        ...
        const items: TableMenuItem[] = [
            {
                ...
                disabled: r === 0,
                onSelect: () =>
                    commitStructural({ r, c }, g => insertRow(g, r)),
            },
            {
                label: 'Insert row below',
                icon: 'ArrowDown',
                onSelect: () =>
                    commitStructural({ r: r + 1, c }, g => insertRow(g, r + 1)),
            },
            {
                label: 'Delete row',
                icon: 'Trash2',
                ...
                onSelect: () =>
                    commitStructural({ r: Math.min(r, rowCount - 2), c }, g =>
                        deleteRow(g, r),
                    ),
            },
            {
                ...
                separatorBefore: true,
                onSelect: () =>
                    commitStructural({ r, c }, g => insertColumn(g, c)),
            },
            {
                label: 'Insert column right',
                icon: 'ArrowRight',
                onSelect: () =>
                    commitStructural({ r, c: c + 1 }, g =>
                        insertColumn(g, c + 1),
                    ),
            },
            {
                ...
                disabled: cols <= 1,
                onSelect: () =>
                    commitStructural({ r, c: Math.min(c, cols - 2) }, g =>
                        deleteColumn(g, c),
                    ),
            },
            {
                ...
                onSelect: () => {
                    ...
                    const to =
                        range.from > 0
                            ? range.to
                            : Math.min(range.to + 1, view.state.doc.length)
            ...
            },
            {
                ...
                onSelect: () => {
                    ...
                    const spec: TransactionSpec = {
                        ...
                        effects: setActiveTableEffect.of(line),
                    }
                    if (pretty !== src)
                        spec.changes = {
                            from: range.from,
    ...
    class {
        ...
        apply(view: EditorView): void {
            const wraps = Array.from(
                view.contentDOM.querySelectorAll<HTMLElement>('.cm-table-wrap'),
            )
            if (wraps.length === 0) return
            for (const w of wraps) clearTableFindHighlights(w)
            const query = getSearchQuery(view.state)
            if (!searchPanelOpen(view.state) || !query.valid || !query.search)
                return
            const doc = view.state.doc
            const { blocks } = groupTableBlocks(doc)
            ...
            const activeBlock = blocks.find(
                b =>
                    sel.from >= doc.line(b.startLine).from &&
                    sel.from <= doc.line(b.endLine).to,
            )
            const activeCoord = activeBlock
                ? cellCoordForOffset(activeBlock, doc, sel.from)
                : null
            for (const wrap of wraps) {
                ...
                const block = blocks.find(
                    b =>
                        pos >= doc.line(b.startLine).from &&
                        pos <= doc.line(b.endLine).to,
                )
                for (const cell of Array.from(
                    wrap.querySelectorAll<HTMLElement>('[data-cell]'),
                )) {
                ...
                if (
                    ...
                    const mark = activeCell?.querySelector<HTMLElement>(
                        `mark.${TABLE_FIND_MATCH_CLASS}`,
                    )
                    if (mark) {
