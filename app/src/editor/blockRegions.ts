...
export function computeBlockRegions(doc: Text): BlockRegions {
        ...
        while (i <= doc.lines) {
            ...
            if (m) {
                ...
                if (j <= doc.lines) {
                    ...
                    for (let k = open; k <= j; k++)
                        codeBlockByLine.set(k, block)
                    i = j + 1
                    continue
                }
    ...
    if (fmRange) {
        ...
        const lastBodyLine =
            fmRange.to > fmRange.from
                ? doc.lineAt(fmRange.to).number
                : firstLine - 1
    ...
    const { blocks: tableBlocks, byLine: tableBlockByLine } =
        groupTableBlocks(doc)
    ...
    for (const c of scanCalloutLineBlocks(doc)) {
        for (let k = c.fromLine; k <= c.toLine; k++)
