...
function ensureSetup(): Promise<void> {
    if (!setupPromise) {
        ...
        setupPromise = (async () => {
            ...
            const dictWords = [
                ...new Set(words.map(normalizeDictWord).filter(Boolean)),
            ]
...
export async function removeDictionaryWord(word: string): Promise<void> {
    ...
    const remaining = [
        ...new Set(
            loadHarperState().words.map(normalizeDictWord).filter(Boolean),
        ),
    ]
    await ensureSetup()
    const inst = getLinter()
...
export function harperSpellcheck(opts: HarperOpts): Extension {
    ...
    return linter(
        async (view): Promise<Diagnostic[]> => {
            ...
            const lints = await getLinter().lint(bodyText, {
                language: 'markdown',
            })
            ...
            const visible = lints.filter(l =>
                isSpellingLint(l) ? opts.spelling : opts.grammar,
            )
