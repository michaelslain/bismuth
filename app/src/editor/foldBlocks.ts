...
function indentRegionEnd(
    doc: Text,
    i: number,
    indent: number,
): { last: number; sawChild: boolean } {
    let last = i
    let sawChild = false
...
function buildDeco(
    ...
    for (const b of scanFoldables(doc, mode)) {
        ...
        const base =
            b.kind === 'h'
                ? '-24px'
                : `${(b.depth * LIST_STEP - 1.45).toFixed(2)}em`
        ...
        const left =
            hasGutter && b.depth === 0
                ? `calc(-1 * var(--bismuth-fold-gutter-w, 0px) - 18px)`
                : base
        ranges.push(foldableLine.range(b.anchorFrom))
        ranges.push(
            Decoration.widget({
                widget: new FoldArrowWidget(
                    b.id,
                    isFolded,
                    locked.has(b.id),
                    left,
...
const withAdded = (s: Set<string>, id: string): Set<string> =>
    new Set(s).add(id)
...
function regionLineEls(
    ...
    for (let n = startLine; n <= endLine && n <= doc.lines; n++) {
        ...
        const visible = view.visibleRanges.some(
            r => line.from <= r.to && line.to >= r.from,
        )
        if (!visible) continue
    ...
    const run = () => {
        ...
        els.forEach((el, i) => {
            ...
            const anim = el.animate([from, to], {
                duration: ANIM_MS,
                easing: ANIM_EASE,
                fill: dir === 'collapse' ? 'forwards' : 'none',
            })
...
export function foldBlocks(
    ...
    const field = StateField.define<FoldState>({
        create(state) {
            ...
            return {
                locked,
                ephemeral: new Set(),
                deco: buildDeco(state.doc, mode, locked, locked, hasGutter),
            }
        },
        update(value, tr) {
            ...
            return {
                locked,
                ephemeral,
                deco: buildDeco(tr.state.doc, mode, folded, locked, hasGutter),
            }
        },
        ...
        if (dir === 'collapse') {
            ...
            view.dom
                .querySelector<HTMLElement>(
                    `.cm-fold-arrow[data-fold-id="${escId}"]`,
                )
            ...
            measure(view: EditorView): void {
                view.requestMeasure({
                    ...
                    write: (w, v) => {
                        ...
                        this.lastWidth = w
                        v.dom.style.setProperty(
                            '--bismuth-fold-gutter-w',
                            `${w}px`,
                        )
                    },
                })
    ...
    const gutterClickThrough = EditorView.theme({
    ...
    return [
        ...
        EditorView.updateListener.of(u => {
            if (
                u.transactions.some(tr =>
                    tr.effects.some(e => e.is(toggleFold) || e.is(toggleLock)),
                )
            ) {
        ...
        EditorView.theme({
            ...
            '.cm-fold-arrow.is-collapsed::before': {
