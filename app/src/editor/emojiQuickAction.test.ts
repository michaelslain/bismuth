...
afterAll(() => {
    ...
    for (const k of EVENT_CLASSES)
        (globalThis as Record<string, unknown>)[k] = savedEventClasses[k]
})

/** Capture the single `bismuth-open-emoji-library` event `run` fires. */
function captureEmojiOpen(
    run: () => void,
    ...
    try {
        ...
        window.removeEventListener('bismuth-open-emoji-library', onOpen)
    }
    return detail
}
...
describe('emojiQuickAction', () => {
    ...
    test("passes no insert for the note editor — App's default (focused editor caret) is correct there", () => {
        const detail = captureEmojiOpen(() =>
            emojiQuickAction({ focus: () => {} }).onSelect(),
        )
...
describe('editorContextMenu — emoji placement (#67)', () => {
    ...
    function rightClick(doc = 'hello world'): EditorMenuEvent {
        ...
        const onMenu = (e: Event) => {
            detail = (e as CustomEvent<EditorMenuEvent>).detail
        }
        ...
        if (!detail)
            throw new Error(
                'right-click dispatched no bismuth-context-menu event',
            )
        return detail
    }
