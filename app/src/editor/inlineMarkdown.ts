        ...
        if (src[i] === '!' && src[i + 1] === '[' && src[i + 2] === '[') {
            ...
            if (close !== -1) {
                ...
                segs.push({
                    type: 'embed',
                    wiki: true,
                    target: src.slice(i + 3, close).trim(),
                    alt: null,
                })
                i = close + 2
                continue
...
export function renderEmbedHtml(
    seg: InlineSeg & { type: 'embed' },
    assetUrl: (t: string) => string,
): string {
    ...
    if (spec.kind === 'pdf' && spec.src) {
        ...
        const params = [spec.page, 'toolbar=0', 'navpanes=0', 'view=FitH']
            .filter(Boolean)
            .join('&')
        return (
...
function renderSeg(seg: InlineSeg, assetUrl: (t: string) => string): string {
    ...
    return inlineMarked.parseInline(iridescentBismuthCell(seg.raw), {
        async: false,
    }) as string
}
...
function mathSpansIn(src: string): { from: number; to: number }[] {
    ...
    while (i < src.length) {
        if (src[i] === '$' && src[i + 1] === '$') {
            i += 2
            continue
        } // display-math fence — not inline math
        if (src[i] === '$') {
            const end = matchMath(src, i)
            if (end !== -1) {
...
function spanningEmphasisTokens(src: string): EmphToken[] {
    ...
    const inMath = (a: number, b: number): boolean =>
        math.some(s => a < s.to && b > s.from)
...
function renderInlineRun(src: string, assetUrl: (t: string) => string): string {
    ...
    if (tokens.length === 0)
        return tokenizeInline(src)
            .map(seg => renderSeg(seg, assetUrl))
            .join('')
    let out = ''
    let i = 0
    for (const t of tokens) {
        ...
        out +=
            t.open +
            renderInlineRun(
                src.slice(t.start + t.markLen, t.end - t.markLen),
                assetUrl,
            ) +
            t.close
        i = t.end
