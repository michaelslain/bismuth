...
export function latexTokenDecorations(
    offset: number,
    src: string,
): Range<Decoration>[] {
    return tokenizeLatex(src).map(tok =>
        mark(tok.cls).range(offset + tok.from, offset + tok.to),
    )
}
...
export const latexHighlightTheme = EditorView.theme({
    ...
    // colored tokens read as code instead of the body serif. The SIZE is global.css's size-reset
    // list (--code-font-size), the one place every mono run in a note is sized.
    '.cm-math-src': {
