...
test('appendToCell joins onto existing cell content with a <br>', () => {
    ...
    expect(appendToCell(g, 1, 0, '![[cat.png]]').cells).toEqual([
        ['A'],
        ['hello<br>![[cat.png]]'],
    ])
})
...
test('appendToCell can target the header row', () => {
    ...
    expect(appendToCell(g, 0, 1, '![[d.pdf]]').cells[0]).toEqual([
        'A',
        'B<br>![[d.pdf]]',
    ])
})
...
test("decideCellKey lets Cmd/Ctrl app shortcuts PASS THROUGH to App's global handler", () => {
    ...
    expect(
        decideCellKey(key({ key: 'd', metaKey: true, shiftKey: true })),
    ).toBe('pass-through')
})
...
test('decideCellKey keeps the cell-local Cmd combos it owns', () => {
    ...
    for (const k of ['b', 'i', 'u'])
        expect(decideCellKey(key({ key: k, ctrlKey: true }))).toBe(
            'block-format',
        )
})
...
test('parseRowCellSpans locates the column an in-row offset falls in', () => {
    ...
    const spans = parseRowCellSpans(line)
    const colAt = (off: number): number =>
        spans.findIndex(s => off >= s.start && off <= s.end)
...
test('surgicalTableEdit returns null on a row-count change (structural op)', () => {
    const before = '| a |\n| --- |\n| one |'
    expect(
        surgicalTableEdit(before, [['a'], ['one']], [['a'], ['one'], ['two']]),
    ).toBeNull()
})

test('surgicalTableEdit header edits land on line 0', () => {
    const before = '| a | b |\n| --- | --- |\n| x | y |'
    const out = surgicalTableEdit(
        before,
        [
            ['a', 'b'],
            ['x', 'y'],
...
test('remapCursorOffTable: heads outside the block are untouched', () => {
    ...
    expect(remapCursorOffTable(CURSOR_DOC, bFrom - 1, bFrom - 1, null)).toBe(
        bFrom - 1,
    ) // empty line above
    expect(remapCursorOffTable(CURSOR_DOC, bTo + 1, bTo + 1, null)).toBe(
        bTo + 1,
    ) // empty line below
})
...
test('remapCursorOffTable: moving FORWARD onto the block skips past it', () => {
    ...
    expect(remapCursorOffTable(CURSOR_DOC, bFrom, bFrom - 2, null)).toBe(
        bTo + 1,
    )
...
test('remapCursorOffTable: moving BACKWARD onto the block skips above it', () => {
    expect(remapCursorOffTable(CURSOR_DOC, bTo, CURSOR_DOC.length, null)).toBe(
        bFrom - 1,
    )
    expect(
        remapCursorOffTable(CURSOR_DOC, bFrom, CURSOR_DOC.length, null),
    ).toBe(bFrom - 1)
})
...
test('rectFromCells builds the bounding region from any two corners', () => {
    expect(rectFromCells({ r: 2, c: 3 }, { r: 1, c: 1 })).toEqual({
        r: 1,
        c: 1,
        rowSpan: 2,
        colSpan: 3,
    })
    expect(rectFromCells({ r: 0, c: 0 }, { r: 0, c: 0 })).toEqual({
        r: 0,
        c: 0,
        rowSpan: 1,
        colSpan: 1,
...
test('regionsOverlap detects touching vs disjoint rectangles', () => {
    ...
    expect(regionsOverlap(a, { r: 2, c: 0, rowSpan: 1, colSpan: 1 })).toBe(
        false,
    ) // just below
    expect(regionsOverlap(a, { r: 0, c: 2, rowSpan: 1, colSpan: 1 })).toBe(
        false,
    ) // just right
})
...
test('coveredCells hides every cell of a region except its anchor', () => {
    ...
    expect([...covered].sort()).toEqual(
        [cellKey(0, 1), cellKey(1, 0), cellKey(1, 1)].sort(),
    )
})
...
test('addMergeRegion replaces overlapping merges and unmerge removes by any covered cell', () => {
    ...
    regions = addMergeRegion(
        regions,
        { r: 0, c: 0, rowSpan: 2, colSpan: 2 },
        4,
        4,
    )
    expect(mergeAnchorAt(regions, 0, 0)).toEqual({
        r: 0,
        c: 0,
        rowSpan: 2,
        colSpan: 2,
    ...
    expect(mergeContaining(regions, 2, 2)).toEqual({
        r: 1,
        c: 1,
        rowSpan: 2,
        colSpan: 2,
