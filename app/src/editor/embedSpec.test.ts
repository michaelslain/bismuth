...
describe('specForWikiEmbed', () => {
    ...
    test('html embed carries src and passes the fragment through as its page (deep-link via location.hash)', () => {
        ...
        expect(specForWikiEmbed('viz.html', asset)).toMatchObject({
            kind: 'html',
            src: asset('viz.html'),
        })
    })
    test('bare target is a note; .draw is dropped', () => {
        expect(specForWikiEmbed('Some Note', asset)).toEqual({
            kind: 'note',
            target: 'Some Note',
        })
...
describe('specForMarkdownImage', () => {
    test('remote/data URLs render as-is with alt-carried width', () => {
        expect(
            specForMarkdownImage('https://x/y.png', 'cap|80', asset),
        ).toEqual({
            kind: 'image',
            src: 'https://x/y.png',
            alt: 'cap',
        ...
        expect(
            specForMarkdownImage('data:image/png;base64,AAAA', '', asset)?.src,
        ).toBe('data:image/png;base64,AAAA')
    })
    test('a bare vault path is classified by extension', () => {
        expect(
            specForMarkdownImage('attachments/testimg.png', '', asset),
        ).toMatchObject({
            kind: 'image',
            src: asset('attachments/testimg.png'),
        })
        expect(specForMarkdownImage('clip.mp4', '', asset)?.kind).toBe('video')
        expect(specForMarkdownImage('doc.pdf#page=2', '', asset)).toMatchObject(
            { kind: 'pdf', page: 'page=2' },
        )
        expect(
            specForMarkdownImage('viz.html#region=form', '', asset),
        ).toMatchObject({
            kind: 'html',
            src: asset('viz.html'),
            page: 'region=form',
