...
import { applyChatFrame, buildTranscript, type AssistantItem, type TurnItem } from "./chatTranscript";
...
describe("applyChatFrame — user turns", () => {
  it("pushes a replayed user turn as its own item", () => {
    const t: TurnItem[] = [];
    expect(applyChatFrame(t, { type: "user-message", text: "hello there" })).toBe(true);
    expect(t).toEqual([{ role: "user", text: "hello there", images: undefined }]);
  });

  it("keeps a replayed turn's image attachments so an image-only turn survives replay", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "user-message", text: "", images: ["data:image/png;base64,AAAA"] });
    expect(t).toEqual([{ role: "user", text: "", images: ["data:image/png;base64,AAAA"] }]);
  });
  ...
  it("keeps consecutive user turns as separate items", () => {
    ...
    expect(t.map((i) => (i as { text: string }).text)).toEqual(["first", "second"]);
  });
});
...
describe("applyChatFrame — streamed assistant prose", () => {
  it("opens an assistant turn on the first delta", () => {
    const t: TurnItem[] = [];
    expect(applyChatFrame(t, { type: "assistant-text", text: "Hel" })).toBe(true);
  ...
  it("MERGES consecutive text deltas into one part rather than one part per delta", () => {
    const t: TurnItem[] = [];
    for (const chunk of ["Hel", "lo, ", "world"]) applyChatFrame(t, { type: "assistant-text", text: chunk });
    expect(assistantTail(t).parts).toEqual([{ kind: "text", text: "Hello, world" }]);
  });
  ...
  it("starts a NEW text part after an interleaved tool call, so prose splits into two bubbles", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "assistant-text", text: "Reading the note. " });
    applyChatFrame(t, { type: "tool-use", id: "t1", name: "Read", input: { file_path: "a.md" } });
  ...
  it("starts a fresh assistant turn when the previous item is a user turn", () => {
    ...
    expect(assistantTail(t).parts).toEqual([{ kind: "text", text: "second reply" }]);
  });
  ...
  it("does NOT flag a turn answering ordinary prose that merely mentions a slash", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "user-message", text: "what does /context do?" });
...
describe("applyChatFrame — tool calls", () => {
  it("pushes a pending chip carrying the frame's name, input, and machine kind", () => {
    const t: TurnItem[] = [];
    expect(
      applyChatFrame(t, { type: "tool-use", id: "t1", name: "Read", kind: "read", input: { file_path: "notes/a.md" } }),
    ).toBe(true);
    expect(assistantTail(t).parts).toEqual([
      {
        kind: "tool",
  ...
  it("leaves toolKind undefined for a backend that carries no machine token", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "tool-use", id: "t1", name: "Read", input: {} });
    expect((assistantTail(t).parts[0] as { toolKind?: string }).toolKind).toBeUndefined();
  });

  it("resolves the matching chip when its result lands", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "tool-use", id: "t1", name: "Read", input: {} });
    expect(applyChatFrame(t, { type: "tool-result", id: "t1", content: "file body", isError: false })).toBe(true);
  ...
  it("records a FAILED tool result on the chip", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "tool-use", id: "t1", name: "Bash", input: { command: "false" } });
    applyChatFrame(t, { type: "tool-result", id: "t1", content: "exit 1", isError: true });
    const part = assistantTail(t).parts[0] as { result: string | null; isError: boolean; pending: boolean };
  ...
  it("matches a chip in an EARLIER turn — results can arrive out of band", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "tool-use", id: "slow", name: "Task", input: {} });
    applyChatFrame(t, { type: "user-message", text: "meanwhile…" });
    applyChatFrame(t, { type: "assistant-text", text: "still working" });
    applyChatFrame(t, { type: "tool-result", id: "slow", content: "done at last", isError: false });
    const firstTurn = t[0] as AssistantItem;
    expect((firstTurn.parts[0] as { result: string | null; pending: boolean }).result).toBe("done at last");
  ...
  it("resolves only the chip with the MATCHING id, leaving its siblings pending", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "tool-use", id: "a", name: "Read", input: {} });
    applyChatFrame(t, { type: "tool-use", id: "b", name: "Grep", input: {} });
    applyChatFrame(t, { type: "tool-result", id: "b", content: "2 matches", isError: false });
    const parts = assistantTail(t).parts as { id: string; result: string | null; pending: boolean }[];
    expect(parts[0]).toMatchObject({ id: "a", result: null, pending: true });
    expect(parts[1]).toMatchObject({ id: "b", result: "2 matches", pending: false });
  });
  ...
  it("drops a result for an unknown id WITHOUT creating a turn or throwing", () => {
    ...
    expect(applyChatFrame(t, { type: "tool-result", id: "ghost", content: "x", isError: false })).toBe(true);
    expect(t).toEqual([]);
  });
});
...
describe("applyChatFrame — inline prompts", () => {
  it("pushes an unanswered permission card", () => {
    const t: TurnItem[] = [];
    expect(
      applyChatFrame(t, { type: "permission", id: "p1", toolName: "Bash", input: { command: "rm -rf build" } }),
    ).toBe(true);
    expect(assistantTail(t).parts).toEqual([
      { kind: "permission", id: "p1", toolName: "Bash", input: { command: "rm -rf build" }, answered: null },
    ]);
  });
  ...
  it("pushes an unanswered AskUserQuestion card carrying every option", () => {
    ...
    const t: TurnItem[] = [];
    expect(applyChatFrame(t, { type: "question", id: "q1", questions })).toBe(true);
    expect(assistantTail(t).parts).toEqual([{ kind: "question", id: "q1", questions, answered: null }]);
  });
});
...
describe("applyChatFrame — the result footer", () => {
  it("stamps turns + cost onto the turn it ends", () => {
    ...
    expect(applyChatFrame(t, { type: "result", isError: false, numTurns: 3, costUsd: 0.0142 })).toBe(true);
    expect(assistantTail(t).footer).toEqual({ numTurns: 3, costUsd: 0.0142 });
  });

  it("keeps a null cost (a backend that reports no price) as null, not 0", () => {
    const t: TurnItem[] = [];
    applyChatFrame(t, { type: "assistant-text", text: "done" });
    applyChatFrame(t, { type: "result", isError: false, numTurns: 1, costUsd: null });
  ...
  it("opens an assistant turn for a result that arrives with no prose before it", () => {
    ...
    applyChatFrame(t, { type: "result", isError: true, numTurns: 1, costUsd: null });
...
describe("applyChatFrame — the transcript-NEUTRAL frames", () => {
  ...
  const neutral: ChatFrame[] = [
    { type: "manifest", manifest: { model: "opus", permissionMode: "default", slashCommands: [], tools: [], mcpServers: [] } },
    { type: "done" },
    { type: "models", models: [{ value: "opus", label: "Opus", description: "", effortLevels: [] }] },
    ...
    { type: "context", percentage: 12, totalTokens: 24000, maxTokens: 200000 },
  ...
  it("covers every error CODE without ever touching the transcript", () => {
    const codes = ["no-claude", "no-opencode", "no-binary", "visibility-refused", "spawn", "exit", "error"] as const;
    for (const code of codes) {
      const t: TurnItem[] = [];
      expect(applyChatFrame(t, { type: "error", code, message: `failed: ${code}` })).toBe(false);
      expect(t).toEqual([]);
    }
  });
});
...
describe("buildTranscript", () => {
  ...
  it("folds a whole replayed conversation — prose, thinking, a resolved tool call, and a footer", () => {
    const frames: ChatFrame[] = [
      ...
      { type: "tool-use", id: "t1", name: "Read", kind: "read", input: { file_path: "journal/2026-08-04.md" } },
      { type: "tool-result", id: "t1", content: "- [ ] ship the refactor", isError: false },
      { type: "assistant-text", text: "One open task: ship the refactor." },
    ...
    expect(t.length).toBe(2);
    expect(t[0]).toEqual({ role: "user", text: "What's in my daily note?", images: undefined });

    const turn = t[1] as AssistantItem;
    expect(turn.command).toBe(false);
    expect(turn.footer).toEqual({ numTurns: 2, costUsd: 0.031 });
    expect(turn.parts.map((p) => p.kind)).toEqual(["thinking", "text", "tool", "text"]);
    expect(turn.parts[0]).toEqual({ kind: "thinking", text: "The daily note lives under journal/." });
    ...
    expect(turn.parts[3]).toEqual({ kind: "text", text: "One open task: ship the refactor." });
  });
