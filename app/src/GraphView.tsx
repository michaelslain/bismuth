...
export function GraphView(props: {
    ...
    // graph size (bismuth-design/ascii .../guidelines/ascii-zoom.card.html; asciiGrid.ts DEEPEST_WORLD_PER_CELL).
    ...
    const closeMenu = () => {
        setMenuOpen(false)
        renderer.setSearchMatches(new Set())
    ...
    const localLaidOut = createMemo<GraphData>(() => {
        ...
        const pos2 = computeLayout(input, {
            dimensions: 2,
            refineTicks: LOCAL_REFINE_TICKS,
        ...
        return {
            nodes: g.nodes.map(n => ({
                ...
                position2d: pos2[n.id]
                    ? ([pos2[n.id][0], pos2[n.id][1]] as [number, number])
                    : n.position2d,
            })),
            edges: g.edges,
        }
        ...
        const cfg: GraphConfig = {
            ...
            edgeColor: ap.isLight
                ? mixHex(
                      ...
                      hexToIntT(ap.background, 0xffffff),
                      0.45,
                  )
                : hexToIntT(ap.neutral, 0xaeb4c2),
            ...
            labelTextColor: ap.isLight
                ? ap.foreground
                : 'rgba(232,232,238,0.95)',
    ...
    const mountRenderer = () => {
        ...
        renderer.setActiveFile(
            props.active ? props.active.replace(/\.md$/, '') : null,
        )
        if (switcherHadMatch && props.searchMatchIds?.length)
            renderer.setSearchMatches(new Set(props.searchMatchIds))
