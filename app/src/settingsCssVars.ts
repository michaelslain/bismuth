...
export function settingsToCssVars(s: Settings): Record<string, string> {
    ...
    const mix = (a1: string, p: number, a2: string) =>
    ...
    return {
        ...
        '--border-soft':
            a.borderSoft ??
            `color-mix(in srgb, ${a.foreground} 10%, transparent)`,
        '--text-muted': a.neutral,
        // Tertiary / disabled text (design's --faint): the .4–.5 opacity-on-fg used app-wide.
        '--faint':
            a.faint ?? `color-mix(in srgb, ${a.foreground} 42%, transparent)`,
        '--panel': a.surface,
        '--hover-bg':
            a.hoverBg ?? `color-mix(in srgb, ${a.foreground} 8%, transparent)`,
        '--surface-1': a.surface,
        '--surface-2': a.surface2,
        '--surface-3':
            a.surface3 ??
            `color-mix(in srgb, ${a.foreground} 14%, transparent)`,
        ...
        '--scrim-bg':
            a.scrimBg ??
            (light
                ? mix(a.neutral, 32, 'transparent')
                : mix(a.foreground, 62, 'transparent')),
        ...
        '--label-halo':
            a.labelHalo ?? (light ? mix('#fff', 90, 'transparent') : '#05060a'),
        ...
        '--vignette-edge':
            a.vignetteEdge ??
            (light
                ? mix(a.background, 50, a.border)
                : mix(a.background, 70, '#000')),
        ...
        '--accent-soft':
            a.accentSoft ?? `color-mix(in srgb, ${a.accent} 14%, transparent)`,
        ...
        '--grad': (() => {
            const g0 = palette[0] ?? a.accent,
                g1 = palette[1] ?? a.accent,
                g2 = palette[2] ?? a.accent
            const g3 = palette[3] ?? a.accent,
                g4 = palette[4] ?? a.accent
