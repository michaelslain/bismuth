...
test('machine-level tools map to daemon status/devices/owner (no --vault needed)', () => {
    expect(daemonCliArgs('daemon_status', {}, V)).toEqual([
        'daemon',
        'status',
        '--pretty',
    ])
    expect(daemonCliArgs('daemon_devices', {}, V)).toEqual([
        'daemon',
        'devices',
        '--pretty',
    ])
    // owner: read vs claim
    expect(daemonCliArgs('daemon_owner', {}, V)).toEqual([
        'daemon',
        'owner',
        '--pretty',
    ])
...
test('cron tools map to daemon cron run / toggle', () => {
    ...
    expect(
        daemonCliArgs('cron_toggle', { name: 'dream', enabled: false }, V),
    ).toEqual(['daemon', 'cron', 'toggle', 'dream', '--vault', V, '--off'])
    // enabled:true is the default → no --off
    expect(
        daemonCliArgs('cron_toggle', { name: 'dream', enabled: true }, V),
    ).toEqual(['daemon', 'cron', 'toggle', 'dream', '--vault', V])
})
...
test('page tools map to page list / create / resolve', () => {
    expect(daemonCliArgs('page_list', {}, V)).toEqual([
        'page',
        'list',
        '--vault',
        V,
    ...
    expect(
        daemonCliArgs('page_create', { slug: 's', actions: '[]' }, V),
    ).toEqual([
        'page',
        'create',
        's',
    ...
    expect(
        daemonCliArgs(
            'page_resolve',
            { path: '.daemon/pages/x.md', action: 'ok' },
            V,
        ),
    ).toEqual([
        'page',
...
test('a missing required arg throws (server turns it into an isError result)', () => {
    ...
    expect(() => daemonCliArgs('cron_toggle', {}, V)).toThrow(
        "'name' is required",
    )
    expect(() => daemonCliArgs('process_toggle', {}, V)).toThrow(
        "'name' is required",
    )
    expect(() => daemonCliArgs('page_create', {}, V)).toThrow(
        "'slug' is required",
    )
    expect(() => daemonCliArgs('page_resolve', { path: 'x' }, V)).toThrow(
        "'action' is required",
    )
