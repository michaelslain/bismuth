...
try {
  ...
  console.error(`--vault ${VAULT} does not exist or is not accessible: ${(err as Error).message}`);
  process.exit(1);
}
...
const RESERVED_OPT_KEYS = ["dimensions", "refineTicks", "initialPositions", "fixedIds"] as const;
for (const k of RESERVED_OPT_KEYS) {
  if (k in EXTRA) {
    console.error(`--opts may not set "${k}" — it is part of the harness's fixed cold-path contract. --opts may only ADD forces, never redefine dimensions/refineTicks/initialPositions/fixedIds.`);
    process.exit(1);
  }
}
...
function fingerprint(nodeIds: string[], graphEdges: { from: string; to: string; kind: string }[]): string {
...
function sanitizeNaN(dimLabel: "d2" | "d3", obj: Record<string, number>): Record<string, number | string> {
  const out: Record<string, number | string> = {};
  for (const [k, v] of Object.entries(obj)) {
    if (!Number.isFinite(v)) {
      const path = `${dimLabel}.${k}`;
      console.error(`WARNING: ${path} is ${v} — the measurement pool was empty (see bench/layoutmetrics.ts doc comments). This is NOT a valid "0" measurement.`);
      nanFields.push(path);
      out[k] = String(v);
    } else {
      out[k] = v;
