 ...
 */
export function neighbourhoodPreservation(
    pos: Pt[],
    adj: number[][],
    k: number | 'degree',
): number {
 ...
 * crossings".
 */
export function edgeCrossingRate(
    pos: Pt[],
    ...
    let crossings = 0,
        tried = 0
    const consider = (
        e1: { a: number; b: number },
        e2: { a: number; b: number },
    ) => {
        if (e1.a === e2.a || e1.a === e2.b || e1.b === e2.a || e1.b === e2.b)
            return
        tried++
        if (segIntersect(pos[e1.a], pos[e1.b], pos[e2.a], pos[e2.b]))
            crossings++
    }
    if (totalPairs <= EXACT_PAIR_LIMIT) {
        for (let i = 0; i < m; i++)
            for (let j = i + 1; j < m; j++) consider(edges[i], edges[j])
    } else {
        const rnd = lcg(777)
        for (let s = 0; s < samples; s++) {
            const e1 = edges[(rnd() * m) | 0],
                e2 = edges[(rnd() * m) | 0]
            if (e1 === e2) continue
            consider(e1, e2)
        }
...
export function nearestNeighbourStats(
    pos: Pt[],
    dim: 2 | 3,
): { cv: number; min: number; median: number } {
    const n = pos.length
