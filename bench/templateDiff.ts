...
type Babel = { transformSync: (code: string, opts: Record<string, unknown>) => BabelResult };
const { transformSync } = (await import(Bun.resolveSync("@babel/core", VPS))) as Babel;
...
for (let i = 0; i < argv.length; i++) {
  const a = argv[i]!;
  if (!a.startsWith("--")) { sides.push(a); continue; }
...
const LINES_A = opts.get("lines-a") ?? LINES, LINES_B = opts.get("lines-b") ?? LINES;
...
function readSide(spec: string): string {
  ...
  if (m) {
    ...
    if (r.exitCode !== 0) { console.error(`cannot read "${spec}" as a file or as a git ref:\n${r.stderr.toString()}`); process.exit(2); }
...
function prepare(src: string, range: string, spec: string): string {
  ...
  if (!m) { console.error(`--lines wants N-M, got "${range}"`); process.exit(2); }
  const lines = src.split("\n").slice(Number(m[1]) - 1, Number(m[2]));
  if (lines.length === 0) { console.error(`${spec}: line range ${range} is empty`); process.exit(2); }
...
function templates(src: string, spec: string): string[] {
  let code: string;
  try {
    code = transformSync(src, {
      filename: spec.replace(/^.*:/, ""),
      presets: [[PRESET, {}]],
      parserOpts: { plugins: ["typescript", "jsx"] },
      configFile: false, babelrc: false, cloneInputAst: false,
    })?.code ?? "";
  } catch (e) {
    console.error(`${spec}: failed to compile — ${(e as Error).message.split("\n")[0]}`);
    process.exit(2);
  }
  // The literal handed to _$template. Both quote styles, because the preset picks per content.
  return [...code.matchAll(/_\$template\(\s*(`(?:[^`\\]|\\.)*`|"(?:[^"\\]|\\.)*"|'(?:[^'\\]|\\.)*')/g)].map((m) => m[1]!);
}
...
const stripClassAttrs = (t: string) =>
  t.replace(/ class=(?:"[^"]*"|'[^']*'|[^\s>]*)/g, (m, off: number, s: string) =>
...
if (INDEX !== null) {
  if (!a[INDEX] || !b[INDEX]) { console.error(`--index ${INDEX} out of range (A has ${a.length}, B has ${b.length})`); process.exit(2); }
  a = [a[INDEX]!]; b = [b[INDEX]!];
}
...
if (a.length === 0 && b.length === 0) {
  ...
  console.error(`both sides emitted ZERO templates — nothing was compared. Wrong --lines range, or neither side contains JSX.`);
  process.exit(2);
}

const norm = (t: string) => (MODULO_CLASS ? stripClassAttrs(t) : t);
const label = MODULO_CLASS ? "modulo class attributes" : "byte-for-byte";
console.log(`A: ${specA}${LINES_A ? ` (lines ${LINES_A})` : ""} — ${a.length} template(s)`);
console.log(`B: ${specB}${LINES_B ? ` (lines ${LINES_B})` : ""} — ${b.length} template(s)`);
console.log(`comparing ${label}\n`);

const diffs: string[] = [];
if (a.length !== b.length) diffs.push(`template COUNT differs: A has ${a.length}, B has ${b.length}`);
for (let i = 0; i < Math.max(a.length, b.length); i++) {
  const x = a[i] === undefined ? null : norm(a[i]!);
  const y = b[i] === undefined ? null : norm(b[i]!);
  if (x === y) { if (VERBOSE) console.log(`[${i}] same: ${x}`); continue; }
  if (x === null || y === null) { diffs.push(`[${i}] present on only one side:\n  ${x ?? y}`); continue; }
...
if (diffs.length === 0) {
  for (const [i, t] of a.entries()) if (!VERBOSE) console.log(`[${i}] ${norm(t)}`);
  console.log(`\nMATCH (${label}) — emitted markup is identical: tags, nesting, attribute names and values, and every text node.`);
  if (MODULO_CLASS) console.log(`NOTE: class attributes were excluded by --modulo-class. This says NOTHING about whether they were converted correctly — pair it with bench/probeStory.ts or bench/moduleClassCheck.ts.`);
  process.exit(0);
}
