...
if (!has("skip-build")) {
  ...
  const r = spawnSync("bun", ["run", "build"], { cwd: APP, stdio: ["ignore", "ignore", "inherit"] });
  if (r.status !== 0) { log("BUILD FAILED"); process.exit(2); }
}
if (!existsSync(ASSETS)) { log(`no build output at ${ASSETS} — drop --skip-build`); process.exit(2); }
...
const allFiles = (dir: string, acc: string[] = []): string[] => {
  for (const e of readdirSync(dir, { withFileTypes: true })) {
    ...
    if (e.isDirectory()) allFiles(p, acc); else acc.push(p);
  }
  return acc;
};
...
function stripGlobal(css: string): string {
  ...
  for (;;) {
    ...
    o += css.slice(i, at);
    let depth = 0, j = at + ":global".length;
    for (; j < css.length; j++) { if (css[j] === "(") depth++; else if (css[j] === ")" && --depth === 0) break; }
    i = j + 1;
  }
}
const classesIn = (css: string) =>
  new Set((stripGlobal(stripComments(css)).match(/\.[A-Za-z_][A-Za-z0-9_-]*/g) ?? []).map((s) => s.slice(1)));

const moduleFiles = allFiles(SRC).filter((f) => f.endsWith(".module.css"));
if (moduleFiles.length === 0) { log("found no *.module.css under app/src — nothing to check (is this the right tree?)"); process.exit(2); }
const moduleClasses = new Map<string, Set<string>>();   // "FileTree.module.css" -> {ft-row, …}
for (const f of moduleFiles) moduleClasses.set(relative(SRC, f), classesIn(readFileSync(f, "utf8")));
log(`${moduleFiles.length} CSS modules, ${[...moduleClasses.values()].reduce((n, s) => n + s.size, 0)} declared class names`);
...
for (const f of allFiles(SRC).filter((f) => f.endsWith(".css") && !f.endsWith(".module.css")))
...
const css = new Map(cssFiles.map((f) => [f, readFileSync(join(ASSETS, f), "utf8")]));
const js = new Map(jsFiles.map((f) => [f, readFileSync(join(ASSETS, f), "utf8")]));
log(`${cssFiles.length} css + ${jsFiles.length} js chunks in ${relative(ROOT, ASSETS)}`);
...
for (const [file, text] of css) {
  for (const m of text.matchAll(HASHED_IN_CSS)) {
    const [, , local, hash] = m;
    addLocal(hash, local);
    if (!cssHomeOf.has(`${hash}:${local}`)) cssHomeOf.set(`${hash}:${local}`, file);
  }
}
if (localsByHash.size === 0) {
  out.push("FATAL: no hashed CSS-Modules class found in any emitted stylesheet.");
  out.push("Either the build output is stale/empty or Vite's scoped-name format changed — either way this check is not looking at anything.");
...
for (const [file, text] of js) {
  ...
  for (const m of text.matchAll(ALIAS)) if (HASHED.test(m[2])) aliases.set(m[1], m[2]);

  for (const m of text.matchAll(OBJECT)) {
    const ident = m[1], body = m[2];
    ...
    for (const [, value] of entries) {
      ...
      for (const t of toks.slice(1)) {
        ...
        if (c) { addLocal(c[2], c[1]); bump(composed, c[2], c[1]); }
      }
    }

    // Reads of this map. Word-boundary-anchored so a longer identifier ending in `ident` is not a hit.
    const esc = ident.replace(/\$/g, "\\$");
    for (const r of text.matchAll(new RegExp(`(?<![A-Za-z0-9_$])${esc}\\.([A-Za-z_$][\\w$]*)`, "g"))) bump(readKeys, hash, r[1]);
    for (const r of text.matchAll(new RegExp(`(?<![A-Za-z0-9_$])${esc}\\[\\s*("([^"]*)"|'([^']*)')\\s*\\]`, "g"))) bump(readKeys, hash, r[2] ?? r[3]!);
    ...
    if (new RegExp(`(?<![A-Za-z0-9_$])${esc}\\[\\s*(?!["'])`).test(text)) bump(dynamic, hash, file);
  }
}
...
for (const [hash, locals] of localsByHash) {
  const fits = [...moduleClasses].filter(([, defined]) => [...locals].every((l) => defined.has(l)));
  if (fits.length === 1) moduleOf.set(hash, fits[0][0]);
    ...
    unattributed.push(`AMBIGUOUS: hash ${hash} fits ${fits.length} modules (${fits.map((f) => f[0]).join(", ")}); reporting as ${fits[0][0]}`);
  } else unattributed.push(`UNATTRIBUTED: hash ${hash} (locals: ${[...locals].sort().join(", ")}) matches no *.module.css class set`);
}
/** hashId of a module, inverted — used to notice a module that emitted nothing at all. */
const hashOf = new Map([...moduleOf].map(([h, m]) => [m, h]));
const name = (hash: string) => moduleOf.get(hash) ?? `<unknown module, hash ${hash}>`;
...
for (const [hash, locals] of [...localsByHash].sort()) {
  ...
  if (!mapsFound.has(hash)) {
    findings.push(`${mod}: NO EXPORT MAP found in any JS chunk — every class in this module is unreachable (module imported by nothing, or the emit shape changed)`);
    continue;
  }
  if (dynamic.has(hash)) {
    skippedDynamic.push(`${mod}: indexed by a runtime key in ${[...dynamic.get(hash)!].join(", ")} — reachability UNCHECKED for this module`);
    continue;
  }
  ...
  for (const local of [...locals].sort()) {
    ...
    const home = cssHomeOf.get(`${hash}:${local}`) ?? "(no rule; exported only)";
    findings.push(`${home}: _${local}_${hash}_… (from ${mod}) — hashed class is in the CSS but nothing reads styles["${local}"]`);
  }
}
for (const [mod] of moduleClasses) if (!hashOf.has(mod)) findings.push(`${mod}: emitted NOTHING into the bundle — no hashed class from this module is present (dead module, or it failed to build)`);
...
for (const [hash, locals] of [...localsByHash].sort()) {
  ...
  for (const local of [...locals].sort()) {
    if (globalClasses.has(local)) {
      collisions.push(`${mod}: "${local}" is declared by a global stylesheet too — check B skipped (indistinguishable in the bundle); if the global rule is a LEFTOVER of this migration, delete it`);
      continue;
    }
    const inJs = literalClasses.get(local);
    if (inJs) findings.push(`${[...inJs].join(", ")}: class=${local} (from ${mod}) — UNHASHED literal reaches the DOM and matches no rule`);
  }
}
...
// 8. Report
// ---------------------------------------------------------------------------------------------
log(`${localsByHash.size} modules present in the bundle: ${[...localsByHash.keys()].map((h) => `${name(h)}=${h}`).join(", ")}`);
const warnings = [...unattributed, ...skippedDynamic, ...collisions];
out.push(`WARNINGS (${warnings.length}) — reviewed by a human, never a failure on their own:`);
if (has("verbose")) for (const s of warnings) out.push(`  ${s}`);
else {
  for (const s of [...unattributed, ...skippedDynamic]) out.push(`  ${s}`);
  if (collisions.length) out.push(`  ${collisions.length} name(s) shared with a global stylesheet — check B skipped for them (--verbose to list)`);
}
out.push("");
out.push(`FINDINGS (${findings.length}) — each is a class that cannot be reaching the DOM as intended:`);
...
if (findings.length) {
  ...
  try { rmSync(DUMP, { force: true }); } catch {}
}
