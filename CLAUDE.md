...
**The original ASCII design handoff is retired — it lives only in git history.** Source comments citing `bismuth-design/ascii/…` or `bismuth-design/ascii-extended/…` name files that were deleted in `8335a574`; read one with `git show 8335a574^:design/ascii/<rest of path>`. It was taken out of the tree because its prototype components sat beside the real ones, free to drift and easy to mistake for shipping code. The live design system is `DESIGN.md`, the tokens and `app/src/ui/`.
...
- **The design-system gate + `tokenLint`** also run in `scripts/gate.ts`, as one combined step, whenever a staged path touches `app/src/`, `DESIGN.md`, `design/` or `scripts/designSystem/` — component/story/token conformance against `DESIGN.md`'s governance block — including **`globalReach`**, which flags a component stylesheet reaching a class it does not own via `:global()` (the one-importer rule in a spelling the import graph cannot see; exempt via `externalClasses` for DOM built outside the bundler). Also **`oneGlobalFile`**: the global layer is ONE stylesheet plus N `<Component>.module.css` — Bismuth is now down to that shape, a single `app/src/global.css`. Ratcheted by `design/baseline.json` (`DESIGN.md` stays at the root — impeccable reads it only there); its `accepted` array is now empty. The only in-line `design-system-ignore globalReach` exemptions left are 7 document-level/third-party rules — pdf.js's own `.markedContent` spans (`preview/PdfPageCanvas.module.css`) and the `html.kb-dragging`/`html.view-dragging` drag-state classes on `<html>` (`bases/KanbanView.module.css`, `shell/GraphFloater.module.css`, `shell/PaneOverlay.module.css`); reaches into `cm-*`/`xterm*`/`bismuth-*`/`callout-*` DOM are exempt wholesale via `externalClasses`. Permanent exceptions go in `DESIGN.md`'s governance block, single lines in a `design-system-ignore <check-id>: <reason>` comment. A FOURTH, independent step runs `bench/moduleClassCheck.ts` (builds the app, ~11s) whenever a staged path matches `app/src/**/*.css`, catching a stale string-literal class the other checks can't see. Detail: `docs/contributing/testing.md`.
...
- `chat.ts` — the visual chat (`/chat` WS): one long-lived Agent-SDK `query()` per chat over the user's own binary. `chatProviders/`+`agentBackends/` — the provider seam + capability catalog behind **nine** backends (claude, opencode, codex + six ACP-based). Refs: `docs/chat/{overview,backends}.md`.
...
**Caching + data flow**: `cachedGraph`/`cachedTree` persist until vault/memory files change; `rowsCache`/`tasksCache` are `createAsyncCache` instances (in-flight dedup). A change → 250ms debounce → `changeClassifier.ts` marks caches dirty (content-only edits stay silent), bumps `version`, pushes SSE `{version, paths, dirty}` on `/events`. The frontend keeps one `EventSource("/events")`, with a `/version` poll as fallback. **Self-write suppression** (`core/src/selfWriteMarks.ts`): a mutation marks the paths it writes so the watcher drops its own echo (one echo, re-armed with a 2s grace, released on throw/≥400); `POST /daily-note` + the `GET /file` `.settings` reconcile do NOT mark yet. Ref: `docs/overview/data-flow.md`.

### Frontend App (`app/`)

Solid.js + TypeScript, CSS Modules.

- `App.tsx` — root: tab + pane tree, active-file routing, graph mode, settings persistence, global keys. It composes its render out of **`shell/`** (`AppFrame` `TopStrip` `TabRail` `Sidebar` `StatusBar` `EditorPane` `GraphFloater` `PaneOverlay` `DragGhost` …) — presentational, slot-driven components that own no signal and never fetch, so every one renders in Storybook with stubs. `panes.ts` — pure binary-tree model for splits (Leaf/Split), unit-tested. `PaneTree.tsx`/`PaneContent.tsx` route a Leaf to a note, Bases view, `.sheet`, `.draw`, calendar, tasks, flashcards, terminal, chat, preview, or export view.
- `tabIds.ts` — sentinel ids for non-file panes (`::graph`, `::daemon`, `::empty`, prefixed `::flashcards:`/`::term:`/`::export:`/`::chat:`); notes/bases/sheets/drawings/settings route by path. No `::search` — search is the Cmd+O switcher takeover (`palette/SwitcherBar.tsx`), the app's ONE search surface.
...
- **Ink lives IN the note, as a ` ```draw ` fence** — no sidecar. **The fence's MODE IS ITS INFO STRING** (` ```draw ` = attached over the block above, ` ```draw block ` = standalone) — never infer it from a blank line. The coordinate contract is **asymmetric** between the two, and anything rendering a note must tell a draw fence from an ordinary one (export does: `app/src/export/inkHtml.ts`). `core/src/drawing/drawBlocks.ts` (pure) · `editor/InkOverlay.tsx`+`inkCommit.ts` · `editor/drawBlock.ts`. Full contract: `docs/editor/ink.md`.
...
- **`ViewBar` is THE view header** (graph, bases, calendar, flashcards, chat) — six NAMED SLOTS, not positional children: `identity` `locus` `facet` lead, `readouts` `config` `actions` trail. A control's region is decided by the QUESTION it answers, not its shape. A Bases view *kind* that contributes controls returns a `ViewBarSlots` object rather than stacking its own bar (`calendarSlots()`, `flashcardsSlots()`); `BaseView` owns the single bar. Collapse is ONE shared ladder in `ui/ViewBar.module.css` that a control opts into by TAGGING — `data-bar-drop` on a control, `<BarLabel long short drop>` on a word. Both hooks are `data-*`, never classes — see the hashing trap under **Styling** (`ui/barDropLevels.test.ts` pins every tag to a level the ladder actually defines). The chat controls row has no bar (it renders under the composer, not in a `ViewBar`) and so keeps its OWN, SEPARATE collapse ladder — a `@container chatrow` query (named on `.row` itself, since the row's available width differs between the chat tab and the daemon page) tagging a word with `data-row-label` to drop it (icon-only bracket) and reserving `data-row-drop` for a control a future tier hides whole, both in `chat/ChatControls.module.css`, guarded the same way by a dedicated test (`chatRowDrop.test.ts`).
...
- **Settings have no GUI page** — the "settings page" IS `.settings` (a hidden extensionless file per vault) opened in the editor like any note, with schema-aware autocomplete + lint. `core/src/schema/settingsSchema.ts` is the source of truth.
...
**Typography**: exactly two font-family settings, both schema-driven. `appearance.uiFont` (Monaspace, default `Monaspace Xenon`) governs all chrome (rail, tabs, buttons, menus, calendar chips) AND the mono constructs inside a note — code blocks, inline code, frontmatter, math, in-note tags — plus config buffers (`.settings`, `*.yaml`), which render entirely in it. `appearance.proseFont` (default `Lora`, family string `'Lora Variable'`) governs note prose, note headings, note tables, chat message bodies and the chat composer, sized by `--prose-font-size` = `--editor-font-size` × `--prose-scale` (`1.04`, re-derived from measured x-height whenever the face changes). Anything pulled back OUT of prose — code, frontmatter + both fence rows, inline code, `#tags`, task fields — returns to `--ui-font-stack` at ONE size, `--code-font-size` (prose × `--code-scale` 0.89: Monaspace at prose x-height is 24% wider + 16% inkier than Lora, so it sits one step down; inline code takes the same ratio off `1em` as `--fs-rel-code`). It is set in exactly one place per surface — `global.css`'s editor size-reset list, `chat/ChatTextBubble.module.css` — and livePreview.ts sets no size on those classes (two equal-specificity rules racing on source order is how code, frontmatter and checkboxes once drifted to three sizes). KaTeX output keeps its own size (`ui/_fontFace.ts` story helpers assert against the live tokens, so a rule that paints in the wrong face or size is caught). Ref: `docs/settings/themes.md`.

**Styling**: **CSS Modules are the rule** — every component's rules live in a colocated `<Component>.module.css` (144 of them). `global.css` is the GLOBAL layer ONLY: design tokens, the element reset, and classes written into runtime-generated HTML strings — one file, no `@import`s. Two traps: **CSS `@import` HOISTS**, so relocating a rule out of `global.css` silently flips precedence (`cssLayering.test.ts` guards this); and a module class is **hashed at build time**, so a call site holding the old literal (`class="ft-row"`) compiles, renders, and matches nothing (`bench/moduleClassCheck.ts` catches it). Colour is centralized in **`core/src/theme/tokens.ts`** (4 themes ink/paper/cathode/riso + semantic tokens + category swatches — in `core` so gcal/drawing/export/schema can import it). Ref: `docs/settings/themes.md`.

**Storybook is THE visual-verification surface** — `bun run storybook` from `app/` (:6006, Storybook 9 + `storybook-solidjs-vite`). `app/.storybook/preview.ts` already projects the real theme tokens and installs an in-memory `fakeTransport` — **never re-solve either per story, never hardcode a stand-in token**. A story id comes from `meta.title`, not the file path: copy keys from `curl -s :6006/index.json`. `GraphView` pauses rAF when the tab is `hidden`, so bench tools drive their own Chrome (`bench/chromeSession.ts`, not `core/src/render/`'s namesake). Rules: `docs/contributing/testing.md`. `app/.storybook/preview.ts` does two things globally that must NOT be re-solved per story: projects the real theme tokens (so **never hardcode a stand-in for a design token**) and installs an in-memory `fakeTransport` (without it every mount-fetching component renders a permanent "Loading…" that reads as a passing story). Fixtures: `app/src/ui/_*`. **A story's id comes from its `meta.title`, NOT its file path**, — resolve every id from the running server (`curl -s :6006/index.json | jq -r '.entries|keys[]'`) and copy the key whole; a hand-built path renders a red "Couldn't find story matching" panel instead of erroring. Rules + URL shape: `docs/contributing/testing.md`. **Caveat:** `GraphView` pauses its rAF loop when `document.visibilityState === "hidden"`, so a backgrounded automation tab samples a 0%-inked canvas — indistinguishable from a broken renderer; every `bench/` tool drives its own Chrome (`bench/chromeSession.ts`) for that reason — **not** `core/src/render/chromeSession.ts`, a separate launcher of the same name that backs headless export.
...
**Owner-token reach** (`cli/src/http.ts`): `call()` attaches `X-Bismuth-Token` from the vault's `0600` run record, **loopback only**, so the owner's shell reads owner-gated routes. The agent boundary is `BISMUTH_AGENT_CHANNEL` — an env var, **not** a cryptographic one. **Adding a command**: a `Command` in a `src/commands/<group>.ts` map (or a new group imported in `index.ts`).

### Bases (`core/src/bases/` + `app/src/bases/`)

> Deep reference: `docs/bases/` + `docs/bases/views/` (per view kind). This is the summary.

A query/view system. A **base is a `type: base` md file** whose frontmatter declares filters, formulas and views over the vault's notes. There is **no `.base` extension**.
...
Task metadata is **bracketed fields** on the checkbox line (`[due 2026-09-14]`, `[high]`, `[every week]`) — the same `[key value]` grammar used elsewhere, and **the only spelling the parser reads**. The Obsidian-Tasks **emoji signifiers are no longer a read path**: a line still carrying one is just a task whose description contains an emoji. A vault is converted on first open (after a git snapshot) or by `bismuth task migrate --dry-run`. Tasks are a **base source** (`source: tasks`, optionally `from: [[Base]]`), never standalone, and `mode: tasks` is what makes any other view render its rows as tasks (checkbox + chips via the shared `bases/TaskRow.tsx`). `taskParse.ts`/`taskFields.ts` read the grammar, `taskLegacy.ts`+`taskMigrate*.ts` convert, `taskReorder.ts` sinks resolved items; `bases/taskDsl.ts` still translates a legacy `tasks: <dsl>` block (`bismuth base migrate-queries` rewrites them); `POST /tasks/toggle` and `POST /tasks/reschedule` write the line back. Ref: `docs/tasks/`.
...
A tab's content is a binary tree of Leaves and Splits (`app/src/panes.ts` — pure, unit-tested); each Leaf holds a content id (a note path or a `tabIds.ts` sentinel). `PaneContent.tsx` routes it; per-window layout is keyed by `windowId.ts`.
...
- **Commands**: `core/src/commands.ts` (`COMMAND_CATALOG`, 51 entries → the `toolbar.command` enum) + `app/src/commands.ts` (`bindCommands` → a live `{id,label,icon,action}` map). The bar above the file tree is configured by `toolbar:` in `.settings`. A spec's `interactive: true` means the action only OPENS a modal a person must finish, so app control reports that rather than implying completion; `UI_CONTROL_BLOCKLIST` bars a few outright. A toolbar item's `commands:` list is a **fallback** — only the first resolvable id runs. **Adding one:** `COMMAND_CATALOG` + an `action` in `bindCommands`. Full list: `docs/settings/toolbar-commands.md`.
- **Keybindings**: `core/src/keybindings.ts` (`KEYBINDING_CATALOG`, 49 entries) + `app/src/keybindings.ts` (`matchesKeybinding`). App.tsx's global handler is catalog-driven, with two narrow deliberate literals — the undiscoverable reset combo and a backup Escape for the Cmd+O switcher; see `app/src/keybindingCoverage.test.ts`. `"Mod"` = Cmd/Ctrl; `"Ctrl"`/`"Cmd"`/`"Meta"` are now separate EXACT tokens (independent of `Mod`), for pinning a combo to one physical key; matching is **exact**; combos comma-separated; matches the produced key OR physical `event.code`. Two ids — `ui-dismiss`/`ui-confirm` — are shared across every transient modal/menu/inline input via `app/src/ui/widgetKeys.ts`'s `isDismissKey`/`isConfirmKey`, rather than each widget getting its own catalog entry. Ref: `docs/settings/keybindings.md`.
...
- **Every component has a colocated `<Component>.module.css`** — never a parallel `styles/` tree. `app/src/global.css` is the only non-module stylesheet.
- **A shared stylesheet means a missing component.** Two components importing one `.module.css` = a component nobody extracted; extract it and compose. Do NOT copy the stylesheet per component.
...
- **Reach through the tree, not through the DOM.** Never `closest('.some-class')` or `classList` against a **class name** inside a component — the hashing trap under **Styling** applies, and the guard silently stops firing. The child declares the event is its own via `stopPropagation`. **Trap:** that does *not* stop `onPointerDown`/`onDblClick`. Leave alone: `closest()` on **tag** selectors or **data attributes**, plain-DOM libraries (CodeMirror, the canvas renderer), and `document.documentElement.classList`.
- **Two element-hook forms, meaning different things.** `data-<name>` (`data-pane-leaf`, `data-tabstrip`, `data-ft-path`) is a **runtime** hook — production CSS/JS reads it, so renaming one breaks the app. `data-testid` is **test-only**: stories and tests query it, and **nothing in production may read it**. Pick by who consumes it; never invent a third form.
...
Bun `workspaces` in the root `package.json`: `core` exports `@bismuth/core`, imported by `app`/`cli`/`mcp`. Add a dep with `cd <workspace> && bun add <package>`; `bun install` at root syncs all. Purposes are in **Architecture** above. **Exhaustive per-file map: `docs/contributing/codebase-map.md`.** Non-workspace top-level dirs: `bench/` (the visual gate), `skills/` (agent skills shipped to all 9 backends), `scripts/` (gate + docs check).
...
**`docs/contributing/codebase-map.md` → "Where to Add Things"** is the lookup table: HTTP endpoint (read vs mutating), graph node/edge kind, setting (plain and CSS-driven), command, keybinding, Bases view kind, Bases function, SRS scheduler variant, graph source, pane file type, shell component, and how to verify a visual change. Two that bite: a **setting** is schema-first (`settingsSchema.ts` → `app/src/settings.ts`, `settings.parity.test.ts` enforces parity; default = the current hardcoded value), and a **new/changed component gets a story** or it is invisible to visual verification.
...
### Graph Modes
`GraphMode` (`app/src/commands.ts`) is exactly `"2nd" | "3rd" | "both" | "local"` — no "agents" or "daemon" mode. **2nd** vault + tags · **3rd** memory · **both** + cross-edges · **local** the open note's neighborhood (`localSubgraph`, the one mode laid out in the browser). Switched from the graph pane's ViewBar; 2D/3D is a transient localStorage flag. Daemon crons/processes live on `::daemon` (`GET /daemon/snapshot`); `daemonGraph()` still backs `bismuth daemon graph`. Ref: `docs/graph/overview.md`.
...
A stdio MCP server serving the `docs/` reference + `bismuth` CLI **token-frugally**: 6 always-on tools (`bismuth_docs_{list,search,read}`, `bismuth_cli`, `bismuth_cli_help`, `bismuth_skill`) + 3 daemon-gated memory tools (`remember`/`recall`/`forget`). **Dev**: auto-attaches per-tab via relay's `.mcp.json`. **Bundled app**: installed machine-wide on boot (`core/src/bismuthInstall.ts`). **App control** adds **ZERO new MCP tools** — it rides `bismuth_cli` via the `app`+`page` groups → core's `/ui/*` WS. Detail: `docs/mcp/{overview,app-control}.md`.
...
- **Runs as a launchd/systemd service, NOT a Tauri child** — it must outlive the app to keep firing crons. `core/src/daemon.ts`/`daemonGraph.ts` are Bismuth's READ window (the daemon page `::daemon` — `app/src/daemon/DaemonPageHost.tsx`, fed by `GET /daemon/snapshot` + `GET /daemon/logs`, with an inline `::chat:daemon` chat in its centre column (`daemon/DaemonChat.tsx`) that only a trusted user gesture on its composer arms — opening the page spawns no session; `daemonGraph()`/`buildDaemonGraph()` themselves still exist and back the CLI's `bismuth daemon graph`, but are no longer a `GraphView` mode).
...
- Cron outcomes, process lifecycle and brain-starts append to `<vault>/.daemon/logs/activity-YYYY-MM-DD.jsonl` (`readActivity()`, `GET /daemon/logs`, `bismuth daemon logs`).
