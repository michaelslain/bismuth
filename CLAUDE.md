...
- `backup.ts` — local-only vault git snapshots. Its `.git/info/exclude` list is an **allow-list**, not a deny-list (a deny-list fails open): `.daemon/*` is excluded, then `identity.md`, `PAGES.md` and `crons/`/`processes/`/`pages/` re-included — so `.settings` and the daemon's *definitions* are tracked, `memory/` (its own repo), `logs/` and `session-id*` are not. Order is load-bearing; old blanket lines are PRUNED on the next backup.

**Caching + data flow**: `cachedGraph`/`cachedTree` persist until vault/memory files change; `rowsCache`/`tasksCache` are `createAsyncCache` instances (in-flight dedup). A change → 250ms debounce → `changeClassifier.ts` marks caches dirty (content-only edits stay silent), bumps `version`, pushes SSE `{version, paths, dirty}` on `/events`. The frontend keeps one `EventSource("/events")`, with a `/version` poll as fallback. **Self-write suppression** (`core/src/selfWriteMarks.ts`): `mutatingHandler` marks only the paths its own route extractor returns (a route with no extractor, or a vault-wide scope that returns none, marks nothing); `PUT /file` and the boot-time `.settings` reconcile mark their own writes inline the same way. `POST /daily-note` and the `.settings` reconcile triggered from `GET /file` write WITHOUT marking (deferred, not yet covered). A marked path: the watcher drops that write's own echo; the mark is consumed on first read (one echo only, so a later external write still schedules), RE-armed — only if still present, so a batch write's already-consumed path is never resurrected — with a 2s grace once the write actually resolves (a write slower than the debounce would otherwise let its echo through), and taken back off when the mutation throws *or* returns ≥400. The change tracker is seeded from the boot-time task-migration scan, so a note's first save after boot can be classified content-only instead of forced structural. Ref: `docs/overview/data-flow.md`.
...
- **Ink lives IN the note, as a ` ```draw ` fence** — no `.ink` sidecar; strokes are delta-coded + deflated base64, one fence per markdown block, and **the fence's MODE IS ITS INFO STRING** (` ```draw ` = attached, painted over the block above reserving no height; ` ```draw block ` = standalone). Never infer the mode from a blank line. **The coordinate contract is ASYMMETRIC** (attached stores y in unscaled px below its block's top; standalone uses the logical scale), and **anything rendering a note must know a draw fence from an ordinary one** — export does (`app/src/export/inkHtml.ts`), Blocks mode does NOT. `core/src/drawing/drawBlocks.ts` (pure) · `editor/InkOverlay.tsx`+`inkCommit.ts` · `editor/drawBlock.ts`. Full contract: `docs/editor/ink.md`.
...
Bun `workspaces` in the root `package.json`: `core` exports `@bismuth/core`, imported by `app`/`cli`/`mcp`. Add a dep with `cd <workspace> && bun add <package>`; `bun install` at root syncs all. Purposes are in **Architecture** above; this is the layout. **Exhaustive per-file map: `docs/contributing/codebase-map.md`.**
...
core/src/
  ...
  relay.ts uiControl.ts runRegistry.ts daemon*.ts     # relay registry; /ui; ports; daemon
  ...
  chat.ts chatProviders/ agentBackends/   # visual chat + the nine backends
  fileAccess.ts localBackend.ts   # mobile IO seam + no-HTTP backend
  bases/ srs/ gcal/ drawing/      # Bases DSL · SRS · gcal sync · .draw vector docs
  render/                         # headless-Chrome HTML→PDF/PNG (the CLI's rasterizer)
...
app/src/
  ...
  ui/          # primitives: Text Heading Button Modal ViewBar … (+ ascii/); _* = fixtures
  styles/      # the only global stylesheets: tokens cmu reset content icons
  Editor.tsx editor/ · BlockEditor.tsx blocks/   # CodeMirror · Milkdown
  GraphView.tsx graph/            # the ONE renderer (AsciiGraphRenderer)
...
app/scripts/     # dev.ts (THE dev entry point) + devVault.ts (the example vault)
bench/           # NOT a workspace — the visual gate (invariants storyAudit playCheck …)

cli/src/    # `bismuth` binary: index.ts dispatcher + args.ts seam + commands/<group>.ts
mcp/src/    # stdio MCP server: 6 always-on + 3 daemon-gated (app control adds ZERO)
relay/      # Claude Code plugin: hooks/ (→ POST /relay/*) + shim/ + .mcp.json
daemon/src/ # per-vault brain: daemon/ (session, cron, process, seeds) + lib/
skills/     # NOT a workspace — agent skills shipped to all 9 backends
```
...
### Graph Modes
`GraphMode` (`app/src/commands.ts`) is exactly `"2nd" | "3rd" | "both" | "local"` — there is no "agents" mode (`relay.ts` feeds chat's subagent tracking + terminal pruning, not a graph), and no "daemon" mode any more. **2nd** vault notes + tags · **3rd** memory · **both** everything + cross-edges · **local** the open note's neighborhood only (`localSubgraph`), the one mode laid out in the browser. Switched from the graph pane's own ViewBar — **not** the status bar. The **2D/3D toggle** is a transient localStorage flag, not a `.settings` key. The daemon's crons/processes moved off the graph onto their own page (`::daemon`), fed by `GET /daemon/snapshot` rather than a graph mode; `core/src/daemonGraph.ts`'s `daemonGraph()`/`buildDaemonGraph()` are unchanged and still back the CLI's `bismuth daemon graph`. Ref: `docs/graph/overview.md`.
...
- **Runs as a launchd/systemd service, NOT a Tauri child** — it must outlive the app to keep firing crons. `core/src/daemon.ts`/`daemonGraph.ts` are Bismuth's READ window (the daemon page `::daemon` — `app/src/daemon/DaemonPageHost.tsx`, fed by `GET /daemon/snapshot` + `GET /daemon/logs`, with a docked `::chat:daemon` chat that only a trusted user gesture on its band arms — opening the page spawns no session; `daemonGraph()`/`buildDaemonGraph()` themselves still exist and back the CLI's `bismuth daemon graph`, but are no longer a `GraphView` mode).
...
- **Every cron outcome, process lifecycle moment and brain-start is appended to a per-vault activity log** (`<vault>/.daemon/logs/activity-YYYY-MM-DD.jsonl`) — the durable history `.last-fired.json` can't give, since it keeps one entry per cron. Read via `readActivity()`, `GET /daemon/logs`, `bismuth daemon logs`, or the `daemon_logs` MCP tool.
