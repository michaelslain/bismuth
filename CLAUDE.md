...
- **The design-system gate + `tokenLint`** also run in `scripts/gate.ts`, as one combined step, whenever a staged path touches `app/src/`, `DESIGN.md`, `design-system.baseline.json` or `scripts/designSystem/` — component/story/token conformance against `DESIGN.md`'s governance block, ratcheted by `design-system.baseline.json`; detail: `docs/contributing/testing.md`.
