...
**Typography**: exactly two font-family settings, both schema-driven. `appearance.uiFont` (Monaspace, default `Monaspace Xenon`) governs all chrome (rail, tabs, buttons, menus, calendar chips) AND the mono constructs inside a note — code blocks, inline code, frontmatter, math, in-note tags — plus config buffers (`.settings`, `*.yaml`), which render entirely in it. `appearance.proseFont` (default `Lora`, family string `'Lora Variable'`) governs note prose, note headings, note tables, chat message bodies and the chat composer, sized by `--prose-font-size` = `--editor-font-size` × `--prose-scale` (`1.04`, re-derived from measured x-height whenever the face changes). Anything pulled back OUT of prose — code, frontmatter, `#tags` — returns to `--ui-font-stack` at `--editor-font-size`, not a scaled multiple (`ui/_fontFace.ts` story helpers assert against the live tokens, so a rule that paints in the wrong face is caught). Ref: `docs/settings/themes.md`.
