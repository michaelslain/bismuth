...
- **Runs as a launchd/systemd service, NOT a Tauri child** — it must outlive the app to keep firing crons. `core/src/daemon.ts`/`daemonGraph.ts` are Bismuth's READ window (the daemon's own page + `DaemonList.tsx`, fed by `GET /daemon/snapshot`; `daemonGraph()`/`buildDaemonGraph()` themselves still exist and back the CLI's `bismuth daemon graph`, but are no longer a `GraphView` mode).
