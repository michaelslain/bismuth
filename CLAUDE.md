...
- **The design-system gate + `tokenLint`** also run in `scripts/gate.ts`, as one combined step, whenever a staged path touches `app/src/`, `DESIGN.md`, `design-system.baseline.json` or `scripts/designSystem/` — component/story/token conformance against `DESIGN.md`'s governance block, with **no baseline — the gate runs at zero** (`design-system.baseline.json` was deleted once its last three entries turned out to be checker false positives; both call sites still pass `--baseline` if the file exists, so re-adding it re-arms the ratchet). Permanent exceptions go in `DESIGN.md`'s governance block, single lines in a `design-system-ignore <check-id>: <reason>` comment. Detail: `docs/contributing/testing.md`.
