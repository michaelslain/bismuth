...
- `settings.daemon.enabled` is the master switch for the whole 3rd-brain/assistant surface (memory injection + `.daemon` folder visibility + 3rd-brain & daemon graph modes). The daemon's **name + personality** live together in **`<vault>/.daemon/identity.md`** (a normal markdown file shown in the sidebar): the `name:` frontmatter drives the folder label, the daemon-graph hub, and the bot's self-identity (`daemonIdentityName()` in core; the registry in `@bismuth/daemon`), and the body is its system prompt (read fresh per session via `appendSystemPrompt`). **Seeding** is one declarative, incremental pass — `reconcileSeeds(ctx)` (`daemon/src/daemon/seeds.ts`, the daemon's analog of core's `reconcileSettings`) writes any seeded default that's MISSING on every brain-start: the editable `identity.md` and the default crons (`dream` = hourly memory consolidation, `vault-review` = 4-hourly model-of-the-user pass, in `defaultCrons.ts`). A default added in a later version lands in already-set-up vaults on next boot; existing files are never clobbered (disable a cron rather than delete it). Add a new seedable by appending one entry to `seedsFor()`. (`daemon.name`/`daemon.home`/`daemon.autoUpdate` were removed from settings.)
