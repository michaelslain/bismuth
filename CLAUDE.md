...
Spaced-repetition reviews. Flashcards are a **Bases view kind** (`flashcards`) over a base's rows — UI is `app/src/bases/FlashcardsView.tsx`. Two code paths share `srs/scheduler.ts` (SM-2) + `srs/types.ts`: **Markdown cards** (`srs/parser.ts` + `srs/cards.ts` = model + `applyReview`) and **Row cards** (base rows with front/back/due/ease/interval columns; `srs/reviewRow.ts`). **Markdown card syntax** — a note is collected ONLY if it carries the `flashcards` tag (`BASE_TAG`; `flashcards/sub` = sub-deck), so an untagged note with perfect syntax yields nothing: **inline** `front::back` (basic) or `front:::back` (reversed, 2 sub-cards); **multi-line** separated by a lone `?`/`??` on its OWN line (a trailing inline `?` is NOT a card); **cloze** via `==x==`, `{{x}}` or `**x**` (`CLOZE_RE`). Queue logic is pure + unit-tested (`app/src/bases/flashcardsQueue.ts`). **Bidirectional** (toggle in `BaseSettings.tsx`): each row yields forward + reverse entries, reverse scheduled in `*Back` columns. **Cram mode** ignores due dates, never writes scheduling. Endpoints: `GET /cards/{decks,all,note,due}`; `POST /cards/review` dual-mode; card CRUD → `POST /row/{update,delete,reorder}`. Detail: `docs/flashcards/srs.md`.
