...
test("sanitizeGitEnv drops every repo-location var and keeps everything else", () => {
  ...
  for (const k of [
    ...
    "GIT_COMMON_DIR", "GIT_NAMESPACE", "GIT_OBJECT_DIRECTORY", "GIT_ALTERNATE_OBJECT_DIRECTORIES",
  ]) {
...
test("a leaked GIT_DIR makes a nested `git -C <other>` write THIS repo's index — sanitizing stops it", () => {
  const git = (args: string[], cwd: string, env: Record<string, string | undefined>) =>
    spawnSync("git", args, { cwd, encoding: "utf8", env: env as NodeJS.ProcessEnv });
  ...
  for (const a of [["init", "-q"], ["add", "-A"], ["-c", "user.email=t@t", "-c", "user.name=t", "commit", "-qm", "init"]]) {
    git(a, victim, process.env);
  }
  const victimIndex = () => git(["ls-files"], victim, process.env).stdout.trim().split("\n").filter(Boolean);
  ...
  const leaked = { ...process.env, GIT_DIR: join(victim, ".git"), GIT_WORK_TREE: vault };
