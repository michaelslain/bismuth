...
const WORKSPACES = ["core", "app", "cli", "mcp", "relay", "memory", "daemon"] as const;
type Workspace = (typeof WORKSPACES)[number];

/** Root-level paths that can affect every workspace, so touching one widens the gate to all. */
const SHARED_PREFIXES = ["package.json", "bun.lock", "tsconfig.base.json", "bunfig.toml", "scripts/", ".githooks/"];
...
export function affectedWorkspaces(staged: string[]): Workspace[] {
  if (staged.some((f) => SHARED_PREFIXES.some((p) => f === p || f.startsWith(p)))) return [...WORKSPACES];
...
function main(): void {
  if (process.env.BISMUTH_SKIP_GATE === "1") {
    process.stdout.write("\x1b[33m[gate] skipped (BISMUTH_SKIP_GATE=1)\x1b[0m\n");
    process.exit(0);
  }

  const staged = stagedFiles();
  if (staged.length === 0) {
    process.stdout.write("\x1b[2m[gate] nothing staged — nothing to check\x1b[0m\n");
    process.exit(0);
  }
  ...
  if (targets.length === 0) {
    ...
    process.stdout.write("\x1b[2m[gate] no source workspace touched — skipping tests\x1b[0m\n");
    process.exit(0);
  }
  ...
  process.stdout.write("\x1b[32m[gate] passed\x1b[0m — slow suites deferred to pre-push\n");
}
