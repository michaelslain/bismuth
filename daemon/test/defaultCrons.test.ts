...
function git(repoRoot: string, ...args: string[]): string {
  return execFileSync("git", ["-C", repoRoot, ...args], { encoding: "utf-8", stdio: ["ignore", "pipe", "ignore"] })
}
...
function cronRevisions(repoRoot: string = REPO_ROOT, path: string = CRONS_PATH): CronRevision[] | null {
  let raw: string
  try {
    raw = git(repoRoot, "log", "--follow", "--format=%x00%H", "--name-only", "--", path)
  } catch {
    return null
  }
...
async function cronsAtRevision(rev: CronRevision, repoRoot: string = REPO_ROOT): Promise<RevisionLoad> {
  let source: string
  try {
  ...
  try {
    ...
    if (!Array.isArray(crons) || crons.length === 0) {
      return { status: "unloadable", reason: "the module imported but exported no DEFAULT_CRONS entries" }
    }
    return { status: "loaded", crons }
  } catch (err) {
    return { status: "unloadable", reason: (err as Error)?.message ?? String(err) }
  } finally {
...
test("every historical shipped version of each default cron is either current or listed in PRIOR_SEED_HASHES", async () => {
  ...
  const currentByName = new Map(DEFAULT_CRONS.map((c) => [c.name, sha256(c.content)]))
  ...
  for (const rev of revisions) {
    ...
    if (loaded.status === "unloadable") {
      unloadable.push(`${rev.path} @ ${rev.commit.slice(0, 8)} — ${loaded.reason}`)
      continue
    }
...
test("the history walk follows a rename and resolves each revision's own path", () => {
  ...
  const g = (...args: string[]) =>
    execFileSync("git", ["-C", repo, "-c", "user.email=t@example.com", "-c", "user.name=t", "-c", "commit.gpgsign=false", ...args], {
      encoding: "utf-8",
  ...
  try {
    ...
    writeFileSync(before, 'export const DEFAULT_CRONS = [{ name: "dream", content: "v1" }]\n', "utf-8")
    g("add", "-A")
    g("commit", "-qm", "add")
    g("mv", "daemon/src/daemon/oldName.ts", "daemon/src/daemon/defaultCrons.ts")
    g("commit", "-qm", "rename")
    writeFileSync(after, 'export const DEFAULT_CRONS = [{ name: "dream", content: "v2" }]\n', "utf-8")
    g("add", "-A")
    g("commit", "-qm", "edit")
    ...
    for (const rev of revisions!) expect(g("show", `${rev.commit}:${rev.path}`)).toContain("DEFAULT_CRONS")
  } finally {
...
test("a revision that exists but cannot be reconstructed reports `unloadable`, never a silent skip", async () => {
  ...
  try {
    ...
    const broken = await cronsAtRevision({ commit: withImport, path: CRONS_PATH }, repo)
    expect(broken.status).toBe("unloadable")
    expect(broken.status === "unloadable" ? broken.reason : "").toContain("not-a-real-sibling")

    const empty = await cronsAtRevision({ commit: withoutExport, path: CRONS_PATH }, repo)
    expect(empty.status).toBe("unloadable")
    expect(empty.status === "unloadable" ? empty.reason : "").toContain("no DEFAULT_CRONS entries")
    ...
    const gone = await cronsAtRevision({ commit: withImport, path: "daemon/src/daemon/never-existed.ts" }, repo)
...
test("the specific stock version the first real install shipped with is listed (regression: it was not)", () => {
  ...
  expect(PRIOR_SEED_HASHES.dream).toContain("751039390e12c74e9bb98044b97eb7bf508e5ec2a73dc71a42942eb61121e870")
  // sha256 of the v1 `vault-review` body (7e1ad46, and unchanged through f48076b), likewise
  // unlisted before this fix.
  expect(PRIOR_SEED_HASHES["vault-review"]).toContain("355f4e794b4eb3860f30d271b0622c4a11e7d1d51c240159d77b1ead4bf38a39")
})
...
test("PRIOR_SEED_HASHES never lists the CURRENT content (that would make an up-to-date file look like a stale prior)", () => {
  for (const cron of DEFAULT_CRONS) {
    expect(PRIOR_SEED_HASHES[cron.name] ?? []).not.toContain(sha256(cron.content))
  }
})
...
test("dream forbids writing a memory note about its own runs, and tells it to delete one it inherits", () => {
  ...
  expect(DREAM).toContain("NEVER write a memory note about yourself, this cron, or how a run went")
  expect(DREAM).toContain("It is not a note")
  expect(DREAM).toContain("`forget` it on this run")
  expect(DREAM).toContain("Write a memory note about this cron, its runs, or its results")
})
...
test("dream collapses date-stamped snapshots and refuses the 'historical record' excuse", () => {
  expect(DREAM).toContain("Collapse date-stamped snapshots into ONE canonical living note")
  expect(DREAM).toContain('"It is a historical record" is NOT a reason to keep a duplicate')
  // The real filenames from the user's graph, kept as the worked example so the rule is unambiguous.
  expect(DREAM).toContain("michael-vault-review-july-26-evening-escalation.md")
...
test("dream's cluster-detection shell one-liner survives template-literal escaping verbatim", () => {
  ...
  expect(DREAM).toContain("awk '{ bytes += $5 } END { printf \"%d notes, %d KB of markdown\\n\", NR, bytes/1024 }'")
...
function shippedClusterCommand(): string {
  ...
  expect(match, "Step 3's cluster-detection bash block is gone from the dream prompt").not.toBeNull()
  return match![1]!
}

/** Pull one of Step 3's worked-example filename lists out of the shipped prompt. */
function shippedExample(marker: string): string[] {
  const match = DREAM.match(new RegExp(`${marker}\\n\\n\`\`\`\\n([\\s\\S]*?)\\n\`\`\``))
  expect(match, `Step 3's worked example "${marker}" is gone from the dream prompt`).not.toBeNull()
...
test("vault-review names canonical notes and forbids dated ones as an instruction, not a preference", () => {
  ...
  expect(VAULT_REVIEW).toContain("**Never create a note whose name contains a date or a month.**")
  expect(VAULT_REVIEW).toContain("michael-vault-review-july-27-evening-critical-update")
  expect(VAULT_REVIEW).toContain("This is an instruction, not a preference")
  expect(VAULT_REVIEW).toContain("Never write a note about this review itself")
  // It must REWRITE the canonical note rather than create a sibling.
  expect(VAULT_REVIEW).toContain("`remember` the SAME name with the full rewritten body")
