...
import { reconcileSeeds, seedsFor, PRIOR_SEED_HASHES } from "../src/daemon/seeds.ts"
 ...
 */
async function shippedCronAtCommit(commit: string, name: string): Promise<string | null> {
  const repoRoot = resolve(dirname(fileURLToPath(import.meta.url)), "..", "..")
  let source: string
  try {
    source = execFileSync("git", ["-C", repoRoot, "show", `${commit}:daemon/src/daemon/defaultCrons.ts`], {
      encoding: "utf-8",
...
test("an existing vault stuck on stock v1 dream.md (2026-06-28) is upgraded — the case that was silently broken", async () => {
  ...
  expect(sha256(v1)).toBe("751039390e12c74e9bb98044b97eb7bf508e5ec2a73dc71a42942eb61121e870")
...
test("an existing vault stuck on stock v1 vault-review.md (2026-06-28) is upgraded too", async () => {
  ...
  if (!v1) return
  expect(sha256(v1)).toBe("355f4e794b4eb3860f30d271b0622c4a11e7d1d51c240159d77b1ead4bf38a39")
...
test("a vault on stock v3 dream.md (2026-07-27, the incremental-scoping release) is upgraded to v4", async () => {
  ...
  if (!v3) return
  expect(sha256(v3)).toBe("d324876622fd7a3453217a90605521f13f17e538ac10da8cbf36464e7c559a1c")
...
test("an existing vault whose vault-review.md still matches stock v2 (2026-07-06) is upgraded in place", async () => {
  mkdirSync(ctx.cronsDir, { recursive: true })
  writeFileSync(join(ctx.cronsDir, "vault-review.md"), VAULT_REVIEW_V2_CONTENT, "utf-8")
