...
test('shouldCatchUp always returns false for a file-change cron, regardless of lastFired history', async () => {
    ...
    const longAgo: Record<string, LastFiredEntry> = {
        watcher: {
            timestamp: new Date(Date.now() - 365 * 24 * 3600_000).toISOString(),
            result: 'failed',
        },
    }
...
async function scheduleJob(
    ...
    if (job.catchup !== catchup)
        throw new Error('catchup frontmatter did not round-trip')
    return job
}
...
function firedAt(
    name: string,
    minutesAgo: number,
    entry: Omit<LastFiredEntry, 'timestamp'>,
) {
...
test('classifyFailure tags every environment error observed in 29 days of real daemon logs', () => {
    ...
    for (const msg of observed)
        expect(classifyFailure(new Error(msg))).toBe('environment')
    ...
    expect(classifyFailure('Claude Code process exited with code 1')).toBe(
        'job',
    )
})
...
test('classifyFailure judges the DETAIL after "API Error:", not the envelope — permanent 4xx are NOT environment', () => {
    ...
    for (const msg of permanent)
        expect(classifyFailure(new Error(msg))).toBe('job')
    ...
    expect(
        classifyFailure(new Error('API Error: something we have never seen')),
    ).toBe('job')
})
...
test('a permanent request error reaches a LONG backoff by the honest route — the streak, not a misclassification', async () => {
    ...
    expect(backoffCooldownMs(interval, 'job', 1)).toBe(
        retryCooldownMs(interval),
    )
    expect(backoffCooldownMs(interval, 'job', 1)).toBeLessThan(
        backoffCooldownMs(interval, 'environment', 1),
    )
    ...
    expect(
        [1, 2, 3, 4, 5, 9].map(
            n => backoffCooldownMs(interval, 'job', n) / MIN,
        ),
...
test('an hourly cron that failed once on a transient network error is NOT eligible again after 5 minutes', async () => {
    const job = await scheduleJob('dream', HOURLY)
    const entry = nextLastFired(undefined, {
        result: 'failed',
        cause: 'environment',
    })
...
test('consecutive transient failures produce a strictly increasing cooldown', () => {
    ...
    const cooldowns = [1, 2, 3].map(n =>
        backoffCooldownMs(daily, 'environment', n),
    )
    ...
    const kills = [1, 2, 3, 4, 5].map(n =>
        backoffCooldownMs(daily, 'timeout', n),
    )
    for (let i = 1; i < kills.length; i++)
        expect(kills[i]!).toBeGreaterThan(kills[i - 1]!)
...
test('the backoff cooldown is clamped to max(interval, 5min): the interval for real crons, the 5-minute floor for sub-floor ones', async () => {
    ...
    for (const cron of [dream.cron, review.cron]) {
        ...
        for (const n of [1, 2, 3, 5, 10, 1000]) {
            expect(
                backoffCooldownMs(interval, 'environment', n),
            ).toBeLessThanOrEqual(interval)
        }
    ...
    expect(backoffCooldownMs(everyMinute, 'environment', 10)).toBe(
        retryCooldownMs(everyMinute),
    )
    expect(backoffCooldownMs(everyMinute, 'environment', 10)).toBe(
        5 * everyMinute,
    )
...
test('a single timeout/kill still retries far sooner than the next scheduled tick', async () => {
    ...
    expect(backoffCooldownMs(interval, 'timeout', 1)).toBe(
        retryCooldownMs(interval),
    )
...
test('a success resets the consecutive-failure counter to zero', async () => {
    ...
    const restarted = nextLastFired(ok, {
        result: 'failed',
        cause: 'environment',
    })
...
test('nextLastFired carries a streak across mixed failure kinds and leaves non-failures clean', () => {
    let entry = nextLastFired(undefined, {
        result: 'failed',
        cause: 'environment',
    })
    ...
    const skipped = nextLastFired(entry, {
        result: 'skipped',
        detail: 'skipped: no changes since X',
    })
...
test('loadLastFired reads a real legacy file off disk and drops only garbage in the NEW fields', async () => {
    ...
    writeFileSync(
        ...
        JSON.stringify({
            ...
            dream: { timestamp: '2026-01-15T11:54:00.000Z', result: 'failed' },
            'bogus-cause': {
                timestamp: '2026-01-15T11:54:00.000Z',
                result: 'failed',
                cause: 'wat',
            },
            'bogus-streak': {
    ...
    const loaded = await loadLastFired({
        ...ctx,
        lastFiredFile,
    } as unknown as VaultContext)
    expect(loaded['ancient']).toEqual({
        timestamp: '2026-01-01T00:00:00.000Z',
        result: 'success',
    })
    ...
    expect(
        (loaded['from-the-future'] as unknown as Record<string, unknown>)[
            'somethingNew'
        ],
    ).toBe(7)
})
...
test('a garbage .last-fired.json value is inert — it must never CAUSE a fire', async () => {
    ...
    expect(shouldCatchUp(job, loaded, notAtScheduledMinute.getTime())).toBe(
        false,
    )
    expect(isBackingOff(job, loaded, notAtScheduledMinute.getTime())).toBe(
        false,
    )
    ...
    expect(
        shouldFireOnTick(
            job,
            loaded,
            new Date(Date.UTC(2026, 0, 15, 13, 0, 0)),
        ),
    ).toBe(true)
})
...
test('an entry-shaped object with a NON-STRING timestamp is inert too', async () => {
    ...
    writeFileSync(
        ...
        JSON.stringify({
            dream: {
                timestamp: new Date(
                    Date.UTC(2026, 0, 15, 11, 0, 0),
                ).toISOString(),
                result: 'success',
            },
        }),
    )
    const real = await loadLastFired({
        ...ctx,
        lastFiredFile,
    } as unknown as VaultContext)
...
function replayFailing(
    ...
    for (let minute = 0; minute <= hours * 60; minute++) {
        ...
        const lastFired: Record<string, LastFiredEntry> = entry
            ? { [job.name]: entry }
            : {}
...
test('INVARIANT replay: under a persistent failure, no shape fires fewer times than its bare schedule', async () => {
    ...
    for (const shape of SHAPES) {
        ...
        const { fires, scheduled } = replayFailing(
            job,
            shape.hours,
            'classified',
        )
        expect(scheduled).toBeGreaterThan(0) // the replay window actually covers the schedule
        expect({ shape: shape.name, ok: fires >= scheduled }).toEqual({
            shape: shape.name,
            ok: true,
        })
...
test('INVARIANT replay with `catchup: false`: the schedule is served exactly, never throttled', async () => {
    ...
    for (const shape of SHAPES) {
        ...
        expect({ shape: shape.name, fires }).toEqual({
            shape: shape.name,
            fires: scheduled,
        })
    }
...
test('INVARIANT stated structurally: when the schedule matches, the tick fires in EVERY backoff state', async () => {
    ...
    for (const result of ['failed', 'killed'] as const)
        for (const cause of ['environment', 'timeout', 'job', undefined] as (
            ...
            for (const state of states) {
                for (const minutesAgo of agos) {
                    ...
                    expect({
                        ...
                        state,
                        minutesAgo,
                        fires: shouldFireOnTick(job, lastFired, fireAt!),
                    }).toEqual({
                        shape: shape.name,
                        catchup,
                        state,
...
test('replay: the number of EXTRA retries is a small constant — it does not grow with the outage', async () => {
    ...
    for (const job of [dream, review]) {
        ...
        expect(replayFailing(job, 24, 'legacy').fires).toBeGreaterThan(
            replayFailing(job, 8, 'legacy').fires * 2,
        )
    }
...
test("buildCronPrompt names the firing vault's own memory dir", () => {
    const a = buildCronPrompt({
        jobName: 'j',
        body: 'b',
        memoryDir: '/vault-a/.daemon/memory',
        notify: false,
    })
    const b = buildCronPrompt({
        jobName: 'j',
        body: 'b',
        memoryDir: '/vault-b/.daemon/memory',
        notify: false,
    })
