...
test("checkpointDirFor: undefined/\"vault\" resolves to ctx.root, \"memory\" resolves to ctx.memoryDir", () => {
  const ctx = { root: "/vault", memoryDir: "/vault/.daemon/memory" } as unknown as VaultContext
...
test("decideIncrementalRun: no ref yet (base=null) never skips, even with an empty file list", () => {
  const d = decideIncrementalRun({ base: null, files: [] }, { refCommitIso: null })
...
test("decideIncrementalRun: ref exists + nothing changed -> skip, with the exact 'skipped: no changes since <iso>' note", () => {
  const d = decideIncrementalRun({ base: "abc123", files: [] }, { refCommitIso: "2026-07-20T10:00:00Z" })
  expect(d.skip).toBe(true)
  if (d.skip) expect(d.note).toBe("skipped: no changes since 2026-07-20T10:00:00Z")
})

test("decideIncrementalRun: ref exists + no resolvable commit time -> skip note falls back gracefully", () => {
  const d = decideIncrementalRun({ base: "abc123", files: [] }, { refCommitIso: null })
  expect(d.skip).toBe(true)
...
test("applyIncrementalPlaceholder substitutes {{changedSinceLastRun}} verbatim", () => {
  ...
  expect(out).toBe("Review the vault.\n\n## Scope\n\nChanged: a.md, b.md\n\nGo.")
...
beforeEach(async () => {
  ...
  ctx = { root: dir, memoryDir: join(dir, ".daemon", "memory") } as unknown as VaultContext
})
...
test("resolveIncrementalRun: first run (no ref yet) never skips, regardless of content", async () => {
  write("note.md", "# Note")
  await commit("init")
  const plan = await resolveIncrementalRun(ctx, { name: "vault-review", prompt: `Body ${CHANGED_SINCE_PLACEHOLDER} end` })
...
test("resolveIncrementalRun: a changed markdown note since the ref -> runs with the file injected into the prompt", async () => {
  ...
  const plan = await resolveIncrementalRun(ctx, { name: "vault-review", prompt: `Body\n${CHANGED_SINCE_PLACEHOLDER}\nend` })
  expect(plan.skip).toBe(false)
  if (!plan.skip) {
...
test("resolveIncrementalRun: a change OUTSIDE the markdown filter (e.g. a .daemon-ish or non-md file) still skips", async () => {
  ...
  const plan = await resolveIncrementalRun(ctx, { name: "vault-review", prompt: `Body ${CHANGED_SINCE_PLACEHOLDER}` })
  expect(plan.skip).toBe(true)
})
...
test("resolveIncrementalRun respects checkpointDir: \"memory\" checks ctx.memoryDir, independent of ctx.root", async () => {
  ...
  await advanceIncrementalCheckpoint(ctx.memoryDir, incrementalRefName("dream"))
  ...
  const plan = await resolveIncrementalRun(ctx, { name: "dream", prompt: CHANGED_SINCE_PLACEHOLDER, checkpointDir: "memory" })
  expect(plan.skip).toBe(true)
})
