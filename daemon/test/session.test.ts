...
test('DEFAULT_DAEMON_IDENTITY names .daemon/memory as the single source of truth', () => {
    ...
    expect(DEFAULT_DAEMON_IDENTITY.toLowerCase()).toContain(
...
test('DEFAULT_DAEMON_IDENTITY does not present claude-bot / a built-in store as authoritative', () => {
    ...
    const disregardClause =
        /disregard[\s\S]*claude code's built-in memory|claude code's built-in memory[\s\S]*disregard/i
...
test("composeBackendRefusalNote: passes the base text through unchanged when there's no refusal", () => {
    expect(composeBackendRefusalNote('Consolidated 3 notes.', undefined)).toBe(
        'Consolidated 3 notes.',
    )
})
...
test("composeBackendRefusalNote: the refusal alone is the whole note when there's no base text", () => {
    ...
    ).toBe("cannot enforce this vault's visibility gate")
})
...
test('buildCodexEnv augments PATH with the CLI install dirs and preserves the inherited env', () => {
    const env = buildCodexEnv(ctx, '/vault/.daemon/codex', {
        PATH: '/usr/bin',
        HOME: '/home/me',
    })
