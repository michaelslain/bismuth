...
function parseProcessFrontmatter(
    name: string,
    ...
    const restart = (frontmatter.restart ??
        'on-failure') as ProcessDef['restart']
 ...
 * works.)
 */
export async function loadProcessDefs(
    ctx: VaultContext,
): Promise<ProcessDef[]> {
    let files: string[]
    try {
 ...
 */
async function writeProcessFile(
    filePath: string,
    frontmatter: Record<string, string>,
    body: string,
): Promise<void> {
...
async function removePidFile(ctx: VaultContext, name: string): Promise<void> {
    try {
        await unlink(join(pidsDirFor(ctx), `${name}.pid`))
    } catch {}
}
...
async function scanPs(): Promise<PsRow[]> {
    try {
        const proc = Bun.spawn(['ps', '-ww', '-eo', 'pid,command'], {
            stdout: 'pipe',
            stderr: 'ignore',
        })
...
async function stopAndClear(
    entries: Array<[string, ManagedProcess]>,
    timeoutMs: number,
): Promise<void> {
 ...
 */
export async function stopProcessesForVault(
    ctx: VaultContext,
    timeoutMs: number = 3000,
): Promise<void> {
    const entries = Array.from(managed.entries()).filter(
        ([, mp]) => mp.ctx.root === ctx.root,
    )
...
export async function listProcesses(
    ctx: VaultContext,
): Promise<{ processes: ProcessInfo[]; orphans: OrphanInfo[] }> {
    const psRows = await scanPs()
    ...
    for (const mp of managed.values()) {
        ...
        if (mp.proc?.pid) {
            if (isAlive(mp.proc.pid)) {
                ...
                console.warn(
                    `[process] "${mp.def.name}" pid ${mp.proc.pid} no longer alive — clearing stale ref`,
                )
                mp.proc = null
                status = 'stale'
                await removePidFile(mp.ctx, mp.def.name)
 ...
 */
export async function enableProcess(
    name: string,
    ctx: VaultContext,
): Promise<{ ok: boolean; error?: string }> {
...
export async function processProcessTriggers(ctx: VaultContext): Promise<void> {
    ...
    for (const name of triggers) {
        ...
        if (name.includes('/') || name.includes('\\')) {
            console.warn(
                `[process] Trigger with invalid name "${name}" — skipping`,
            )
            continue
        }
...
export function startProcessTriggers(ctx: VaultContext): void {
    ...
    const interval = setInterval(() => {
        void processProcessTriggers(ctx)
    }, TRIGGER_CHECK_INTERVAL_MS)
