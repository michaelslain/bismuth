...
async function runCodexExec(
  ...
  const proc = Bun.spawn([bin, ...args], { cwd, stdin: "pipe", stdout: "pipe", stderr: "pipe", env, signal })
  try {
    ...
    for await (const chunk of proc.stdout as ReadableStream<Uint8Array>) {
      ...
      while ((nl = pending.indexOf("\n")) >= 0) {
        ...
        if (ev && typeof ev === "object") {
          ...
          if (e.type === "thread.started" && typeof e.thread_id === "string" && e.thread_id) {
            threadId = e.thread_id
            onThreadId(threadId)
          } else if (e.type === "item.completed" && isRecord(e.item) && e.item.type === "agent_message" && typeof e.item.text === "string") {
...
export async function sendCodexMessage(message: string, ctx: VaultContext, opts?: SendOptions): Promise<BotResponse> {
  ...
  try {
    ...
    const args = buildCodexExecArgs({ jsonFlag, cwd: ctx.root, model: opts?.model, effort: asEffort(opts?.effort), threadId: existingThreadId })
    let result = await runCodexExec(bin, args, ctx.root, env, message, ac?.signal, onThreadId)
    ...
    if (!result.parsedAnyLine && result.exitCode !== 0 && !ac?.signal?.aborted) {
      const retryFlag = fallbackJsonFlag(jsonFlag)
      const retryArgs = buildCodexExecArgs({ jsonFlag: retryFlag, cwd: ctx.root, model: opts?.model, effort: asEffort(opts?.effort), threadId: existingThreadId })
      const retryResult = await runCodexExec(bin, retryArgs, ctx.root, env, message, ac?.signal, onThreadId)
      if (retryResult.parsedAnyLine) jsonFlagByRoot.set(ctx.root, retryFlag)
      result = retryResult
    }
