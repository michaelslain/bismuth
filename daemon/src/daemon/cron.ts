...
export async function loadCronJobs(ctx: VaultContext): Promise<CronJob[]> {
    ...
    for (const file of files) {
        ...
        try {
            ...
            const job = parseCronFrontmatter(
                file.replace(/\.md$/, ''),
                frontmatter,
                body,
            )
...
const FAILURE_CAUSES = new Set<string>([
    'environment',
    'timeout',
    'job',
] satisfies FailureCause[])
 ...
 */
export function normalizeLastFired(
    parsed: unknown,
): Record<string, LastFiredEntry> {
    ...
    for (const [key, value] of Object.entries(
        ...
        if (typeof v.cause !== 'string' || !FAILURE_CAUSES.has(v.cause))
            delete entry.cause
        const streak =
...
export async function loadLastFired(
    ctx: VaultContext,
): Promise<Record<string, LastFiredEntry>> {
    try {
        return normalizeLastFired(
            JSON.parse(await readFile(ctx.lastFiredFile, 'utf-8')),
        )
    } catch {
        return {}
    }
...
const PERMANENT_REQUEST_ERROR_TYPES =
    /\b(invalid_request_error|authentication_error|permission_error|not_found_error|request_too_large)\b/i
...
export function classifyFailure(
    ...
    const detail = envelope
        ? text.slice(envelope.index + envelope[0].length)
        : text
    ...
    return ENVIRONMENT_ERROR_PATTERNS.some(re => re.test(detail))
        ? 'environment'
        : 'job'
}
...
export function nextLastFired(
    ...
    const prevStreak =
        prev && (prev.result === 'failed' || prev.result === 'killed')
...
export async function loadRunning(
    ctx: VaultContext,
): Promise<Record<string, RunningEntry>> {
    try {
...
export function getIntervalMs(cron: CronExpression): number {
    ...
    if (cron.minute.startsWith('*/'))
        return parseInt(cron.minute.slice(2), 10) * 60_000
    if (cron.hour.startsWith('*/'))
        return parseInt(cron.hour.slice(2), 10) * 3600_000

    // Weekly: specific day-of-week with wildcard day-of-month
    if (cron.dayOfWeek !== '*' && cron.dayOfMonth === '*')
...
function backoffWindow(
    ...
    return {
        ...
        cooldown: backoffCooldownMs(
            getIntervalMs(job.cron),
            last.cause,
 ...
 *  such an entry. */
const inBackoffWindow = (w: { elapsed: number; cooldown: number }): boolean =>
    w.elapsed <= w.cooldown
 ...
 */
export function shouldFireOnTick(
    job: CronJob,
    lastFired: Record<string, LastFiredEntry>,
    now: Date,
): boolean {
...
async function restoreDir(
    ...
    for (const [file, content] of snapshot) {
        try {
            ...
            if (current !== content) {
                console.warn(
                    `[cron] Guard: "${jobName}" modified ${dir}/${file} — reverting`,
                )
            ...
            console.warn(
                `[cron] Guard: "${jobName}" deleted ${dir}/${file} — restoring`,
            )
            await writeFile(join(dir, file), content, 'utf-8')
        }
    }
    ...
    try {
        ...
        for (const f of currentFiles) {
            ...
            if (!snapshot.has(f)) {
                console.warn(
                    `[cron] Guard: "${jobName}" created ${dir}/${f} — removing`,
                )
...
async function waitForProcessPattern(
    ...
    while (Date.now() < deadline) {
        ...
        console.log(
            `[cron] "${jobName}": waiting for processes matching "${pattern}"`,
        )
    ...
    if (await hasMatchingProcesses(pattern)) {
        console.warn(
            `[cron] "${jobName}": timed out waiting for processes matching "${pattern}"`,
        )
    }
}
    ...
    const sessionPromise = (async () => {
        try {
            ...
            if (job.waitFor) {
                ...
                const remainingMs =
                    job.timeout > 0
                        ? Math.max(
                              0,
                              job.timeout * 1000 - (Date.now() - startedAt),
                          )
                        : Number.MAX_SAFE_INTEGER
                if (remainingMs > 0) {
                    await waitForProcessPattern(
                        job.waitFor,
                        remainingMs,
                        job.name,
                    )
            ...
            if (job.notify) {
                const status =
                    result === 'success'
                        ...
                        : result === 'failed'
                          ? 'failed'
                          : 'completed (unknown result)'
                const notifyMsg =
                    parseNotifyMessage(response.result) || `Cron job ${status}.`
            ...
            if (ac.signal.aborted) {
                ...
                lastFired[job.name] = await updateLastFired(
                    ctx,
                    job.name,
                    prev =>
                        nextLastFired(prev, {
            ...
            console.error(
                `[cron] Failed to fire job "${job.name}" (${cause}):`,
                err,
            )
            lastFired[job.name] = await updateLastFired(ctx, job.name, prev =>
            ...
            if (ownCronContent !== null) {
                try {
                    ...
                    if (current !== ownCronContent) {
                        console.warn(
                            `[cron] Guard: "${job.name}" modified its own definition — reverting`,
                        )
                    ...
                    console.warn(
                        `[cron] Guard: "${job.name}" deleted its own definition — restoring`,
                    )
                    await writeFile(ownCronFile, ownCronContent, 'utf-8')
                }
            }
 ...
 */
export async function fireFileChangeCron(
    ctx: VaultContext,
    job: FileChangeCronJob,
    changedPaths: string[],
): Promise<void> {
    ...
    console.log(
        `[cron] File-change firing: ${job.name} (${changedPaths.join(', ')})`,
    )
    await fireJob(ctx, job, lastFired, {
        triggerContext: changedPaths.join(', '),
    })
}
 ...
 */
export async function recoverInterruptedCrons(
    ctx: VaultContext,
): Promise<void> {
    ...
    console.log(
        `[cron] Recovering interrupted crons for ${ctx.name}: ${names.join(', ')}`,
    )
...
export async function waitForRunningJobs(
    ...
    console.log(
        `[cron] Waiting for ${runningJobs.size} running job(s) to finish (timeout: ${timeoutMs}ms)...`,
    )
    ...
    if (runningJobs.size > 0) {
        console.warn(
            `[cron] Shutdown timeout — ${runningJobs.size} job(s) still running, aborting`,
        )
