...
async function ensureVaultDirs(ctx: VaultContext): Promise<void> {
    ...
    if (seeded.refreshed.length > 0)
        log(
            `[${ctx.name}] upgraded seeded crons: ${seeded.refreshed.join(', ')}`,
        )
    if (seeded.customized.length > 0)
        log(
            `[${ctx.name}] left customized crons untouched: ${seeded.customized.join(', ')}`,
        )
}
...
async function reconcileVaults(): Promise<void> {
    ...
    for (const { ctx, enabled } of all) {
        ...
        if (enabled && !active) {
            log(
                `Reconcile: vault enabled, starting brain for ${ctx.name} (${ctx.root})`,
            )
            await startVault(ctx, { boot: false })
        } else if (!enabled && active) {
            log(
                `Reconcile: vault disabled, pausing brain for ${ctx.name} (${ctx.root})`,
            )
            await stopVault(ctx)
        }
    }
    ...
    for (const root of [...activeVaults]) {
        if (!seen.has(root)) {
            log(
                `Reconcile: vault no longer registered, pausing brain for ${root}`,
            )
...
async function main(): Promise<void> {
    ...
    if (!(await isOwner()))
        log('Not the owner device — idling (heartbeating only, no sessions)')
...
async function ensureInstalled(): Promise<void> {
    ...
    if (plan === 'skip') {
        console.log(
            `[daemon] service already current and running — leaving it alone (${LAUNCHD_LABEL})`,
        )
        return
    }
    const res =
        plan === 'reload'
            ? await reloadDaemon(configPath, config)
            : await installDaemon(configPath, config)
    if (!res.ok) {
        console.error(`[daemon] install failed: ${res.error}`)
        process.exit(1)
    }
    console.log(
        `[daemon] service ${plan === 'reload' ? 'reloaded' : 'installed'}: ${configPath} (${LAUNCHD_LABEL})`,
    )
}
