    ...
    for (const [folder, folderNotes] of groups) {
        ...
        for (let i = 0; i < folderNotes.length; i += BATCH_SIZE) {
            ...
            try {
                ...
                console.error(
                    `[dream] Failed to dispatch folder=${folder || '<root>'} batch ${i / BATCH_SIZE + 1}:`,
                    err,
                )
                continue
            }
            ...
            for (const merge of result.merge) {
                ...
                const existing = batch.find(
                    n => parseNoteRef(n.name).name === keepName,
                )
                await writeNote(
                    keepName,
                    {
            ...
            for (const imp of result.improve) {
                ...
                const existing = batch.find(
                    n => parseNoteRef(n.name).name === impName,
                )
 ...
 */
export function getDreamConfig(
    ctx: VaultContext,
): DreamConfig & { active: boolean } {
 ...
 * already running.
 */
export function updateDreamConfig(
    ctx: VaultContext,
    config: Partial<DreamConfig>,
): void {
    const wasActive = dreamTimers.has(ctx.root)
    const next: DreamConfig = {
        ...(dreamConfigs.get(ctx.root) ?? DEFAULT_CONFIG),
        ...config,
    }
