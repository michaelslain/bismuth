...
export async function installDaemon(
    ...
    if (IS_LINUX) {
        ...
        if (reload.status !== 0)
            return {
                ok: false,
                error: `daemon-reload failed: ${reload.stderr?.toString()}`,
            }
        const enable = spawnSync('systemctl', [
            '--user',
            'enable',
            '--now',
            SYSTEMD_SERVICE_NAME,
        ...
        if (enable.status !== 0)
            return {
                ok: false,
                error: `enable failed: ${enable.stderr?.toString()}`,
            }
        return { ok: true }
    }
    await Bun.write(configPath, config)
    ...
    if (load.status !== 0)
        return {
            ok: false,
            error: `launchctl load failed: ${load.stderr?.toString()}`,
        }
    return { ok: true }
}
...
export function unloadDaemon(configPath: string): {
    ok: boolean
    error?: string
} {
    if (IS_LINUX) {
        const stop = spawnSync('systemctl', [
            '--user',
            'stop',
            SYSTEMD_SERVICE_NAME,
        ])
        if (stop.status !== 0)
            return {
                ok: false,
                error: `stop failed: ${stop.stderr?.toString()}`,
            }
        const disable = spawnSync('systemctl', [
            '--user',
            'disable',
            SYSTEMD_SERVICE_NAME,
        ])
        if (disable.status !== 0)
            return {
                ok: false,
                error: `disable failed: ${disable.stderr?.toString()}`,
            }
        return { ok: true }
    }
    ...
    if (unload.status !== 0)
        return {
            ok: false,
            error: `launchctl unload failed: ${unload.stderr?.toString()}`,
        }
    return { ok: true }
}
...
export async function reloadDaemon(
    ...
    if (IS_LINUX) {
        ...
        const restart = spawnSync('systemctl', [
            '--user',
            'restart',
            SYSTEMD_SERVICE_NAME,
        ])
        if (restart.status !== 0)
            return {
                ok: false,
                error: `restart failed: ${restart.stderr?.toString()}`,
            }
        return { ok: true }
    }
...
export function restartDaemon(): { ok: boolean; error?: string } {
    if (IS_LINUX) {
        ...
        if (r.status !== 0)
            return {
                ok: false,
                error: `systemctl restart failed: ${r.stderr?.toString()}`,
            }
        return { ok: true }
    }
    ...
    if (r.status !== 0)
        return {
            ok: false,
            error: `launchctl kickstart failed: ${r.stderr?.toString()}`,
        }
    return { ok: true }
}
