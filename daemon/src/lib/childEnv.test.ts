...
test('home-relative dirs use os.homedir() with no hardcoded username', () => {
    ...
    expect(augmentPath(BARE_LAUNCHD_PATH, '/home/alice')).toContain(
        '/home/alice/.bismuth/bin',
    )
    expect(augmentPath(BARE_LAUNCHD_PATH, '/home/bob')).toContain(
        '/home/bob/.bismuth/bin',
    )
    // Default arg resolves to the real homedir().
    expect(augmentPath(BARE_LAUNCHD_PATH)).toContain(
        join(homedir(), '.bismuth', 'bin'),
    )
})
