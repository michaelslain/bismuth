...
import { VAULTS_FILE, VAULTS_SEEN_FILE, vaultPaths, type VaultContext } from "./config.ts"
...
export function stampVaultsSeen(
  ...
  if (raw && typeof raw === "object" && !Array.isArray(raw)) {
    for (const [path, iso] of Object.entries(raw as Record<string, unknown>)) {
...
export async function refreshVaultsSeen(
  ...
  try {
    ...
    const { seen, changed } = stampVaultsSeen(raw, roots, new Date(nowMs).toISOString())
    if (!changed) return
