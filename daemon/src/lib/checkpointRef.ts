...
async function git(dir: string, args: string[]): Promise<{ stdout: string; ok: boolean }> {
  try {
    const { stdout } = await execFileAsync("git", ["-C", dir, ...args], { maxBuffer: 64 * 1024 * 1024 })
...
export async function checkpointRefSha(dir: string, ref: string): Promise<string | null> {
...
export async function commitTimeIso(dir: string, sha: string): Promise<string | null> {
 ...
 */
export async function checkpointDelta(dir: string, ref: string): Promise<CheckpointDelta> {
  ...
  if (refSha === null) {
    const ls = await git(dir, ["ls-tree", "-r", "--name-only", "-z", "HEAD"])
    committed = ls.stdout.split("\0").filter(Boolean).map((path) => ({ status: "A", path }))
  } else {
    const d = await git(dir, ["diff", "--name-status", "-z", refSha, "HEAD"])
    committed = parseNameStatus(d.stdout)
  }

  const wt = await git(dir, ["diff", "--name-status", "-z", "HEAD"])
  const tracked = parseNameStatus(wt.stdout)
  const untrackedOut = await git(dir, ["ls-files", "--others", "--exclude-standard", "-z"])
  const untracked: ChangedFile[] = untrackedOut.stdout.split("\0").filter(Boolean).map((path) => ({ status: "A", path }))
 ...
 */
export async function advanceCheckpointRef(dir: string, ref: string): Promise<string | null> {
  if (!(await isGitRepo(dir))) return null
  const head = await headSha(dir)
  if (!head) return null
