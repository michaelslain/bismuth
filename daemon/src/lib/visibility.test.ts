...
test("buildDenyPaths: 'hidden' AND 'chat-only' are both daemon-restricted (only 'all' is not)", async () => {
    ...
    const denied = (await buildDenyPaths(vault)).sort((a, b) =>
...
test('resolveDenyPlan: an ABSENT .settings is determined — a vault that hides nothing is an answer', async () => {
    ...
    expect(await resolveDenyPlan(vault)).toEqual({
        determined: true,
        entries: [],
    })
})
...
test('buildDenyPaths: a symlinked directory is walked, so the subtree behind it is denied', async () => {
    ...
    expect(entries.map(e => e.rel).sort()).toEqual([
        'link-to-hidden/inside.md',
        'real-hidden/inside.md',
    ])
    ...
    expect(buildManagedSettingsDeny(entries)).toContain(
        `Read(${join(canonical, 'real-hidden/inside.md')})`,
    )
})
