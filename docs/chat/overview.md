...
If `claude` is not on PATH the driver pushes a single `{ type: "error", code: "no-claude" }` frame and returns — it **never falls back to an API**. The session (`app/src/chat/chatSession.ts`) catches that frame and sets `setupError`; `app/src/chat/ChatSetupGate.tsx` (composed by both `ChatView` and the daemon page's `DaemonChat`) reads it and swaps the transcript for a setup state ("Claude Code isn't available"):
...
When the **`chat.computerUse`** setting is on (a boolean, default `false`, in `core/src/schema/settingsSchema.ts` under the `chat` section), `spawnChatQuery` passes `extraArgs: { chrome: null }` (a bare `--chrome` boolean flag) to `query()`, so the spawned `claude` process can launch and control a **Chromium browser** for Claude's browser/computer-use tools. It requires a Chromium-based browser on the system (Chrome/Edge/Brave). The capability is stashed on `session.computerUse`, so a visibility respawn preserves the flag (like `effort`/`model`). It's threaded end-to-end: `sendMessage`/`resumeSession`/`openSession` all take a `computerUse?` argument. The value is **per-chat and client-driven**: the session (`app/src/chat/chatSession.ts`) stamps the chat's own choice onto every `open`/`user`/`resume` message, and the server falls back to `appConfig.chat.computerUse` only when the client sends nothing — so `chat.computerUse` acts as the *default for a chat that hasn't chosen*, set from the composer via the client-side **`/chrome [on|off]`** command or the header Globe pill (see [Client-side slash commands](#client-side-slash-commands)).

Because `--chrome` is a **spawn-fixed CLI flag**, a mid-chat change cannot alter the running `query()`. `sendMessage` reconciles the client's choice against the live session (`computerUseChange`) and, when it differs, marks `spawnOptionsDirty` and **respawns `query()` before running the turn** — passing `resume: session.sessionId`, so the *same conversation* continues with its full history, now with (or without) the browser. This is why the transcript note says "takes effect from your next message": the flag can only matter *during* a turn, and the next turn already has it, so the live chat gains the capability at the earliest moment it could possibly be observed — no new chat, nothing lost.
...
- `{type:"session", sessionId}` — the SDK `session_id` this chat is bound to, emitted the moment the drain loop first learns it (and again if it ever changes, e.g. after a resume). The session (`app/src/chat/chatSession.ts`) persists it keyed by the chat **tab** id (`app/src/chatSessionStore.ts`), so **Reopen closed tab** (Cmd+Shift+T) can resume the *same* conversation — a reopened `::chat:` tab reads the remembered `sessionId` on mount and calls `resumeSession()` instead of spawning a blank session. Durable (the CLI's on-disk session store), so it survives a relaunch too.
...
The header is its own component, **`app/src/chat/ChatHeader.tsx`** (with `ChatHeader.stories.tsx`): an identity crumb plus the controls from **`app/src/chat/ChatControls.tsx`**'s `chatControlSlots(session)`, taking the `ChatSession` whole rather than individual props. Its controls sit in a `ViewBar` (`app/src/ui/ViewBar.tsx`), grouped into four regions by what each control answers rather than source order — **identity** (which chat this is, and whose), **readouts** (tools/MCP/context counts you can't click), **config** (provider · model · effort · browser · permission mode — what governs the session), and **actions** (auth · history · new chat, primary last) — with the config/readout controls each tagged a measured `data-bar-drop` collapse tier so the bar narrows predictably at a smaller pane width instead of overflowing (the permission-mode picker is deliberately never tagged for the ladder — its armed tone is the only signal that the session can write to the vault unconfirmed, and a control that disappears at a narrow pane takes that signal with it). The two popovers (auth, history) are `ChatAuthPanel.tsx` and `ChatHistoryPanel.tsx`, opened from `ChatControls.tsx`.

In the header, `ChatHeader` renders:
...
1. **The picker values and the manifest model are different namespaces.** `Query.supportedModels()` returns short **aliases** (`"default"`, `"sonnet"`, `"haiku"`, `"opus[1m]"` — the `[1m]` suffix is the 1M-context variant, a *distinct* choice), while each `init` manifest reports the **fully-resolved id** the session actually runs (`"claude-haiku-4-5-20251001"`, `"claude-opus-4-8[1m]"`). Every comparison/display therefore goes through `app/src/chatModelResolution.ts` (`modelsCorrespond` / `modelOptionFor` / `modelLabelFor`, pure + unit-tested): a persisted alias *corresponds to* its resolved id (same family, same `[1m]` suffix; `"default"` corresponds to anything), the header `Select`'s value is mapped into picker-space, and the Effort picker keys its levels off the mapped value. Without this, the first turn's manifest "adopted" the resolved id over the persisted alias — deselecting the picker and corrupting the saved choice on every chat.
...
Three slash commands are **intercepted client-side** by the session (`app/src/chat/chatSession.ts`'s `applyLocalCommand`) before a turn is sent — they act on the chat tab or app settings and never reach the model. `app/src/chatSlashCommands.ts` (pure, unit-tested) parses them:
...
- **`/chrome [on|off]`** — set the `--chrome` browser/computer-use capability **for this chat** (BUG #87). It is an **enable verb, not a toggle**: a bare `/chrome` (or `/chrome on`) always ends *enabled* and is idempotent — running it on an already-enabled chat keeps the browser on and answers "already enabled". Only an explicit `/chrome off` disables it. This is deliberate and was the bug's root cause: the command mirrors the `--chrome` CLI flag and reads as "give this chat the browser", so implementing it as a blind toggle (`next = !current`) made it *disable* the browser and report "disabled" whenever the chat was already on — which is the normal state for a user who wants the browser (a vault with `chat.computerUse: true`, or one previous enable). No choice of starting default can fix that; the semantics have to be an assignment. The pure mapping is `computeChromeCommand(current, arg)` in `app/src/chatSlashCommands.ts` (unknown arg → `undefined` → the caller reports it, like `/color`).

  The header **Globe pill** stays a true toggle (`computeChromeToggle`) — a pill renders the state it is about to flip, so toggling is correct there. Both write the same per-chat state (`app/src/chatComputerUse.ts`, localStorage-backed, keyed by the chat tab id), so the pill and the command always agree. A chat that has made no choice of its own falls back to the vault's `chat.computerUse` setting — that fallback is what makes the setting real, since the session stamps this value onto every `open`/`user`/`resume` message and the server only consults `appConfig.chat.computerUse` when the client sends nothing.
...
The chat's rules are factored into small pure modules so they're testable without a live session, CodeMirror, localStorage, or a Solid signal; the stateful half lives in `app/src/chat/chatSession.ts` and the markup in the `app/src/chat/` components (`ChatHeader`, `ChatControls`, `ChatTranscript`, `ChatComposerBar`, …), each covered by stories rather than unit tests. The controls are the biggest consumer of several of these, especially `chatProvider.ts` and `chatModelResolution.ts`:

- **`app/src/chatProvider.ts`** — provider-choice helpers the header and the setup screen both need: `CHAT_PROVIDER_OPTIONS` (every non-`hidden` backend from the catalog, in catalog order), `sanitizeChatProvider`/`providerStorageKey` (persist and guard a chat tab's chosen backend), `modelStorageKeys(provider, chatId)` (backend-scoped model-key namespacing — a Claude model id must never seed an opencode session's `-m` flag, so each backend keeps its own per-chat + global localStorage keys, with Claude and opencode kept on their original unsuffixed/`oc`-suffixed keys for backward compatibility), `providerCan(provider, cap)` (the generic capability lookup every gated header control uses, replacing the old `provider === "claude"` check), `providerLabel`/`providerInstallHint`, and opencode-specific formatting (`modelPriceBadge` for the model picker's Free/Paid badge, `opencodeAuthSummary` for the auth pill's tri-state label).
...
Source: `core/src/chat.ts`, `core/src/server.ts` (`/chat` WS + `GET /chat/sessions` + `GET /chat/session-messages` + `POST /chat/search`), `core/src/chatProviders/index.ts` (`sessionHistoryFrames` provider dispatch), `core/src/ownerToken.ts` + `core/src/runRegistry.ts` (the owner-token gate these three routes enforce), `core/src/schema/settingsSchema.ts` (`chat.computerUse`), `app/src/ChatView.tsx`, `app/src/chat/ChatHeader.tsx`, `app/src/ChatComposer.tsx`, `app/src/chatContext.ts`, `app/src/chatEditorContext.ts`, `app/src/chatComposerKeys.ts`, `app/src/chatHistory.ts`, `app/src/chatQueueRestore.ts`, `app/src/chatSlashCommands.ts`, `app/src/chatProvider.ts`, `app/src/chatModelResolution.ts`, `app/src/chatPermissionMode.ts`, `app/src/chatEffort.ts`, `app/src/chatSessionStore.ts`, `app/src/chatColors.ts`, `app/src/chatTitles.ts`, `app/src/chatOrigin.ts`, `app/src/tabIds.ts`, `app/src/PaneContent.tsx`, `app/src/api.ts`, `app/src/App.tsx`, `app/src/commands.ts`, `core/src/commands.ts`, `core/src/keybindings.ts`, `cli/src/commands/chat.ts`, `cli/src/http.ts`.
