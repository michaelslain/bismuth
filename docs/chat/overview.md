...
The header is its own component, **`app/src/chat/ChatHeader.tsx`** (with a full `ChatHeader.stories.tsx`) — extracted out of `ChatView.tsx`, which shrank by roughly 200 lines, because it was the one view toolbar in the app with no story of its own: nothing visual verification could see. It is props-only — no signal, no socket, no api call; every value and every callback arrives as a prop from `ChatView`, so a story can render all of its controls from literals with no transport, session, or manifest. Its controls sit in a `ViewBar` (`app/src/ui/ViewBar.tsx`), regrouped into four regions by what each control answers rather than source order — **identity** (which chat this is, and whose), **readouts** (tools/MCP/context counts you can't click), **config** (provider · model · effort · browser · permission mode — what governs the session), and **actions** (auth · history · new chat, primary last) — with the config/readout controls each tagged a measured `data-bar-drop` collapse tier so the bar narrows predictably at a smaller pane width instead of overflowing (the permission-mode picker is deliberately never tagged for the ladder — its armed tint is the only signal that the session can write to the vault unconfirmed, and a control that disappears at a narrow pane takes that signal with it). The two popovers (auth, history) stay in `ChatView.tsx` and arrive as JSX slots (`authPanel`/`historyPanel`), since they close over session state — the history list, its search, the auth frame — that the header itself is not allowed to own.
...
`ChatView.tsx` is a large component; its rules are factored into small pure modules so they're testable without a live session, CodeMirror, localStorage, or a Solid signal. The header itself, **`app/src/chat/ChatHeader.tsx`**, is not on this list — it's presentational (props-only, no signal, no fetch; covered by `ChatHeader.stories.tsx` rather than a unit test) — but it's the biggest consumer of several of these, especially `chatProvider.ts` and `chatModelResolution.ts`:
