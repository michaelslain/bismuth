...
- **`sync.json`** (`manifest.ts`, mode `0600`) — the `SyncManifest`, now **keyed by base path**: `{ bases: { [basePath]: BaseSync } }` where each `BaseSync` = `{ lastSyncAt, syncToken, calendarId, links }` and `links` maps a Google event id → `{ bismuthId, etag, updated, sig }`. Per-base keying means every synced calendar keeps its own link map + incremental token + last calendar target — no cross-base contamination (the old single-base `basePath`-binding + retarget guard is gone). `baseSyncOf(m, basePath)` gets or creates a base's entry. `readManifest` **migrates** the old flat single-base shape (`{ links, basePath, syncToken }`) by nesting it under its `basePath`. Kept here, not as columns on the rows, because the frontend calendar serializer only re-emits known event fields and would drop extra sync columns on the next in-app edit. Reads never throw.
