...
4. **The manual `POST /gcal/sync` route is gated the same way.** Unless `gcalAutoSyncEnabled()` is true it answers **`403`** with a JSON `{ error }` that names `BISMUTH_GCAL_AUTOSYNC=1`, and logs `[gcal] manual sync off outside the installed app (set BISMUTH_GCAL_AUTOSYNC=1 to enable)` — before it reads the base, marks a self-write or invalidates anything, so no Google call is made. An ungated manual sync was not safe just because it is "a person clicking Sync now": `bismuth gcal sync <basePath>` goes straight to this route and is reachable by any agent through the CLI or MCP's `bismuth_cli`, and a sync from a dev core on a vault copy used the machine-wide refresh token — its first sync re-linked every event through the `bismuthId` self-heal, later syncs pushed the copy's edits, and Phase C deleted real events whose rows the copy lacked. So a dev/test/agent core cannot sync at all, manually or automatically, until a human sets `BISMUTH_GCAL_AUTOSYNC=1` on it; the CLI prints the route's error sentence as `error: POST /gcal/sync → 403: …`. Opting in this way still does **not** set `claimLegacy` (only `BISMUTH_APP_PATH` does), so an opted-in dev core starts from its own empty namespaced entry rather than a real vault's legacy links.
