...
3. **The background auto-sync ticker itself doesn't exist unless `gcalAutoSyncEnabled()` is true** — `!!process.env.BISMUTH_APP_PATH || process.env.BISMUTH_GCAL_AUTOSYNC === '1'`. `server.ts` checks this once at startup: if false, `setInterval` is never called (not created-then-skipped — never created at all) and it logs once: `[gcal] auto-sync off outside the installed app (set BISMUTH_GCAL_AUTOSYNC=1 to enable)`. This means every dev/test/agent core is silent and inert with respect to Google Calendar unless a human deliberately opts in with `BISMUTH_GCAL_AUTOSYNC=1` (e.g. to test auto-sync itself against a throwaway calendar) — opting into the *ticker* this way does **not** also set `claimLegacy`, so it still can't touch a real vault's legacy links.

**Residual scope, by design:** the manual `POST /gcal/sync` route (a person clicking "Sync now") is unchanged — it still resolves `claimLegacy` the same way (`!!process.env.BISMUTH_APP_PATH` unless the caller of `syncEvents` overrides it), so it is not itself gated by `gcalAutoSyncEnabled()`. A person who deliberately runs a dev core against the **real** vault and clicks "Sync now" can still sync (and, if `BISMUTH_APP_PATH` happens to be set, still claim a legacy entry) — this is considered acceptable because it requires a human in the loop clicking a button, unlike the background ticker.
...
- **`gcal health --vault <dir> [<basePath>]`** reads `readManifest()` (`manifest.ts`) directly — also headless, since the manifest is a plain file at `gcalDir()` (`~/.bismuth/gcal/sync.json`, or `BISMUTH_GCAL_DIR`). Reports `{ basePath, calendarId, lastSyncAt?, linkedEvents, hasSyncToken, legacy? }` per base. Because this file lives **outside every vault**, no vault-scoped command could reach it before this — an agent had no way to answer "when did this calendar last sync, and how many events are linked?" without triggering a sync itself. Per-sync `conflicts` counts are NOT in this output (they're not persisted in `BaseSync` — see `SyncResult` above); they're in `gcal sync`'s own printed result instead. **Requires `--vault`** to resolve `manifestKey(vault, basePath)` — it looks up the namespaced entry first, falling back to a legacy bare-`basePath` entry (marked `legacy: true`) only when the namespaced one is absent. **READ-ONLY**: unlike `syncEvents`'s `baseSyncFor`, this command never creates, moves or claims a manifest entry — it does its own plain, non-mutating lookup inline (`cli/src/commands/gcal.ts`), so running `gcal health` can never itself decide which vault gets a pre-namespacing legacy entry.
...
| `syncIntervalMinutes` | number, `15` (min 1, max 1440) | Auto-sync cadence in minutes for every synced calendar (manual "Sync now" is always available). |
