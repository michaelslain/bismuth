...
- **`gcal health [<basePath>]`** wraps `readManifest`/`baseSyncOf` (`manifest.ts`) — also headless, since the manifest is a plain file at `gcalDir()` (`~/.bismuth/gcal/sync.json`, or `BISMUTH_GCAL_DIR`). Reports `{ basePath, calendarId, lastSyncAt?, linkedEvents, hasSyncToken }` per base. Because this file lives **outside every vault**, no vault-scoped command could reach it before this — an agent had no way to answer "when did this calendar last sync, and how many events are linked?" without triggering a sync itself. Per-sync `conflicts` counts are NOT in this output (they're not persisted in `BaseSync` — see `SyncResult` above); they're in `gcal sync`'s own printed result instead. **Known gap:** this command has no vault argument and still resolves entries by the bare `basePath` (`baseSyncOf`), so for a real, already-syncing vault it now reads the pre-namespacing bare key rather than that vault's actual `manifestKey(vault, basePath)` entry — it will under-report (typically `linkedEvents: 0`, no `lastSyncAt`) until that command is updated to take `--vault` and call `baseSyncFor` instead.
