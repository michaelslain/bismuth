...
- Frontmatter fields of interest: `tags`, `aliases`, `icon` (emoji or Lucide name), `type` (`base` marks a Bases data file), plus any user-defined property types registered under `settings.yaml → properties:`
...
The file tree (`listTree`) surfaces the following extensions to the UI; all others (images, `.txt`, etc.) are omitted from the tree but are still accessible as embeds:

| Extension | Purpose |
|-----------|---------|
| `.md` | Notes, bases, templates, settings |
| `.draw` | Vector drawing documents (JSON `DrawingDoc`) |
| `.sheet` | Univer workbook snapshots (JSON) |
| `.yaml` / `.yml` | Shown in tree (e.g. `settings.yaml`) |

Generated sidecars `.draw.png` and `.draw.pdf` are explicitly **excluded** from the tree even though they live in the vault.
...
Pasted or dropped media (images, PDFs, audio, video) are written into the vault at a configurable location. The default folder is `attachments/` (relative to the vault root), controlled by `settings.yaml → attachments.folder`. Special values:
...
`settings.yaml` is the **single source of truth** for all user-visible settings. The backend is the sole writer; the frontend never directly modifies the file. Filename is the constant `SETTINGS_FILE = "settings.yaml"` in `core/src/settings.ts`.
...
| Vault opened, file present | `reconcileSettings` fills any missing schema keys while preserving existing values, comments, key order, and unknown keys; skips if the file is corrupt |
...
### 2.3 Concurrent Write Safety

All writes to `settings.yaml` are serialized through a per-vault **promise-chain mutex** (`settingsMutexes` map in `settings.ts`). Concurrent `POST /set-setting` requests chain onto the same promise, so 100+ simultaneous requests all persist without TOCTOU clobbering.

### 2.4 Git Exclusion

`backup.ts` adds `settings.yaml` to `<vault>/.git/info/exclude` (idempotent) so git snapshots never track the settings file. This prevents committing personal appearance preferences, API keys, or per-device paths into a vault's git history.
...
- `daemon` — `enabled: false`, `home: ""` (overrides daemon home; see §6)
...
Templates are `.md` files under `settings.yaml → templates.folder` (defaults to `""`, i.e. the vault root). `listTemplates` walks that subfolder recursively, skipping dotfiles and non-`.md` files, and returns sorted `{ name, path }` pairs where `name` is the basename without the `.md` extension.
...
These run only when `.git/` is absent. On every call (including existing repos) it also calls `ensureExclude` to add `settings.yaml` to `.git/info/exclude`.
...
Everything staged by `git add -A` except `settings.yaml` (excluded via `.git/info/exclude`). This includes notes, drawings, sheets, templates, and attachment files. Dotfile directories (`.trash`) are not explicitly excluded from git; if they exist they will be committed unless the user adds them to `.gitignore`.
...
## 6. Claude-bot Daemon Home

`core/src/daemon.ts` reads (and minimally writes) the claude-bot daemon's shared state from a configurable home directory. Bismuth never starts or stops the daemon; it only reads the on-disk contract.

### 6.1 Home Directory Resolution
...
<home>/
  ...
  crons/
    <name>.md            — cron def; YAML frontmatter { name?, schedule, enabled? }
...
Every read is fault-tolerant (`try/catch` returning `null` or empty on failure):
...
- `crons/<name>.md` → `daemonSnapshot()`: reads frontmatter for `name`, `schedule`, `enabled`
- `crons/.last-fired.json` → last-fired timestamp + result per cron
- `crons/.running.json` → whether a cron is currently running
- `processes/<name>.md` → reads frontmatter for `name`, `enabled`

### 6.4 Files Bismuth Writes

Only two write operations; both are byte-compatible with what claude-bot reads:

**`owner.json`** (`setOwner(deviceId)`):
...
$OA_VAULT/                         # vault root (required)
  ...
  **/*.sheet                         # Univer workbook snapshots
  settings.yaml                      # app settings (excluded from git)
...
~/.claude-bot/  (or $OA_CLAUDEBOT_HOME or daemon.home)
  ...
  crons/<name>.md                    # cron definitions
