...
**`PUT /file` marks its own write too.** It bypasses `mutatingHandler` (it's a read-table route, not a `mutatingRoutes` entry — see §11), so before Task 4 it never marked its write at all: every save produced two SSE events, the handler's direct `invalidate()` and the watcher's unsuppressed echo of the same write. It now follows the identical mark → write → (unmark on throw | rearm on success) → `invalidate()` sequence inline.
...
3. Immediately enqueues an unconditional `: connected\n\n` comment. This exists because some HTTP clients (Bun's own `fetch` included) don't consider a streaming response "open" until the first chunk of body data arrives — without this, a client connecting while `version === 0` (nothing else to send: no catch-up snapshot below, no real event yet) got nothing at all until the next heartbeat tick, up to `sseHeartbeatMs` (5s default) later. This was masked for most of the server's life by an unrelated accident (the boot-time `.settings` reconcile write used to bump `version` to 1 a beat after boot, via its own unsuppressed watcher echo — see §2 — so `version > 0` was usually already true by the time a client subscribed); Task 4's self-write-suppression fix correctly stopped that spurious bump, which is what surfaced this connection-stall path.
