...
3. Immediately enqueues an unconditional `: connected\n\n` comment. Measured: without this, a client connecting while `version === 0` (nothing else to send: no catch-up snapshot below, no real event yet) received nothing at all until the next heartbeat tick, up to `sseHeartbeatMs` (5s default) later. This single enqueued comment is enough to make that connection resolve/flush right away instead. This had gone unnoticed for most of the server's life because the boot-time `.settings` reconcile write used to bump `version` to 1 a beat after boot, via its own unsuppressed watcher echo (see §2) — so `version > 0` was usually already true by the time a client subscribed. Suppressing that echo correctly (so it no longer spuriously bumps `version`) is what surfaced this connection-stall path.
