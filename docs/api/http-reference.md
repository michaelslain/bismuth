...
All seven routes below are **owner-only** (`403 "forbidden"` for any non-owner channel) — CORS is `*`, so any local page could otherwise flip, run, create or delete a cron/process; the daemon itself never calls these routes (it acts through the headless CLI), so gating costs it nothing.
...
- **`POST /daemon/cron/create`** / **`POST /daemon/process/create`** — body `{ name? }`. `createCron`/`createProcess(name, vaultDaemonDir(cfg.vault))`: slug = kebab-case of `name` (lowercase, `[a-z0-9-]` only, collapsed/trimmed dashes, then capped at 100 chars with trailing dashes trimmed again); writes `<vault>/.daemon/{crons,processes}/<slug>.md` from a template (`name: <display name, JSON-quoted>`, plus `schedule: 0 9 * * *`/`enabled: false` for a cron, or `command: echo` / `args: ["replace me"]` / `enabled: false` for a process, and a placeholder prompt/description body) — the file is `<slug>.md`; the name is quoted so both core's YAML and the daemon's `parseFrontmatter` (which unquotes) read it back identically. The write is atomic (`{ flag: 'wx' }`), which is what produces the 409 below on a clash rather than a lost `existsSync` race. Response `{ ok: true, file: slug }`. Empty/invalid slug → `AppError("EINVAL")` → `400`; an existing file at that slug → `AppError("EEXIST")` → `409`, both via the dispatch catch.
...
Mobile/iPad (`core/src/localBackend.ts`'s in-process transport) has no notion of an owner channel at all, so it refuses every one of these seven with `NOT_SUPPORTED` (`501`) rather than running them unauthenticated — see the mobile overview doc.
