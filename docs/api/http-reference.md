...
- **`POST /daemon/cron/create`** / **`POST /daemon/process/create`** — body `{ name? }`. `createCron`/`createProcess(name, vaultDaemonDir(cfg.vault))`: slug = kebab-case of `name` (lowercase, `[a-z0-9-]` only, collapsed/trimmed dashes); writes `<vault>/.daemon/{crons,processes}/<slug>.md` from a template (`name`, plus `schedule: "0 9 * * *"`/`enabled: false` for a cron, `command: echo "replace me"`/`enabled: false` for a process, and a placeholder prompt/description body). Response `{ ok: true, file: slug }`. Empty/invalid slug → `AppError("EINVAL")` → `400`; an existing file at that slug → `AppError("EEXIST")` → `409`, both via the dispatch catch.
- **`POST /daemon/cron/delete`** / **`POST /daemon/process/delete`** — body `{ name? }`. `deleteCron`/`deleteProcess(name, vaultDaemonDir(cfg.vault))`: unlinks the definition `.md` (a cron delete also drops a reconcile trigger for processes, mirroring `setProcessEnabled`). Response `{ ok: true }`. Unknown name → `404`; a cron listed as running in `<vault>/.daemon/crons/.running.json` → `AppError("EBUSY")` → `409`, leaving the file in place.
- **`POST /daemon/memory/forget`** — body `{ path? }` (VAULT-relative, `.daemon/memory/<name>.md`, as returned by `GET /daemon/memory`). `forgetDaemonMemory(effectiveMemoryDir(), path)` — rejects anything outside `.daemon/memory/` or carrying a `..` segment with `AppError("EINVAL")` → `400` before it reaches the filesystem. Response `{ ok: true }`. Unknown note → `404`; daemon disabled (no memory dir) → `404`.
