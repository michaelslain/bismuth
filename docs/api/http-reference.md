...
### `GET /daemon/memory`
- **Params:** `?q=<text>&limit=<n>` (limit default 50, max 500) — both optional.
- **Response:** `DaemonMemoryList` = `{ total: number, items: DaemonMemoryItem[] }`, `DaemonMemoryItem = { path, name, type, updated, excerpt }` (`path` is VAULT-relative, `.daemon/memory/<name>.md`; `updated` is the note's `updated` frontmatter — a local calendar date `YYYY-MM-DD` (memory/src/dates.ts), not a timestamp; `excerpt` is the note's first non-empty content line, ≤ 160 chars). This vault's daemon-visible 3rd-brain notes for the daemon page's memory panel, read from the server's own memory dir (`effectiveMemoryDir()` — `<vault>/.daemon/memory` when the daemon is enabled). No `q`: every visible note sorted by `updated` desc. With `q`: `@bismuth/memory`'s `searchMemory` relevance ranking. `total` is always the full visible count, independent of `q`/`limit`. **Never throws** — degrades to `{ total: 0, items: [] }`, including when the daemon is disabled.
...
- **`POST /daemon/cron/create`** / **`POST /daemon/process/create`** — body `{ name? }`. `createCron`/`createProcess(name, vaultDaemonDir(cfg.vault))`: slug = kebab-case of `name` (lowercase, `[a-z0-9-]` only, collapsed/trimmed dashes); writes `<vault>/.daemon/{crons,processes}/<slug>.md` from a template (`name: <slug>`, plus `schedule: 0 9 * * *`/`enabled: false` for a cron, or `command: echo` / `args: ["replace me"]` / `enabled: false` for a process — written bare; the daemon's frontmatter reader keeps quotes — and a placeholder prompt/description body). Response `{ ok: true, file: slug }`. Empty/invalid slug → `AppError("EINVAL")` → `400`; an existing file at that slug → `AppError("EEXIST")` → `409`, both via the dispatch catch.
- **`POST /daemon/cron/delete`** / **`POST /daemon/process/delete`** — body `{ name? }`. `deleteCron`/`deleteProcess(name, vaultDaemonDir(cfg.vault))`: unlinks the definition `.md` (a process delete also drops a reconcile trigger, mirroring `setProcessEnabled`, so a running daemon stops it). Response `{ ok: true }`. Unknown name → `404`; a cron listed as running in `<vault>/.daemon/crons/.running.json` → `AppError("EBUSY")` → `409`, leaving the file in place. **Owner-only** (`403 "forbidden"` for any non-owner channel) — CORS is `*`, so any local page could otherwise delete a service/cron.
- **`POST /daemon/memory/forget`** — body `{ path? }` (VAULT-relative, `.daemon/memory/<name>.md`, as returned by `GET /daemon/memory`). `forgetDaemonMemory(effectiveMemoryDir(), path)` — rejects anything outside `.daemon/memory/` or carrying a `..` segment, or a name that doesn't start with an alphanumeric or ends in `.`/`-`, with `AppError("EINVAL")` → `400` before it reaches the filesystem. Response `{ ok: true }`. Unknown note → `404`; daemon disabled (no memory dir) → `404`. **Owner-only** (`403 "forbidden"` for any non-owner channel) — CORS is `*`, so any local page could otherwise forget memory.
