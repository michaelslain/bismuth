...
**Owner-token gated, and the CLI does not yet supply one.** The route exists in `core/src/server.ts`, but it's blanket owner-only (see [../api/http-reference.md](../api/http-reference.md#visibility-gating) — a `RelaySubagent.lastMessage` is free-text final output that can quote vault content, the same reason `GET /chat/sessions` is gated). `resolveCore`/`call` (`cli/src/http.ts`) never attach an `X-Bismuth-Token` header, so a plain shell invocation of `relay list` today gets a `403`, not the sessions/subagents list — a separate, still-open gap from wiring the CLI's HTTP client to present the boot token (available via the run registry's `RunRecord.token`, `core/src/runRegistry.ts`). It fails honestly (a normal HTTP error) rather than silently returning an empty result.
```bash
...
| `relay list` | relay.ts | **no** (needs a running server; `GET /relay/snapshot` is owner-token-gated and the CLI doesn't supply one yet — see the section above) | JSON |
