...
**"Files already in the folder" and "future files" are covered for free, with no migration**: resolution is computed from a file's CURRENT path at read/gate time and never stamped onto the file. Move a note into or out of a restricted folder and its effective visibility changes on the very next resolve — no batch job, no per-file frontmatter rewrite. The deny list covers every file the sidebar surfaces (`.md`/`.yaml`/`.sheet`/`.draw`/images/PDF), not just markdown, so a hidden folder of non-`.md` files is enforced, not merely badged. The daemon rebuilds the deny list **fresh on every message** (a separate process per turn). A chat session is long-lived, so its deny list is built at spawn AND rebuilt whenever the vault's visibility settings change: `invalidateChatVisibility()` flags every open chat, and the next turn respawns `query()` (resuming the same conversation) with a fresh gate — managedSettings/sandbox are fixed at spawn and can't be updated on a running session, so a respawn is the only way to re-gate them.
...
|---|---|---|---|
| Chat native Read/Edit/Grep/Glob of a restricted file | MECHANICAL | `managedSettings.permissions.deny` in `chat.ts`, built via `buildDenyPaths(cwd, "chat")` at spawn and rebuilt (respawn) on any visibility change — PLUS a path-aware `canUseTool` auto-deny (reads the deny set live). When ANY file is restricted, `Grep`/`Glob` are additionally disabled outright: a per-file deny can't stop an unscoped whole-vault `Grep(pattern)` that returns a hidden file's matching lines | **Yes** — live spike + `core/test/chat.test.ts`'s "visibility" test |
...
| Daemon native Read/Edit/Grep/Glob | MECHANICAL | Same `managedSettings.permissions.deny`, recomputed per `sendMessage` from `ctx.root` (`daemon/src/lib/visibility.ts`, a ported copy), and `Grep`/`Glob` disabled whenever any file is restricted (the daemon has no `canUseTool`, so this outright disable is the only reliable gate against a broad scan) | **Yes** — Step-0 spike confirms deny survives `permissionMode: "bypassPermissions"` |
