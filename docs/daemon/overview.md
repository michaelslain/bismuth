...
- **Centre** — the living `.:[00]:.` face (`DaemonFace`) with the daemon's name under it. Its status — `asleep // daemon is off`, `working // <cron> +N`, `watching // last: <cron> <age>`, or `watching // nothing has run yet` — is now the view bar's first readout, ahead of the cron and service counts. Its mood (`deriveMood`) comes from, first match wins: daemon off/not running → asleep; the daemon chat streaming → talking; a draft in the daemon chat → listening; an enabled cron failed in the last 30 minutes → hurt; a cron or inbox page running → busy; inbox pages due → alert; otherwise idle.
- **Centre, below the face** — its own chat (`DaemonChat`), inline in the same column, not a band across the bottom.
- **Right** — the inbox (`DaemonInbox`, up to half the column) over the activity log (`DaemonLog`).

The left and right columns run the full height of the stage with the same inset top and bottom; nothing sits below it. Below 760px of the page's own width the columns stack (face, chat, services, inbox + log) in a scrolling stage instead. With `daemon.enabled: false` the page shows only the sleeping face and a note on how to enable it — no panels and no chat.
