...
- **Top** — a `ViewBar`: the `Bot` crumb + the daemon's name (`daemonIdentity.ts`) on the left, and, trailing, the status ALONE (`working // <cron> +N`, `watching // last: <cron> <age>`, `asleep // daemon is off`, …) on the right. No facet toggle, no counts in the bar (`daemonPageModel.ts`'s `barReadouts`).
