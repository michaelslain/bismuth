...
- **No system prompt** — `codex exec` has no system-prompt flag, so there is no Codex equivalent of `buildSystemPrompt`. Instead, `refreshIdentityAgentsMd` writes a managed block into the vault's `AGENTS.md` with the same `identity.md`-derived text — including the absolute memory-graph path, that `remember` is the only way to write to it, and that a `memory/` folder relative to `cwd` is the user's vault and not the graph — gated on `settings.codex.writeAgentsMd` (`VaultContext.codexWriteAgentsMd`). That setting is off by default, so a vault with `daemon.backend: "codex"` and `codex.writeAgentsMd: false` runs with **no persona and no instructions about its memory** — an honest, logged degrade, not a silent one. (The memory *location* still reaches it via the env above; what it lacks is anything telling it to use that rather than improvise.)
...
Layout — "hub + one panel":

- **Top** — a `ViewBar`: the daemon's name (`daemonIdentity.ts`), a `SegmentedToggle` across five facets — `inbox` `crons` `services` `memory` `log`, each labelled with its own count (a count omitted when it isn't known yet, e.g. `memory` before its first fetch — never a false zero) — and, trailing, the status ALONE (`working // <cron> +N`, `watching // last: <cron> <age>`, `asleep // daemon is off`, …). The cron/service/inbox counts that used to ride alongside the status moved onto the facet labels; the trailing group carries only the one, user-data-length status string now (`daemonPageModel.ts`'s `barReadouts`).
- **Left — the hub** (`DaemonHub.tsx`) — the living `.:[00]:.` face (`DaemonFace`), under it the daemon's name, its one-line personality blurb (`identity.blurb`, omitted when empty), and a `[ edit ]` button to `.daemon/identity.md`; under THAT, its own chat (`DaemonChat`), inline in the same column, not a band across the bottom. Its mood (`deriveMood`) comes from, first match wins: daemon off/not running → asleep; the daemon chat streaming → talking; the daemon chat busy with no reply text yet → thinking; a draft in the daemon chat → listening; an enabled cron failed in the last 30 minutes → hurt; a cron or inbox page running → busy; inbox pages due → alert; otherwise idle. Once the chat has messages (or a full-height pane like chat history has taken the chat region), the face collapses to a one-line header — a small glyph beside the name, left-aligned — handing the rest of the column to the chat; the blurb and `[ edit ]` drop out in that form.
- **Right — the panel** — whichever ONE of `DaemonInbox` / `DaemonCrons` / `DaemonProcesses` / `DaemonMemory` / `DaemonLog` the selected facet names, at least as wide as the hub column. `DaemonCrons`/`DaemonProcesses` rows share one column grid (`dot // name // schedule // status // [ run ]`, minus the schedule/`[ run ]` for services); each panel's own head row (when it has one) carries only its actions — `[ new cron ]`/`[ new service ]` (swaps to an inline name field; on success the new file opens and the row appears disabled) — never a title or count, since the facet toggle is now the panel's heading. A row's own `[ delete ] [ cancel ]` confirm lives in that row's trailing actions cell instead, armed from its context menu, not the panel head row. `DaemonMemory` adds a search field over the top of its rows.

Which facet the page opens on (`daemonPageModel.ts`'s `initialFacet`): anything due in the inbox wins outright; otherwise the facet this browser window had open last (remembered in `localStorage`, key `bismuth.daemonFacet.<windowId>` — `app/src/windowId.ts`); otherwise `crons`.

The hub and panel columns run the full height of the stage with the same inset top and bottom; nothing sits below it. Below 760px of the page's own width they stack (hub, then panel) in a scrolling stage instead. With `daemon.enabled: false` the page shows only the sleeping face and a note on how to enable it — no facet toggle and no panel.

**Where the data comes from.** `app/src/daemon/DaemonPageHost.tsx` polls `GET /daemon/snapshot` every 4s and `GET /daemon/logs?limit=60` every 5s, only while the page is mounted and the daemon is enabled, skipping ticks while the document is hidden. The inbox pages are App's shared `/daemon/pages` poll (`app/src/daemonInbox.ts`). While the `memory` facet is showing, a keystroke in its search field debounces ~200ms before `GET /daemon/memory` refetches (same hidden-tab skip). The pure derivations (status caption, readouts, the recent-failure window, the initial/segment facet logic) are `app/src/daemon/daemonPageModel.ts`. Every cron/service/memory mutation (run, enable/disable, create, delete, forget) goes through `app/src/api.ts` and reports a toast on failure.
