...
#### `selfUpdate.ts`
Git-based self-update for the bundled Bismuth app. The bundled app is built from a local clone; the build bakes a `build-origin.json` (repo root + sha) into the tools resource, which `getUpdateStatus()` uses to detect when the INSTALLED build is behind `origin/main` — measured from the built sha, not the clone's live HEAD, so a developer who commits+pushes from the build-source clone still sees the update. `startUpdate()` runs `git pull --ff-only` + `bun install --frozen-lockfile` + `bun run tauri build`, then hands off to a detached script that waits for the app to quit, swaps the `.app` bundle, and relaunches; `getUpdateProgress()` reports `UpdatePhase = "idle" | "pulling" | "building" | "ready" | "error"`. Every step's command + full output is appended to a persistent log at `join(tmpdir(), "bismuth-update-build.log")` (truncated per run), and a failing step's `message` names that path. Self-disables (never throws) when there's no source build — e.g. `bun run dev:browser` — surfacing as an `"error"` phase with a reason instead. Backs `cli/src/commands/update.ts` and `GET /update/status` / `POST /update/apply`.
...
#### `PaneTree.tsx`
Renders the binary pane tree; manages pane drag-and-drop via `dnd/viewDrag.ts`. Handles split/close/resize interactions. `PaneLeaf` (one pane's own content + focus/right-click reporting + view-drag drop target (tab/pane/tree-row POINTER drags via dnd/viewDrag.ts — the old HTML5 `application/x-bismuth-path` drag-to-split path is gone; nothing ever set that MIME)) was promoted out of this file into its own `PaneLeaf.tsx`, which in turn delegates its mini view-bar breadcrumb to `PaneHeader.tsx` and its split/chat-reference drop affordances to `PaneDropZone.tsx` — `PaneTree.tsx` itself is now just the tree walk. Each of the four now has its own colocated module (`PaneTree.module.css`, `PaneLeaf.module.css`, `PaneHeader.module.css`, `PaneDropZone.module.css`); a rule that used to cross the file boundary now reads a `data-*` hook instead — `PaneLeaf.tsx` sets `data-pane-focused` on its own root when its leaf is focused, and `PaneHeader.module.css`'s `[data-pane-focused] .pane-header` rule reads it, since a data attribute is never hashed the way a class is.
...
The app's only tab presentation — a right-edge vertical rail, collapsed to 48px and expanding to 232px on hover/focus-within. `TabRail` is the rail shell (actions + rows as slots); `TabRailRow` is one row (icon, label or inline rename input, close-X/pin). Each now owns its own module (`TabRail.module.css`, `TabRailRow.module.css`) — the selectors that used to cross the file boundary reach the ancestor through the stable, unhashed `[data-tab-rail]`/`[data-rail-pinned]` attributes `TabRail.tsx` sets on its own root, rather than a class.
...
#### `StatusBar.tsx`
The status-bar field-log line: one `location` readout (the focused file's absolute path, or `<vault path> // <label>` for a graph/terminal/chat pane — issue #10), connection health (from `serverVersion`'s `ConnectionState`), then right-aligned the inbox indicator and the daemon state, closed by a blinking caret. `onCopyLocation` (click-to-copy, which pushes a toast) and `onOpenInbox` (which opens a tab) stay callback props since a presentational component must not do either itself.
...
Inlines the DOCUMENT faces — note prose (Lora Variable) and the mono face (Monaspace Xenon) — as base64 `data:` URIs for export, the same technique `katexCss.ts` already used for math glyphs. Without it an export's font stack silently fell through to Georgia (measured under the previous CMU-Serif prose face: a prose run rendering pixel-identical to Georgia, nothing like the real prose face) because the app loads these fonts through Vite at runtime, which a standalone exported document — or the headless Chrome the PDF path rasterizes in — has no way to resolve. `docFontInlineCss()`.
...
#### `export/htmlToPdf.ts`
The FALLBACK PDF rasterizer — html2canvas + `jspdf` — used only where there is no native WebKit print engine to hand the document to (browser dev, iPad); the desktop app prints through WebKit instead (`pdfPrint.ts`). Rasterizes the document to a canvas, then slices it across Letter pages via `pageGeometry.ts`'s `pageSlices`, gated by the painted cut gate (`classifyRows`/`cutSafe`/`formulaBands`) so a slice never lands on inked pixels.

#### `export/pageGeometry.ts`
Pure geometry for the html2canvas FALLBACK PDF exporter (`htmlToPdf.ts`) — the desktop app's WebKit path (`pdfPrint.ts`) does its own native pagination and never touches this module. US Letter portrait (8.5in × 11in), 1in margins on every side, computed explicitly (rather than left to a `@page` CSS rule) because html2canvas rasterizes the whole document to a single canvas and ignores `@page` rules, so `htmlToPdf.ts` slices that canvas across pages itself. `PAGE_W_PT`/`PAGE_H_PT`/`MARGIN_PT`/`CONTENT_W_PT`/`CONTENT_H_PT` (PDF points, 72pt/in) and their pixel (96dpi) counterparts, `snapDownToGrid(raw, unit)`. Matches the headless CLI path's geometry (`core/src/render/htmlRaster.ts`) so both rasterizers agree on the same page box.

Also the painted-blank cut gate: a DOM-legal cut stop can still land on inked pixels because html2canvas doesn't paint pixels exactly where the DOM says they are (measured drift of ±40-110 canvas px at 2x scale between a DOM-measured atom edge and where its ink actually lands on the raster). `classifyRows(...)` samples the raster and classifies each row (blank / rule / inked); `cutSafe(...)` and `formulaBands(...)` refine the DOM-legal stops against those pixel classes so a cut only lands where the pixels are actually blank (or on a legitimate full-width rule, like a table border). `CutGate` is the interface tying the classification to `pageSlices`.

#### `export/pdfPrint.ts`
The PDF engine chooser + fallback for the browser exporter: `pickPdfEngine(env)` picks `'webkit'` inside a Tauri webview or `'canvas'` everywhere else; `printPdf(html, title, engine, printers, warn?)` runs whichever was picked and falls back from a failing webkit print to canvas rather than losing the export outright (a message of exactly `'unsupported'` — the expected answer on iPad/non-macOS — is swallowed rather than warned). `makeWebkitPrinter(invoke)` calls the `print_pdf` Tauri command (`app/src-tauri/src/print_pdf.rs`) with the print head injected via `printCss.ts`'s `injectHead`. `realPrinters` wires the real webkit (dynamic `@tauri-apps/api/core` import, so the Tauri bridge never lands in a non-desktop bundle) and canvas (lazy `htmlToPdf.ts` import) printers; `htmlToPdfBytes(html, title)` is what `ExportView` wires into `ExportDeps.htmlToPdf`.

#### `export/printCss.ts`
CSS + head markup the PDF printers inject into an export document, shared by the WebKit printer (`pdfPrint.ts`) and the html2canvas fallback (`htmlToPdf.ts`) so both engines lay the body out at the same printable width. `PDF_BODY_OVERRIDE` drops the shared template's reading column so the only margin is the 1in page margin. `PRINT_READY_TITLE` is the marker the export document sets on `document.title` once its embedded fonts are ready — the native printer polls for it (`WKWebView.title` is readable without a delegate) before printing. `WEBKIT_PRINT_HEAD` (webkit path only) adds print-colour-adjust so backgrounds print, `break-inside: avoid` rules measured in the WebKit print spike (`table`/`img`/`svg`/`.katex-display`; `tr` alone is ignored by WebKit), and the fonts-ready marker script. `injectHead(html, markup)` inserts markup immediately before `</head>`.
...
`app/src/daemon/` (daemon-overview plan): the sections the daemon's own page composes. Each colocated `<Name>.module.css` has exactly one importer — none of these reach into another's stylesheet.

#### `daemon/DaemonSection.tsx`
The shared frame for one right-column section on the daemon page (inbox / crons / services / log): a lowercase heading + optional count `Badge`, an empty one-liner (`nothing needs you`, `no crons yet // ask the daemon`, `no services yet // ask the daemon`, `nothing logged yet`) while the section has nothing to show, then the section's own rows as opaque children. Every section sizes to its content — no section scrolls on its own; `DaemonOverview` owns the column's one scroll and row-limits what each section renders.

#### `daemon/DaemonOverview.tsx`
The daemon page's right column: `inbox`, `crons`, `services`, `log` stacked top to bottom, each slot a function of that section's computed row limit (`limit: () => RowLimit`) so the caller's list component reads it reactively without losing its own expanded state — `DaemonPageHost` wires the four list components in. ONE scroll for the whole column. Measures its own height (`ResizeObserver`) and, from `rows: Record<DaemonSectionKey, {total, attention, extraLines?}>` and the live row-height tokens (`--row-h`/`--sp-1`/`--sp-2`/`--sp-6`), works out each section's dynamic limit via `daemonRowBudget.ts`'s `allocateRows` so all four fit with no scrollbar at rest — a floor of `max(3, attention)` per section so a limit never hides a row that needs the user. Narrow (below the page's own 760px container query, read off a `--overview-fit` custom property rather than a class) drops the measurement (this component's own height is `auto` there — a feedback loop) for static per-section caps (5 rows inbox/crons/services, 10 log), each still raised to its attention count.

#### `daemon/daemonRowBudget.ts` + `daemon/daemonAttention.ts` + `daemon/DaemonMoreLine.tsx`
`daemonRowBudget.ts` is the pure row-budget math `DaemonOverview` measures against: `allocateRows(available, needs)` gives each section either `undefined` (show everything) or a row count, respecting each section's `floor` and spending one row on a cut-off section's own `+N more` line, spreading any leftover round-robin rather than starving a short section for a long one; `rowUnits(heightPx, overheadPx, pitchPx)` turns a measured pixel height into whole row units. `daemonAttention.ts` is the pure "needs the user" predicate per row kind (`cronNeedsAttention`, `processNeedsAttention` — inbox attention is just due+failed, already in `daemonInboxLogic.ts`) plus `attentionFirst()`, the stable sort every row-limited list applies before slicing so its own floor lands on the rows that matter. `DaemonMoreLine.tsx` is the one `+N more // show` / `// hide` toggle line every row-limited section renders when its list is cut short — `DaemonInbox`/`DaemonCrons`/`DaemonProcesses`/`DaemonLog` all compose it, keyed off their own `limit` prop.

#### `daemon/DaemonCrons.tsx` + `daemon/DaemonProcesses.tsx` + `daemon/DaemonRow.tsx`
The "crons" and "services" sections — presentational, data + callbacks only (the host wires `api`/`pushToast`/store), each wrapped in its own `DaemonSection`. Both share one row primitive, `DaemonRow.tsx` (a subgrid row so dot/name/meta/status/actions line up across the whole list); `DaemonCrons` supplies a `schedule` column and `[ run ]`, `DaemonProcesses` doesn't (a background service has neither). Right-click keeps the shared `<ContextMenu>` (Run now / Enable / Disable / Delete); a row click opens `.daemon/crons/<name>.md` or `.daemon/processes/<name>.md`; a delete confirm becomes that row's own trailing `[ delete ] [ cancel ]`. `daemon/cronStatus.ts`/`daemon/failedResult.ts` derive the enabled/running/failed/idle tone; `daemon/cronFrequency.ts` converts a cron expression to a short human string ("every 5m"). `DaemonProcesses`' "live" always means enabled AND the daemon process itself is up — `GET /daemon/snapshot`'s per-process `running` is always `false` (no trustworthy per-process liveness).

#### `daemon/DaemonInbox.tsx` + `daemon/InboxRow.tsx`
The content of the deleted `InboxView.tsx` (the former `::inbox` tab) minus its own `ViewBar`, wrapped in one `DaemonSection`: ONE flat list — due, then failed, then scheduled (each group's existing sort) — with no `Needs review`/`Scheduled` sub-headings, over the daemon's pages (`core/src/daemonPages.ts`), sorted/grouped by `app/src/daemonInboxLogic.ts`. Once anything is resolved, a trailing `N resolved // show` line toggles to `// hide` and reveals the resolved rows beneath it (shown under the empty line too when nothing is open). `pages` is now a plain prop rather than a module-level signal read directly by the component. `InboxRow.tsx` is one row — ONE line, status dot + title + age, click opens the page — whose only control is a hover/focus `[ archive ]` (`api.archiveDaemonPage`, deletes the page); a page's own actions live on `InboxPageView`'s bottom bar, not in the row. Extracted from the deleted `InboxView.tsx`'s `PageRow`.
...
#### `daemon/DaemonHub.tsx`
The daemon page's left column: the living face (`DaemonFace`), the daemon's name + one-line personality blurb (`identity.blurb`, omitted when empty) and an `[ edit ]` bracket button to `.daemon/identity.md`, then its own chat underneath. Resting, the identity sits centred under the full-size face; once the chat has taken the column (conversing, or a full-height pane like chat history), the face collapses to `DaemonFace`'s own `compact` one-line-header form and the identity shrinks to just the name — the blurb and `[ edit ]` drop out. Off (`enabled === false`): no identity, no chat.

#### `daemon/DaemonPage.tsx` + `daemon/DaemonPageHost.tsx` + `daemon/DaemonChat.tsx` + `daemon/daemonPageModel.ts`
The daemon's own page (`::daemon`, routed lazily by `PaneContent.tsx`) — "hub + full overview". `DaemonPage` is presentational: a `ViewBar` with just the `Bot` crumb + name plus a trailing status readout — no facet toggle, no counts in the bar — over a two-column stage — `DaemonHub` (face, then `DaemonIdentity` — the name, whose hover/focus card holds the personality blurb + `[ edit ]` — then chat) on the left, the overview column (`props.overview`, the host's `<DaemonOverview/>`: inbox/crons/services/log stacked) on the right, handed in as a slot so `DaemonPage` never imports the sections directly; with the daemon off, `DaemonHub` itself sleeps and the overview column drops entirely, leaving one `EmptyState` under the face. `DaemonPageHost` is the container: polls `GET /daemon/snapshot` (4s) and `GET /daemon/logs` (5s) while mounted and enabled, reads the shared inbox store, derives the mood, wires all four sections' callbacks into one `<DaemonOverview/>`, looks up `chatSession(DAEMON_CHAT_ID)` (`chat/chatSessions.ts` — undefined until armed) and passes `<DaemonChat/>` as the hub's chat slot. `DaemonChat` renders `ChatTranscript` over the shared `ChatComposerBar` (with `ChatControls` as its `below`) — the composer renders identically whether or not a session exists, and a trusted press/focus on it arms the chat (`daemonChatArming.ts` — pure `isArmingGesture`/`stayArmed`; the signal is `app/src/daemon/daemonChatArm.ts`), after which App's `chatContents` memo retains the `::chat:daemon` session and the same (already-focused) composer starts driving it. Opening the page alone never spawns a chat session (see `UI_CONTROL_BLOCKLIST`'s comment in `core/src/commands.ts`). `daemonPageModel.ts` is the pure layer — `hasRecentFailure`, `faceCaption`, `barReadouts`, `RECENT_FAILURE_MS`. See `docs/daemon/overview.md` → "Daemon page".
...
| `uiLint.ts` | Pure dev-time lint helpers (tested): `labelCaseWarning(children)` flags a `TextButton`/`IconTextButton` label that isn't all-lowercase; components call these behind an `import.meta.env.DEV` guard |
