...
#### `daemon/DaemonMemory.tsx` + `daemon/MemoryRow.tsx` + `daemon/memoryListModel.ts`
The "memory" facet panel: a `SearchBar` over the vault's `mem:` items (`GET /daemon/memory`), recent first — the host sorts and debounces; this only renders what it's given, wrapped in one `DaemonPanel`. Each row is a `MemoryRow` (name + type + age, then a one-line excerpt; click opens the note the memory is about; a trailing `[ forget ]` asks `[ forget ] [ cancel ]` inline on first press, `POST /daemon/memory/forget`). `memoryListModel.ts` is the pure layer behind both — `memoryEmptyMessage` (the panel's empty-state text) and a deterministic relative age (`memoryAge`) — no Solid imports, so it's unit-tested without mounting.
...
The daemon's own page (`::daemon`, routed lazily by `PaneContent.tsx`) — "hub + one panel". `DaemonPage` is presentational: a `ViewBar` with a facet `SegmentedToggle` across five facets (`inbox`/`crons`/`services`/`memory`/`log`, each labelled with its own count) plus a trailing status readout, over a two-column stage — `DaemonHub` (face, name, personality blurb, `[ edit ]`, chat) on the left, whichever ONE panel the current facet names (`DaemonCrons`/`DaemonProcesses`/`DaemonMemory`/`DaemonInbox`/`DaemonLog`) on the right, handed in as a slot so `DaemonPage` never imports the panels directly; with the daemon off, `DaemonHub` itself sleeps and the facet toggle + panel drop entirely, leaving one `EmptyState` under the face. `DaemonPageHost` is the container: polls `GET /daemon/snapshot` (4s) and `GET /daemon/logs` (5s) while mounted and enabled, reads the shared inbox store, derives the mood, picks + wires the current facet's panel, looks up `chatSession(DAEMON_CHAT_ID)` (`chat/chatSessions.ts` — undefined until armed) and passes `<DaemonChat/>` as the hub's chat slot. `DaemonChat` renders `ChatTranscript` over the shared `ChatComposerBar` (with `ChatControls` as its `below`) — the composer renders identically whether or not a session exists, and a trusted press/focus on it arms the chat (`daemonChatArming.ts` — pure `isArmingGesture`/`stayArmed`; the signal is `app/src/daemon/daemonChatArm.ts`), after which App's `chatContents` memo retains the `::chat:daemon` session and the same (already-focused) composer starts driving it. Opening the page alone never spawns a chat session (see `UI_CONTROL_BLOCKLIST`'s comment in `core/src/commands.ts`). `daemonPageModel.ts` is the pure layer (`DAEMON_FACETS`, `facetCount`, `initialFacet`, `barReadouts`, `hasRecentFailure`). See `docs/daemon/overview.md` → "Daemon page".
