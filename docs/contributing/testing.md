...
2. **Mocked agent-CLI integration tests** (`core/test/chatProviders/*Mocked.test.ts`,
   ...
   **account** isn't logged in — a missing-binary skip and a missing-account skip must never be
   ...
   item covers EIGHT files total, not seven — the six `*Mocked.test.ts` files plus two fake-agent
   files, `acpFakeAgent.test.ts` and `clineAuthFakeAgent.test.ts`, neither of which is a real CLI):
   - `claudeMocked`/`opencodeMocked`/`codexMocked`/`gooseMocked`/`geminiMocked`/`clineMocked.test.ts`
     drive a REAL agent CLI binary (`claude`/`opencode`/`codex`/`goose`/`gemini`/`cline`) through
     ...
     turn E2E vs. handshake-only). `clineMocked.test.ts` is two describe blocks, not one: an original
     one proving the DEFAULT (no-bypass-configured) path fails safely against an isolated,
     never-authenticated `CLINE_DIR`, plus a later "real E2E" block proving a full turn completes once
     a real, source-cited env-var bypass (`CLINE_API_KEY`) is applied — see the table's `cline` row.
