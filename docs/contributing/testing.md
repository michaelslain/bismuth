...
every file under `core/test/` (and `app/src/`, which the `core` workspace resolution also pulls in)
already has `core` as a substring of its own path, keeping `core` in the pattern list matches the
same full set no matter what you append after it — confirmed live: `bun test core -- wikilinks`
still ran all 139 files. This has already cost one agent a full-suite run it believed was scoped to
one file.

To actually filter, drop the workspace argument and pass either an exact path or a bare pattern:
