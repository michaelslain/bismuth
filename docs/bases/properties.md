...
2. **Kanban card meta.** A kanban without an `order:` shows the declared properties (minus the title and description slots) as each card's read-only meta rows — previously only an explicit `order:` produced meta. Empty values are still dropped per `hasValue`.
