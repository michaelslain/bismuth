...
**Cancellation and shared work**: `attachLayout(graph, vaultKey, { signal })` and `computeViewLayouts(graph, vaultKey, { signal })` take an optional `AbortSignal`. Concurrent callers for the same graph signature share ONE computation (an in-flight map keyed by signature) — first launch used to compute the view layouts twice at once. A caller whose signal aborts stops waiting and rejects with an `AbortError`; the shared computation itself is aborted only when its last waiting caller has gone, and a caller without a signal holds it to completion. **A superseded build never writes a seed**: a build that predates a `renameLayoutIds` call — one that already finished its ticks, or took the zero-compute path — would otherwise overwrite the seed that call just remapped, and the renamed note would jump. Two checks guard every seed write in both functions: the signal (a caller that aborts the build), and a per-vault *rename epoch* that `renameLayoutIds` increments. Each build captures the epoch before it reads its seed and awaits its layout, and writes no seed if the epoch has moved — which also covers a build that has no signal and so cannot be aborted at all. `createAsyncCache` (`core/src/asyncCache.ts`) hands each build its own signal and aborts it from `invalidate()`; a build still queued when it was invalidated is never started, and a running build that passes the signal on (e.g. into `attachLayout`) stops at once — its layout worker is terminated (see "Where the settle runs" below), or in-process it stops at its next tick — instead of making the next build wait behind it. A build that ignores its signal runs to completion as before, and its result is still discarded.
