...
The `three` and `d3-force-3d` npm packages are still listed in `app/package.json` and still have a manual Vite chunk-splitting rule (`app/vite.config.ts`), but nothing under `app/src` imports from either anymore — `AsciiGraphRenderer.ts` does not use `THREE.*` or `d3-force-3d` at all (the frontend's own `d3-force-3d.d.ts` type-declaration file, `app/src/graph/d3-force-3d.d.ts`, is itself unreferenced). `core/src/layout.ts` is the only remaining live consumer of `d3-force-3d` in the app (server-side force refinement — see "Backend-Precomputed 2D/3D Layout" above); its own type stub is `core/src/d3-force-3d.d.ts`. `app/src/App.tsx` still carries a comment ("Lazy: GraphView pulls in three.js + d3-force-3d…") justifying why `GraphView` is lazy-loaded; that comment's premise (that the graph chunk actually pulls in `three`) is no longer accurate now that nothing imports it, though lazy-loading the graph pane is still reasonable on its own merits.
