...
**Warm starts**: the last layout per vault is kept as a *seed* — `lastFullLayout` for the full graph, `lastSecondLayout`/`lastThirdLayout` for the brain views — in memory and on disk (`seed-<CACHE_VERSION>-<hash>.json`). A seed carries the positions plus the graph's sorted `from|to|kind` edge keys (`edges`), so the next build can diff both nodes and edges against it. On a cache miss, `diffPlan(seed, graph)` decides how much has to be computed:
...
**Renames and moves**: a rename would otherwise read as a removal plus an add, and the renamed note would re-settle from scratch. `renameLayoutIds(vaultKey, fromRel, toRel)` rewrites the in-memory seeds (full, 2nd, 3rd brain; loading the on-disk seed first if none is in memory) before the graph is rebuilt, so the next diff is empty. `fromRel`/`toRel` are vault-relative paths as `POST /move` receives them: a note file (`a/b.md`) renames the one id `a/b`; a folder (`a/b`) rewrites every id under `a/b/`. The two are deliberately kept apart, so a note `proj.md` beside a folder `proj/` is not dragged along by a move of the other. Ids inside the seed's `edges` are rewritten the same way, and the seed is copied, never edited in place (its positions are shared with the pre-rename graph's cached entry).

**Cancellation and shared work**: `attachLayout(graph, vaultKey, { signal })` and `computeViewLayouts(graph, vaultKey, { signal })` take an optional `AbortSignal`. Concurrent callers for the same graph signature share ONE computation (an in-flight map keyed by signature) — first launch used to compute the view layouts twice at once. A caller whose signal aborts stops waiting and rejects with an `AbortError`; the shared computation itself is aborted only when its last waiting caller has gone, and a caller without a signal holds it to completion. **An aborted build never writes a seed**: both functions check the signal immediately before every seed write, because a superseded build that already finished its ticks (or took the zero-compute path) would otherwise overwrite a seed that `renameLayoutIds` has just remapped, and the renamed note would jump. `createAsyncCache` (`core/src/asyncCache.ts`) hands each build its own signal and aborts it from `invalidate()`; a build still queued when it was invalidated is never started, and a running build that passes the signal on (e.g. into `attachLayout`) stops at its next tick instead of making the next build wait behind it. A build that ignores its signal runs to completion as before, and its result is still discarded.
