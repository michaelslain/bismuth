...
>
> **The "daemon" graph mode was removed too.** `GraphMode` (`app/src/commands.ts`) no longer has a `"daemon"` value; `GraphView` no longer polls or renders a daemon-mode legend card, and the `graph-daemon` command is gone from `COMMAND_CATALOG`. The daemon's crons/processes now get their own page (content id `::daemon` — see [the daemon page](../daemon/overview.md#daemon-page)), fed by `GET /daemon/snapshot` (`{ daemon: { label, running, home }, crons, processes }`) rather than `GET /daemon/graph`. `core/src/daemonGraph.ts`'s `daemonGraph()`/`buildDaemonGraph()` are unchanged and still live — the CLI's `bismuth daemon graph` command calls them directly — and the `"daemon"`/`"cron"`/`"process"` node kinds, the `"supervises"` edge kind, and `daemonViz.ts`'s visual-state mapping all stay exactly as documented below; only the app's GRAPH MODE and the `GET /daemon/graph` HTTP route are gone.
...
1. **In-memory**: `Map<sig, Layout>` within a server run, bounded to `MEM_CACHE_MAX` (16) entries — every structural edit mints new signatures, so it evicts the least-recently-set entry (a hit re-sets its entry). An evicted entry is re-read from disk.
...
`CACHE_VERSION` **must be bumped whenever the layout output changes** (constants, the small-graph boost, the reel-in, the incremental-rebuild scheme) — a stale cached layout computed under different rules would mismatch what the renderer settles to. The one exception is a change that only decides *which* nodes an incremental rebuild pins while leaving every full settle byte-identical: the incremental diff below did not bump, because every cached `<sig>.json` is still a valid layout under the same physics, and a bump would force a ~20 s cold settle on every user's next launch. The version comments record the full history in `layout-cache.ts` itself (not duplicated here, to avoid this doc drifting stale again): `v5` = collide iterations 3→6 + padding 1.25→1.55; `v6` = small-graph linkDist boost added; `v7` = stronger `400/n` (cap 8) boost; `v8` = reel disconnected components into the main mass via virtual tether links; `v9` = incremental "add-only" rebuilds pin pre-existing nodes; `v10`–`v20` = disc-flatten bias, repulsion/collide tuning, community-aware clustering forces, and — most recently — the LinLog energy model + degree-proportional repulsion default (`v20`; see `layout-cache.ts`'s inline history for each step and exactly what it changed).
...
**Warm starts**: the last layout per vault is kept as a *seed* — `lastFullLayout` for the full graph, `lastSecondLayout`/`lastThirdLayout` for the brain views — in memory and on disk (`seed-<CACHE_VERSION>-<hash>.json`). A seed carries the positions plus the graph's sorted `from|to|kind` edge keys (`edges`), so the next build can diff both nodes and edges against it. On a cache miss, `diffPlan(seed, graph)` (`core/src/layoutDiff.ts`, the pure, fs-free half of the cache alongside the edge-key and id-remap helpers) decides how much has to be computed:

- **removed** = seed ids no longer in the graph; **added** = graph ids not in the seed; **survivors** = ids in both.
- **movable** = the added nodes, plus both endpoints of every edge in the symmetric difference of the seed's edges and the graph's *whose two endpoints are both survivors*. An edge touching an added or removed node never counts — it appeared or vanished only because its node did.
- **fixed** = every other graph id, pinned exactly where the seed has it.

What that means per edit:

| Edit | movable | Compute |
|---|---|---|
| Delete a note | nobody | **none** — the layout is the seed minus the removed ids, every survivor at its exact prior position |
| Rename / move a note or folder, seeds remapped first by `renameLayoutIds` (below) | nobody | **none** — the moved notes keep their exact positions |
| Rename / move without that remap | the moved note(s), which read as removed + added | pinned incremental settle |
| Add or remove a link between two existing notes | exactly those two notes | pinned incremental settle |
| Create a note | the new note(s) | pinned incremental settle |
| More movable nodes than `INCREMENTAL_MAX_ADD` (25) or `INCREMENTAL_MAX_FRAC` (10%) of the graph, whichever is larger | — | full warm settle (a large batch is better re-optimized globally) |

The **pinned incremental settle** runs `computeLayoutAsync` with `fixedIds` at `REFINE_TICKS_INCREMENTAL = 60` (vs the full `REFINE_TICKS = 240`), with a convergence early-exit; only the movable nodes move. Its cost is per-tick force evaluation over the *whole* graph, pinned nodes included: on a 2,336-node vault a link added between a lecture note and a hub took 2.6 s, against 11.6 s for the full warm settle it replaced (a removal and a remapped rename: 10–12 ms). A full warm settle — no seed, an empty seed, or a diff over the cap — seeds the 3D pass from the prior positions (`initialPositions`), skipping PivotMDS.

A seed written before seeds carried `edges` is still valid: it cannot diff edges, so it keeps the older rule (a pure add pins; anything else is a full warm settle) once, and the seed written after that build has edges.

**Renames and moves**: a rename would otherwise read as a removal plus an add, and the renamed note would re-settle from scratch. `renameLayoutIds(vaultKey, fromRel, toRel)` lets a caller that moves files remap the in-memory seeds (full, 2nd, 3rd brain; loading the on-disk seed first if none is in memory) before the graph is rebuilt, so the next diff is empty. `fromRel`/`toRel` are vault-relative paths, in the shape `POST /move` receives: a note file (`a/b.md`) renames the one id `a/b`; a folder (`a/b`) rewrites every id under `a/b/` (`idRenamer`). The two are deliberately kept apart, so a note `proj.md` beside a folder `proj/` is not dragged along by a move of the other. Ids inside the seed's `edges` are rewritten the same way, and the seed is copied, never edited in place (its positions are shared with the pre-rename graph's cached entry).

**Cancellation and shared work**: `attachLayout(graph, vaultKey, { signal, epoch })` and `computeViewLayouts(graph, vaultKey, { signal, epoch, speculative })` take an optional `AbortSignal`. Concurrent callers for the same graph signature share ONE computation (an in-flight map keyed by signature) — first launch used to compute the view layouts twice at once. **Only a build that is cheap to redo accepts cancellation** — a pinned diff or a zero-compute one. The plan (`diffPlan`) is decided before a caller creates or joins the build: for a cancellable build, a caller whose signal aborts stops waiting and rejects with an `AbortError`, and the shared computation is aborted only when its last waiting caller has gone. A **full settle** (no seed, or more movable nodes than the incremental cap) is joined without the signal and runs to completion even when its caller aborts — its seed is what turns every later build into a small pinned diff. Cancelling it starved the graph: a structural change inside one 10–20 s settle aborted it, the retry had no seed and started another full settle from scratch, so a steady trickle of edits (agents creating notes, daemon memory writes) kept the graph empty or stale on a first open, after a cleared cache or after a batch import. Held to completion, `asyncCache` queues the next build behind it, and that build diffs against the fresh seed as a pinned settle. The one exception is a **`speculative`** request — unrequested work nothing awaits, i.e. the server's boot view warm-up, which `graphCache.invalidate()` aborts — where even a full settle may be cancelled, so it never holds the layout worker ahead of a requested rebuild; a requested caller joining the same full settle holds it regardless. `layoutBuildCounts(vaultKey)` reports how many `full` / `pinned` / `reused` (zero-compute) builds a vault key has started — the tests read it instead of timing. **A superseded build never writes a seed**: a build that predates a `renameLayoutIds` call — one that already finished its ticks, or took the zero-compute path — would otherwise overwrite the seed that call just remapped, and the renamed note would jump. Two checks guard every seed write in both functions: the signal (a cancellable caller that aborts), and a per-vault *rename epoch* that `renameLayoutIds` increments. A build writes no seed if the epoch has moved since it was captured — before the seed is read, or earlier when the caller passes `epoch` (`layoutEpoch(vaultKey)`): the server's `graphCache` captures it before its vault walk, because a full settle ignores its signal and the epoch is then its only guard. `computeViewLayouts` on a graph `attachLayout` returned uses that call's epoch. `POST /move` also invalidates `graphCache` in the same synchronous step as `renameLayoutIds`, so a cancellable build that read the tree before the move is aborted before it can lay it out. `createAsyncCache` (`core/src/asyncCache.ts`) hands each build its own signal and aborts it from `invalidate()`; a build still queued when it was invalidated is never started, and a running cancellable build that passes the signal on stops at once — its layout worker is terminated (see "Where the settle runs" below), or in-process it stops at its next tick — instead of making the next build wait behind it. A build that ignores its signal (a full settle included) runs to completion, and its result is still discarded by the cache.

The pure 3D-then-2D compute is `computeLayoutPair(job, signal)` in `core/src/layoutCompute.ts` (a `LayoutJob` = `{ input, refineTicks, seed?, fixedIds? }`; both types are re-exported from `layout-cache.ts`): with `fixedIds` + `seed` it is the pinned settle, otherwise a full settle warm-started from `seed.pos3d` when there is one. It holds no cache or seed state, so it can run anywhere unchanged.

**Where the settle runs** (`core/src/layoutRunner.ts`): `layout-cache.ts` hands every job to `runLayoutJob(job, signal)`, which runs it in ONE persistent Bun Worker (`core/src/layoutWorker.ts`), created on the first job and fed FIFO, one job at a time. The worker runs the same `computeLayoutPair`, so the positions are byte-identical to the in-process path. On the request thread a full settle starved everything else even while yielding every 8 ms: on a 2,294-note vault clone, `GET /file` + `PUT /file` round trips took a median 32 ms during a settle in-process (145 ms on the earlier diagnosis host) against 0.2 ms with the worker, and an unrelated competing task ran at 3 % of its idle rate in-process against ~100 % with the worker (`core/test/layoutRunner.test.ts`).
- **Cancellation**: aborting the running job terminates the worker and rejects the job with the signal's reason — an `AbortError` for a plain `abort()`; the next queued job gets a fresh worker. Aborting a queued job just removes it.
- **In-process fallback** (the old path, `computeLayoutPair` on the request thread): when there is no `Bun` global (the mobile in-process backend, `localBackend.ts`, runs inside a WKWebView), when `BISMUTH_LAYOUT_IN_PROCESS=1`, or when the worker cannot be used — either `new Worker` throws, or the worker fires `error`/`close` (which is what Bun does for a worker file that is not there: it does not throw). The last case logs `[layout] worker unavailable, computing in-process: <message>` once, settles every job already handed to the worker in-process, and runs every later job in-process too.
- **Compiled sidecar**: `bun build --compile` embeds a worker only when it is passed as an extra entrypoint, so `app/scripts/build-core-sidecar.ts` compiles `core/src/server.ts` AND `core/src/layoutWorker.ts`. A compiled binary without it still works, on the fallback, and says so with the log line above.
...
**Yielding**: `computeLayoutAsync()` yields to the event loop (via `setImmediate`) whenever its own compute has run for `YIELD_BUDGET_MS` (8 ms) of wall-clock time, so no single stretch of a settle holds its thread for long. That bounds each stall but not the starvation, which is why the server runs settles in the layout worker (above); on the in-process fallback the yields are what keeps concurrent requests moving at all. It checks its `signal` before PivotMDS and before every tick, so an abort stops the settle at the next tick boundary. Output is numerically identical to the sync path, with or without a signal.

### Attaching Positions

`attachLayout(graph, vaultKey, opts?)` returns a copy of the graph whose nodes carry `position: [x,y,z]` and `position2d: [x,y]`. The `position2d` field is always two elements (the trailing `z=0` is stripped). Nodes not in the backend's computed layout get no position from this step. Unlike the old `CanvasGraphRenderer`, `AsciiGraphRenderer` does not special-case a `"self"` node's position at all in this step — see "The 'You' Self Node" above.
...
### Display layers: `[clusters]` and `[gradient]`

Two buttons in the full-pane graph's ViewBar, both defaulting to on, backed by `app/src/graph/graphLayers.ts`. `[clusters]` is `GraphConfig.showLodMasses`, and it works in both 2D and 3D. On, notes read as named community groups: in 2D the far-zoom band draws each community as one aggregate territory mass (see "The three-band zoom ladder" below); 3D has no masses, so there it is the cluster-name ladder over the cloud. Off, the field is a plain every-note graph in either dimension — every glyph at every zoom, and `layoutLabels()` swaps the cluster names for note names at every zoom (collisions still drop a name rather than overprint it). Local mode forces `showLodMasses` off regardless of the toggle (a neighbourhood carries no community hierarchy to summarize — see "Mode 4" above). `[gradient]` mounts/unmounts `GraphAtmosphere` (the phosphor bloom + depth vignette, see "Graph Atmosphere" below) — off, no bloom canvas and no vignette exist in the DOM at all, not merely hidden. Both are per-window `localStorage` choices (`bismuth:graph:clusters`, `bismuth:graph:gradient`), shared by the full-pane graph and the sidebar mini graph (one module-level signal each, exactly like the 2D/3D toggle), and never written to `.settings`. The mini graph and the full-pane graph's own `≤520px` layout show neither button.
