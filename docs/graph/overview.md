...
The pure 3D-then-2D compute is `computeLayoutPair(job, signal)` (a `LayoutJob` = `{ input, refineTicks, seed?, fixedIds? }`): with `fixedIds` + `seed` it is the pinned settle, otherwise a full settle warm-started from `seed.pos3d` when there is one. It holds no cache or seed state, so it can run anywhere unchanged.
...
**Server hot path**: `computeLayoutAsync()` yields to the event loop (via `setImmediate`) whenever its own compute has run for `YIELD_BUDGET_MS` (8 ms) of wall-clock time, so a large graph settle doesn't block concurrent requests. It checks its `signal` before PivotMDS and before every tick, so an abort stops the settle at the next tick boundary. Output is numerically identical to the sync path, with or without a signal.
