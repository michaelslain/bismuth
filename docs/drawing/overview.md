...
Toggles carry no frame at rest (full-contrast muted ink) and a 1px accent frame when on, so a freshly opened PDF paints no frame at all; nothing else in the bar is ever framed. The page readout is unframed at rest too. Narrow panes use only the shared view-bar collapse ladder (`data-bar-drop`, `app/src/ui/ui.css`): the zoom steps (−, %, +) and the file actions drop below its 650px tier, the page readout below 500px; FIT, highlight draw scratch and bookmarks stay at every width. The filename ellipsizes first and keeps about six characters. There is no second bar row.
