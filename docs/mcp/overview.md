...
The server (`mcp/src/server.ts`, low-level `@modelcontextprotocol/sdk` `Server` + `StdioServerTransport`, raw JSON-Schema — no zod) also sets a server-level **`instructions`** string (`mcp/src/instructions.ts`'s `SERVER_INSTRUCTIONS`, pinned by `mcp/src/serverInstructions.test.ts` to stay under 120 words) — text the client reads before ever calling a tool. Its one job is heading off the most common tagging mistake: it states that an image/PDF's tags live in its hidden companion note `<file>.<ext>.md`, set via `bismuth prop set <file.pdf> tags '[...]'`, and that an agent must never create a separate `<name>.md` that just embeds the file to hold tags. See [`vault/frontmatter.md`](../vault/frontmatter.md#companion-notes-frontmatter-for-binary-files-imagespdfs) for the full companion-note model.
