...
Every source calls the shared `applyInsert(view, completion, from, to, insert, cursorOffset)` helper in `autocomplete.ts` which dispatches a single `view.dispatch({changes, selection, annotations: pickedCompletion.of(completion)})`. Using `pickedCompletion.of` is mandatory so CodeMirror's internal bookkeeping (closing the popup, etc.) stays correct.

Several sources use `startCompletion(view)` after the dispatch to immediately re-open the popup for the next step (e.g. after inserting a task emoji to show date options, or after picking a query key to show value options).
