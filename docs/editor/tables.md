...
| `"tag"`      | `<span class="cm-tag" data-tag="<name>">#<name></span>` — the editor's tag mark (gold, in the UI/mono face — `.cm-tag { color: var(--gold); font-family: var(--ui-font-stack) }`, `livePreview.ts`), so a tag in a cell reads identically to one in the note body (#41). Display-only, like tags in the editor body (no click navigation). |
