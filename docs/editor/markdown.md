...
- **Body lines**: each gets `cm-codeblock cm-block-mid` (Monaspace Xenon, `--mono-scale` font-size, `line-height:1.5`; fill + accent left edge from `cm-block-mid`) and a 1-based in-block line number via `numberedLine("cm-codeblock cm-block-mid", lineNumber - openLine)`.
