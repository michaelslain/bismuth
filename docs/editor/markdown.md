...
**Rendering behavior** (bug #10, 4th round — a spec change, not a bug: earlier rounds tried a hairline edge, then a full rounded-corner bordered CARD spanning every line; the user saw the card in their real app and dialed it back to just the fence line): **only** the opening/closing `---` delimiter lines get any chrome — a self-contained grey bar (`cm-fence-bar`: a subtly distinct background + a GREY, never `var(--accent)`, left accent edge, rounded on all four corners), ALWAYS visible, on and off cursor. Property rows in between render as **plain text** — no fill, no border, no left/right edge, same background as the rest of the editor.

- **Opening and closing `---` delimiters** always carry `cm-fence-bar` — regardless of cursor state, both delimiters render identically (there's no card roof/floor left to distinguish). The literal dashes stay **always visible** in very dim mono (`cm-fence-syntax`, `color-mix(fg 30%)`) — the #10 ask ("the em dashes should always be visible") — and brighten to `cm-syntax-mark` when the cursor is on that exact delimiter line. Never `display:none`-hidden: a fully-hidden line collapses to zero height, which would erase that line's rounded corners. This ensures both delimiters look the same even though the markdown tokenizer handles them differently, and the bar itself never flickers as the cursor enters/leaves.
- **Property rows** (lines between the delimiters): each gets plain `cm-frontmatter` (Monaspace Xenon, `--mono-scale` font-size) — no shared chrome with the fence bar. In-block 1-based line numbers are displayed via the `numberedLine` mechanism (see [Code Line Numbers](#code-line-numbers-in-block-line-numbers) below).
...
Like frontmatter, ONLY the opening/closing ` ``` ` fence lines render with any chrome (bug #10, 4th round) — `cm-fence-bar` on both, regardless of cursor state, so the bar never flickers as the cursor enters/leaves. Body lines are plain text. See [YAML Frontmatter](#yaml-frontmatter) above for the shared class design.

### Rendered mode (cursor outside the block)

- **Opening fence line** (`codeBlock.open`): gets `cm-fence-bar`, and its text is replaced by a `CodeHeaderWidget`. The widget mounts the `<CodeHeader>` Solid component, which renders a dim language label on the left (`cm-code-lang`, shows `"text"` if no info string) and a copy-to-clipboard icon button on the right (`cm-code-copy`) — riding the bar (it inherits `cm-fence-bar`'s padding).
- **Closing fence line** (`codeBlock.close`): gets `cm-fence-bar`, with its raw ` ``` ` always visible in very dim mono (`cm-fence-syntax` — same rationale as the frontmatter dashes: always-visible per the #10 ask, and a hidden line would collapse and erase the bar's rounded corners).
- **Body lines**: each gets plain `cm-codeblock` (Monaspace Xenon, `--mono-scale` font-size, `line-height:1.5`, no fill/border) and a 1-based in-block line number via `numberedLine("cm-codeblock", lineNumber - openLine)`.
...
- In edit mode, the opening/closing fence lines keep their `cm-fence-bar` styling (the bar itself never keys off reveal state) but show their raw ` ``` ` text at full mono contrast instead of the header widget / dim `cm-fence-syntax`; body lines are unaffected.
