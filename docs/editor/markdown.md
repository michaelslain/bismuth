...
**Rendering behavior**: the whole block renders as **one homogeneous container** — a flat `var(--surface-1)` fill across ALL lines and a continuous `2px` inset `var(--accent)` left edge running the block's full height, with no rounded corners and no distinct band colour on the fence rows, so the block reads as one continuous surface. Built from per-line classes, since a CodeMirror line decoration can't span multiple lines: `cm-block-mid` on every body line, plus `cm-block-top` / `cm-block-bottom` on the opening/closing `---` (same fill and edge; they differ only in padding and outer margin).

**The fill is painted by an `::after` pseudo-element, not a `background` on the line** — and that is load-bearing. CodeMirror paints text selection into `.cm-selectionLayer`, an absolutely-positioned sibling of `.cm-content` at a *negative* z-index inside `.cm-scroller`'s stacking context. In CSS painting order a negative-z-index child paints before in-flow block backgrounds, so an opaque `background` on these lines painted straight over the selection rect: dragging a selection across a code block or frontmatter panel made the highlight vanish for exactly those rows while the transparent prose lines around them kept it. The inset accent edge moved with it, since an inset `box-shadow` paints in the same step. So the three classes carry only `position: relative` (no `z-index` — that would make each line its own stacking context and trap the pseudo above the layer again, i.e. the bug straight back), and a `::after` with `inset: 0`, `z-index: -10` and `pointer-events: none` carries the fill and the edge. `inset: 0` resolves against the padding box, which is the same rect `background` covered. `-10` rather than `-1`: the selection layer's z-index is computed as `(above ? 150 : -1) - pos` over all registered layers, so it measures `-2` in practice and shifts again if another layer is added; `-10` clears the range without relying on a tree-order tie. It is `::after` and not `::before` because `codeLineNumbers.ts` already owns `::before` on these lines for the in-block gutter number. `app/src/editor/BlockSelection.stories.tsx` is the regression story.

- **Opening and closing `---` delimiters** always carry `cm-block-top` / `cm-block-bottom` — the container's roof/floor — regardless of cursor state. The literal dashes stay **always visible** in very dim mono (`cm-fence-syntax`, `color-mix(fg 30%)`) — the #10 ask ("the em dashes should always be visible") — and brighten to `cm-syntax-mark` when the cursor is on that exact delimiter line. Never `display:none`-hidden: a fully-hidden line collapses to zero height, which would break the block's fill at that row. This ensures both delimiters look the same even though the markdown tokenizer handles them differently, and the container itself never flickers as the cursor enters/leaves.
- **Property rows** (lines between the delimiters): each gets `cm-frontmatter cm-block-mid` (Monaspace Xenon at `--code-font-size`; the container's fill + accent left edge come from `cm-block-mid`). In-block 1-based line numbers are displayed via the `numberedLine` mechanism (see [Code Line Numbers](#code-line-numbers-in-block-line-numbers) below).
- **`key:` portion**: The regex `/^(\s*(?:-\s+)?)([A-Za-z0-9_$.-]+)\s*:/` (`FM_KEY_RE`; the leading group swallows a sequence dash, so `- type: x` marks `type`) detects the key name on each property row. The key receives `cm-fm-key` (color: `--text-muted`), while the value portion inherits `--fg`. The body lines of a ` ```yaml ` / ` ```yml ` fenced block get the same mark, so a key reads identically in both blocks.
...
Like frontmatter, the whole block ALWAYS renders as one homogeneous container — `cm-block-top` on the opening fence, `cm-block-mid` on every body line, `cm-block-bottom` on the closing fence, all three carrying the same flat fill and accent left edge via the `::after` described under [YAML Frontmatter](#yaml-frontmatter) — regardless of cursor state, so the container never flickers as the cursor enters/leaves. See [YAML Frontmatter](#yaml-frontmatter) above for the shared class design.
...
- The table's lines get `cm-table` (Monaspace Xenon at `--code-font-size`) so pipe structure is readable.
...
**Inline code styling** (`cm-inline-code`): Monaspace Xenon at `--fs-rel-code` (`1em` × `--code-scale` × `--mono-scale` — the code-to-prose ratio taken off the surrounding text, so it scales inside a heading), `--surface-2` background, `3px` border-radius, `0 3px` padding. The size is set in `global.css`, not the theme.
...
The entire tag (including the `#`) gets `cm-tag` (`color: var(--gold)`, `font-family: var(--ui-font-stack)`). Body `#hashtags` deliberately read **gold**, not teal — the ASCII redesign's call, so a `#tag` reads identically wherever it appears (editor, chat transcript, the Milkdown chip, the in-table chip), all bound to the same `--gold`/`--ui-font-stack` pair. The font-family is set explicitly (rather than relying on inherited styling) so `appearance.uiFont` still governs it even inside surfaces that would otherwise render in a different face (note prose defaults to `appearance.proseFont`, a different, proportional face). No text is hidden — the `#` is always visible and colored. This applies on both cursor and non-cursor lines (there is no reveal/hide for tags).
...
| `#hashtag` | Gold (`--gold`) text including `#`, in the UI mono face (`--ui-font-stack`) at editor size; nothing hidden | Always shown gold, no hide/reveal |
