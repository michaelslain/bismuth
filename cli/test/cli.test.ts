...
test('`bismuth graph --vault <dir>` prints graph JSON with the vault nodes', async () => {
    ...
    const proc = Bun.spawn(
        ['bun', 'run', 'cli/src/index.ts', 'graph', '--vault', vault],
        {
            stdout: 'pipe',
        },
    )
...
test('`checkpoint diff` refuses under an agent channel against a REAL git repo, via --dir', async () => {
    ...
    expect(
        JSON.parse(ownerOut).files.some(
            (f: any) => f.path === 'Private/secret.md',
        ),
    ).toBe(true)
}, 30_000)
...
beforeEach(() => {
    savedEnv = {
        api: process.env.BISMUTH_API,
        relay: process.env.CLAUDE_RELAY_URL,
        run: process.env.BISMUTH_RUN_DIR,
        vault: process.env.BISMUTH_VAULT,
    }
...
test('resolveCore precedence: --api > BISMUTH_API > CLAUDE_RELAY_URL > run-registry > :4321', async () => {
    ...
    expect(resolveCore(['--api', 'http://localhost:7000'])).toBe(
        'http://localhost:7000',
    )
})
...
function mockUiControlServer(reply: unknown = { ok: true }): {
    url: string
    calls: any[]
    stop: () => void
} {
    ...
    const server = Bun.serve({
        ...
        fetch: async req => {
            if (
                req.method === 'POST' &&
                new URL(req.url).pathname === '/ui/command'
            ) {
...
test('`bismuth calendar …` create → category → add → list/range/search → move → delete, headlessly', async () => {
    ...
    expect(range.json.map((e: any) => e.date)).toEqual([
        '2026-01-05',
        '2026-01-06',
        '2026-01-12',
    ])
    ...
    expect(
        (await runCli(vault, 'calendar', 'get', cal, singleId)).json.category,
    ).toBe('Job')
    ...
    expect(
        (await runCli(vault, 'calendar', 'delete', cal, singleId)).json,
    ).toEqual({ ok: true })
...
test('`card review` honours settings.srs instead of the hardcoded defaults', async () => {
    ...
    const before = parseBaseFile(await readNote(vault, deckPath), {
        name: 'Deck',
        path: deckPath,
    })
    const expected = applyReviewToRow(
        before.rows[0].note,
        'easy',
        today(),
        { ...DEFAULT_SRS, easyGraduatingInterval: 9 },
        undefined,
    )
    ...
    const after = parseBaseFile(await readNote(vault, deckPath), {
        name: 'Deck',
        path: deckPath,
    })
...
test('`card review` (legacy inline note card) honours settings.srs instead of the hardcoded defaults', async () => {
    ...
    const { schedule, DEFAULT_SRS } =
        await import('../../core/src/srs/scheduler')
    const { today } = await import('../src/args')
    ...
    const expected = schedule(null, 'easy', today(), {
        ...DEFAULT_SRS,
        easyGraduatingInterval: 9,
    })
    // Sanity: the configured value actually diverges from what the hardcoded default would produce.
    ...
    const reviewed = (await noteCards(vault, notePath)).find(
        c => c.id === cardId,
    )
...
test('`bismuth page create` + `page list` author and read back a page headlessly', async () => {
    ...
    expect(JSON.parse(createOut)).toMatchObject({
        path: '.daemon/pages/cli-page.md',
        slug: 'cli-page',
    })

    const list = Bun.spawn(
        ['bun', 'run', 'cli/src/index.ts', 'page', 'list', '--vault', vault],
        { stdout: 'pipe' },
    )
    ...
    expect(
        pages.some((p: any) => p.slug === 'cli-page' && p.title === 'From CLI'),
    ).toBe(true)
})
...
test('a vault-wide replace leaves a git snapshot to undo from', async () => {
    ...
    const log = await Bun.spawn(['git', '-C', vault, 'log', '--oneline'], {
        stdout: 'pipe',
    })
...
test('a snapshot failure warns on stderr but the replace still proceeds', async () => {
    ...
    const before = await Bun.spawn(['git', '-C', vault, 'log', '--oneline'], {
        stdout: 'pipe',
    })
    const commitsBefore = (await new Response(before.stdout).text())
        .trim()
        .split('\n').length
    ...
    const after = await Bun.spawn(['git', '-C', vault, 'log', '--oneline'], {
        stdout: 'pipe',
    })
    const commitsAfter = (await new Response(after.stdout).text())
        .trim()
        .split('\n').length
...
test('`render --theme light` produces bytes that differ from the default dark theme', async () => {
    ...
    expect(
        Buffer.compare(readFileSync(darkOut), readFileSync(lightOut)),
    ).not.toBe(0)
})
...
test('`base create` refuses to clobber an existing file', async () => {
    const vault = makeVault({})
    expect(
        (await runCli(vault, 'base', 'create', 'Board.md', '--view', 'table'))
            .code,
    ).toBe(0)
    expect(
        (await runCli(vault, 'base', 'create', 'Board.md', '--view', 'table'))
...
test('`base validate` on a base with an unknown view type reports it AND exits non-zero', async () => {
    ...
    expect(result.json.errors.some((e: string) => e.includes('gantt'))).toBe(
        true,
    )
    expect(
        result.json.errors.some((e: string) =>
            e.includes('not a valid view type'),
        ),
    ).toBe(true)
})
...
test('`base validate` flags a declared property default that fails its own type', async () => {
    ...
    expect(
        result.json.errors.some((e: string) =>
            e.includes('properties.age.default'),
        ),
    ).toBe(true)
})
...
test("`base validate` flags a source ref that doesn't resolve to a file in the vault", async () => {
    ...
    expect(result.json.ok).toBe(false)
    expect(
        result.json.errors.some((e: string) => e.includes('Nonexistent')),
    ).toBe(true)
})
...
test('`base render` on a stat base returns a computed aggregate, not the row list', async () => {
    ...
    const byKey = Object.fromEntries(
        result.json.chart.points.map((p: any) => [p.key, p.value]),
    )
...
async function captureLogsAndExit(
    ...
    try {
        ...
        if (!(e instanceof Error) || e.message !== '__test_process_exit__')
            throw e
    } finally {
...
test('`daemon stop` calls unloadDaemon(daemonConfigPath()) and reports {ok:true} on success', async () => {
    ...
    platformMock.unload = configPath => {
        calledWith = configPath
        return { ok: true }
    }

    const { logs, code } = await captureLogsAndExit(() =>
        daemonCommands['daemon stop'].run([]),
    )
...
test('`daemon stop` exits non-zero and reports {ok:false,error} when unloadDaemon reports failure', async () => {
    platformMock.unload = () => ({
        ok: false,
        error: 'launchctl unload failed: boom',
    })
...
test('`daemon restart` calls restartDaemon() and forwards {ok:true}, exit 0', async () => {
    ...
    platformMock.restart = () => {
        restartCalled = true
        return { ok: true }
    }

    const { logs, code } = await captureLogsAndExit(() =>
        daemonCommands['daemon restart'].run([]),
    )
...
test('`daemon restart` exits non-zero when restartDaemon() reports {ok:false,error}', async () => {
    platformMock.restart = () => ({
        ok: false,
        error: 'launchctl kickstart failed: boom',
    })
...
test('`daemon restart --pretty` pretty-prints the JSON result (argument parsing works)', async () => {
    ...
    const { logs } = await captureLogsAndExit(() =>
        daemonCommands['daemon restart'].run(['--pretty']),
    )
...
function mockUpdateServer(
    replies: { status?: unknown; apply?: unknown } = {},
): {
    url: string
    ...
    const server = Bun.serve({
        ...
        fetch: req => {
            ...
            if (req.method === 'GET' && pathname === '/update/status') {
                return new Response(
                    JSON.stringify(
                        replies.status ?? { available: false, behind: 0 },
                    ),
                    {
                        headers: { 'content-type': 'application/json' },
                    },
            ...
            if (req.method === 'POST' && pathname === '/update/apply') {
                return new Response(
                    JSON.stringify(replies.apply ?? { phase: 'idle' }),
                    {
                        headers: { 'content-type': 'application/json' },
                    },
                )
...
test("`bismuth update status` GETs /update/status and prints the route's JSON", async () => {
    ...
    try {
        ...
        const [outText, , code] = await Promise.all([
            new Response(proc.stdout).text(),
            new Response(proc.stderr).text(),
            proc.exited,
        ])
        expect(code).toBe(0)
        expect(mockSrv.calls).toEqual([
            { method: 'GET', path: '/update/status' },
        ])
...
test("`bismuth update apply` POSTs /update/apply and prints the route's JSON", async () => {
    const mockSrv = mockUpdateServer({
        apply: { phase: 'pulling', message: 'pulling latest…' },
    })
    try {
        const proc = Bun.spawn(
            [
...
function mockGcalServer(
    replies: { status?: unknown; authStart?: unknown; sync?: unknown } = {},
): {
    url: string
    ...
    const server = Bun.serve({
        ...
        fetch: async req => {
            ...
            if (req.method === 'GET' && pathname === '/gcal/status')
            ...
            if (req.method === 'POST' && pathname === '/gcal/credentials')
                return Response.json({ ok: true })
            if (req.method === 'POST' && pathname === '/gcal/auth/start')
                return Response.json(
                    replies.authStart ?? {
            ...
            if (req.method === 'POST' && pathname === '/gcal/disconnect')
                return Response.json({ ok: true })
            return new Response('not found', { status: 404 })
        },
    })
...
test("`gcal status` GETs /gcal/status and prints the route's JSON", async () => {
    ...
    try {
        ...
        expect(code).toBe(0)
        expect(mockSrv.calls).toEqual([
            { method: 'GET', path: '/gcal/status', body: undefined },
        ])
        expect(JSON.parse(outText)).toEqual({
            connected: true,
            needsCredentials: false,
...
test('`gcal connect --client-id --client-secret` POSTs credentials then auth/start, prints the consent URL + a browser note (never claims the flow completed)', async () => {
    const mockSrv = mockGcalServer({
        authStart: { url: 'https://accounts.google.com/fake-consent' },
    })
    try {
        const proc = Bun.spawn(
            [
        ...
        expect(code).toBe(0)
        expect(
            mockSrv.calls.map(c => ({ method: c.method, path: c.path })),
        ).toEqual([
            { method: 'POST', path: '/gcal/credentials' },
            { method: 'POST', path: '/gcal/auth/start' },
        ])
        expect(mockSrv.calls[0].body).toEqual({
            clientId: 'cid',
...
test('`gcal sync <basePath>` POSTs {basePath} to /gcal/sync and prints the SyncResult', async () => {
    ...
    try {
        ...
        expect(mockSrv.calls[0]).toMatchObject({
            method: 'POST',
            path: '/gcal/sync',
            body: { basePath: 'Bases/Team.md' },
        })
...
test('`gcal disconnect` POSTs /gcal/disconnect and prints {ok:true}', async () => {
    ...
    try {
        ...
        expect(code).toBe(0)
        expect(mockSrv.calls).toEqual([
            { method: 'POST', path: '/gcal/disconnect', body: undefined },
        ])
...
test("`relay list` GETs /relay/snapshot and passes the route's JSON straight through", async () => {
    ...
    const server = Bun.serve({
        ...
        fetch: req => {
            const { pathname } = new URL(req.url)
            calls.push({ method: req.method, path: pathname })
            if (req.method === 'GET' && pathname === '/relay/snapshot')
...
test('http.ts call() attaches X-Bismuth-Token when the run registry has a token for the target port', async () => {
    ...
    try {
        writeRunRecord({
            port: server.port!,
            vault: '/v/chat-owner-token-test',
            pid: process.pid,
            token: 'TEST-OWNER-TOKEN-1',
        })
...
test("`chat list [--scope]` GETs /chat/sessions and passes the route's JSON straight through", async () => {
    ...
    try {
        ...
        expect(calls).toEqual([
            { method: 'GET', path: '/chat/sessions', search: '?scope=daemon' },
        ])
...
test('`chat read <id> [--provider]` GETs /chat/session-messages and passes frames straight through', async () => {
    ...
    const server = Bun.serve({
        ...
        fetch: req => {
            ...
            if (req.method === 'GET' && u.pathname === '/chat/session-messages')
...
test('SECURITY: a RESTRICTED agent channel refuses `chat list` before ever reaching the network — the seeded transcript never appears in stdout', async () => {
    ...
    const server = Bun.serve({
        ...
        fetch: req => {
            ...
            return Response.json({
                sessions: [
                    { sessionId: 's1', summary: 'SEEDED-TRANSCRIPT-SECRET-99' },
                ],
            })
        },
    })
...
test('`calendar bases` lists only calendar-view bases (type:base + view:calendar), skipping other bases and notes', async () => {
    ...
    expect(
        (await runCli(vault, 'calendar', 'create', 'Cal.md', '--title', 'Cal'))
            .code,
    ).toBe(0)
    expect(
        (await runCli(vault, 'calendar', 'category', 'add', 'Cal.md', 'Work'))
            .code,
    ).toBe(0)
    expect(
        (
    ...
    expect(result.code).toBe(0)
    expect(result.json).toEqual([
        { path: 'Cal.md', title: 'Cal', events: 1, categories: ['Work'] },
    ])
})
...
test('`calendar search --from/--to` searches EXPANDED instances of a recurring event; without a window it searches the raw master', async () => {
    ...
    expect(windowed.code).toBe(0)
    expect(windowed.json.map((e: any) => e.date)).toEqual([
        '2026-01-05',
        '2026-01-12',
        '2026-01-19',
    ]) // 3 expanded Mondays
...
test('`calendar overlaps <path> <date>` detects intersecting timed events on THAT day, ignores a non-overlapping one, and ignores a same-time overlap on a DIFFERENT day', async () => {
    ...
    expect(
        [
            result.json.overlaps[0].a.title,
            result.json.overlaps[0].b.title,
        ].sort(),
    ).toEqual(['A', 'B'])
})
...
test('`calendar move <path> <bogus-id>` fails naming the id and leaves the file unchanged', async () => {
    ...
    expect((await runCli(vault, 'calendar', 'list', cal)).json).toEqual(
        before.json,
...
test('`calendar override` splits a weekly series and overrides exactly ONE occurrence (verified via `range`)', async () => {
    ...
    const byDate = Object.fromEntries(
        range.json.map((e: any) => [e.date, e.title]),
    )
...
test('`calendar delete-occurrence` removes exactly ONE occurrence from a weekly series, no replacement (verified via `range`)', async () => {
    ...
    expect(range.json.map((e: any) => e.date)).toEqual([
        '2026-01-05',
        '2026-01-19',
    ]) // Jan 12 gone, nothing put in its place
})
...
test('`calendar category add` a duplicate name fails; categories on disk unchanged', async () => {
    ...
    expect((await runCli(vault, 'calendar', 'categories', cal)).json).toEqual([
        { name: 'Work', color: '#111' },
    ])
})
...
test('`calendar category remove` (no --reassign) clears the category off every event referencing it', async () => {
    ...
    expect((await runCli(vault, 'calendar', 'create', cal)).code).toBe(0)
    expect(
        (await runCli(vault, 'calendar', 'category', 'add', cal, 'Work')).code,
    ).toBe(0)
    const added = await runCli(
        vault,
        'calendar',
...
test('`calendar category remove --reassign <other>` reassigns events instead of clearing them', async () => {
    ...
    expect(
        (await runCli(vault, 'calendar', 'category', 'add', cal, 'Personal'))
            .code,
    ).toBe(0)
    const added = await runCli(
        vault,
...
test('`calendar category remove` errors: unknown category, unknown reassign target, and reassigning to itself — none of them write', async () => {
    ...
    const unknownReassign = await runCli(
        vault,
        'calendar',
        'category',
        'remove',
    ...
    const selfReassign = await runCli(
        vault,
        'calendar',
        'category',
        'remove',
...
const calendarArgValidationCases: {
    label: string
    args: string[]
    expect: string
}[] = [
    {
        label: '`calendar create` (no basePath)',
        args: ['calendar', 'create'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar list` (no basePath)',
        args: ['calendar', 'list'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar range` (no basePath)',
        args: ['calendar', 'range'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar range <path>` (missing <from>/<to>)',
        args: ['calendar', 'range', 'X.md'],
        expect: '<from> and <to> (YYYY-MM-DD) required',
    },
    {
        label: '`calendar get` (no basePath)',
        args: ['calendar', 'get'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar get <path>` (no id)',
        args: ['calendar', 'get', 'X.md'],
        expect: '<id> required',
    },
    {
        label: '`calendar search` (no basePath)',
        args: ['calendar', 'search'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar search <path>` (no text)',
        args: ['calendar', 'search', 'X.md'],
        expect: '<text> required',
    },
    {
        label: '`calendar day` (no basePath)',
        args: ['calendar', 'day'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar day <path>` (no date)',
        args: ['calendar', 'day', 'X.md'],
        expect: '<date> (YYYY-MM-DD) required',
    },
    {
        label: '`calendar overlaps` (no basePath)',
        args: ['calendar', 'overlaps'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar overlaps <path>` (no date)',
        args: ['calendar', 'overlaps', 'X.md'],
        expect: '<date> (YYYY-MM-DD) required',
    },
    {
        label: '`calendar add` (no basePath)',
        args: ['calendar', 'add'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar add <path>` (no --date)',
        args: ['calendar', 'add', 'X.md'],
        expect: '--date (YYYY-MM-DD) required',
    },
    {
        label: '`calendar move` (no basePath)',
        args: ['calendar', 'move'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar move <path>` (no id)',
        args: ['calendar', 'move', 'X.md'],
        expect: '<id> required',
    },
    {
        label: '`calendar move <path> <id>` (nothing to update)',
        args: ['calendar', 'move', 'X.md', 'some-id'],
        expect: 'nothing to update',
    },
    {
        label: '`calendar delete` (no basePath)',
        args: ['calendar', 'delete'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar delete <path>` (no id)',
        args: ['calendar', 'delete', 'X.md'],
        expect: '<id> required',
    },
    {
        label: '`calendar override` (no basePath)',
        args: ['calendar', 'override'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar override <path>` (no id)',
        args: ['calendar', 'override', 'X.md'],
        expect: '<id> (recurring event) required',
    },
    {
        label: '`calendar override <path> <id>` (no date)',
        args: ['calendar', 'override', 'X.md', 'some-id'],
        expect: '<date> (YYYY-MM-DD occurrence) required',
    },
    {
        label: '`calendar delete-occurrence` (no basePath)',
        args: ['calendar', 'delete-occurrence'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar delete-occurrence <path>` (no id)',
        args: ['calendar', 'delete-occurrence', 'X.md'],
        expect: '<id> (recurring event) required',
    },
    {
        label: '`calendar delete-occurrence <path> <id>` (no date)',
        args: ['calendar', 'delete-occurrence', 'X.md', 'some-id'],
        expect: '<date> (YYYY-MM-DD occurrence) required',
    },
    {
        label: '`calendar categories` (no basePath)',
        args: ['calendar', 'categories'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar category add` (no basePath)',
        args: ['calendar', 'category', 'add'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar category add <path>` (no name)',
        args: ['calendar', 'category', 'add', 'X.md'],
        expect: '<name> required',
    },
    {
        label: '`calendar category update` (no basePath)',
        args: ['calendar', 'category', 'update'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar category update <path>` (no name)',
        args: ['calendar', 'category', 'update', 'X.md'],
        expect: '<name> required',
    },
    {
        label: '`calendar category update <path> <name>` (nothing to update)',
        args: ['calendar', 'category', 'update', 'X.md', 'SomeCat'],
        expect: 'nothing to update',
    },
    {
        label: '`calendar category remove` (no basePath)',
        args: ['calendar', 'category', 'remove'],
        expect: '<basePath> required',
    },
    {
        label: '`calendar category remove <path>` (no name)',
        args: ['calendar', 'category', 'remove', 'X.md'],
        expect: '<name> required',
    },
]
...
test('`checkpoint ref` reports sha:null before any checkpoint (auto-inits the repo, like `checkpoint diff` does); `checkpoint advance` commits pending changes and moves the ref — verified via a FRESH `checkpoint ref` read and the git log', async () => {
    const repo = makeVault(
        { 'note.md': 'hello\n' },
        'bismuth-checkpoint-advance-',
    )

    const before = await spawnCli([
        'checkpoint',
        'ref',
        'cron-x',
        '--dir',
    ...
    const log = await Bun.spawn(['git', '-C', repo, 'log', '--oneline'], {
        stdout: 'pipe',
    })
    const commits = (await new Response(log.stdout).text())
        .trim()
        .split('\n')
        .filter(Boolean)
...
test('`checkpoint advance`/`checkpoint ref` (no ref) fail with a usage message naming the command; (no --dir/--vault, BISMUTH_VAULT unset) fails naming the missing dir', async () => {
    ...
    const noRefAdvance = await spawnCli([
        'checkpoint',
        'advance',
        '--dir',
        repo,
    ...
    const noDir = await spawnCli(['checkpoint', 'advance', 'some-ref'], {
        ...process.env,
...
test('`page resolve <path> <actionId>` on a pure-dismiss action (no prompt) settles the page to dismissed and writes NO trigger file', async () => {
    ...
    expect(list.json.find((p: any) => p.slug === 'dismiss-me').status).toBe(
        'dismissed',
    )
    ...
    const { join } = await import('node:path')
    expect(
        existsSync(join(vault, '.daemon', 'pages', '.triggers', 'dismiss-me')),
    ).toBe(false)
})
...
test("`page resolve <path> <actionId>` on an approve action (has a prompt) sets status:working and drops a trigger file for the daemon's processPageTriggers to poll", async () => {
    ...
    const create = await runCli(
        ...
        'approve-me',
        '--actions',
        JSON.stringify([{ id: 'go', label: 'Go', prompt: 'do the thing' }]),
    )
    const path = create.json.path as string
    ...
    const list = await runCli(vault, 'page', 'list')
    expect(list.json.find((p: any) => p.slug === 'approve-me').status).toBe(
        'working',
    )
    ...
    const { join } = await import('node:path')
    expect(
        existsSync(join(vault, '.daemon', 'pages', '.triggers', 'approve-me')),
    ).toBe(true)
})
...
test("`page resolve <path> <bogus-action>` fails naming the action; the page's state is left untouched", async () => {
    ...
    const list = await runCli(vault, 'page', 'list')
    expect(list.json.find((p: any) => p.slug === 'unknown-action').status).toBe(
        'pending',
    ) // untouched
})
...
test("`page mark-failed <path>` forces a stuck 'working' page to failed (verified via `page list`), but never re-marks an already-settled (dismissed) page", async () => {
    const { vault } = await makeSampleVault()
    const stuck = await runCli(
        vault,
        'page',
        'create',
        'stuck',
        '--actions',
        JSON.stringify([{ id: 'go', label: 'Go', prompt: 'do it' }]),
    )
    const stuckPath = stuck.json.path as string
    expect(
        (await runCli(vault, 'page', 'resolve', stuckPath, 'go')).json.status,
    ).toBe('working')
    ...
    expect(list1.json.find((p: any) => p.slug === 'stuck').status).toBe(
        'failed',
    )
    ...
    const markFailed2 = await runCli(
        vault,
        'page',
        'mark-failed',
        dismissedPath,
    ...
    expect(
        list2.json.find((p: any) => p.slug === 'already-dismissed').status,
    ).toBe('dismissed') // NOT overwritten
})
...
test('`prop set <file> <key> <value>` JSON-parses the value when possible, else keeps it a raw string — verified by reading the note back from disk', async () => {
    ...
    expect(
        (await runCli(vault, 'prop', 'set', 'Note.md', 'priority', '3')).code,
    ).toBe(0)
    expect(
        (await runCli(vault, 'prop', 'set', 'Note.md', 'done', 'true')).code,
    ).toBe(0)
    expect(
        (await runCli(vault, 'prop', 'set', 'Note.md', 'status', 'in progress'))
            .code,
    ).toBe(0)
...
test("`bismuth tree` lists the vault's real file tree, including nested folders and file/dir kinds", async () => {
    ...
    const byPath = Object.fromEntries(
        result.json.map((e: any) => [e.path, e.kind]),
    )
...
test('`bismuth tree` (no vault, BISMUTH_VAULT unset) fails cleanly naming the missing vault', async () => {
    const result = await spawnCli(['tree'], {
        ...process.env,
        BISMUTH_VAULT: undefined,
    })
    expect(result.code).toBe(1)
...
test('`backup` (no vault, BISMUTH_VAULT unset) fails cleanly naming the missing vault', async () => {
    const result = await spawnCli(['backup'], {
        ...process.env,
        BISMUTH_VAULT: undefined,
    })
    expect(result.code).toBe(1)
