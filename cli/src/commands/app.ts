...
const unreachable = (base: string) =>
    `could not reach a running Bismuth app at ${base} — open the app, or pass --api <url>`
...
async function command(
    ...
    if (!reply || reply.ok !== true)
        fail(reply?.error ?? 'app control command failed')
    return reply.result
}
...
export const commands: CommandMap = {
    'app windows': {
        summary:
            'List open Bismuth windows (id, label, active tab, tab count) from a running app',
    ...
    'app open': {
        ...
        run: async args => {
            ...
            if (!content)
                fail(
                    'usage: bismuth app open <content> [--new-tab] [--window <id>]',
                )
            out(
                await command(args, 'open-tab', {
                    content,
                    newTab: bool(args, 'new-tab'),
                }),
                args,
            )
        },
    ...
    'app rename': {
        summary:
            'Rename a tab (custom label overriding its auto content label)',
        usage: '<tabId> <name> [--window <id>]',
        run: async args => {
            const [tabId, name] = positionals(args)
            if (!tabId || !name)
                fail('usage: bismuth app rename <tabId> <name> [--window <id>]')
    ...
    'app pin': {
        ...
        run: async args => {
            const [tabId] = positionals(args)
            if (!tabId)
                fail('usage: bismuth app pin <tabId> [--off] [--window <id>]')
            out(
                await command(args, 'pin-tab', {
                    tabId,
                    pinned: !bool(args, 'off'),
                }),
                args,
            )
        },
    ...
    'app reorder': {
        ...
        run: async args => {
            ...
            if (!tabId || indexRaw === undefined)
                fail(
                    'usage: bismuth app reorder <tabId> <index> [--window <id>]',
                )
            const index = Number(indexRaw)
            if (!Number.isInteger(index))
                fail(`invalid index "${indexRaw}" — must be an integer`)
            out(await command(args, 'reorder-tab', { tabId, index }), args)
        },
    },
    'app run': {
        summary:
            'Run a safe UI command by id (see `app commands`); chat + heavyweight verbs are blocked',
    ...
    'app commands': {
        summary:
            'List the command ids `app run` accepts (the command catalog minus the app-control blocklist)',
        usage: '[--pretty]',
