...
export const commands: CommandMap = {
    'daemon status': {
        summary:
            "Print the daemon's liveness, this device id, and current owner",
        usage: '[--pretty]',
        run: args => {
    ...
    'daemon owner': {
        summary:
            'Print the current owner device, or claim a device as owner when <deviceId> is given',
    ...
    'daemon stop': {
        summary:
            "Stop the installed daemon service (launchctl unload / systemctl stop+disable) — it won't restart on its own",
        usage: '[--pretty]',
        run: args => {
    ...
    'daemon restart': {
        summary:
            'Restart the running daemon service in place (launchctl kickstart -k / systemctl restart) without rewriting its config',
        usage: '[--pretty]',
        run: args => {
    ...
    'daemon graph': {
        summary:
            "Build this vault's daemon-mode graph (daemon hub → crons + processes) and print it as JSON",
    ...
    'daemon cron toggle': {
        summary:
            'Enable (or, with --off, disable) a cron in this vault by flipping its `enabled` frontmatter',
        usage: '<name> --vault <dir> [--off]',
        run: args => {
            const [name] = positionals(args)
            if (name === undefined)
                fail('usage: daemon cron toggle <name> --vault <dir> [--off]')
            setCronEnabled(
                name,
                !bool(args, 'off'),
    ...
    'daemon cron run': {
        summary:
            'Request the daemon to run a cron in this vault NOW (drops a trigger file the daemon polls)',
        usage: '<name> --vault <dir>',
        run: args => {
            const [name] = positionals(args)
            if (name === undefined)
                fail('usage: daemon cron run <name> --vault <dir>')
    ...
    'daemon process toggle': {
        summary:
            'Enable (or, with --off, disable) a background process in this vault by flipping its `enabled` frontmatter',
        usage: '<name> --vault <dir> [--off]',
        run: args => {
            const [name] = positionals(args)
            if (name === undefined)
                fail(
                    'usage: daemon process toggle <name> --vault <dir> [--off]',
                )
            setProcessEnabled(
                name,
                !bool(args, 'off'),
