...
export const commands: CommandMap = {
  "gcal status": {
    summary: "Google Calendar connection status: connected?, needs credentials?, account, timezone (requires a running server)",
    usage: "[--api <url>]",
    run: async (args) => out(await call(resolveCore(args), "GET", "/gcal/status", undefined, unreachable), args),
  },

  "gcal connect": {
    summary: "Start Google OAuth: prints the consent URL. A PERSON must finish sign-in in a browser — this cannot complete on its own (requires a running server)",
    ...
    run: async (args) => {
      ...
      if (clientId || clientSecret) {
        if (!clientId || !clientSecret) fail("usage: gcal connect --client-id <id> --client-secret <secret>");
        await call(resolveCore(args), "POST", "/gcal/credentials", { clientId, clientSecret }, unreachable);
      }
      const res = (await call(resolveCore(args), "POST", "/gcal/auth/start", undefined, unreachable)) as { url: string };
      out(
        {
          url: res.url,
          note:
  ...
  "gcal sync": {
    summary: "Two-way sync ONE calendar base against Google now; returns the SyncResult counts (pulled/pushed/deleted/conflicts/…) (requires a running server)",
    usage: "<basePath> [--api <url>]",
    run: async (args) => {
      const [basePath] = positionals(args);
      if (!basePath) fail("usage: gcal sync <basePath>");
      out(await call(resolveCore(args), "POST", "/gcal/sync", { basePath }, unreachable), args);
    },
  },

  "gcal disconnect": {
    summary: "Disconnect Google Calendar: revokes the token and wipes local sync state. Permanent — event links are not recoverable (requires a running server)",
    usage: "[--api <url>]",
    run: async (args) => out(await call(resolveCore(args), "POST", "/gcal/disconnect", undefined, unreachable), args),
  },

  "gcal targets": {
    summary: "List calendar bases with Google sync enabled — the same scan the auto-sync ticker runs (headless: reads the vault + settings directly, no server needed)",
    usage: "",
    run: async (args) => {
  ...
  "gcal health": {
    ...
    run: async (args) => {
      ...
      out(basePath ? summarize(basePath) : Object.keys(manifest.bases).map(summarize), args);
    },
  },
};
