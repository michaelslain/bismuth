...
describe('MemoryGraph', () => {
    ...
    test('writeNote and readNote round-trip', async () => {
        ...
        await writeNote(
            'Alice',
            fm,
            'Alice is a software engineer. She knows [[Bob]].',
            tempDir,
        )
    ...
    test("isMemoryNoteVisibleToDaemon: true only when visibility is absent ('all')", async () => {
        ...
        expect(
            isMemoryNoteVisibleToDaemon((await readNote('A', tempDir))!),
        ).toBe(true)
        expect(
            isMemoryNoteVisibleToDaemon((await readNote('B', tempDir))!),
        ).toBe(false)
        expect(
            isMemoryNoteVisibleToDaemon((await readNote('C', tempDir))!),
        ).toBe(false)
    })
    ...
    test('backlinks are extracted from content', async () => {
        ...
        expect(note!.backlinks.sort()).toEqual([
            'Auth Module',
            'Deploy Process',
        ])
    })
    ...
    test('duplicate backlinks are deduplicated', async () => {
        ...
        await writeNote(
            'DupLinks',
            fm,
            'See [[Alpha]] and again [[Alpha]] and [[Alpha]].',
            tempDir,
        )
    ...
    describe('loadAllNotes', () => {
        ...
        test('handles mix of valid and files with missing frontmatter', async () => {
            ...
            await Bun.write(
                join(tempDir, 'raw-content.md'),
                'Just raw content, no frontmatter.',
            )
    ...
    describe('folder support', () => {
        ...
        test('parseNoteRef splits folder-prefixed and bare references', () => {
            ...
            expect(parseNoteRef('moltbook/foo')).toEqual({
                folder: 'moltbook',
                name: 'foo',
            })
            expect(parseNoteRef('moltbook/foo.md')).toEqual({
                folder: 'moltbook',
                name: 'foo',
            })
            expect(parseNoteRef('/foo')).toEqual({ name: 'foo' })
