...
import { writeNote, type NoteFrontmatter } from '../src/graph.ts'
import { parseQuery, executeQuery, query } from '../src/query.ts'

function makeTempDir(): string {
    return join(
        tmpdir(),
        `claude-bot-query-test-${randomBytes(8).toString('hex')}`,
    )
}
...
describe('executeQuery', () => {
    ...
    test("excludes 'hidden' and 'chat-only' memory notes from every query, even match-all", async () => {
        await writeNote(
            'SecretNote',
            { ...mkFm('fact', ['secret']), visibility: 'hidden' },
            'a hidden fact',
            tempDir,
        )
        await writeNote(
            'DraftNote',
            { ...mkFm('fact', ['draft']), visibility: 'chat-only' },
            'a draft fact',
            tempDir,
        )
    ...
    test('filters by tag', async () => {
        ...
        expect(results.map(n => n.name).sort()).toEqual([
            'AuthModule',
            'ProjectAuth',
        ])
    })
    ...
    test('filters by multiple tags (AND)', async () => {
        const results = await executeQuery(
            parseQuery('tag:active tag:backend'),
            tempDir,
        )
        expect(results).toHaveLength(1)
    ...
    test('combined type + tag + keyword', async () => {
        const results = await executeQuery(
            parseQuery('type:project tag:active keyword:auth'),
            tempDir,
        )
...
describe('executeQuery — edge cases', () => {
    ...
    test('multiple filters with no matching note returns empty array', async () => {
        ...
        const results = await executeQuery(
            parseQuery('type:person tag:active'),
            tempDir,
        )
        expect(results).toEqual([])
    })
    ...
    test('keyword matches in note name', async () => {
        ...
        const results = await executeQuery(
            parseQuery('keyword:DeployWorkflow'),
            tempDir,
        )
        expect(results).toHaveLength(1)
        expect(results[0]!.name).toBe('DeployWorkflow')
    ...
    test('multiple type filters act as OR — matches either type', async () => {
        const results = await executeQuery(
            parseQuery('type:person type:workflow'),
            tempDir,
        )
        const names = results.map(n => n.name).sort()
    ...
    test('multiple link filters — note must link to ALL targets', async () => {
        ...
        const results = await executeQuery(
            parseQuery('link:Alice link:AuthModule'),
            tempDir,
        )
        expect(results).toEqual([])
    })
