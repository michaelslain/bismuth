...
import { extractKeywords, searchMemory } from '../src/search.ts'

function makeTempDir(): string {
    return join(
        tmpdir(),
        `claude-bot-search-test-${randomBytes(8).toString('hex')}`,
    )
}
...
describe('searchMemory', () => {
    ...
    beforeEach(async () => {
        ...
        await writeNote(
            'Alice',
            mkFm('person', ['friend']),
            'Alice is an engineer who works on authentication.',
            tempDir,
        )
        await writeNote(
            'DeployGuide',
            mkFm('workflow', ['devops']),
            'Steps to deploy the backend service.',
            tempDir,
        )
        await writeNote(
            'ProjectAuth',
            mkFm('project', ['active']),
            'The auth project for login and sessions.',
            tempDir,
        )
    })
    ...
    test('ranks exact matches above stem matches', async () => {
        const results = await searchMemory(
            'authentication project login sessions',
            tempDir,
        )
        const names = results.map(n => n.name)
        ...
        expect(names).toContain('Alice')
        expect(names.indexOf('ProjectAuth')).toBeLessThan(
            names.indexOf('Alice'),
        )
    })
    ...
    test("never surfaces a 'hidden' memory note, even on a strong keyword match", async () => {
        await writeNote(
            'SecretAuth',
            { ...mkFm('fact', ['auth']), visibility: 'hidden' },
    ...
    test("never surfaces a 'chat-only' memory note either (recall is daemon-facing)", async () => {
        await writeNote(
            'DraftAuth',
            { ...mkFm('fact', ['auth']), visibility: 'chat-only' },
            'authentication draft notes',
            tempDir,
        )
        const results = await searchMemory(
            'authentication draft notes',
