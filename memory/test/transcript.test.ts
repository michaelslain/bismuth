...
describe('extractTurns', () => {
    ...
    test('clamps a single oversized message with a marker', () => {
        ...
        expect(turns[0].claude.length).toBeLessThanOrEqual(
            PER_MESSAGE_CHARS + 8,
        )
    ...
    test('strips the demarcated <bismuth-memory> recall envelope (isolation: 3rd brain never re-collected)', () => {
        ...
        const turns = extractTurns([
            user(injected),
            assistant('15 minutes, per the note.'),
        ])
...
describe('trimToBudget', () => {
    test('keeps whole turns and inserts one omission marker, never splitting a turn', () => {
        ...
        expect(
            trimmed[trimmed.length - 1].user.startsWith('question 39 '),
        ).toBe(true)
    })
...
describe('buildAutoNoteBody', () => {
    test('renders the paired format', () => {
        const body = buildAutoNoteBody([
            user('what is bismuth? '.repeat(3)),
            assistant('a knowledge tool '.repeat(3)),
        ])
    ...
    test('MIN check sums BOTH roles — a tiny prompt with real work is kept', () => {
        const body = buildAutoNoteBody([
            user('continue'),
            assistant('I refactored the entire parser and added twelve tests.'),
        ])
    ...
    test('drops trivial and cron-fired sessions', () => {
        ...
        expect(
            buildAutoNoteBody([
                user('[Cron: dream] consolidate memory now please and thanks'),
                assistant('done, consolidated everything'),
            ]),
        ).toBeNull()
    })
    ...
    test('respects the whole-body cap', () => {
        ...
        for (let i = 0; i < 60; i++) {
            entries.push(
                user(`q${i} ` + 'u'.repeat(400)),
                assistant(`a${i} ` + 'c'.repeat(400)),
            )
        }
    ...
    test('SDK SessionMessage-shaped input produces identical output to raw JSONL shape', () => {
        const raw: TranscriptEntry[] = [
            user('hello there friend of mine'),
            assistant('greetings, wonderful human being'),
        ]
