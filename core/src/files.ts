...
export async function listTree(
    ...
    const entries = await walkDir(
        ...
        (d, rel) => {
            ...
            // still surfaces while the drawing-export artifact stays hidden.) The extension set itself
            // lives in fileKinds.ts, shared with the mobile mirror + the preview surface.
...
export async function resolveAsset(
    ...
    const hit = (
        await walkDir(root, d => !d.isDirectory() && d.name === base)
    )[0]
...
function carrySidecars(
    ...
    const pairs: Array<[string, string]> = wasDir
        ...
        : [
              ...
              ...(isCompanionable(from)
                  ? ([[companionPathFor(from), companionPathFor(to)]] as Array<
                        [string, string]
                    >)
                  : []),
              ...(isPageMove
    ...
    if (!wasDir && PAGE_MD_RE.test(from) && !PAGE_MD_RE.test(to)) {
        try {
            rmSync(join(root, `.daemon/pages/.triggers/${pageSlug(from)}`), {
                force: true,
            })
        } catch {
            /* best-effort */
...
export function createEntry(
    ...
    if (existsSync(abs))
        throw createError('EEXIST', `already exists: ${path}`, 409)
    if (kind === 'dir') {
