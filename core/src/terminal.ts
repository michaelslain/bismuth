...
// rc-loaded PATH).
const RELAY_PLUGIN_DIR =
    process.env.BISMUTH_RELAY_BUNDLE ??
    resolve(import.meta.dir, '..', '..', 'relay')
...
export function shimSpecsFor(
    ...
    for (const b of backends) {
        ...
        if (
            b.relayReporting === 'wrapper' &&
            (!opts.wrapperReportingEnabled || b.id === 'claude')
        )
            continue
        specs.push({
            id: b.id,
            binary: b.binary,
            realPath: resolve(b.binary),
            mode: b.relayReporting,
        })
    }
    return specs
}
...
export function serializeShimSpecs(specs: readonly ShimSpec[]): string {
    return specs
        .map(s =>
            [s.id, s.binary, s.realPath ?? '', s.mode].join(SHIM_FIELD_SEP),
        )
        .join(SHIM_RECORD_SEP)
}
...
const SHIM_SPECS: ShimSpec[] = shimSpecsFor(
    BACKEND_SHIM_CANDIDATES,
    whichBinary,
    {
...
function buildWrapperShimDir(specs: readonly ShimSpec[]): string | undefined {
    ...
    try {
        ...
        for (const s of wrapperEntries)
            symlinkSync(AGENT_SHIM_SCRIPT, join(dir, s.binary))
        return dir
    } catch {
        return undefined
    }
...
export function buildPtyEnv(p: PtyEnvParams): Record<string, string> {
    ...
    if (p.shimAvailable) {
        ...
        if (p.shimSpecs && p.shimSpecs.length)
            env.BISMUTH_SHIM_SPECS = serializeShimSpecs(p.shimSpecs)
        if (p.realClaude) {
        ...
        if (
            p.wrapperShimDir &&
            p.shimSpecs?.some(s => s.mode === 'wrapper' && s.realPath)
        ) {
            env.PATH = env.PATH
                ? `${p.wrapperShimDir}:${env.PATH}`
                : p.wrapperShimDir
        }
    }
    return env
    ...
    while (pool.length < POOL_SIZE) {
        ...
        try {
            ...
            s = spawnSession({
                ...
                cols: 80,
                rows: 24,
                relayPort: poolRelayPort,
                memoryDir: poolMemoryDir,
            })
        } catch {
 ...
 */
export function claimPooledSession(opts: {
    termId?: string
    cols: number
    rows: number
}): Session | undefined {
