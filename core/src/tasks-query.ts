...
function parseBool(toks: Tok[], today: string, errors: string[]): Predicate {
    ...
    function parseFactor(): Predicate {
        const tk = peek()
        if (!tk) {
            errors.push('unexpected end of filter')
            return () => true
        }
        if (tk.t === '(') {
            next()
        ...
        if (tk.t === 'leaf') {
            ...
            if (!p) {
                errors.push(`unrecognized filter: ${tk.v}`)
                return () => true
            }
            return p
        }
...
function makeSorter(
    ...
    if (key === 'priority') {
        return (a, b) =>
            dir * (PRIORITY_RANK[a.priority] - PRIORITY_RANK[b.priority])
    }
    if (key === 'description') {
