...
export function opencodeErrorMessage(ev: Record<string, unknown>): string {
    ...
    const err = (
        ev.error && typeof ev.error === 'object' ? ev.error : {}
    ...
    if (typeof err.name === 'string' && err.name)
        return `opencode reported an error (${err.name})`
...
function unseenSuffix(
    state: OpencodeTurnState | OpencodeServerTurnState,
    partId: string,
    text: string,
): string {
...
function toolPartFrames(
    ...
    if (
        ...
        frames.push({
            ...
            id: callId,
            content: toolContent(
                status === 'error' ? (st.error ?? st.output) : st.output,
            ),
...
export function translateOpencodeEvent(
    ...
    const part = (
        ev.part && typeof ev.part === 'object' ? ev.part : {}
    ) as Record<string, unknown>
    const frames: ChatFrame[] = []
    ...
    switch (ev.type) {
        ...
        case 'error': {
            ...
            frames.push({
                type: 'error',
                code: 'error',
                message: opencodeErrorMessage(ev),
            })
            return frames
        }
        default:
 ...
 * frames:[] }.
 */
export function translateOpencodeExport(raw: unknown): {
    title: string | null
    frames: ChatFrame[]
} {
    ...
    const title =
        typeof doc.info?.title === 'string' && doc.info.title.trim()
            ? doc.info.title.trim()
            : null
 ...
 */
export function translateOpencodeSessionMessages(
    messages: unknown,
): ChatFrame[] {
 ...
 */
export function parseOpencodeModelsVerbose(
    stdout: string,
): OpencodeModelEntry[] {
    const seen = new Set<string>()
    ...
    for (const seg of segments) {
        ...
        try {
            const meta = JSON.parse(nl >= 0 ? seg.slice(nl + 1) : '') as Record<
                string,
                unknown
            >
            if (typeof meta.name === 'string' && meta.name.trim())
                name = meta.name.trim()
            const cost = (
                meta.cost && typeof meta.cost === 'object' ? meta.cost : null
...
function finalizeModelEntries(
    entries: { id: string; name: string | null; free?: boolean }[],
): OpencodeModelEntry[] {
 ...
 */
export function modelEntriesFromProviders(
    providers: OpencodeApiProvider[],
): OpencodeModelEntry[] {
    const seen = new Set<string>()
    const entries: { id: string; name: string | null; free?: boolean }[] = []
 ...
 */
export function commandEntriesFromApi(
    commands: OpencodeApiCommand[],
): OpencodeCommandEntry[] {
    const out: OpencodeCommandEntry[] = []
...
export function withOpencodeBuiltinCommands(
    commands: OpencodeCommandEntry[],
): OpencodeCommandEntry[] {
    const out = [...commands]
    const names = new Set(commands.map(c => c.name))
    for (const b of OPENCODE_BUILTIN_COMMANDS)
        if (!names.has(b.name)) out.push(b)
    return out
}
...
export function parseOpencodeRunCommand(
    ...
    return {
        command: name,
        args: sp === -1 ? '' : trimmed.slice(sp + 1).trim(),
    }
}
 ...
 */
export function parseOpencodeAuthList(
    stdout: string,
...
export function zenFreeModelIds(models: OpencodeModelEntry[]): string[] {
    return models
        .filter(m => m.free === true && m.value.startsWith('opencode/'))
        .map(m => m.value)
}
...
export function withZenFreeRotate(
...
export function pickZenFreeModel(
    freeIds: string[],
    turnIndex: number,
): string | null {
    if (!freeIds.length) return null
    return freeIds[
        ((turnIndex % freeIds.length) + freeIds.length) % freeIds.length
    ]
}
 ...
 */
export function translateOpencodeServerEvent(
    raw: unknown,
    state: OpencodeServerTurnState,
): ChatFrame[] {
    if (!raw || typeof raw !== 'object') return []
    const ev = raw as Record<string, unknown>
    const props = (
        ev.properties && typeof ev.properties === 'object' ? ev.properties : {}
    ) as Record<string, unknown>

    if (ev.type === 'message.updated') {
        const info = (
            props.info && typeof props.info === 'object' ? props.info : {}
        ) as Record<string, unknown>
        const id = typeof info.id === 'string' ? info.id : null
        if (id && info.role === 'user') state.messageRoles.set(id, 'user')
        else if (id && info.role === 'assistant')
            state.messageRoles.set(id, 'assistant')
        return []
    }
    ...
    if (ev.type === 'message.part.delta') {
        ...
        state.emitted.set(
            partId,
            (state.emitted.get(partId) ?? 0) + delta.length,
        )
    ...
    if (ev.type === 'message.part.updated') {
        const part = (
            props.part && typeof props.part === 'object' ? props.part : {}
        ) as Record<string, unknown>
        const messageId =
 ...
 */
export function reconcileOpencodeFinalParts(
    parts: unknown,
    state: OpencodeServerTurnState,
): ChatFrame[] {
 ...
 */
export function parseOpencodePermissionAsk(
    raw: unknown,
    ...
    return {
        id,
        toolName,
        input: patterns !== undefined ? { pattern: patterns } : {},
    }
}
 ...
 */
export function opencodePermissionResponse(
    behavior: 'allow' | 'deny',
    always?: boolean,
): 'once' | 'always' | 'reject' {
...
export function buildOpencodePromptParts(
    ...
    for (const im of images ?? []) {
        parts.push({
            type: 'file',
            mime: im.media_type,
            url: `data:${im.media_type};base64,${im.data}`,
        })
    }
    return parts
}
