...
export function encodeJsonRpcNotification(
    method: string,
    params?: unknown,
): string {
    return line({
...
function selectOptionValue(
    o: AcpSessionConfigSelectOption | undefined,
): string {
...
function pickModelOption(
    candidates: AcpSessionConfigOption[],
    r: AcpNewSessionResult,
): AcpSessionConfigOption {
    const yieldsModels = (o: AcpSessionConfigOption) =>
        flattenSelectOptions(o.options).some(
            x => selectOptionValue(x).length > 0,
        )
    ...
    const availIds = new Set(
        (Array.isArray(r.models?.availableModels)
    ...
    if (availIds.size) {
        const byIntersection = candidates.find(o =>
            flattenSelectOptions(o.options).some(x =>
                availIds.has(selectOptionValue(x)),
            ),
        )
...
export function detectModelShape(newSessionResult: unknown): AcpModelShapeInfo {
    const r = (
        newSessionResult && typeof newSessionResult === 'object'
            ? newSessionResult
            : {}
    ) as AcpNewSessionResult

    const configOptions = Array.isArray(r.configOptions)
        ? r.configOptions
        : null
    // EVERY category:"model" entry, not just the first — cline emits two (see pickModelOption).
    const modelCandidates = (configOptions ?? []).filter(
        o => o && o.category === 'model' && Array.isArray(o.options),
    )
    if (modelCandidates.length) {
        const modelOpt = pickModelOption(modelCandidates, r)
        const thoughtOpt = configOptions?.find(
            o =>
                o && o.category === 'thought_level' && Array.isArray(o.options),
        )
        const effortLevels = flattenSelectOptions(thoughtOpt?.options)
            .map(selectOptionLabel)
            .filter(Boolean)
        ...
        return {
            ...
            effortConfigId:
                typeof thoughtOpt?.id === 'string' ? thoughtOpt.id : null,
        }
    }

    const modelsField =
        r.models && typeof r.models === 'object' ? r.models : null
    if (modelsField) {
        const avail = Array.isArray(modelsField.availableModels)
        ...
        const models: AcpModelEntry[] = avail
            ...
            .map(m => ({
                value: m.modelId,
                label:
                    typeof m.name === 'string' && m.name ? m.name : m.modelId,
                description:
        ...
        return {
            ...
            models,
            currentModelId:
                typeof modelsField.currentModelId === 'string'
...
export function contentBlockText(block: unknown): string {
    ...
    switch (b.type) {
        ...
        case 'resource_link': {
            const name =
                typeof b.name === 'string' && b.name ? b.name : 'resource'
 ...
 * an empty `{}`.
 */
export function toolCallInput(u: {
    title?: unknown
    kind?: unknown
    rawInput?: unknown
}): unknown {
 ...
 */
export function toolCallName(u: {
    name?: unknown
    title?: unknown
    kind?: unknown
}): string {
...
export function newAcpTranslateState(
    manifest: AcpManifestBaseline,
): AcpTranslateState {
    return {
        manifest,
        toolCalls: new Map(),
        toolsResolved: new Set(),
        costUsd: null,
        usedTokens: null,
        maxTokens: null,
    }
...
export function translateSessionUpdate(
    ...
    switch (u.sessionUpdate) {
        ...
        case 'available_commands_update': {
            const raw2 = Array.isArray(u.availableCommands)
                ? u.availableCommands
                : []
            const commands = raw2
                .filter(
                    (c): c is Record<string, unknown> =>
                        !!c &&
                        typeof c === 'object' &&
                        typeof (c as Record<string, unknown>).name ===
                            'string' &&
            ...
            state.manifest.commandDetails = Object.fromEntries(
                commands
        ...
        case 'usage_update': {
            ...
            if (amount != null && amount > 0)
                state.costUsd = (state.costUsd ?? 0) + amount
...
export function choosePermissionOption(
    ...
    const usable = options.filter(
        (o): o is { kind: string; optionId: string; name?: string } =>
            typeof o.kind === 'string' && typeof o.optionId === 'string',
    )
