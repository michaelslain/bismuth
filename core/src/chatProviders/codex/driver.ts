...
function createSession(
    ...
    if (!bin) {
        sink({
            ...
            binary: 'codex',
            message:
                'The `codex` CLI was not found. Install OpenAI Codex (`npm i -g @openai/codex`) and run `codex` once to log in.',
        })
        return null
    }
    ...
    if (s.threadId)
        emit(s, { type: 'session', sessionId: s.threadId, origin: 'user' })
...
function materializeImages(images: ChatImage[] | undefined): {
    paths: string[]
    cleanup: () => void
} {
...
async function runTurn(
    ...
    try {
        ...
        for await (const chunk of proc.stdout as ReadableStream<Uint8Array>) {
            ...
            while ((nl = pending.indexOf('\n')) >= 0) {
                ...
                parsedAnyLine = true
                for (const frame of translateThreadEvent(
                    ev,
                    s.translateState,
                )) {
    ...
    if (failed && !sawErrorFrame) {
        ...
        emit(s, {
            type: 'error',
            code: 'error',
            message: tail || `codex exited with code ${exitCode}`,
        })
    }
    emit(s, {
        type: 'result',
...
export const codexBackend: ChatBackend = {
    ...
    sendMessage: (c: ChatTurnContext & { text: string }) =>
        sendMessage(c.chatId, c.text, c.cwd, c.sink, c.images),
    resumeSession: (c: ChatTurnContext & { sessionId: string }) =>
        resumeSession(c.chatId, c.sessionId, c.cwd, c.sink),
