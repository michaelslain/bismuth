...
export function newCodexTranslateState(threadId: string | null = null): CodexTranslateState {
  return { text: new Map(), toolUseSeen: new Set(), toolResultSeen: new Set(), threadId };
}
...
function textDelta(state: CodexTranslateState, id: string, full: string): string {
...
function toolInputFor(item: Record<string, unknown>): unknown {
  switch (item.type) {
    ...
    case "mcp_tool_call":
      return { server: str(item.server) ?? "", tool: str(item.tool) ?? "", arguments: item.arguments };
    case "web_search":
...
function toolResultContentFor(item: Record<string, unknown>): string {
  switch (item.type) {
    ...
    case "file_change": {
      ...
      return changes
        .map((c) => (isRecord(c) ? `${str(c.kind) ?? "update"} ${str(c.path) ?? ""}`.trim() : ""))
        .filter(Boolean)
        .join("\n");
    }
    case "mcp_tool_call": {
      ...
      if (isRecord(result)) {
        if (Array.isArray(result.content)) {
          return result.content
            .map((c) => (isRecord(c) && typeof c.text === "string" ? c.text : JSON.stringify(c)))
            .join("\n");
        }
        if (result.structured_content !== undefined) return JSON.stringify(result.structured_content);
      }
      return "";
    }
    case "web_search":
...
function frameForItem(item: unknown, state: CodexTranslateState): ChatFrame[] {
  ...
  if (type === "agent_message" || type === "reasoning") {
    ...
    if (!delta) return [];
    return [type === "agent_message" ? { type: "assistant-text", text: delta } : { type: "thinking", text: delta }];
  }

  if (type === "error") {
    return [{ type: "error", code: "error", message: str(item.message) ?? "Codex reported an error." }];
  }
  ...
  if (TOOL_ITEM_TYPES.has(type)) {
    ...
    if (!state.toolUseSeen.has(id)) {
      ...
      frames.push({ type: "tool-use", id, name: toolNameFor(item), input: toolInputFor(item) });
    }
    const status = str(item.status);
    if ((status === "completed" || status === "failed") && !state.toolResultSeen.has(id)) {
      state.toolResultSeen.add(id);
      frames.push({ type: "tool-result", id, content: toolResultContentFor(item), isError: toolIsError(item) });
    }
    return frames;
  }
 ...
 */
export function translateThreadEvent(event: unknown, state: CodexTranslateState): ChatFrame[] {
  ...
  switch (type) {
    ...
    case "turn.failed": {
      ...
      const message = isRecord(err) && typeof err.message === "string" ? err.message : "Codex turn failed.";
      return [{ type: "error", code: "error", message }];
    }
    case "error":
      return [{ type: "error", code: "error", message: str(event.message) ?? "Codex reported a fatal error." }];
    default:
