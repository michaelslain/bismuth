...
export function buildRRule(
    ...
    if (rec.endDate)
        parts.push(
            `UNTIL=${allDay ? compact(rec.endDate) : timedUntil(rec.endDate, timeZone)}`,
        )
...
export function parseRRule(
    ...
    if (recurrence.some(r => r.startsWith('RDATE') || r.startsWith('EXDATE')))
        return null
    ...
    if (until) {
        ...
        if (/^\d{8}$/.test(ymd))
            rec.endDate = `${ymd.slice(0, 4)}-${ymd.slice(4, 6)}-${ymd.slice(6, 8)}`
    }
    return rec
}
...
export function firstOccurrence(rec: BismuthRecurrence): string {
    if (
        (rec.type === 'weekly' || rec.type === 'biweekly') &&
        rec.daysOfWeek &&
        ...
        if (!Number.isNaN(t)) {
            for (let i = 0; i < 7; i++) {
                const d = new Date(t)
                if (rec.daysOfWeek.includes(d.getUTCDay()))
...
export function recurrenceSignature(
    rec: BismuthRecurrence | undefined,
): string {
    if (!rec) return ''
    return JSON.stringify([
        rec.type,
        rec.daysOfWeek ? [...rec.daysOfWeek].sort((a, b) => a - b) : null,
        rec.startDate,
        rec.endDate ?? null,
    ])
}
