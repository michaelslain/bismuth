...
export function eventFieldsOf(note: Record<string, unknown>): MappedEvent {
    ...
    if (raw) {
        try {
            recurrence =
                typeof raw === 'string'
                    ? (JSON.parse(raw) as BismuthRecurrence)
                    : (raw as BismuthRecurrence)
        } catch {
...
export function signature(m: MappedEvent): string {
    return JSON.stringify([
        ...
        m.location ?? '',
        m.description ?? '',
        recurrenceSignature(m.recurrence),
        m.category ?? '',
    ])
}
...
export function toGoogle(
    ...
    if (m.recurrence)
        body.recurrence = buildRRule(m.recurrence, !m.startTime, timeZone)
