...
export async function primaryInfo(accessToken: string): Promise<PrimaryInfo> {
    ...
    return {
        account: obj.summary ?? 'Google Calendar',
...
export async function insertEvent(
    ...
    const text = await res.text()
    if (!res.ok)
        throw new Error(`google calendar insert ${res.status}: ${text}`)
...
export async function patchEvent(
    ...
    const text = await res.text()
    if (res.status === 412)
        throw new PreconditionFailed(`event ${eventId} changed on the server`)
