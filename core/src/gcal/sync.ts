...
const readBid = (ev: GEvent): string | undefined =>
    ev.extendedProperties?.private?.[BID_PROP]
...
function applyRemoteToNote(note: Record<string, unknown>, ev: GEvent): void {
    ...
    note.recurrence = mapped.recurrence
...
export async function syncEvents(opts: SyncOpts): Promise<SyncResult> {
    ...
    rows.forEach(r => {
        if (r.note.id != null) byBid.set(String(r.note.id), r)
    })
    ...
    const fullWindow = {
        ...
        timeMax: new Date(now + 365 * DAY_MS).toISOString(),
        showDeleted: true,
    }
    let listed: ListResult
    if (bs.syncToken) {
        try {
            listed = await listEvents(accessToken, calendarId, {
                syncToken: bs.syncToken,
            })
        } catch (e) {
    ...
    for (const ev of events) {
        ...
        if (ev.status === 'cancelled') {
            if (link) {
                if (byBid.has(link.bismuthId)) {
                    deleteBids.add(link.bismuthId)
                    res.deletedLocal++
                }
        ...
        if (!link) {
            ...
            if (bid && byBid.has(bid)) {
                link = {
                    ...
                    etag: ev.etag,
                    updated: ev.updated,
                    sig: sigOfNote(byBid.get(bid)!.note),
                }
                bs.links[ev.id] = link
                res.relinked++
                continue
        ...
        if (!link) {
            ...
            const row: Row = {
                file: baseFile,
                note: buildNote(bid, mapped, ev.updated),
                formula: {},
            }
            newRows.push(row)
            byBid.set(bid, row)
        ...
        if (remoteChanged && localChanged) {
            ...
            if (winner === 'remote') {
                ...
                bs.links[ev.id] = {
                    bismuthId: link.bismuthId,
                    etag: ev.etag,
                    updated: ev.updated,
                    sig: sigOfNote(row.note),
                }
                res.pulledUpdate++
            } else {
    ...
    const bidToEntry = new Map<
        string,
        { gcalId: string; etag?: string; sig?: string }
    >()
    for (const [gcalId, link] of Object.entries(bs.links)) {
        bidToEntry.set(link.bismuthId, {
            gcalId,
            etag: link.etag,
            sig: link.sig,
        })
    ...
    for (const row of rows) {
        ...
        try {
            if (!entry) {
                ...
                try {
                    const ev = await insertEvent(
                        accessToken,
                        calendarId,
                        stamped(toGoogle(fields, timeZone, colorMap), bid),
                        googleEventId(bid),
                    )
                    bs.links[ev.id] = {
                        bismuthId: bid,
            ...
            res.failed++
            console.error(
                `[gcal] push failed for event ${bid}: ${(e as Error).message}`,
            )
        }
    }
    ...
    for (const row of rows)
        if (!deleteBids.has(String(row.note.id ?? '')))
            currentBids.add(String(row.note.id ?? ''))
    ...
    for (const [gcalId, link] of Object.entries(bs.links)) {
        ...
        try {
            ...
            if (!(e instanceof PreconditionFailed)) {
                res.failed++
                console.error(
                    `[gcal] delete failed for ${gcalId}: ${(e as Error).message}`,
                )
            }
        }
    }
    ...
    for (const { gcalId, bid } of toStamp) {
        ...
        try {
            const ev = await patchEvent(
                ...
                calendarId,
                gcalId,
                { extendedProperties: { private: { [BID_PROP]: bid } } },
                link.etag,
            )
            link.etag = ev.etag
    ...
    if (newRows.length || deleteBids.size || res.pulledUpdate) {
        const finalRows = rows
            .filter(r => !deleteBids.has(String(r.note.id ?? '')))
            .concat(newRows)
