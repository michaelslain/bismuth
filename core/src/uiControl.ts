...
const pending = new Map<
    string,
    { resolve: (r: UiReply) => void; timer: ReturnType<typeof setTimeout> }
>()
...
export function registerWindow(id: string, send: Send, now = Date.now()): void {
    windows.set(id, {
        id,
        send,
        snapshot: windows.get(id)?.snapshot ?? null,
        lastSeen: now,
    })
}
...
export function resolveTarget(windowId?: string): TargetResolution {
    if (windowId) {
        ...
        return {
            ok: false,
            status: 404,
            error: `no Bismuth window "${windowId}" is connected`,
        }
    }
    const ids = [...windows.keys()]
    if (ids.length === 0)
        return { ok: false, status: 404, error: 'no Bismuth window is open' }
    if (ids.length === 1) return { ok: true, id: ids[0] }
    return {
        ok: false,
        status: 409,
        error: `multiple Bismuth windows open — pass --window <id> (open: ${ids.join(', ')})`,
    }
}
...
export function sendCommand(
    ...
    if (!w)
        return Promise.resolve({
            ok: false,
            error: `no Bismuth window "${id}" is connected`,
        })
    ...
    return new Promise<UiReply>(resolve => {
        const timer = setTimeout(() => {
            ...
            resolve({
                ok: false,
                error: `window "${id}" did not respond within ${timeoutMs}ms`,
            })
        }, timeoutMs)
        ...
        try {
            w.send({
                ...
                action,
                args,
            } satisfies UiCommandFrame)
        } catch (e) {
            clearTimeout(timer)
            pending.delete(reqId)
            resolve({
                ok: false,
                error: `failed to reach window "${id}": ${(e as Error).message}`,
            })
        }
    })
}
