...
function parseStatement(line: string): Statement | { error: string } | null {
    ...
    if (!first)
        return {
            error: `expected a node id (a bare word or a "quoted" string)`,
        }
    ...
    if (two === '->' || two === '--') {
        ...
        if (!second)
            return { error: `expected a target node id after "${two}"` }
...
function parseLabel(
    ...
    if (line[i] !== ':')
        return {
            error: `unexpected text ${JSON.stringify(line.slice(i))} — expected "->", "--", or ": label"`,
        }
...
export function parseGraphBlock(body: string): GraphBlockParseResult {
    ...
    for (let li = 0; li < lines.length; li++) {
        ...
        if ('error' in stmt) {
            errors.push({ line: li + 1, message: stmt.error })
            continue
        }
        if (stmt.kind === 'node') {
            ...
            if (existing?.explicit) {
                errors.push({
                    line: li + 1,
                    message: `duplicate node "${stmt.id}"`,
                })
                continue
            }
            if (existing) {
                ...
                const node: GraphBlockNode =
                    stmt.label !== undefined
                        ? { id: stmt.id, label: stmt.label }
                        : { id: stmt.id }
...
export function addNode(
    spec: GraphBlockSpec,
    label?: string,
): { spec: GraphBlockSpec; id: string } {
    const id = freshNodeId(spec)
    const node: GraphBlockNode = label?.trim()
        ? { id, label: label.trim() }
        : { id }
