 ...
 */
export function coreLaunchArgv(
    serverEntry: string,
    vault: string,
    memory: string,
    port: number,
    ...
    return compiled
        ? [process.execPath, ...tail]
        : [process.execPath, 'run', serverEntry, ...tail]
}
 ...
 * once `waitMs` elapses.
 */
export async function spawnVaultBackend(
    opts: SpawnOptions,
    ...
    for (let attempt = 1; attempt <= attempts; attempt++) {
        ...
        const child = spawn(
            coreLaunchArgv(opts.serverEntry, vault, opts.memory, port),
            cwd,
        )
        ...
        if (!died) {
            ...
            throw createError(
                'INTERNAL_ERROR',
                `backend for ${vault} did not become ready`,
                500,
            )
        }
