export type NodeKind =
    'note' | 'memory' | 'agent' | 'tag' | 'self' | 'daemon' | 'cron' | 'process'
export type EdgeKind =
    'link' | 'message' | 'about' | 'tag' | 'open' | 'supervises'
...
export function localSubgraph(
    ...
    if (!centerId || !g.nodes.some(n => n.id === centerId))
        return { nodes: [], edges: [] }
    ...
    const nodes = g.nodes
        ...
        .map(
            ({ community, communityPath, communityPathLabels, ...rest }) =>
                rest,
        )
    return {
        nodes,
