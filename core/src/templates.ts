    ...
    if (name === 'date') {
        switch (unit) {
            case 'd':
                ...
                break
            case 'w':
                clone.setDate(clone.getDate() + 7 * n)
                break
            case 'm':
                clone.setMonth(clone.getMonth() + n)
                break
            case 'y':
                clone.setFullYear(clone.getFullYear() + n)
                break
            default:
                return null // unknown unit → invalid
        }
    } else {
        // name === "time"
        switch (unit) {
            case 'h':
                clone.setHours(clone.getHours() + n)
                break
            case 'm':
                clone.setMinutes(clone.getMinutes() + n)
                break
            default:
                return null
        }
...
export const TEMPLATE_TOKENS: TemplateToken[] = [
    {
        token: '{{date}}',
        doc: 'Current date (YYYY-MM-DD). Offset/format: {{date+1d}}, {{date:YYYY-MM}}.',
    },
    {
        token: '{{time}}',
        doc: 'Current time (HH:mm). Offset/format: {{time+1h}}, {{time:h:mm A}}.',
    },
    { token: '{{title}}', doc: "The note's title (its filename without .md)." },
    {
        token: '{{cursor}}',
        doc: 'Where the caret lands after the template is inserted.',
    },
]
