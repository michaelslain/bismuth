...
const debounceMs = (): number =>
    Number(process.env.BISMUTH_BACKUP_DEBOUNCE_MS) || 30_000 // ~30s after the last save
const maxWaitMs = (): number =>
    Number(process.env.BISMUTH_BACKUP_MAX_WAIT_MS) || 300_000 // ...but at least every 5 min
...
export function scheduleBackup(dir: string, message: () => string): void {
    ...
    pendingBackups.set(dir, {
        first,
        message,
        timer: setTimeout(() => fireBackup(dir), debounceMs()),
    })
}
...
export async function checkpointRef(
    dir: string,
    ref: string,
): Promise<string | null> {
    await ensureRepo(dir)
...
export async function checkpointDelta(
    ...
    if (!refExists) {
        ...
        return {
            base: null,
            head,
            files: all.map(path => ({ status: 'A', path })),
        }
    }
