...
function drawBackground(
    ...
    for (const l of paperLines(paper.bg, w, h)) {
        ctx.beginPath()
        ctx.moveTo(l.x1, l.y1 + 0.5)
        ctx.lineTo(l.x2, l.y2 + 0.5)
        ctx.stroke()
    }
    ctx.fillStyle = paperDotColor(t)
    for (const d of paperDots(paper.bg, w, h)) {
        ctx.beginPath()
...
function fillPolygon(ctx: Ctx2D, fill: number[][]) {
    ...
    const sx = (fill[n - 1][0] + fill[0][0]) / 2,
        sy = (fill[n - 1][1] + fill[0][1]) / 2
    ctx.moveTo(sx, sy)
    for (let i = 0; i < n; i++) {
        const a = fill[i],
            b = fill[(i + 1) % n]
...
export function renderDocStacked(
    ...
    doc.pages.forEach((pg, i) =>
        translate(ctx, 0, i * h, () =>
            renderPage(ctx, pg, doc.paper, t, w, h, resolveImage),
        ),
    )
}
