...
export function schedule(
    ...
    return {
        due: addDaysISO(today, clampedInterval),
...
function scheduleNew(
    ...
    if (response === 'easy') {
        return {
            interval: cfg.easyGraduatingInterval,
            ease: cfg.baseEase + cfg.easeStep,
        }
    }
...
function scheduleExisting(
    ...
    if (response === 'hard') {
        ...
        interval = Math.max(
            1,
            Math.round(prev.interval * cfg.lapsesIntervalChange),
        )
...
const SCHEDULE_ENTRY_RE = /!(\d{4}-\d{2}-\d{2}),(\d+),(\d+)/g
...
export function parseScheduling(text: string): SchedulingInfo[] {
    ...
    SCHEDULE_ENTRY_RE.lastIndex = 0
    while ((e = SCHEDULE_ENTRY_RE.exec(m[0])) !== null) {
