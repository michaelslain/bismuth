...
function renderCloze(text: string, n: number): [string, string] {
    let i = 0
    const question = text.replace(CLOZE_RE, m =>
        i++ === n ? '[...]' : strip(m),
    )
 ...
 */
export async function noteCards(
    vault: string,
    notePath: string,
): Promise<Card[]> {
...
export async function applyReview(
    ...
    if (!pc)
        throw createError('CARD_NOT_FOUND', `card not found: ${cardId}`, 404)
    if (subIndex < 0 || subIndex >= pc.subCount) {
        throw createError(
            'CARD_NOT_FOUND',
            `sub-card not found: ${cardId}`,
            404,
        )
    }
    ...
    if (
        ...
        subQuestion(pc, subIndex) !== expectedQuestion
    ) {
        throw createError(
