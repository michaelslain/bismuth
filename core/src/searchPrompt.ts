            ...
            items: {
                ...
                properties: {
                    path: {
                        type: 'string',
                        description:
                            'Exact path of a candidate note that answers the question.',
                    },
                    quote: {
                        type: 'string',
                        description:
                            "A span copied verbatim from that note's text.",
                    },
                    reason: {
                        type: 'string',
                        description:
                            'One short line on why it answers the question.',
                    },
                },
            },
        },
...
export function buildCandidateContext(
    ...
    for (const c of cands.slice(0, MAX_CANDIDATES)) {
        ...
        if (blocks.length > 0 && total + block.length > TOTAL_CONTEXT_CHARS)
            break
        blocks.push(block)
...
export function validateResults(
    ...
    const seen = new Set<string>()
    for (const r of raw) {
        if (!r || typeof r.path !== 'string' || typeof r.quote !== 'string')
            continue
    ...
    for await (const msg of iterator) {
        ...
        if (msg.subtype === 'success') {
            ...
            if (m.is_error) {
                throw new AppError(
                    'INTERNAL_ERROR',
                    `AI search failed: ${m.result || 'unknown error'}`,
                    500,
                )
            }
        ...
        throw new AppError(
            'INTERNAL_ERROR',
            `AI search did not complete (${msg.subtype})`,
            500,
        )
    }
...
const runModelReal: ModelRunner = async ({
    bin,
    root,
    question,
    context,
    ...
    try {
        ...
        throw new AppError(
            'INTERNAL_ERROR',
            `AI search failed to start: ${(e as Error).message}`,
            500,
        )
    }
    return raceWithTimeout(
        consumeModelStream(iterator),
        abort,
...
export async function promptSearch(
    ...
    if (!bin)
        throw new AppError(
            'EINVAL',
            'AI search needs Claude Code installed',
            400,
        )
    ...
    try {
        out = await searchPromptDeps.runModel({
            bin,
            root,
            question: q,
            context,
        ...
        throw new AppError(
            'INTERNAL_ERROR',
            `AI search failed: ${(e as Error).message}`,
            500,
        )
    }
    return validateResults(
        parseModelOutput(out.structured, out.resultText),
        bodiesByPath,
        q,
    )
}
