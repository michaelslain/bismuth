    ...
    switch (node.type) {
        ...
        case 'lambda': {
            ...
            const fn = (...args: unknown[]) => {
                ...
                for (let k = 0; k < params.length; k++)
                    bindings[params[k]] = args[k]
                const inner: EvalContext = {
                    ...ctx,
                    scope: { bindings, parent: parentScope },
                }
        ...
        case 'call': {
            ...
            if (callee.type === 'ident')
                return callFunction(callee.name, args, ctx)
...
function evalPlus(l: unknown, r: unknown): unknown {
    ...
    if (l instanceof Date && typeof r === 'number')
        return new Date(l.getTime() + r)
    if (r instanceof Date && typeof l === 'number')
        return new Date(r.getTime() + l)
