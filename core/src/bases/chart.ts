...
function autoX(rows: Row[], cols: string[]): string | undefined {
    for (const c of cols)
        if (fractionMatching(rows, c, v => toISODate(v) !== null) >= 0.5)
            return c
    return cols[0]
}
...
export function buildChartData(rows: Row[], view: ViewConfig): ChartData {
    ...
    const agg: Aggregate =
        (view.aggregate as Aggregate) ?? (y ? 'sum' : 'count')
    const bin: Bin = (view.bin as Bin) ?? 'day'

    const isDate = x
        ? fractionMatching(rows, x, v => toISODate(v) !== null) >= 0.5
        : false
...
export function buildHeatmapWeeks(points: ChartPoint[]): {
    ...
    while (cursor <= end || col.length > 0) {
        col.push({
            date: cursor,
            value: byDate.has(cursor) ? byDate.get(cursor)! : null,
        })
        if (col.length === 7) {
            weeks.push(col)
            col = []
