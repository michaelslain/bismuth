...
test('wikilink inside a fenced code block produces no edge', async () => {
    ...
    const { graph: g } = await buildVaultGraph(dir)
    expect(g.edges.some(e => e.from === 'index' && e.to === 'target')).toBe(
        false,
    )
})
...
test('note→tag edges are created', async () => {
    ...
    expect(tagEdges.some(e => e.from === 'note' && e.to === 'tag:foo')).toBe(
        true,
    )
    expect(tagEdges.some(e => e.from === 'note' && e.to === 'tag:bar')).toBe(
        true,
    )
})
...
test('links ignore missing targets (no broken edges)', async () => {
    ...
    expect(
        edges.every(e => e.to === 'exists' || g.nodes.some(n => n.id === e.to)),
    ).toBe(true)
})
