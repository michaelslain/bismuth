...
process.env.BISMUTH_DAEMON_BIN = join(
    tmpdir(),
...
test('GET /graph returns the merged brain graph', async () => {
    ...
    await writeNote(
        vault,
        '.daemon/memory/michael-profile.md',
...
test('GET /asset stamps a locked-down CSP on embedded .html (blocks vault exfil), not on other assets', async () => {
    ...
    writeFileSync(
        join(vault, 'viz.html'),
        "<!doctype html><body><script>document.body.textContent='hi'</script>",
    )
    const server = createServer({ vault, memory, port: 0 })
    const base = `http://localhost:${server.port}`
    try {
...
test('relay ingest routes reject missing required fields', async () => {
    ...
    try {
        expect((await post('/relay/session', { sessionId: 's1' })).status).toBe(
            400,
        ) // no terminalId
        expect(
            (await post('/relay/subagent/start', { agentId: 'a1' })).status,
        ).toBe(400) // no parentSessionId
    } finally {
        server.stop(true)
    }
})
...
test('POST /relay/session + /relay/subagent/start reach the relay registry', async () => {
    ...
    try {
        const sessionRes = await post('/relay/session', {
            sessionId: 'sess-1',
            terminalId: 'tab-1',
            cwd: '/x/my-proj',
        })
        expect(sessionRes.status).toBe(200)
        const subagentRes = await post('/relay/subagent/start', {
...
test('GET /relay/snapshot: owner gets the full snapshot, non-owner gets it redacted (no content leak)', async () => {
    ...
    try {
        await post('/relay/session', {
            sessionId: 'sess-1',
            terminalId: 'tab-1',
            cwd: '/x/my-proj',
        })
        await post('/relay/subagent/start', {
            parentSessionId: 'sess-1',
            agentId: 'ag-1',
            agentType: 'Explore',
        })
        await post('/relay/subagent/stop', {
            agentId: 'ag-1',
            lastMessage: SECRET,
        })
        ...
        const withToken = await fetch(`${base}/relay/snapshot`, {
        ...
        const nonOwnerSub = nonOwnerBody.subagents.find(
            (s: { agentId: string }) => s.agentId === 'ag-1',
        )
        expect(nonOwnerSub).toMatchObject({
            agentId: 'ag-1',
            parentSessionId: 'sess-1',
            agentType: 'Explore',
            done: true,
...
test('POST /set-properties applies batched frontmatter writes across notes in one request', async () => {
    ...
    try {
        ...
        const housing = (await (
            await fetch(`${base}/meta?path=housing.md`)
        ).json()) as Record<string, unknown>
        ...
        const ok2 = await fetch(`${base}/set-properties`, {
            ...
            body: JSON.stringify({
                writes: [
                    { path: 'nope-does-not-exist.md', key: 'x', value: 1 },
                    { path: 'essay.md', key: 'flag', value: true },
                ],
            }),
        })
        ...
        while (Date.now() - start < 2000) {
            ...
            for (const f of frames) {
                ...
                if (
                    ...
                    payload.paths.includes('.daemon/pages/reply-drafts.md')
                ) {
...
test('GET /tree returns array of { path } entries', async () => {
    ...
    try {
        ...
        expect(
            entries.every((e: any) => e.kind === 'file' || e.kind === 'dir'),
        ).toBe(true)
    } finally {
        server.stop(true)
    }
...
test('POST /folder-icon persists folderIcons into settings.yaml', async () => {
    ...
    try {
        ...
        expect(
            (res!.data.folderIcons as Record<string, unknown>).projects,
        ).toBe('Folder')
    } finally {
        server.stop(true)
    }
...
test('PUT /file with stale baseText 409s with current on-disk content and does NOT overwrite', async () => {
    ...
    try {
        ...
        const res = await fetch(`${base}/file`, {
            ...
            body: JSON.stringify({
                path: 'concurrent.md',
                contents: 'locally edited content\n',
                baseText: 'original content\n',
            }),
        })
        ...
        expect(await readNote(vault, 'concurrent.md')).toBe(
...
test('POST /create then /move then /delete then /restore round-trips a file', async () => {
    ...
    try {
        expect(
            (await post('/create', { path: 'fresh.md', kind: 'file' })).status,
        ).toBe(200)
        let paths = (await (await fetch(`${base}/tree`)).json()).map(
            (e: any) => e.path,
        )
        expect(paths).toContain('fresh.md')

        expect(
            (await post('/move', { from: 'fresh.md', to: 'renamed.md' }))
                .status,
        ).toBe(200)
        paths = (await (await fetch(`${base}/tree`)).json()).map(
            (e: any) => e.path,
        )
        expect(paths).toContain('renamed.md')
        expect(paths).not.toContain('fresh.md')

        const { trashPath } = await (
            await post('/delete', { path: 'renamed.md' })
        ).json()
        ...
        expect(
            (await post('/restore', { trashPath, to: 'renamed.md' })).status,
        ).toBe(200)
        paths = (await (await fetch(`${base}/tree`)).json()).map(
            (e: any) => e.path,
        )
...
test('GET /cards/note returns all cards for one note (tagless ok)', async () => {
    ...
    try {
        const cards = await (
            await fetch(`${base}/cards/note?path=${encodeURIComponent('n.md')}`)
        ).json()
    ...
    try {
        ...
        while (Date.now() - start < 2000) {
            ...
            for (const f of frames) {
                ...
                if (
                    Array.isArray(payload.paths) &&
                    payload.paths.includes('tracked-path.md')
                ) {
    ...
    try {
        const rows = await (
            await fetch(`${base}/rows`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    spec: { kind: 'tasks', from: '[[Keep]]' },
                }),
            })
        ).json()
        expect(rows.map((r: any) => r.note.description)).toEqual([
            'scoped task',
        ])
    } finally {
        server.stop(true)
    ...
    const resolveNotes = async () =>
        (await (
            await fetch(`${base}/rows`, {
                ...
                headers: { 'Content-Type': 'application/json' },
                body: JSON.stringify({
                    spec: { kind: 'notes', where: 'file.hasTag("book")' },
                }),
            })
        ).json()) as any[]
    try {
...
test('GET /templates lists .md files in the templates folder', async () => {
    ...
    try {
        ...
        expect(list.find((t: any) => t.name === 'Daily').path).toBe(
            'Templates/Daily.md',
        )
    } finally {
        server.stop(true)
    ...
    try {
        ...
        while (Date.now() - start < 2000) {
            ...
            for (const f of frames) {
                ...
                if (
                    Array.isArray(payload.paths) &&
                    payload.paths.includes(r1.path)
                ) {
                    sawPath = true
                    break
                }
        ...
        const readVersion = async () =>
            (await (await fetch(`${base}/version`)).json()).version
...
test('a structural mutation invalidates the cached /graph', async () => {
    ...
    try {
        ...
        await fetch(`${base}/create`, {
            ...
            headers: { 'Content-Type': 'application/json' },
            body: JSON.stringify({
                path: 'brand-new-note-mutation-test.md',
                kind: 'file',
            }),
        })
...
test('daemon routes: status + devices read shared state, owner round-trips', async () => {
    ...
    try {
        ...
        expect(devices.devices.map((d: any) => d.deviceId).sort()).toEqual([
            'dev-a',
            'dev-b',
        ])
...
test('POST /tasks/archive removes resolved tasks from a single note', async () => {
    ...
    await writeNote(
        vault,
        'todo.md',
        ['- [ ] keep', '- [x] done', '- [-] cancelled'].join('\n'),
    )
    const server = createServer({ vault, memory, port: 0 })
    const base = `http://localhost:${server.port}`
    try {
        const res = await (
            await fetch(`${base}/tasks/archive`, {
                method: 'POST',
                headers: { 'content-type': 'application/json' },
...
test("POST /search-prompt returns 400 when Claude Code isn't installed", async () => {
    ...
    try {
        const res = await fetch(`${base}/search-prompt`, {
            ...
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
                query: 'where did I write about the essay',
            }),
        })
        expect(res.status).toBe(400)
...
test('POST /search-prompt maps a stubbed model result to a byte-exact snippet', async () => {
    ...
    searchPromptDeps.runModel = async () => ({
        structured: {
            results: [
                {
                    path: 'essay.md',
                    quote: 'Religion and historical materialism.',
                    reason: "It is about the essay's subject.",
                },
            ],
        },
    ...
    try {
        const res = await fetch(`${base}/search-prompt`, {
            ...
            headers: { 'content-type': 'application/json' },
            body: JSON.stringify({
                query: 'what did I write about religion and materialism',
            }),
        })
        expect(res.status).toBe(200)
        ...
        const line =
            body.split('\n')[
                /* 1-based line */ (
                    results[0].snippets[0] as unknown as { line: number }
                ).line - 1
            ]
