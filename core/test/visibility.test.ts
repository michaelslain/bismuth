...
test("resolveVisibility: absence inherits the nearest ancestor folder's setting", () => {
    ...
    expect(resolveVisibility('notes/private/a.md', undefined, folders)).toBe(
        'hidden',
    )
})
...
test('resolveVisibility: nearest ancestor wins over a shallower one', () => {
    const folders = {
        notes: 'hidden' as const,
        'notes/private': 'chat-only' as const,
    }
    expect(resolveVisibility('notes/private/a.md', undefined, folders)).toBe(
        'chat-only',
    )
...
test("resolveVisibility: explicit file value overrides an ancestor folder's rule", () => {
    ...
    expect(resolveVisibility('notes/private/a.md', 'chat-only', folders)).toBe(
        'chat-only',
    )
})
...
test("resolveVisibility: a root-level file with no ancestors inherits 'all'", () => {
    expect(resolveVisibility('a.md', undefined, { notes: 'hidden' })).toBe(
        'all',
    )
})
...
test("buildDenyPaths: 'hidden' file denies for both channels; 'chat-only' denies only the daemon", async () => {
    ...
    expect(chatDeny.map(pair)).toEqual([
        { rel: 'secret.md', abs: join(root, 'secret.md') },
    ])
    expect(
        [...daemonDeny].sort((a, b) => a.rel.localeCompare(b.rel)).map(pair),
    ).toEqual(
        [
...
test('buildDenyPaths: folder-level rule cascades to files with no explicit visibility', async () => {
    ...
    expect(
        [...denied].sort((a, b) => a.rel.localeCompare(b.rel)).map(pair),
    ).toEqual(
        [
            { rel: 'private/a.md', abs: join(root, 'private/a.md') },
            { rel: 'private/b.md', abs: join(root, 'private/b.md') },
        ].sort((a, b) => a.rel.localeCompare(b.rel)),
    )
})
...
test('buildDenyPaths: includes .daemon memory notes (ordinary vault files under the same frontmatter path)', async () => {
    ...
    expect(denied.map(pair)).toEqual([
        {
            rel: '.daemon/memory/note.md',
            abs: join(root, '.daemon/memory/note.md'),
        },
    ])
})
...
test('absDenyPaths: pulls just the absolute form, in order', () => {
    expect(absDenyPaths(SAMPLE_ENTRIES)).toEqual([
        '/vault/secret.md',
        '/vault/private/b.md',
    ])
})
...
describe('buildSandboxDenyPaths', () => {
    ...
    test("the token path is the run record's, not a second spelling of it", () => {
        const out = buildSandboxDenyPaths(
            [{ rel: 's.md', abs: '/vault/s.md' }],
            '/vault',
        )
...
test('buildDenyPaths: a folder rule covers NON-markdown files too (badge vs deny must agree)', async () => {
    ...
    expect(
        (await buildDenyPaths(vault, 'chat')).every(e =>
            e.abs.startsWith(root),
        ),
    ).toBe(true)
})
...
test('setFolderVisibility + resolveVisibility: a trailing-slash folder key still enforces', async () => {
    ...
    expect(resolveVisibility('Private/secret.md', undefined, map)).toBe(
        'hidden',
    )
    // and the deny list actually restricts it
    expect((await buildDenyPaths(vault, 'chat')).map(e => e.rel)).toContain(
        'Private/secret.md',
    )
})
...
test('buildDenyPaths: a memory note under .daemon/memory is gated by its OWN frontmatter, never folder cascade', async () => {
    ...
    const denied = (await buildDenyPaths(vault, 'daemon'))
        .map(e => e.rel)
        .sort()
...
test('isDeniedPath does not match an unrelated path, or a mere prefix collision', () => {
    ...
    expect(
        isDeniedPath([{ rel: 'Private', abs: '/v/Private' }], 'Private2/x.md'),
    ).toBe(false)
})
...
test('buildDenyPaths: frontmatter whose closing fence lies PAST the 512-byte head is still parsed (head-read escalates to a bigger read)', async () => {
    ...
    const filler =
        ...
        Array.from(
            { length: 40 },
            (_, i) => `  line ${i} filler filler filler filler`,
        ).join('\n') +
        '\n'
...
test('buildDenyPaths: a symlinked directory is walked, so the subtree behind it is denied', async () => {
    ...
    expect(await readNote(vault, 'link-to-hidden/inside.md')).toContain(
        '# Inside',
    )
...
test('buildDenyPaths: a file under a symlinked directory denies BOTH absolute spellings, as one entry', async () => {
    ...
    expect(sandboxDenyRead(entries, vault)).toContain(
        join(canonical, 'realdir/secret.md'),
    )
    expect(sandboxDenyRead(entries, vault)).toContain(
        join(canonical, 'linkdir/secret.md'),
...
test('buildDenyPaths: a vault that is not there throws rather than returning an empty list', async () => {
    const missing = join(tmpdir(), `bismuth-absent-${Date.now()}`)
    expect(buildDenyPaths(missing, 'daemon')).rejects.toThrow(
        VisibilityUndeterminedError,
    )
})
...
test('resolveDenyPlan: a corrupt .settings is undetermined, and the folder cascade does not silently vanish', async () => {
    ...
    expect(before.determined && before.entries.map(e => e.rel).sort()).toEqual([
        'Private/secret.md',
        'real-hidden/inside.md',
    ])
...
test("buildDenyPaths: the caller's own root spelling is an alias when it differs from the canonical one", async () => {
    ...
    expect(isDeniedPath(entries, join(canonical, 'Private/secret.md'))).toBe(
        true,
    )
...
test("resolveDenyPlan: exceeding the walk's entry budget is undetermined, not a short list", async () => {
    ...
    expect(ok.determined === true && ok.entries.map(e => e.rel)).toEqual([
        'a.md',
    ])
})
