...
import { assertProcessesGone, pidAlive, waitProcessesGone } from "./acpFakeAgentProcess";
...
async function spawnSigtermIgnoring(): Promise<ReturnType<typeof Bun.spawn>> {
  const proc = Bun.spawn(["bash", "-c", 'trap "" TERM; echo ready; sleep 30'], { stdout: "pipe", stderr: "ignore" });
...
describe("waitProcessesGone / assertProcessesGone", () => {
  test("a pid that ignores SIGTERM is still reported alive, and assertProcessesGone still throws", async () => {
    ...
    try {
      ...
      await expect(assertProcessesGone([proc.pid], 300)).rejects.toThrow(/still alive/);
    } finally {
  ...
  test("waits on multiple survivors concurrently, bounded by one timeout budget rather than their sum", async () => {
    ...
    const procs = await Promise.all([spawnSigtermIgnoring(), spawnSigtermIgnoring(), spawnSigtermIgnoring()]);
    try {
