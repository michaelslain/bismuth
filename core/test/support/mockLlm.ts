...
function resolveLlmockBin(): string {
    const pkgJsonPath = Bun.resolveSync(
        '@copilotkit/aimock/package.json',
        import.meta.dir,
    )
    ...
    if (!rel)
        throw new Error(
            '@copilotkit/aimock\'s package.json has no "llmock" bin entry — package layout changed?',
        )
 ...
 */
const DENYLISTED_ARG_RE =
    /^--record$|^--provider-|^--proxy-only$|^--agui-record$|^--agui-upstream$|^--agui-proxy-only$/

export function startMockLlm(
    fixtureDir: string = DEFAULT_FIXTURE_DIR,
    ...
    return new Promise((resolve, reject) => {
        ...
        const timer = setTimeout(() => {
            finishFail(
                new Error(
                    `mock LLM server ("llmock") did not print its listening banner within ${STARTUP_TIMEOUT_MS}ms`,
                ),
            )
        }, STARTUP_TIMEOUT_MS)
        ...
        void (async () => {
            ...
            finishFail(
                new Error(
                    'mock LLM server ("llmock") exited before printing its listening banner',
                ),
            )
        })()
