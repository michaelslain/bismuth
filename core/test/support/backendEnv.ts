...
export function backendMockEnv(
    backendId: string,
    mockUrl: string,
    workDir?: string,
    ...
    switch (backendId) {
        ...
        case 'openclaw': {
                ...
                JSON.stringify(
                    {
                        ...
                        logging: {
                            file: join(stateDir, 'openclaw.log'),
                            level: 'info',
                            consoleLevel: 'info',
                        },
                        agents: {
            ...
            return {
                OPENCLAW_CONFIG_PATH: join(workDir, 'openclaw.json5'),
        ...
        default:
            throw new Error(
                `backendMockEnv: no mock-env mapping for backend id "${backendId}"`,
            )
    }
}
