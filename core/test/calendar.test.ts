...
function baseFile(
    ...
    return serializeCalendarFile(
        { type: 'base', view: 'calendar', ...extraFm },
        events,
    )
}
...
describe('add / move / delete', () => {
    ...
    test('moveEvent changes date/time and throws on unknown id', () => {
        ...
        const id = start[0].id
        const next = moveEvent(start, id, {
            date: '2026-01-06',
            startTime: '12:00',
            endTime: '13:00',
        })
        expect(findEvent(next, id)).toMatchObject({
            date: '2026-01-06',
            startTime: '12:00',
            endTime: '13:00',
        })
...
describe('queries + overlap detection', () => {
    ...
    test('eventsForRange expands across multiple days', () => {
        const events: CalendarEvent[] = [
            { id: 'r1', title: 'Daily', date: '2026-01-01', recurrence: rec() },
        ]
        expect(eventsForRange(events, '2026-01-01', '2026-01-03')).toHaveLength(
            3,
        )
    })
...
describe('per-occurrence override of a recurring event', () => {
    test('override a middle occurrence splits the series into head + tail + single', () => {
        ...
        const next = overrideOccurrence(events, 'r1', '2026-01-03', {
            startTime: '14:00',
            endTime: '15:00',
        })
    ...
    test('override the FIRST occurrence drops the head (no zombie with endDate < startDate)', () => {
        ...
        const next = overrideOccurrence(events, 'r1', '2026-01-01', {
            startTime: '14:00',
            endTime: '15:00',
        })
        ...
        const events: CalendarEvent[] = [
            {
                ...
                title: 'Daily',
                date: '2026-01-01',
                recurrence: rec({ endDate: '2026-01-05' }),
            },
        ]
    ...
    test('override/delete throw on a non-recurring event', () => {
        const events: CalendarEvent[] = [
            { id: 's1', title: 'One-off', date: '2026-01-03' },
        ]
        expect(() =>
            overrideOccurrence(events, 's1', '2026-01-03', {}),
        ).toThrow()
...
describe('raw window listing + search', () => {
    const events: CalendarEvent[] = [
        ...
        },
        {
            ...
            date: '2026-01-01',
            category: 'Work',
            recurrence: rec({ endDate: '2026-02-01' }),
        },
        {
            id: 'm2',
            title: 'Gym',
            date: '2026-02-15',
            categories: ['Health'],
            recurrence: rec({ startDate: '2026-02-15', seriesId: 's-gym' }),
        },
    ]
    ...
    test('eventsInWindow keeps singles by date and masters by series intersection', () => {
        ...
        // Open-ended bounds.
        expect(
            eventsInWindow(events, '2026-03-01')
                .map(e => e.id)
                .sort(),
        ).toEqual(['m2', 's2'])
        expect(
            eventsInWindow(events, undefined, '2026-01-31').map(e => e.id),
        ).toEqual(['m1'])
...
describe('recurrenceFromRRule (gcal RRULE subset)', () => {
    ...
    test('RRULE: prefix optional, lowercase tolerated, UNTIL → endDate, INTERVAL=2 → biweekly', () => {
        const r = recurrenceFromRRule(
            'rrule:freq=weekly;interval=2;byday=tu,th;until=20260301',
            '2026-01-06',
        )
        expect(r.type).toBe('biweekly')
        expect(r.daysOfWeek).toEqual([2, 4])
        expect(r.endDate).toBe('2026-03-01')
        expect(recurrenceFromRRule('FREQ=DAILY', '2026-01-01').type).toBe(
            'daily',
        )
        expect(recurrenceFromRRule('FREQ=MONTHLY', '2026-01-15').type).toBe(
            'monthly',
        )
    })
    ...
    test('unsupported rules throw (YEARLY, COUNT, arbitrary INTERVAL)', () => {
        ...
        expect(() =>
            recurrenceFromRRule('FREQ=DAILY;COUNT=10', '2026-01-01'),
        ).toThrow()
        expect(() =>
            recurrenceFromRRule('FREQ=DAILY;INTERVAL=3', '2026-01-01'),
        ).toThrow()
    })
    ...
    test('expandRecurrence agrees with a parsed weekly rule', () => {
        ...
        const events: CalendarEvent[] = [
            { id: 'w1', title: 'W', date: r.startDate, recurrence: r },
        ]
...
describe('category mutations', () => {
    ...
    test('addCategory appends; duplicate name throws; other frontmatter preserved', () => {
        ...
        expect(() =>
            addCategory(next, { name: 'Work', color: '#fff' }),
        ).toThrow()
        expect(() => addCategory(next, { name: '  ', color: '#fff' })).toThrow()
    })

    test('updateCategory recolors without touching events', () => {
        const { frontmatter, events } = updateCategory(fm(), evts(), 'Work', {
            color: '#00f',
        })
        expect(categoriesOf(frontmatter)).toContainEqual({
            name: 'Work',
            color: '#00f',
        })
    ...
    test('updateCategory rename cascades into category + categories and stamps localUpdated only on changed events', () => {
        ...
        expect(categoriesOf(frontmatter).map(c => c.name)).toEqual([
            'Job',
            'Home',
        ])
    ...
    test('updateCategory throws on unknown name or rename collision', () => {
        expect(() =>
            updateCategory(fm(), evts(), 'Nope', { color: '#000' }),
        ).toThrow()
        expect(() =>
            updateCategory(fm(), evts(), 'Work', { name: 'Home' }),
        ).toThrow()
    })
    ...
    test('category rename survives a serialize/parse round-trip', () => {
        ...
        expect(categoriesOf(again.frontmatter).map(c => c.name)).toEqual([
            'Job',
            'Home',
        ])
        expect(again.events.find(e => e.id === 'b')!.categories).toEqual([
            'Job',
            'Home',
        ])
