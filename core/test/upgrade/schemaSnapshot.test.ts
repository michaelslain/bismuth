...
const SNAPSHOT_PATH = join(import.meta.dir, "..", "fixtures", "upgrade", "settings-schema-snapshot.json");
...
function describeType(t: PropertyType): unknown {
  ...
  if (t.kind === "list") return { kind: "list", item: t.item ? describeType(t.item) : null };
  if (t.kind === "path") return { kind: "path", only: t.only ?? null, scope: t.scope ?? null };
...
function flatten(schema: Schema, prefix: string[] = [], out: Record<string, unknown> = {}): Record<string, unknown> {
  for (const key of Object.keys(schema).sort()) {
    const entry = schema[key]!;
    const path = [...prefix, key].join(".");
    const isObj = typeof entry.type === "object" && (entry.type as { kind?: string }).kind === "object";
    out[path] = {
    ...
    if (isObj) flatten((entry.type as { kind: "object"; fields: Schema }).fields, [...prefix, key], out);
  }
  return out;
}
...
test("every non-object schema leaf carries a default, so an upgrading user is never left with undefined", () => {
  ...
  for (const [path, meta] of Object.entries(flat)) {
    ...
    const isObject = typeof m.type === "object" && (m.type as { kind?: string }).kind === "object";
