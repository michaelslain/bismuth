...
import { pruneTmpFiles, safeTmpName, stageTmpFile, tmpFilesDir } from "../src/tmpFiles";
...
describe("safeTmpName", () => {
  test("reduces a traversal-shaped name to a single harmless segment", () => {
    expect(safeTmpName("../../.ssh/authorized_keys")).toBe("authorized_keys");
  ...
  test("drops control characters and caps the length", () => {
    ...
    expect(safeTmpName("x".repeat(500) + ".txt").length).toBeLessThanOrEqual(120);
  });
...
describe("stageTmpFile", () => {
  ...
  test("accepts an ArrayBuffer as well as a Uint8Array (the /tmp-file route hands over both)", async () => {
    ...
    const path = await stageTmpFile("b.bin", src.buffer.slice(0) as ArrayBuffer);
