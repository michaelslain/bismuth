...
const CAN_MINT_HEIC = process.platform === "darwin" && existsSync("/usr/bin/sips");
...
async function heicFixture(): Promise<Uint8Array | null> {
  ...
  try {
    const png = join(dir, "s.png"), heic = join(dir, "s.heic");
    ...
    const proc = Bun.spawn(["/usr/bin/sips", "-s", "format", "heic", png, "--out", heic], {
      stdout: "ignore", stderr: "ignore",
    });
    if ((await proc.exited) !== 0) throw new Error("sips could not mint the HEIC fixture");
...
test("POST /convert/heic rejects non-HEIC bytes with 400, not 500", async () => {
  ...
  try {
    const r = await fetch(`http://localhost:${server.port}/convert/heic`, {
      method: "POST",
      headers: { "Content-Type": "application/octet-stream" },
      body: new TextEncoder().encode("this is not an image at all") as unknown as BodyInit,
    });
...
test("POST /tmp-file stages bytes outside the vault and returns the absolute path", async () => {
  ...
  const server = createServer({ vault, memory, port: 0 });
  try {
    const r = await fetch(`http://localhost:${server.port}/tmp-file?name=${encodeURIComponent("report.pdf")}`, {
      method: "POST",
      headers: { "Content-Type": "application/octet-stream" },
      body: new TextEncoder().encode("%PDF-1.4 fake") as unknown as BodyInit,
    });
    expect(r.status).toBe(200);
    ...
    expect(new TextDecoder().decode(await readFile(path))).toBe("%PDF-1.4 fake");
  } finally {
    server.stop();
  }
});
...
test("POST /tmp-file cannot be walked out of the scratch dir by a hostile name", async () => {
  ...
  try {
    ...
    const r = await fetch(`http://localhost:${server.port}/tmp-file?name=${encodeURIComponent(hostile)}`, {
      method: "POST",
      headers: { "Content-Type": "application/octet-stream" },
    ...
    expect((await readdir(SCRATCH)).some((f) => f === "bismuth-escaped-marker.txt")).toBe(true);
  } finally {
    server.stop();
  }
});
