...
const note = (id: string, extra: Partial<GraphNode> = {}): GraphNode => ({
    id,
    label: id,
    kind: 'note',
    ...extra,
...
describe('localSubgraph', () => {
    test('depth 1 keeps the center plus its neighbours in BOTH directions', () => {
        ...
        expect(ids(localSubgraph(g, 'center'))).toEqual([
            'center',
            'in1',
            'in2',
            'out1',
    ...
    test('depth 2 reaches one hop further', () => {
        expect(ids(localSubgraph(g, 'center', 2))).toEqual([
            'center',
            'far',
            'in1',
            'in2',
    ...
    test('an isolated center is just itself', () => {
        expect(localSubgraph(g, 'island')).toEqual({
            nodes: [note('island')],
            edges: [],
        })
    })
        ...
        const clustered: GraphData = {
            nodes: [
                note('center', {
                    community: 3,
                    communityPath: [1, 2, 3],
                    communityPathLabels: ['a', 'b', 'c'],
                }),
                note('in1', {
                    community: 7,
                    communityPath: [1, 5, 7],
                    communityPathLabels: ['a', 'd', 'e'],
                }),
            ],
