...
describe("extractEditorContextPaths (captureToMemory's visibility gate)", () => {
    ...
    test('extracts open tabs (comma-separated)', () => {
        ...
        expect(extractEditorContextPaths(text)).toEqual([
            'a.md',
            'b.md',
            'private/c.md',
        ])
...
describe("sessionModelFromMessages (Bug #89: recover a resumed session's model)", () => {
    ...
    test('finds the model from a /model record (the live record shape)', () => {
        ...
        expect(
            sessionModelFromMessages([modelRecord('claude-opus-4-8[1m]')]),
    ...
    test('the NEWEST record wins when the model changed mid-session', () => {
        const messages = [
            ...
            {
                type: 'user',
                message: { role: 'user', content: 'a normal message' },
            },
        ]
    ...
    test('an empty-args newest record yields null (no usable info), not the older record', () => {
        expect(
            sessionModelFromMessages([modelRecord('haiku'), modelRecord('  ')]),
        ).toBeNull()
    })
...
describe('stripEditorContext (chat history replay)', () => {
    ...
    test('leaves an ordinary message (no preamble) untouched', () => {
        expect(stripEditorContext('just a normal question')).toBe(
...
describe('makeUserMessage (content shape)', () => {
    ...
    test('an empty images array is treated as no images → still a string', () => {
        expect(makeUserMessage('hello world', []).message.content).toBe(
            'hello world',
        )
    })

    test('images present → an ARRAY: a leading text block then one image block per attachment', () => {
        const msg = makeUserMessage('look at this', [
...
describe('unstreamedAssistantFrames (BUG #19: slash-command output de-dupe)', () => {
    ...
    test('thinking not streamed → emit it; text streamed → skip it', () => {
        ...
        expect(unstreamedAssistantFrames(msg, 14, 0)).toEqual([
    ...
    test('non-array content (a plain string) yields nothing', () => {
        expect(
            unstreamedAssistantFrames({ message: { content: 'plain' } }, 0, 0),
        ).toEqual([])
    })
})
...
describe('formatMcpStatus (BUG #39: the /mcp reply body)', () => {
    ...
    test("singular 'tool' for a count of exactly 1", () => {
        const text = formatMcpStatus([
            { name: 'solo', status: 'connected', toolCount: 1 },
        ])
    ...
    test('a tool count of 0 is still shown (an explicit zero, not omitted)', () => {
        const text = formatMcpStatus([
            { name: 'empty', status: 'connected', toolCount: 0 },
        ])
...
describe('withLocalSlashCommands (BUG #39: /mcp shows in the autocomplete)', () => {
    test("appends 'mcp' to the SDK's command list", () => {
        expect(withLocalSlashCommands(['context', 'help', 'cost'])).toEqual([
            'context',
            'help',
            'cost',
            'mcp',
...
describe('computerUseChange (BUG #87: live --chrome toggle → respawn decision)', () => {
    test('off → on: respawn, next=true', () => {
        expect(computerUseChange(false, true)).toEqual({
            next: true,
            respawn: true,
        })
    })
    test('on → off: respawn, next=false', () => {
        expect(computerUseChange(true, false)).toEqual({
            next: false,
            respawn: true,
        })
    })
    test('unchanged on → on: no respawn', () => {
        expect(computerUseChange(true, true)).toEqual({
            next: true,
            respawn: false,
        })
    })
    test('unchanged off → off: no respawn', () => {
        expect(computerUseChange(false, false)).toEqual({
            next: false,
            respawn: false,
        })
    })
    test('undefined (never-set) current is treated as off: undefined→true respawns', () => {
        expect(computerUseChange(undefined, true)).toEqual({
            next: true,
            respawn: true,
        })
    })
    test('undefined current vs false request: same off state, no spurious respawn', () => {
        expect(computerUseChange(undefined, false)).toEqual({
            next: false,
            respawn: false,
        })
    })
    test('undefined request is treated as off (a client that omits the flag never enables it)', () => {
        expect(computerUseChange(true, undefined)).toEqual({
            next: false,
            respawn: true,
        })
        expect(computerUseChange(false, undefined)).toEqual({
            next: false,
            respawn: false,
        })
    })
...
describe("buildChatSandboxOption (the value spawnChatQuery's sandbox option is built from)", () => {
    test('allowUnsandboxedCommands is false when the vault restricts something', () => {
        const o = buildChatSandboxOption(
            [{ rel: 'secret.md', abs: '/vault/secret.md' }],
            '/vault',
        )
    ...
    test('denyRead covers the owner-token run record — the file whose contents defeat every other filter', () => {
        ...
        const denyRead =
            (o?.filesystem as { denyRead?: string[] } | undefined)?.denyRead ??
            []
...
if (!LIVE) {
    ...
    console.warn(
        '[chat.test] live Claude tests skipped — set BISMUTH_LIVE_TESTS=1 to run them (makes real API calls against your Anthropic account).',
    )
}
...
describeOrSkip('visual Claude Code chat driver (live)', () => {
    ...
    test('permission ALLOW: approving Write creates the file', async () => {
        ...
        sendMessage(
            chatId,
            'Create a file named t.txt containing the text hi. Do nothing else.',
            cwd,
            sink,
        )
    ...
    test('BUG #60: bypassPermissions suppresses the permission prompt (canUseTool never fires)', async () => {
        ...
        sendMessage(
            chatId,
            'Create a file named b.txt containing the text hi. Do nothing else.',
            cwd,
            sink,
        )
    ...
    test("visibility: a note marked 'hidden' is never read by chat, even when directly asked", async () => {
        ...
        const leaked = frames.some(f =>
            JSON.stringify(f).includes('SECRET-CODE-CHAT-TEST-7731'),
        )
    ...
    test('listChatSessions returns an array; sessionHistoryFrames replays a real session', async () => {
        ...
        try {
            ...
            if (sessions.length > 0) {
                const frames = await sessionHistoryFrames(
            ...
            const none = await sessionHistoryFrames(
...
describe('chatSnippet (content-search excerpt)', () => {
    test('returns a snippet around the first case-insensitive match, with clip markers', () => {
        const text =
            'the quick brown fox jumps over the lazy dog and keeps running along the river bank'
...
describe('matchChatSession (content-search filter)', () => {
    const doc = (over: Partial<ChatSearchDoc> = {}): ChatSearchDoc => ({
        ...
        lastModified: 1000,
        texts: [
            'We should move the login flow to OAuth',
            'Then wire the token refresh',
        ],
        origin: 'user',
        ...over,
    })
...
describe('buildAskUserQuestionAnswer (host answer → canUseTool PermissionResult)', () => {
    ...
    test("null answers (user skipped) → allow UNCHANGED (tool emits its own 'no answer'; turn continues)", () => {
        expect(buildAskUserQuestionAnswer(toolInput, null)).toEqual({
            behavior: 'allow',
