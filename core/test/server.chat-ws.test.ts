...
interface ChatSocket {
  ...
  waitFor(pred: (f: Frame) => boolean, what: string, timeoutMs?: number): Promise<Frame>;
...
async function openChatWs(base: string, chatId: string, opts: { rebind?: boolean } = {}): Promise<ChatSocket> {
  ...
  await new Promise<void>((resolve, reject) => {
    ws.onopen = () => resolve();
    ws.onerror = () => reject(new Error(`chat ws failed to open for chatId=${chatId}`));
  });
  ...
  return {
    ...
    waitFor: (pred, what, timeoutMs = 15_000) =>
      new Promise<Frame>((resolve, reject) => {
        ...
        const t = setTimeout(
          () => reject(new Error(`timeout waiting for ${what}; frames seen: ${JSON.stringify(frames.map((f) => f.type))}`)),
          timeoutMs,
        );
        waiters.push({
          pred,
...
describe("the /chat websocket handler (fake ACP agent — no CLI, no model, no network)", () => {
  ...
  afterEach(() => {
    ...
    if (savedGrace === undefined) delete process.env.BISMUTH_CHAT_GRACE_MS;
  ...
  async function boot(): Promise<{ base: string; wsBase: string }> {
    ...
    servers.push(server);
    return { base: `http://localhost:${server.port}`, wsBase: `ws://localhost:${server.port}` };
  }
   ...
   *  transcript. */
  async function openLiveChat(wsBase: string, chatId: string): Promise<ChatSocket> {
    chatIds.push(chatId);
    const sock = await openChatWs(wsBase, chatId);
    sock.send({ type: "open", provider: "cline", computerUse: false });
    const sessionFrame = await sock.waitFor((f) => f.type === "session", "the session frame from the fake ACP agent");
  ...
  test(
    "a STALE socket's close after a reconnect must NOT tear down the live session (the identity-guard regression)",
    async () => {
      ...
      await ws2.waitFor((f) => f.type === "done", "the rebind's synthetic done frame on ws2");
      ...
      ws2.send({ type: "user", text: "still there?", provider: "cline", computerUse: false });
      const assistant = await ws2.waitFor((f) => f.type === "assistant-text", "the turn's assistant-text on ws2");
      ...
      expect(types.lastIndexOf("done")).toBeGreaterThan(types.indexOf("assistant-text"));
  ...
  test(
    "a CLEAN close (1000) tears the chat session down immediately — it does not take the grace path",
    async () => {
  ...
  test(
    "an ABNORMAL close arms the grace teardown; reconnecting with rebind=1 after it fired gets the explicit exit frame",
    async () => {
      ...
      const ws2 = await openChatWs(wsBase, chatId, { rebind: true });
      const err = await ws2.waitFor((f) => f.type === "error", "the reconnect-too-late error frame");
  ...
  test(
    "message verbs dispatch over the socket: {type:'set_model'} reaches the agent's wire method",
    async () => {
      ...
      await waitForCondition(
        () => readEchoLines(echoFile).some((l) => l.method === "session/new"),
        10_000,
      ...
      await waitForCondition(
        () => readEchoLines(echoFile).some((l) => l.method === "session/set_model"),
        10_000,
      ...
      ws.send({ type: "user", text: "hi", provider: "cline", computerUse: false });
      const assistant = await ws.waitFor((f) => f.type === "assistant-text", "the assistant-text after set_model");
      expect(assistant.text).toBe(FAKE_TURN_TEXT);
