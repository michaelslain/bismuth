...
test("rename-tab: happy path + invalid tabId round-trip through the registry", async () => {
  const frames: any[] = [];
  registerWindow("w1", (f) => frames.push(f));
  const ok = sendCommand("w1", "rename-tab", { tabId: "t1", name: "Renamed" }, 1000);
  expect(frames).toHaveLength(1);
  expect(frames[0]).toMatchObject({ type: "command", action: "rename-tab", args: { tabId: "t1", name: "Renamed" } });
  resolveReply(frames[0].reqId, { ok: true });
  expect(await ok).toEqual({ ok: true });

  const bad = sendCommand("w1", "rename-tab", { tabId: "bogus", name: "X" }, 1000);
...
test("pin-tab: happy path + invalid tabId round-trip through the registry", async () => {
  ...
  expect(frames[0]).toMatchObject({ type: "command", action: "pin-tab", args: { tabId: "t1", pinned: true } });
  resolveReply(frames[0].reqId, { ok: true });
  expect(await ok).toEqual({ ok: true });

  const bad = sendCommand("w1", "pin-tab", { tabId: "bogus", pinned: false }, 1000);
  resolveReply(frames[1].reqId, { ok: false, error: 'no tab "bogus"' });
  const reply = await bad;
  expect(reply.ok).toBe(false);
  expect(reply.error).toContain("bogus");
...
test("reorder-tab: happy path + invalid tabId round-trip through the registry", async () => {
  ...
  expect(frames[0]).toMatchObject({ type: "command", action: "reorder-tab", args: { tabId: "t1", index: 2 } });
  resolveReply(frames[0].reqId, { ok: true });
  expect(await ok).toEqual({ ok: true });

  const bad = sendCommand("w1", "reorder-tab", { tabId: "bogus", index: 0 }, 1000);
  resolveReply(frames[1].reqId, { ok: false, error: 'no tab "bogus"' });
  const reply = await bad;
  expect(reply.ok).toBe(false);
  expect(reply.error).toContain("bogus");
