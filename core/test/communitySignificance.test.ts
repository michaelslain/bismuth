...
import { modularity, significance, nullModelModularity, nullModelForGraph } from "../src/communitySignificance";
...
function twoCliquesOfSize(k: number): { adj: number[][]; comm: number[] } {
  ...
  for (const base of [0, k]) {
    for (let i = base; i < base + k; i++) for (let j = i + 1; j < base + k; j++) link(i, j);
  }
  link(0, k); // the bridge
  return { adj, comm: Array.from({ length: 2 * k }, (_, i) => (i < k ? 0 : 1)) };
}
...
test("I3: nullModelModularity is invariant to comm — DO NOT wire comm back in", () => {
  ...
  expect(nullModelModularity(adj, a, 5)).toEqual(nullModelModularity(adj, b, 5));
});
