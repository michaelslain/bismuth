...
describe('replaceInText', () => {
    test('replaces all literal occurrences and counts them', () => {
        const r = replaceInText('brown fox brown', 'brown', 'red', {
            caseSensitive: false,
            wholeWord: false,
            regex: false,
        })
    ...
    test('respects whole-word', () => {
        const r = replaceInText('brownish brown', 'brown', 'red', {
            caseSensitive: false,
            wholeWord: true,
            regex: false,
        })
    ...
    test('regex with capture groups', () => {
        const r = replaceInText('2026-05-31', '(\\d{4})-(\\d{2})', '$2/$1', {
            caseSensitive: false,
            wholeWord: false,
            regex: true,
        })
    ...
    test('no matches is a no-op with count 0', () => {
        expect(
            replaceInText('abc', 'zzz', 'q', {
                caseSensitive: false,
                wholeWord: false,
                regex: false,
            }),
        ).toEqual({ text: 'abc', count: 0 })
    })
})
