...
test('preserves first node when ids are duplicated', () => {
    const g1: GraphData = {
        nodes: [{ id: 'x', label: 'First', kind: 'note' }],
        edges: [],
    }
    const g2: GraphData = {
        nodes: [{ id: 'x', label: 'Second', kind: 'note' }],
        edges: [],
    }
    const merged = mergeGraphs([g1, g2])
...
test('handles different node kinds in merge', () => {
    ...
    const g2: GraphData = {
        nodes: [{ id: 'b', label: 'B', kind: 'memory' }],
        edges: [],
    }
    const merged = mergeGraphs([g1, g2])
...
test('preserves node properties (folder, state) in merge', () => {
    const g1: GraphData = {
        nodes: [{ id: 'a', label: 'A', kind: 'note', folder: 'project' }],
        edges: [],
    }
...
test('handles self nodes correctly', () => {
    const g1: GraphData = {
        nodes: [{ id: 'self', label: 'You', kind: 'self' }],
        edges: [],
    }
    const g2: GraphData = {
        nodes: [{ id: 'note', label: 'Note', kind: 'note' }],
        edges: [],
    }
    const merged = mergeGraphs([g1, g2])
