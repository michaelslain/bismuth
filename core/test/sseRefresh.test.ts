...
async function waitForFrame(
  base: string,
  match: (payload: { version: number; paths: string[]; dirty?: { graph: boolean; tree: boolean } }) => boolean,
  trigger: () => Promise<void> | void,
  timeoutMs = 4000,
): Promise<{ version: number; paths: string[]; dirty?: { graph: boolean; tree: boolean } }> {
  ...
  try {
    ...
    throw new Error(`no matching SSE frame within ${timeoutMs}ms; trailing buf=${buf}`);
  } finally {
...
test("a new NOTE on disk marks the tree dirty and lands in GET /tree", async () => {
  ...
  try {
    ...
    const frame = await waitForFrame(
      base,
      (p) => p.dirty?.tree === true,
      () => { writeFileSync(join(vault, "BrandNewNote.md"), "# brand new\n"); },
    );
    expect(frame.dirty?.tree).toBe(true);
    ...
    const frame = await waitForFrame(
      ...
      async () => {
        await fetch(`${base}/create`, {
          method: "POST",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ path: "ViaApi.md", kind: "file" }),
        });
      },
    );
    expect(frame.dirty?.tree).toBe(true);
    ...
    const frame = await waitForFrame(
      ...
      async () => {
        await fetch(`${base}/file`, {
          method: "PUT",
          headers: { "Content-Type": "application/json" },
          body: JSON.stringify({ path: "EditMe.md", contents: "# EditMe\n\njust prose, no links or tags\n" }),
        });
      },
    );
