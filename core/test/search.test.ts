...
describe('findMatches', () => {
    test('finds a literal match with line number and split context', () => {
        ...
        const m = findMatches(body, 'search', {
            caseSensitive: false,
            wholeWord: false,
            regex: false,
        })
        expect(m).toEqual([
            { line: 2, before: 'hello ', match: 'search', after: ' world' },
        ])
    })
    ...
    test('regex mode honors patterns', () => {
        const body = 'a1 b2 c3'
        const m = findMatches(body, '[a-z]\\d', {
            caseSensitive: false,
            wholeWord: false,
            regex: true,
        })
...
describe('searchVault', () => {
    test('ranks a filename match above a body-only match', async () => {
        ...
        const res = await searchVault(root, 'search', {
            caseSensitive: false,
            wholeWord: false,
            regex: false,
        })
    ...
    test('regex mode ranks by match count', async () => {
        ...
        const res = await searchVault(root, '[a-z]\\d', {
            caseSensitive: false,
            wholeWord: false,
            regex: true,
        })
...
describe('rankCandidates (AI prompt-search stage 1)', () => {
    ...
    test('respects the limit', async () => {
        ...
        for (let i = 0; i < 10; i++)
            files[`n${i}.md`] =
                `note ${i} about project alpha and search topics`
...
describe('updateSearchIndex (incremental)', () => {
    ...
    test('adds a newly-created note', async () => {
        ...
        expect(
            (await searchVault(root, 'delta', simple)).map(r => r.path),
        ).toContain('c.md')
        invalidateSearchIndex(root)
    })
