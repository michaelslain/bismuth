...
test('not done / done filter on status', () => {
    const tasks = [
        task({ status: 'todo', description: 'a' }),
        task({ status: 'done', description: 'b' }),
    ]
    expect(
        runTaskQuery(tasks, 'not done', TODAY).tasks.map(t => t.description),
    ).toEqual(['a'])
    expect(
        runTaskQuery(tasks, 'done', TODAY).tasks.map(t => t.description),
    ).toEqual(['b'])
})
...
test('not done excludes cancelled tasks (cancelled is closed, not actionable)', () => {
    ...
    expect(
        runTaskQuery(tasks, 'not done', TODAY)
...
test('is cancelled / is not cancelled', () => {
    const tasks = [
        task({ status: 'cancelled', description: 'c' }),
        task({ status: 'todo', description: 't' }),
    ]
    expect(
        runTaskQuery(tasks, 'is cancelled', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['c'])
    expect(
        runTaskQuery(tasks, 'is not cancelled', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['t'])
})
...
test('done today filters by done date', () => {
    ...
    expect(
        runTaskQuery(tasks, 'done today', TODAY).tasks.map(t => t.description),
    ).toEqual(['today'])
})

test('is recurring / is not recurring', () => {
    const tasks = [
        task({ recurrence: 'every day', description: 'r' }),
        task({ description: 'n' }),
    ]
    expect(
        runTaskQuery(tasks, 'is recurring', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['r'])
    expect(
        runTaskQuery(tasks, 'is not recurring', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['n'])
})
...
test('priority is / is not', () => {
    const tasks = [
        ...
        task({ priority: 'low', description: 'l' }),
    ]
    expect(
        runTaskQuery(tasks, 'priority is high', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['h'])
    expect(
        runTaskQuery(tasks, 'priority is not high', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['l'])
})
...
test('date comparisons: before/after/on and undated excluded', () => {
    ...
    expect(
        runTaskQuery(tasks, 'due before today', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['past'])
    expect(
        runTaskQuery(tasks, 'due today', TODAY).tasks.map(t => t.description),
    ).toEqual(['today'])
    expect(
        runTaskQuery(tasks, 'due after today', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['future'])
})
...
test('relative date expr: due before in 7 days', () => {
    ...
    expect(
        runTaskQuery(tasks, 'due before in 7 days', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['within'])
})
...
test('day-of-week expr: due friday (TODAY is Wed 2026-05-27 → coming Fri 2026-05-29)', () => {
    ...
    expect(
        runTaskQuery(tasks, 'due friday', TODAY).tasks.map(t => t.description),
    ).toEqual(['fri'])
    expect(
        runTaskQuery(tasks, 'due next monday', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual(['mon'])
    expect(
        runTaskQuery(tasks, 'due before friday', TODAY).tasks.map(
            t => t.description,
        ),
    ).toEqual([])
})
...
test('sort by due puts undated last', () => {
    ...
    expect(
        runTaskQuery(tasks, 'sort by due', TODAY).tasks.map(t => t.description),
    ).toEqual(['dated', 'none'])
})
