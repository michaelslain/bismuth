...
test("SETTINGS_SCHEMA nests the app sections, calendar, ui, server, folderIcons and properties", () => {
  expect(Object.keys(SETTINGS_SCHEMA).sort()).toEqual(
    ["appearance", "attachments", "calendar", "daemon", "dailyNotes", "editor", "folderIcons", "googleCalendar", "graph", "keybindings", "properties", "server", "srs", "templates", "terminal", "toolbar", "ui", "update", "vault"].sort(),
  );
});
