...
describe('toolbar setting', () => {
    ...
    it('is a list of objects with command/commands/icon/tooltip fields', () => {
        ...
        expect(Object.keys(t.item.fields).sort()).toEqual([
            'command',
            'commands',
            'icon',
            'tooltip',
    ...
    it('accepts a valid toolbar item and rejects an unknown command id', () => {
        ...
        expect(
            validateValue(t, [{ command: 'terminal', icon: 'SquareTerminal' }]),
        ).toBeNull()
        const bad = validateValue(t, [
            { command: 'no-such-command', icon: 'X' },
        ])
    ...
    it('accepts a multi-command button and rejects an unknown id inside `commands`', () => {
        ...
        expect(
            validateValue(t, [
                { commands: ['new-note', 'terminal'], icon: 'Rocket' },
            ]),
        ).toBeNull()
        const bad = validateValue(t, [
            { commands: ['new-note', 'no-such-command'], icon: 'X' },
        ])
        expect(bad).not.toBeNull()
        expect(bad!.severity).toBe('error')
    })
    ...
    it('warns (does not error) when a toolbar item sets both `command` and `commands`', () => {
        const diags = validateDocument(
            {
                toolbar: [
                    { command: 'new-note', commands: ['terminal'], icon: 'X' },
                ],
            },
            SETTINGS_SCHEMA,
            { mode: 'settings' },
        )
        const conflict = diags.find(
            d =>
                d.severity === 'warning' &&
                /command/.test(d.message) &&
                /commands/.test(d.message),
        )
