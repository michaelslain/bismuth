...
const base = { folder: vault, memory, serverEntry: "/nonexistent/server.ts", waitMs: 2000 };

/** A child that dies immediately — stands in for "another process took the port". */
const deadChild = (pid: number) => ({ pid, kill: () => {}, exited: Promise.resolve(1) });
/** A child that lives. */
const liveChild = (pid: number) => ({ pid, kill: () => {}, exited: new Promise<number>(() => {}) });
  ...
  it("gives up after `attempts` children and reports the exit", async () => {
    ...
    await expect(
      spawnVaultBackend({
        ...base,
        attempts: 2,
        spawn: () => { n++; return deadChild(300 + n); },
        probe: async () => false,
      }),
  ...
  it("does not retry when the child is alive but merely slow — that is a real timeout", async () => {
    ...
    await expect(
      spawnVaultBackend({
        ...base,
        waitMs: 400,
        spawn: () => { n++; return liveChild(400 + n); },
        probe: async () => false,
      }),
