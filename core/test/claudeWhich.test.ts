...
const realUsername = Bun.spawnSync(['/usr/bin/id', '-un'])
    .stdout.toString()
    .trim()
...
const bin = (root: string, v: string) =>
    join(root, 'versions', 'node', v, 'bin')

describe('nvmBinPaths', () => {
    test('returns nothing when NVM_DIR is absent', () => {
        expect(
            nvmBinPaths({ NVM_DIR: join(tmpdir(), 'nvm-missing-xyz-123') }),
        ).toEqual([])
    })
...
describe('claudeLookupPath', () => {
    test("keeps the user's PATH first, then appends homebrew/local and nvm bins", () => {
        ...
        expect(out.indexOf('/usr/bin')).toBeLessThan(
            out.indexOf('/opt/homebrew/bin'),
        )
        expect(out).toContain(bin(dir, 'v20.11.0'))
    })
