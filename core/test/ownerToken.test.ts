...
test('T-H1: GET /file — a hidden note 403s without the token and never leaks its body; 200 with the token; a visible note stays 200 either way', async () => {
    ...
    try {
        ...
        expect(await noTokenHidden.text()).not.toContain(
            'TOKEN-OWNERTOKEN-SECRET-1',
        )

        const withTokenHidden = await fetch(`${base}/file?path=secret.md`, {
            headers: { 'X-Bismuth-Token': token },
        })
        expect(withTokenHidden.status).toBe(200)
        expect(await withTokenHidden.text()).toContain(
            'TOKEN-OWNERTOKEN-SECRET-1',
        )
...
test("T-H2: POST /search omits a hidden note's hits without the token, includes them with it", async () => {
    ...
    const search = async (
        ...
        (await fetch(`${base}/search`, {
            ...
            body: JSON.stringify({
                query: 'TOKEN-SEARCH-ONLY-IN-HIDDEN',
...
test("T-H3: POST /rows omits a hidden note's row without the token, includes it with it", async () => {
    ...
    const rows = async (
        headers?: Record<string, string>,
    ): Promise<Array<{ file: { path: string } }>> =>
    ...
    try {
        ...
        expect(withToken.map(r => r.file.path).sort()).toEqual([
            'public.md',
            'secret.md',
        ])
    } finally {
...
test("GET /graph drops a hidden note's node (and every edge touching it) without the token", async () => {
    ...
    try {
        const token = tokenFor(vault)
        const noToken = (await (await fetch(`${base}/graph`)).json()) as {
        ...
        expect(
            noToken.edges.some(e => e.from === 'secret' || e.to === 'secret'),
        ).toBe(false)
...
test("GET /tasks omits a hidden note's task line without the token", async () => {
    ...
    try {
        const token = tokenFor(vault)
        const noToken = (await (await fetch(`${base}/tasks`)).json()) as Array<{
            path: string
        }>
        ...
        expect(withToken.map(t => t.path).sort()).toEqual([
            'public.md',
            'secret.md',
        ])
    } finally {
...
test('GET /base and GET /meta 403 a hidden note without the token, 200 with it', async () => {
    ...
    try {
        ...
        expect(
            (
                await fetch(`${base}/base?file=secret.md`, {
                    headers: { 'X-Bismuth-Token': token },
                })
            ).status,
        ).toBe(200)
        ...
        const withToken = await fetch(`${base}/meta?path=secret.md`, {
            headers: { 'X-Bismuth-Token': token },
        })
...
test('GET /abs-path 403s a hidden note without the token, 200 with it', async () => {
    ...
    try {
        const token = tokenFor(vault)
        expect((await fetch(`${base}/abs-path?path=secret.md`)).status).toBe(
            403,
        )
        expect(
            (
                await fetch(`${base}/abs-path?path=secret.md`, {
                    headers: { 'X-Bismuth-Token': token },
                })
            ).status,
        ).toBe(200)
...
test('GET /cards/note 403s a hidden note without the token, 200 with it', async () => {
    ...
    try {
        const token = tokenFor(vault)
        expect((await fetch(`${base}/cards/note?path=secret.md`)).status).toBe(
            403,
        )
        expect(
            (
                await fetch(`${base}/cards/note?path=secret.md`, {
                    headers: { 'X-Bismuth-Token': token },
                })
            ).status,
        ).toBe(200)
...
test('T-H4: X-Bismuth-Channel absent defaults to the daemon (stricter) filter, not chat', async () => {
    ...
    try {
        ...
        const chatHeader = await fetch(`${base}/file?path=chatOnly.md`, {
            headers: { 'X-Bismuth-Channel': 'chat' },
        })
        expect(chatHeader.status).toBe(200)
        expect(await chatHeader.text()).toContain(
            'TOKEN-CHATONLY-VISIBLE-TO-CHAT',
        )
    } finally {
        server.stop(true)
...
test('GET /chat/sessions, GET /chat/session-messages, POST /chat/search are owner-only (blanket, not per-path)', async () => {
    ...
    try {
        ...
        expect(
            (await fetch(`${base}/chat/session-messages?id=nonexistent`))
                .status,
        ).toBe(403)
        expect(
            (
...
function claudeShapedProfile(denyRead: string[]): string {
    ...
    for (const p of denyRead)
        lines.push(
            '(deny file-read*',
            `  (subpath ${JSON.stringify(p)})`,
            '  (with message "visibility"))',
        )
    return `${lines.join('\n')}\n`
}

function readUnderProfile(
    profile: string,
    file: string,
...
test.skipIf(!CAN_SANDBOX)(
    ...
    async () => {
        ...
        try {
            ...
            const denyRead =
                (
                    buildChatSandboxOption(entries, vault)?.filesystem as
                        { denyRead?: string[] } | undefined
                )?.denyRead ?? []

            const attack = readUnderProfile(
                claudeShapedProfile(denyRead),
                record,
            )
            ...
            const visible = readUnderProfile(
                claudeShapedProfile(denyRead),
                join(vault, 'public.md'),
            )
...
test('ownerTokenDenyPaths collapses to the single path when no link is involved', () => {
    ...
    try {
        expect(ownerTokenDenyPaths('/some/vault')).toEqual([
            join(dir, `${runKey('/some/vault')}.json`),
        ])
    } finally {
        if (saved === undefined) delete process.env.BISMUTH_RUN_DIR
        else process.env.BISMUTH_RUN_DIR = saved
...
test("the daemon's ported copy honours BISMUTH_RUN_DIR the same way core's does — including the canonical-spelling expansion", () => {
    ...
    try {
        process.env.BISMUTH_RUN_DIR = link
        expect(daemonOwnerTokenDenyPaths('/Users/x/vault')).toEqual(
            ownerTokenDenyPaths('/Users/x/vault'),
        )
