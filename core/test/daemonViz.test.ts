...
test('disabled → dim base node, NO border (regardless of running / recency)', () => {
    ...
    expect(
        nodeVisualState(state({ enabled: false, running: true }), NOW),
    ).toEqual({
        fill: 'base',
        border: 'none',
        opacity: 0.15,
    ...
    expect(
        nodeVisualState(
            state({ enabled: false, lastResult: 'success', lastFiredMs: NOW }),
            NOW,
        ),
    ).toEqual({ fill: 'base', border: 'none', opacity: 0.15 })
})

test('enabled, not running → bg-fill (hollow) node + a crisp palette border ring', () => {
...
test('lastResult / lastFiredMs are ignored — they no longer drive the encoding', () => {
    ...
    const fresh = nodeVisualState(
        state({ lastResult: 'failed', lastFiredMs: NOW - 1000 }),
        NOW,
    )
    const stale = nodeVisualState(
        state({
            lastResult: 'success',
            lastFiredMs: NOW - 99 * 60 * 60 * 1000,
        }),
        NOW,
    )
    const never = nodeVisualState(
        state({ lastResult: null, lastFiredMs: null }),
        NOW,
    )
    const expected: DaemonVisual = { fill: 'bg', border: 'palette', opacity: 1 }
...
test('now is optional / unused — same result with or without it', () => {
    expect(nodeVisualState(state({ running: true }))).toEqual(
        nodeVisualState(state({ running: true }), NOW),
    )
})
