...
import {
    startCaptureLlmServer,
    type CaptureLlmHandle,
} from '../support/captureLlmServer'
import { startMockLlm, type MockLlmHandle } from '../support/mockLlm'
import {
    getFreePort,
    startOpenclawGateway,
    type OpenclawGatewayHandle,
...
const describeOrSkip =
    HAS_OPENCLAW && shouldRunSlowTests(process.env) ? describe : describe.skip

if (!HAS_OPENCLAW) {
    // eslint-disable-next-line no-console
    console.warn(
        '[openclawMocked.test] skipped — the `openclaw` CLI is not installed on this machine (nothing to drive).',
    )
}
...
function chatCompletionsHitCount(metricsText: string): number {
    ...
    for (const line of metricsText.split('\n')) {
        if (
            ...
            !line.includes('path="/v1/chat/completions"')
        )
            continue
...
describeOrSkip(
    'the real openclaw CLI, driven through the ACP driver, against a real Gateway against a mock LLM (zero account API calls)',
    () => {
        ...
        afterEach(async () => {
            for (const id of chatIds.splice(0))
                CHAT_BACKENDS.openclaw.closeChat(id)
            ...
            if (gatewayPid !== undefined && pidAlive(gatewayPid)) {
                throw new Error(
                    `openclawMocked.test: gateway pid ${gatewayPid} still alive after gateway.stop() resolved — a real leak.`,
                )
            }
            if (mockPid !== undefined && pidAlive(mockPid)) {
                throw new Error(
                    `openclawMocked.test: mock pid ${mockPid} still alive after mock.stop() resolved — a real leak.`,
                )
            }
            ...
            while (Date.now() < deadline) {
                remaining = collectDescendantPids(process.pid).filter(pid =>
                    /openclaw/i.test(describePid(pid)),
                )
        ...
        test("a turn sent through CHAT_BACKENDS.openclaw returns the fixture's exact text, then terminates with result + done, proven by a path-specific mock hit", async () => {
            await setup()
            ...
            const before = chatCompletionsHitCount(
                await fetch(`${mock!.url}/metrics`).then(r => r.text()),
            )
            expect(before).toBe(0) // sanity: nothing hit the model endpoint before this turn

            CHAT_BACKENDS.openclaw.sendMessage({
                chatId,
                cwd,
                sink,
                computerUse: false,
            ...
            // reported success.
            const after = chatCompletionsHitCount(
                await fetch(`${mock!.url}/metrics`).then(r => r.text()),
            )
            expect(after).toBeGreaterThan(before)
        }, 60_000)

        test("session isolation: a brand-new second chat's upstream request carries NO content from a completed first chat (closes the cross-chat leak a fixed session key would reopen)", async () => {
