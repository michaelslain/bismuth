...
describeOrSkipSlow(
    "Bismuth's ACP driver against a fake agent reproducing cline's REAL auth gate (zero network, zero cline binary needed)",
    () => {
        let stubDir: string
        ...
        test('gate CLOSED: the real -32000 auth-required refusal surfaces as a clean error ChatFrame, never a hang, never any assistant-text or result frame', async () => {
        ...
        test('gate OPEN (FAKE_ACP_CLINE_AUTHED=1, mirroring the real CLINE_API_KEY bypass): a full turn completes — the fixture sentinel arrives as assistant-text, then result.isError===false, then done, and no error frame ever appears', async () => {
            ...
            if (sessionFrame.type === 'session')
                expect(sessionFrame.sessionId).toStartWith('fake-session-')
