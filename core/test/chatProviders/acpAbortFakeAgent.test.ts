...
describeOrSkipSlow(
    'abortTurn and a never-settling turn, driven through the ACP driver against a fake agent that holds the turn open (zero network, zero CLI dependency)',
    () => {
        ...
        afterEach(async () => {
            ...
            if (stillAlive.length > 0) {
                throw new Error(
                    `acpAbortFakeAgent.test: fake-agent pid(s) ${stillAlive.join(', ')} still alive 5s after closeChat — a real leak.`,
                )
            }
        }, 15_000)
        ...
        test('abortTurn mid-permission-hold: the turn settles cleanly (isError:false, not an error), session/cancel actually went out, and a stale late reply produces zero further frames', async () => {
            ...
            const cancels = readEchoLines(echoFile).filter(
                l => l.method === 'session/cancel',
            )
            ...
            const lateReplies = readEchoLines(echoFile).filter(
        ...
        test('a session/prompt that never settles leaves the chat turnActive indefinitely; closeChat still tears the fake agent process down, with no orphan left behind', async () => {
            ...
            const { permissionIdx, frames, pid } =
                await driveToParkedPermission(chatId)
            ...
            expect(CHAT_BACKENDS.cline.rebindSink(chatId, probe.sink)).toBe(
                true,
            )
            ...
            await waitForCondition(
                () => !pidAlive(pid),
                5_000,
                `pid ${pid} (the fake agent process) to exit after closeChat`,
            )
