...
describeOrSkipSlow(
    'crash mid-turn, malformed output, and chunk-boundary framing, driven through the ACP driver against a fake agent (zero network, zero CLI dependency)',
    () => {
        ...
        afterEach(async () => {
            ...
            if (stillAlive.length > 0) {
                throw new Error(
                    `acpFramingFakeAgent.test: fake-agent pid(s) ${stillAlive.join(', ')} still alive after closeChat — a real leak.`,
                )
            }
        }, 15_000)
        ...
        test('agent process crashes right after its first session/update: exactly one result{isError:true}, done, error{code:exit} — no duplicate terminal frames', async () => {
            ...
            const errors = frames.filter(
                (f): f is Extract<ChatFrame, { type: 'error' }> =>
                    f.type === 'error',
            )
        ...
        test('plain-text and near-JSON-RPC noise interleaved with real session/update frames: noise yields ZERO frames, both real frames arrive in order, turn still reaches done', async () => {
            ...
            expect(
                frames.some(f => JSON.stringify(f).includes(NOISE_LEAK_MARKER)),
            ).toBe(false)
        ...
        test('one large session/update split into raw byte fragments, one cut INSIDE a 4-byte emoji: reassembles byte-exact, emoji intact', async () => {
            ...
            const assistantTextFrames = frames.filter(
                (f): f is Extract<ChatFrame, { type: 'assistant-text' }> =>
                    f.type === 'assistant-text',
            )
