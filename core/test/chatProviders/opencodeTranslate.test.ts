...
function ev(type: string, part: Record<string, unknown>): unknown {
    return {
        type,
        timestamp: 1783830998470,
        sessionID: SID,
        part: { sessionID: SID, messageID: 'msg_1', ...part },
    }
}
...
describe('translateOpencodeEvent', () => {
    test('text part → one assistant-text frame; session id captured', () => {
        ...
        const frames = translateOpencodeEvent(
            ev('text', { id: 'prt_1', type: 'text', text: 'ok' }),
            state,
        )
    ...
    test('repeated part id with growing text emits only the suffix (streaming-tolerant)', () => {
        ...
        expect(
            translateOpencodeEvent(
                ev('text', { id: 'p', type: 'text', text: 'Hel' }),
                state,
            ),
        ).toEqual([{ type: 'assistant-text', text: 'Hel' }])
        expect(
            translateOpencodeEvent(
                ev('text', { id: 'p', type: 'text', text: 'Hello' }),
                state,
            ),
        ).toEqual([{ type: 'assistant-text', text: 'lo' }])
    ...
    test('independent parts each emit their full text', () => {
        const state = newOpencodeTurnState()
        translateOpencodeEvent(
            ev('text', { id: 'p1', type: 'text', text: 'one' }),
            state,
        )
        expect(
            translateOpencodeEvent(
                ev('text', { id: 'p2', type: 'text', text: 'two' }),
                state,
            ),
        ).toEqual([{ type: 'assistant-text', text: 'two' }])
    })
    ...
    test('reasoning part → thinking frame', () => {
        ...
        expect(
            translateOpencodeEvent(
                ev('reasoning', { id: 'r1', type: 'reasoning', text: 'hmm' }),
                state,
            ),
    ...
    test('pending tool_use emits only the chip; the later completed event resolves it once', () => {
        ...
        expect(
            ...
            { type: 'tool-result', id: 'c', content: 'hi', isError: false },
        ])
    ...
    test('error event → error frame; malformed events never throw', () => {
        ...
        ).toEqual([{ type: 'error', code: 'error', message: 'rate limited' }])
        ...
        const frames = translateOpencodeEvent(
            {
                ...
                error: {
                    name: 'APIError',
                    data: {
                        message: 'Unauthorized: no payment method',
                        statusCode: 401,
                        isRetryable: false,
                    },
                },
...
describe('opencodeErrorMessage', () => {
    test('shallowest message wins; error.name backs a message-less error; generic bottom', () => {
        ...
        expect(
            opencodeErrorMessage({ error: { data: { message: 'deep' } } }),
        ).toBe('deep')
        expect(
            opencodeErrorMessage({ error: { name: 'APIError', data: {} } }),
        ).toBe('opencode reported an error (APIError)')
        expect(opencodeErrorMessage({})).toBe('opencode reported an error')
        expect(opencodeErrorMessage({ error: 'string-error' })).toBe(
...
describe('translateOpencodeExport', () => {
    ...
    test('tolerates malformed exports', () => {
        expect(translateOpencodeExport(null)).toEqual({
            title: null,
            frames: [],
        })
        expect(translateOpencodeExport({ info: {} })).toEqual({
            title: null,
            frames: [],
        })
        expect(
            translateOpencodeExport({ messages: [{ info: { role: 'user' } }] })
                .frames,
        ).toEqual([])
    })
})
...
describe('parseOpencodeModelsVerbose', () => {
    ...
    const block = (id: string, meta: Record<string, unknown>) =>
    ...
    const fixture = [
        ...
        block('opencode/claude-sonnet-4-6', {
            id: 'claude-sonnet-4-6',
            providerID: 'opencode',
            name: 'Claude Sonnet 4.6',
            cost: { input: 3, output: 15 },
        }),
        block('opencode/kimi-k2.5', {
            id: 'kimi-k2.5',
            providerID: 'opencode',
            name: 'Kimi K2.5',
            cost: { input: 0.6, output: 2.5 },
        }),
        block('moonshotai/kimi-k2.5', {
            id: 'kimi-k2.5',
    ...
    test('classifies free vs paid off cost metadata and uses display names as labels', () => {
        ...
        expect(
            out.find(m => m.value === 'opencode/claude-sonnet-4-6')?.free,
        ).toBe(false)
        expect(
            out.find(m => m.value === 'opencode/claude-sonnet-4-6')?.label,
        ).toBe('Claude Sonnet 4.6')
    })

    test('disambiguates display-name collisions across providers', () => {
        const out = parseOpencodeModelsVerbose(fixture)
        expect(out.find(m => m.value === 'opencode/kimi-k2.5')?.label).toBe(
            'Kimi K2.5 (opencode)',
        )
        expect(out.find(m => m.value === 'moonshotai/kimi-k2.5')?.label).toBe(
            'Kimi K2.5 (moonshotai)',
        )
    })

    test('a malformed metadata block degrades to the bare id with NO badge (free undefined)', () => {
        const broken = parseOpencodeModelsVerbose(fixture).find(
            m => m.value === 'broken/model',
        )
    ...
    test('returns [] on output with no model ids (caller falls back to the plain list)', () => {
        ...
        expect(
            parseOpencodeModelsVerbose('usage: opencode models [provider]\n'),
        ).toEqual([])
    })

    test('drops duplicate ids, preserves order', () => {
        const out = parseOpencodeModelsVerbose(
            fixture + block('opencode/big-pickle', { name: 'Dup' }),
        )
        expect(out.filter(m => m.value === 'opencode/big-pickle')).toHaveLength(
            1,
        )
...
const DEBUG_CONFIG = JSON.stringify(
    {
        ...
        command: {
            interview: {
                ...
                description: 'Open a localhost interview UI',
            },
            deepwork: {
            ...
            preset: {
                ...
                description: 'Switch agent presets at runtime',
            },
        },
        lsp: true,
    },
...
describe('parseOpencodeDebugConfigCommands', () => {
    ...
    test('tolerates banner noise around the JSON', () => {
        const cmds = parseOpencodeDebugConfigCommands(
            `some log line\n${DEBUG_CONFIG}`,
        )
    ...
    test("skips command names that can't be typed as one /token", () => {
        expect(
            parseOpencodeDebugConfigCommands(
                '{"command": {"has space": {"template": "x"}, "ok": {"template": "y"}}}',
            ),
        ).toEqual([{ name: 'ok', description: 'y' }])
    })
})

describe('withOpencodeBuiltinCommands', () => {
    test("appends /init and /review after the user's own commands", () => {
        const merged = withOpencodeBuiltinCommands([
            { name: 'deepwork', description: 'd' },
        ])
    ...
    test("a same-named custom command overrides the built-in (opencode's documented precedence)", () => {
        const merged = withOpencodeBuiltinCommands([
            { name: 'init', description: 'my own init' },
        ])
        expect(merged).toEqual([
            { name: 'init', description: 'my own init' },
            OPENCODE_BUILTIN_COMMANDS[1],
        ])
    })

    test('bare list = just the built-ins (a config-less install still offers /init + /review)', () => {
        expect(withOpencodeBuiltinCommands([]).map(c => c.name)).toEqual([
            'init',
            'review',
        ])
    })
...
describe('parseOpencodeRunCommand', () => {
    ...
    test('a leading /known-command splits into command + args', () => {
        expect(
            parseOpencodeRunCommand('/init focus on the CLI', NAMES),
        ).toEqual({ command: 'init', args: 'focus on the CLI' })
        expect(parseOpencodeRunCommand('/review', NAMES)).toEqual({
            command: 'review',
            args: '',
        })
        expect(
            parseOpencodeRunCommand('  /deepwork  refactor the parser ', NAMES),
        ).toEqual({ command: 'deepwork', args: 'refactor the parser' })
    })
...
describe('parseOpencodeAuthList', () => {
    ...
    test('no credentials / garbage / empty → []', () => {
        expect(
            parseOpencodeAuthList('┌  Credentials\n└  0 credentials\n'),
        ).toEqual([])
...
describe('Zen free-model rotation', () => {
    const MODELS = [
        ...
        },
        {
            ...
            description: 'opencode/claude-sonnet-4-6',
            effortLevels: [],
            free: false,
        },
        {
    ...
    test('zenFreeModelIds: only $0 Zen (opencode/…) models, order preserved', () => {
        ...
        expect(zenFreeModelIds(MODELS)).toEqual([
            'opencode/big-pickle',
            'opencode/hy3-free',
        ])
    })
...
function messageUpdated(id: string, role: 'user' | 'assistant'): unknown {
    return {
        ...
        properties: { sessionID: SES, info: { id, sessionID: SES, role } },
    }
}
function partUpdated(part: Record<string, unknown>): unknown {
    return {
        type: 'message.part.updated',
        properties: { sessionID: SES, part: { sessionID: SES, ...part } },
    }
}
function partDelta(messageID: string, partID: string, delta: string): unknown {
    return {
        type: 'message.part.delta',
        properties: { sessionID: SES, messageID, partID, field: 'text', delta },
    }
}
...
describe('translateOpencodeServerEvent', () => {
    ...
    test("real token-level deltas stream as assistant-text, keyed off the part's kind learned from its creation event", () => {
        const state = newOpencodeServerTurnState()
        translateOpencodeServerEvent(
            messageUpdated('msg_a1', 'assistant'),
            state,
        )
        ...
        expect(
            translateOpencodeServerEvent(
                partDelta('msg_a1', 'prt_r1', 'The'),
                state,
            ),
        ).toEqual([{ type: 'thinking', text: 'The' }])
        expect(
            translateOpencodeServerEvent(
                partDelta('msg_a1', 'prt_r1', ' user'),
                state,
            ),
        ).toEqual([{ type: 'thinking', text: ' user' }])
        translateOpencodeServerEvent(
            partUpdated({
                id: 'prt_t1',
                messageID: 'msg_a1',
        ...
        expect(
            translateOpencodeServerEvent(
                partDelta('msg_a1', 'prt_t1', 'Hi'),
                state,
            ),
        ).toEqual([{ type: 'assistant-text', text: 'Hi' }])
    })
    ...
    test('a part that never streamed deltas at all still replays in full off its ONE settled snapshot', () => {
        ...
        ).toEqual([{ type: 'assistant-text', text: 'whole thing' }])
    })
    ...
    test('step-start/step-finish/session/permission events carry no frame here (permission is a separate function)', () => {
        const state = newOpencodeServerTurnState()
        expect(
            translateOpencodeServerEvent({ type: 'step-start' }, state),
        ).toEqual([])
        expect(
            translateOpencodeServerEvent(
...
describe('translateOpencodeSessionMessages', () => {
    ...
    test('tolerates malformed input', () => {
        ...
        expect(
            translateOpencodeSessionMessages([{ info: { role: 'user' } }]),
        ).toEqual([])
    })
})
...
describe('parseOpencodePermissionAsk', () => {
    ...
    test('falls back to patterns when metadata is empty', () => {
        ...
        expect(parseOpencodePermissionAsk(ev)).toEqual({
            id: 'per_1',
            toolName: 'webfetch',
            input: { pattern: ['https://example.com'] },
        })
    })
...
describe('splitOpencodeModelId', () => {
    test('splits at the FIRST slash', () => {
        expect(splitOpencodeModelId('opencode/big-pickle')).toEqual({
            providerID: 'opencode',
            modelID: 'big-pickle',
        })
        expect(splitOpencodeModelId('moonshotai/kimi-k2.5')).toEqual({
            providerID: 'moonshotai',
            modelID: 'kimi-k2.5',
        })
    })
...
describe('modelEntriesFromProviders', () => {
    ...
    test('tolerates malformed/missing providers or models', () => {
        ...
        expect(
            modelEntriesFromProviders([
                null as never,
                { id: 'opencode', models: { a: { id: 'a' } } },
            ]),
        ).toEqual([
            {
                value: 'opencode/a',
                label: 'opencode/a',
                description: 'opencode/a',
                effortLevels: [],
            },
        ])
    })
...
describe('buildOpencodePromptParts', () => {
    test('text only', () => {
        expect(buildOpencodePromptParts('hello')).toEqual([
    ...
    test('text + image rides a FilePartInput with a data: URL — verified live (200, model read real pixels)', () => {
        expect(
            buildOpencodePromptParts('what color?', [
...
describe("reconcileOpencodeFinalParts (server mode's lost-reply backfill)", () => {
    ...
    test('a fully-streamed turn reconciles to NOTHING — text is never printed twice', () => {
        ...
        translateOpencodeServerEvent(
            {
                type: 'message.part.updated',
                properties: { part: textPart('p1', 'Hello!') },
            },
            state,
        )
        expect(
            reconcileOpencodeFinalParts([textPart('p1', 'Hello!')], state),
        ).toEqual([])
    })
    ...
    test('a PARTIALLY streamed turn emits only the missing suffix', () => {
        ...
        translateOpencodeServerEvent(
            {
                type: 'message.part.updated',
                properties: { part: textPart('p1', 'Hel') },
            },
            state,
        )
        expect(
    ...
    test('it is idempotent — reconciling the same final parts twice cannot double-print', () => {
        const state = newOpencodeServerTurnState()
        expect(
            reconcileOpencodeFinalParts([textPart('p1', 'Hi')], state),
        ).toHaveLength(1)
        expect(
            reconcileOpencodeFinalParts([textPart('p1', 'Hi')], state),
        ).toEqual([])
    ...
    test('junk in, nothing out — a missing/!array/garbage payload never throws', () => {
        ...
        expect(
            reconcileOpencodeFinalParts([null, 7, {}, { type: 'text' }], state),
        ).toEqual([])
    })
})
