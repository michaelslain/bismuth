...
const describeOrSkip =
    HAS_CLINE && shouldRunSlowTests(process.env) ? describe : describe.skip

if (!HAS_CLINE) {
    // eslint-disable-next-line no-console
    console.warn(
        '[clineMocked.test] skipped — the `cline` CLI is not installed on this machine (nothing to drive).',
    )
}

describeOrSkip(
    "the real cline CLI's ACP mode, driven through the ACP driver — proves the account-auth wall fails SAFELY (see header)",
    () => {
        ...
        test('an isolated, never-authenticated CLINE_DIR makes session/new fail with a clean auth-required error, never a hang or a real account fallback', async () => {
...
// --------------------------------------------------------------------------------------------
describeOrSkip(
    "the real cline CLI's ACP mode, driven through the ACP driver, against a local mock LLM — a full turn completes end to end (see this file's header UPDATE note)",
    () => {
        ...
        test("a real cline binary, pointed at a local mock via the CLINE_API_KEY bypass, completes a full turn: the mock fixture's exact text arrives, its /v1/chat/completions counter increases, then result.isError===false", async () => {
