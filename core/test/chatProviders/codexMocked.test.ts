...
const describeOrSkip =
    HAS_CODEX && shouldRunSlowTests(process.env) ? describe : describe.skip

if (!HAS_CODEX) {
    // eslint-disable-next-line no-console
    console.warn(
        '[codexMocked.test] skipped — the `codex` CLI is not installed on this machine (nothing to drive).',
    )
}

describeOrSkip(
    'the real codex CLI, driven through chatProviders/codex/driver.ts, against a mock LLM (zero account API calls)',
    () => {
        const ENV_KEYS = [
            'CODEX_HOME',
            'OPENAI_BASE_URL',
        ...
        afterEach(async () => {
            for (const id of chatIds.splice(0))
                CHAT_BACKENDS.codex.closeChat(id)
            for (const dir of tempDirs.splice(0)) {
                await rm(dir, { recursive: true, force: true }).catch(() => {})
            }
        })

        test("a turn sent through CHAT_BACKENDS.codex returns the fixture's exact text, then terminates with done", async () => {
            await setup()
            ...
            CHAT_BACKENDS.codex.sendMessage({
                chatId,
                cwd,
                sink,
                computerUse: false,
