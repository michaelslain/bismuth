...
describe("live tool-use (tool_call -> tool_call_update), driven through the ACP driver against a fake agent (zero network, zero CLI dependency)", () => {
  ...
  beforeEach(() => {
    ...
    stubDir = makeAcpFakeAgentStubDir("bismuth-acp-tooluse-stub-", "cline", FAKE_AGENT_SCRIPT, pidFile);
  ...
  afterEach(async () => {
    ...
    if (stillAlive.length > 0) {
      throw new Error(`acpToolUseFakeAgent.test: fake-agent pid(s) ${stillAlive.join(", ")} still alive after closeChat — a real leak.`);
    }
  }, 15_000);
  ...
  test(
    "tool_call -> tool_call_update yields tool-use{id,name,kind} then tool-result{id,isError:false} with equal ids, tool-result strictly after tool-use, exactly one tool-result for that id, and result after both",
    async () => {
      ...
      const toolUseFrames = frames.filter((f): f is Extract<ChatFrame, { type: "tool-use" }> => f.type === "tool-use");
      const toolResultFrames = frames.filter((f): f is Extract<ChatFrame, { type: "tool-result" }> => f.type === "tool-result");
  ...
  test(
    "tool-call mode unset (the fake's default, unchanged): an ordinary turn produces NO tool-use/tool-result frames at all",
    async () => {
