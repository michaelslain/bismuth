...
const describeOrSkip =
    HAS_GOOSE && shouldRunSlowTests(process.env) ? describe : describe.skip

if (!HAS_GOOSE) {
    // eslint-disable-next-line no-console
    console.warn(
        '[gooseMocked.test] skipped — the `goose` CLI is not installed on this machine (nothing to drive).',
    )
}
 ...
 */
const HAS_BISMUTH_MCP = existsSync(
    join(homedir(), '.bismuth', 'bin', 'bismuth-mcp'),
)
...
describeOrSkip(
    'the real goose CLI, driven through the ACP driver, against a mock LLM (zero account API calls)',
    () => {
        const ENV_KEYS = [
            'ANTHROPIC_HOST',
            'ANTHROPIC_API_KEY',
        ...
        async function setup(): Promise<void> {
            ...
            process.env.XDG_CONFIG_HOME = await newTempDir(
                'bismuth-goose-xdgcfg-',
            )
            process.env.XDG_DATA_HOME = await newTempDir(
                'bismuth-goose-xdgdata-',
            )
            process.env.XDG_STATE_HOME = await newTempDir(
                'bismuth-goose-xdgstate-',
            )
        }
        ...
        afterEach(async () => {
            for (const id of chatIds.splice(0))
                CHAT_BACKENDS.goose.closeChat(id)
            for (const dir of tempDirs.splice(0)) {
                await rm(dir, { recursive: true, force: true }).catch(() => {})
            }
        })

        test("a turn sent through CHAT_BACKENDS.goose returns the fixture's exact text, then terminates with result + done", async () => {
            await setup()
            ...
            CHAT_BACKENDS.goose.sendMessage({
                chatId,
                cwd,
                sink,
                computerUse: false,
        ...
        test.if(HAS_BISMUTH_MCP)(
            ...
            async () => {
                ...
                const assistantTexts = frames
                    ...
                    .map(f => (f.type === 'assistant-text' ? f.text : ''))
                expect(assistantTexts.join('')).toBe(
                    'Thanks, got the docs list.',
                )
            },
            60_000,
        )
