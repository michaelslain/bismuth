...
describe('parseJsonRpcLine', () => {
    ...
    test('blank lines, non-JSON noise, and non-2.0 envelopes all yield null', () => {
        ...
        expect(
            parseJsonRpcLine('{"jsonrpc":"1.0","id":1,"method":"x"}'),
        ).toBeNull()
...
describe('encode helpers', () => {
    test('encode a request/notification/result/error as one NDJSON line', () => {
        expect(
            encodeJsonRpcRequest(1, 'initialize', { protocolVersion: 1 }),
        ).toBe(
            '{"jsonrpc":"2.0","id":1,"method":"initialize","params":{"protocolVersion":1}}\n',
        )
        expect(
            encodeJsonRpcNotification('session/cancel', { sessionId: 's1' }),
        ).toBe(
            '{"jsonrpc":"2.0","method":"session/cancel","params":{"sessionId":"s1"}}\n',
        )
        expect(
            encodeJsonRpcResult('req-1', { outcome: { outcome: 'cancelled' } }),
        ).toBe(
    ...
    test('a request/notification with no params omits the field entirely', () => {
        expect(encodeJsonRpcRequest(1, 'authenticate')).toBe(
...
describe('isMethodNotFoundError', () => {
    test('the reserved JSON-RPC code wins over any text', () => {
        expect(isMethodNotFoundError(new AcpRpcError(-32601, 'whatever'))).toBe(
            true,
        )
        expect(
            isMethodNotFoundError(new AcpRpcError(-32000, 'method not found')),
        ).toBe(true) // text sniff still fires
    })
    test("a textual sniff catches agents that don't use the reserved code", () => {
        expect(
            isMethodNotFoundError(new Error('Unknown method: session/resume')),
        ).toBe(true)
...
describe('detectModelShape', () => {
            ...
            configOptions: [
                {
                    ...
                    name: 'Provider',
                    category: 'model',
                    currentValue: 'openai-compatible',
                    options: [
            ...
            configOptions: [
                {
                    id: 'cfg-model',
                    category: 'model',
                    options: [{ id: 'legacy-m1', name: 'Legacy Model One' }],
                },
            ],
        })
        expect(shape.shape).toBe('new')
    ...
    test('NONE: neither shape present, or a malformed/absent result — never throws', () => {
        ...
        expect(detectModelShape({ configOptions: 'not an array' }).shape).toBe(
            'none',
        )
        expect(
            detectModelShape({ models: { availableModels: 'nope' } }).models,
        ).toEqual([])
    })
...
describe('contentBlockText', () => {
    test("text rides through; non-text kinds get a bracketed placeholder; unknown/malformed → ''", () => {
        ...
        ).toBe('[spec.md](file:///spec.md)')
        expect(
            contentBlockText({ type: 'resource_link', uri: 'file:///x' }),
        ).toBe('[resource](file:///x)')
        expect(contentBlockText({ type: 'resource' })).toBe(
            '[embedded resource]',
        )
...
describe('toolCallContentText', () => {
    test('renders content entries and diff summaries; falls back to raw JSON for anything else', () => {
        ...
        expect(toolCallContentText([{ type: 'future_kind', x: 1 }])).toBe(
...
describe('toolCallInput', () => {
    test("carries title as description (summarizeInput's recognized key) and kind", () => {
        expect(
            toolCallInput({ title: 'Reading src/index.ts', kind: 'read' }),
        ).toEqual({
    ...
    test("rawInput's structured arguments ride through alongside the synthesized fields", () => {
        expect(
            toolCallInput({
                ...
                kind: 'read',
                rawInput: { file_path: '/etc/hosts', limit: 40 },
            }),
        ).toEqual({
            file_path: '/etc/hosts',
            limit: 40,
    ...
    test("on a name collision the ToolCall's OWN title/kind win over identically-named rawInput keys", () => {
        ...
        expect(
            toolCallInput({
                ...
                rawInput: {
                    description:
                        'a tool argument that happens to be named description',
                    kind: 'not-a-toolkind',
                    path: '/tmp/foo.txt',
                },
            }),
        ).toEqual({
            description: 'Write foo.txt',
            kind: 'edit',
            path: '/tmp/foo.txt',
        })
    })
    ...
    test('rawInput alone (no title, no kind) still produces an input rather than undefined', () => {
        expect(toolCallInput({ rawInput: { query: 'gcal' } })).toEqual({
            query: 'gcal',
        })
    })
    ...
    test('an EMPTY rawInput contributes nothing, and on its own yields no empty-object chip', () => {
        ...
        expect(
            toolCallInput({
                title: 'bismuth: bismuth docs list',
                rawInput: {},
            }),
        ).toEqual({ description: 'bismuth: bismuth docs list' })
    ...
    test('a rawInput that is not a plain object is ignored rather than spread key-by-key', () => {
        expect(toolCallInput({ title: 'T', rawInput: 'garbage' })).toEqual({
            description: 'T',
        })
        expect(toolCallInput({ title: 'T', rawInput: ['a', 'b'] })).toEqual({
            description: 'T',
        })
        expect(toolCallInput({ title: 'T', rawInput: null })).toEqual({
            description: 'T',
        })
    })
})
...
describe('toolCallName', () => {
    ...
    test('prefers `title` — the only human-readable field a real ToolCall carries — over `kind`', () => {
        expect(toolCallName({ title: 'Write foo.txt', kind: 'edit' })).toBe(
            'Write foo.txt',
        )
    })
...
describe('translateSessionUpdate', () => {
    test('agent_message_chunk / agent_thought_chunk → assistant-text / thinking; empty text → []', () => {
        ...
        ).toEqual([{ type: 'assistant-text', text: 'Hi there' }])
        expect(
            translateSessionUpdate(
                {
    ...
    test('tool_call (pending) emits only the chip; a LATER tool_call_update resolves it once', () => {
        ...
        expect(
        ...
        ).toEqual([
            {
                type: 'tool-use',
                id: 'tc_1',
                name: 'Reading src/index.ts',
                kind: 'read',
                input: { description: 'Reading src/index.ts', kind: 'read' },
            },
        ])
        expect(
            translateSessionUpdate(
        ...
        expect(frames).toEqual([
            {
                ...
                name: 'Search',
                kind: 'search',
                input: { description: 'Search', kind: 'search' },
            },
            {
                type: 'tool-result',
                id: 'tc_2',
            ...
            translateSessionUpdate(
                {
                    ...
                    kind: 'search',
                    status: 'in_progress',
                    rawInput: { query: 'gcal', kind: 'not-a-toolkind' },
                },
                state,
            ),
        ).toEqual([
            ...
            translateSessionUpdate(
                {
                    ...
                    content: [
                        {
                            type: 'content',
                            content: { type: 'text', text: '2 failing' },
                        },
                    ],
                },
                state,
        ...
        expect(frames).toEqual([
            {
                ...
                manifest: {
                    ...
                    commandDetails: { review: 'Review changes' },
                },
            },
        ])
    })
    ...
    test('usage_update tracks used/size as a context frame and accumulates cost across calls', () => {
        ...
        expect(
            translateSessionUpdate(
                { sessionUpdate: 'usage_update', used: 500, size: 2000 },
                state,
            ),
        ).toEqual([
            {
    ...
    test('known-but-UI-inert kinds (plans, mode/config echoes, session metadata, user echo) yield []', () => {
        ...
        for (const kind of [
            ...
            expect(
                translateSessionUpdate({ sessionUpdate: kind }, state),
            ).toEqual([])
        }
    })
    ...
    test('an unknown FUTURE sessionUpdate kind yields [] rather than throwing', () => {
        ...
        expect(
            translateSessionUpdate(
                { sessionUpdate: 'something_from_a_newer_sdk' },
                state,
            ),
        ).toEqual([])
    })
    ...
    test('malformed input (null / non-object / missing sessionUpdate) is tolerated', () => {
        ...
        expect(
            translateSessionUpdate(
                { sessionUpdate: 'tool_call', toolCallId: 42 },
                state,
            ),
        ).toEqual([]) // non-string id
    })
})
...
describe('choosePermissionOption', () => {
    ...
    test('exact allow/deny × once/always matches win', () => {
        expect(choosePermissionOption(options, { behavior: 'allow' })).toBe(
            'opt-allow-once',
        )
        expect(
            choosePermissionOption(options, {
                behavior: 'allow',
                always: true,
            }),
        ).toBe('opt-allow-always')
        expect(choosePermissionOption(options, { behavior: 'deny' })).toBe(
            'opt-reject-once',
        )
        expect(
            choosePermissionOption(options, { behavior: 'deny', always: true }),
        ).toBe('opt-reject-always')
    })

    test('falls back to the right family when the exact once/always variant is missing', () => {
        const onlyAlways = [
            { kind: 'allow_always', optionId: 'a', name: 'Allow always' },
        ]
        expect(choosePermissionOption(onlyAlways, { behavior: 'allow' })).toBe(
            'a',
        ) // wanted allow_once, got allow family
    })
    ...
    test('zero usable options → null (the driver then answers cancelled)', () => {
        ...
        expect(
            choosePermissionOption([{ kind: 'allow_once' }], {
                behavior: 'allow',
            }),
        ).toBeNull() // no optionId — unusable
    })
})
