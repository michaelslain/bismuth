...
if (!HAS_CLAUDE) {
    ...
    console.warn(
        '[claudeMocked.test] skipped — the `claude` CLI is not installed on this machine (nothing to drive).',
    )
}

describeOrSkip(
    "the real claude CLI, driven through chat.ts's sendMessage, against a mock LLM (zero account API calls)",
    () => {
        ...
        test("a turn sent through sendMessage() returns the fixture's exact text in an assistant-text frame, then terminates with result + done", async () => {
            await setup()
            ...
            if (claudeConfigDir)
                expect(readdirSync(claudeConfigDir).length).toBeGreaterThan(0)
        }, 60_000)
        ...
        test("sendMessage()'s reopen branch (reattachSessionSink) flushes a buffered turn to the NEW sink without an extra synthetic done", async () => {
            ...
            const chatId = newChatId()
            chatIds.push(chatId)
            const { sink: sink1, frames: frames1 } =
