...
describe('agent spawn envs carry the channel', () => {
    test('claudeSpawnEnv stamps the channel when asked, and omits it when not', () => {
        ...
        expect(claudeSpawnEnv({}, 'daemon').BISMUTH_AGENT_CHANNEL).toBe(
            'daemon',
        )
    ...
    test('every chat driver passes a channel to claudeSpawnEnv', () => {
        ...
        for (const rel of drivers) {
            ...
            for (const m of src.matchAll(/claudeSpawnEnv\(([^)]*)\)/g)) {
                const args = m[1] ?? ''
                // Either quote style. Pinning this to " made the whole scan report every driver as
                // an offender the moment the repo was formatted to single quotes — a structural
                // guard that fails on formatting is one people learn to silence.
                if (!/['"](chat|daemon)['"]/.test(args)) {
