...
describeOrSkipSlow(
    "resumeSession's session/load -> session/resume fallback, driven through the ACP driver against a fake agent (zero network, zero CLI dependency)",
    () => {
        ...
        beforeEach(() => {
            ...
            echoDir = undefined
            pidDir = undefined
            pidDir = tempDir('bismuth-acp-resume-pid-')
            pidFile = join(pidDir, 'agent.pid')
            stubDir = makeAcpFakeAgentStubDir(
                'bismuth-acp-resume-stub-',
                'cline',
                FAKE_AGENT_SCRIPT,
                pidFile,
            )
            echoDir = tempDir('bismuth-acp-resume-echo-')
        ...
        afterEach(async () => {
            ...
            if (stillAlive.length > 0) {
                throw new Error(
                    `acpResumeFakeAgent.test: fake-agent pid(s) ${stillAlive.join(', ')} still alive after closeChat — a real leak.`,
                )
            }
        }, 15_000)
        ...
        async function openResumedSession(
            ...
            CHAT_BACKENDS.cline.resumeSession({
                chatId,
                cwd,
                sink,
                computerUse: false,
        ...
        test("session/load rejected with -32601: the driver falls back to session/resume, both carrying the caller's session id, and the next turn completes on it", async () => {
            ...
            const gotSessionId = await openResumedSession(
                chatId,
                RESUME_SESSION_ID,
                cwd,
                waitFor,
            ...
            expect(methodOrder.indexOf('session/resume')).toBeGreaterThan(
                methodOrder.indexOf('session/load'),
            )
        ...
        test("session/load accepted (FAKE_ACP_REJECT_SESSION_LOAD unset — the fake's default, unchanged): the driver never attempts session/resume at all", async () => {
            ...
            const loads = readEchoLines(echoFile).filter(
                l => l.method === 'session/load',
            )
            const resumes = readEchoLines(echoFile).filter(
                l => l.method === 'session/resume',
            )
            expect(loads.length).toBe(1)
            expect(paramsSessionId(loads[0].params)).toBe(RESUME_SESSION_ID)
        ...
        test('pins the CODE arm of isMethodNotFoundError: the correct -32601 code with a NON-matching message still triggers the fallback', async () => {
            ...
            process.env.FAKE_ACP_REJECT_SESSION_LOAD_MESSAGE =
                'zzz this text carries none of the fallback trigger keywords zzz'
            ...
            const allLines = readEchoLines(echoFile)
            expect(
                allLines.filter(l => l.method === 'session/load').length,
            ).toBe(1)
            // THE assertion this test exists for: the fallback fired despite a non-matching message.
            expect(
                allLines.filter(l => l.method === 'session/resume').length,
            ).toBe(1)
        }, 20_000)

        test('pins the MESSAGE arm of isMethodNotFoundError: a non-matching code with a message that DOES match still triggers the fallback', async () => {
