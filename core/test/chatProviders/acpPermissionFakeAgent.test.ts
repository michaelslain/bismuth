...
describeOrSkipSlow("the ACP driver's permission round-trip against a fake agent that holds the turn open (zero network, zero CLI dependency)", () => {
  ...
  async function awaitReplyResult(permissionId: string): Promise<unknown> {
    ...
    const replies = readEchoLines(echoFile).filter((l) => l.dir === "in-response" && String(l.id) === permissionId);
  ...
  test(
    'allow: the driver replies {outcome:{outcome:"selected", optionId:<the allow_once option>}} on the correlating rpc id, and only then does the turn settle',
    async () => {
      ...
      expect(await awaitReplyResult(permissionId)).toEqual({ outcome: { outcome: "selected", optionId: PERM_ALLOW_ONCE_ID } });
      ...
      expect(readEchoLines(echoFile).filter((l) => l.dir === "in-response" && String(l.id) === permissionId).length).toBe(1);
    },
    20_000,
  );

  test(
    'deny: the driver replies with the agent\'s reject_once option on the correlating rpc id, and only then does the turn settle',
    async () => {
  ...
  test(
    'cancel: when the agent offers no selectable options, the driver replies {outcome:{outcome:"cancelled"}} — carrying no optionId — and the turn still settles',
    async () => {
