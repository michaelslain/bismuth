...
import { convertHeicToJpeg, isHeicName, jpegNameFor, looksLikeHeic } from "../src/heic";
...
function tinyPng(): Uint8Array {
  ...
  const stored = [0x01, raw.length & 255, (raw.length >> 8) & 255, ~raw.length & 255, (~raw.length >> 8) & 255, ...raw];
  let a = 1, b = 0;
  ...
  return new Uint8Array([
    ...
    ...chunk("IHDR", ihdr), ...chunk("IDAT", zlib), ...chunk("IEND", []),
  ]);
}
...
async function makeHeicFixture(): Promise<Uint8Array | null> {
  ...
  try {
    ...
    if (exit !== 0) throw new Error(`sips could not mint the HEIC fixture (exit ${exit})`);
...
describe("isHeicName", () => {
  ...
  test("rejects other images and extension-less names", () => {
    for (const n of ["photo.jpg", "photo.png", "heic", "photo.heic.txt", "a/b.heicx"]) {
...
describe("convertHeicToJpeg", () => {
  ...
  test("the portable (non-sips) engine decodes the same HEIC", async () => {
    ...
    const { default: convert } = (await import("heic-convert")) as unknown as {
      default: (o: { buffer: Uint8Array; format: "JPEG"; quality?: number }) => Promise<Uint8Array>;
    };
    const jpeg = new Uint8Array(await convert({ buffer: heic!, format: "JPEG", quality: 0.92 }));
    expect([jpeg[0], jpeg[1]]).toEqual([0xff, 0xd8]);
    expect(jpeg.byteLength).toBeGreaterThan(100);
  });
