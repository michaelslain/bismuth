...
import { mkdtempSync, mkdirSync, writeFileSync, readFileSync, rmSync, existsSync, lstatSync, readlinkSync, symlinkSync } from "node:fs";
...
function fakeIO(opts: {
  ...
  const io: InstallIO = {
    ...
    linkClaudeSkill: () => { calls.push("linkClaudeSkill"); return { ok: true }; },
...
test("no-ops when already installed and up to date", async () => {
  const { io, calls } = fakeIO({ hash: "H", marker: "H", cli: true, mcp: true, skill: true });
...
test("reinstalls when the source hash changed", async () => {
  const { io, calls } = fakeIO({ hash: "H2", marker: "H1", cli: true, mcp: true, skill: true });
  const r = await ensureBismuthInstalled("/src", io);
  expect(r.action).toBe("updated");
  expect(calls).toEqual(["installFiles", "linkCli", "linkClaudeSkill", "registerMcp", "writeMarker"]);
});
...
test("reinstalls when marker matches but the cli symlink is missing", async () => {
  const { io, calls } = fakeIO({ hash: "H", marker: "H", cli: false, mcp: true, skill: true });
  const r = await ensureBismuthInstalled("/src", io);
  expect(r.action).toBe("updated");
...
test("reinstalls when marker matches but the Claude Code skill link is missing", async () => {
  const { io, calls } = fakeIO({ hash: "H", marker: "H", cli: true, mcp: true, skill: false });
  const r = await ensureBismuthInstalled("/src", io);
  expect(r.action).toBe("updated");
...
test("getBismuthStatus reflects marker + link + skill + mcp", async () => {
  const s = await getBismuthStatus(fakeIO({ marker: "H", cli: true, mcp: true, skill: true }).io);
  expect(s).toMatchObject({ installed: true, version: "H", cliLinked: true, skillLinked: true, mcpRegistered: true });
});
...
function withTempDirs<T>(fn: (bismuthHome: string, claudeSkillsDir: string, src: string) => T): T {
...
test("skills are staged alongside docs (real fs, temp home)", () => {
  withTempDirs((bismuthHome, _claudeSkillsDir, src) => {
    ...
    expect(existsSync(join(bismuthHome, "skills", SKILL_ID, "SKILL.md"))).toBe(true);
    expect(existsSync(join(bismuthHome, "skills", SKILL_ID, "references", "table.md"))).toBe(true);
  });
});
...
test("the Claude Code skill entry is created as a symlink into the staged skill", () => {
  withTempDirs((bismuthHome, claudeSkillsDir, _src) => {
    ...
    expect(isSkillLinkedToClaudeCode(bismuthHome, claudeSkillsDir)).toBe(false); // not linked yet
    ...
    expect(readlinkSync(linkPath)).toBe(join(bismuthHome, "skills", SKILL_ID));
    // Followed through the symlink, the real content is there.
    expect(existsSync(join(linkPath, "SKILL.md"))).toBe(true);
    expect(isSkillLinkedToClaudeCode(bismuthHome, claudeSkillsDir)).toBe(true);
  });
});
...
test("a pre-existing non-Bismuth Claude Code skill entry is not overwritten and produces a warning", () => {
  withTempDirs((bismuthHome, claudeSkillsDir, _src) => {
    ...
    expect(readFileSync(join(foreignPath, "SKILL.md"), "utf8")).toBe("# Someone else's skill\n");
    expect(isSkillLinkedToClaudeCode(bismuthHome, claudeSkillsDir)).toBe(false);
  });
});
...
test("a foreign symlink pointing elsewhere is also treated as not ours and left alone", () => {
  withTempDirs((bismuthHome, claudeSkillsDir, _src) => {
    ...
    mkdirSync(claudeSkillsDir, { recursive: true });
    const elsewhere = mkdtempSync(join(tmpdir(), "bismuth-install-elsewhere-"));
    try {
      symlinkSync(elsewhere, join(claudeSkillsDir, SKILL_ID));
      const r = linkSkillToClaudeCode(bismuthHome, claudeSkillsDir);
      expect(r.ok).toBe(false);
      expect(r.warning).toContain("wasn't created by Bismuth");
      expect(readlinkSync(join(claudeSkillsDir, SKILL_ID))).toBe(elsewhere);
    } finally {
