...
import { mkdtempSync, writeFileSync, existsSync, chmodSync, symlinkSync, mkdirSync } from "node:fs";
...
test("shimSpecsFor produces claude's spec matching today's behavior (mode hooks, resolved path passed through)", () => {
  ...
  expect(specs).toEqual([{ id: "claude", binary: "claude", realPath: "/usr/local/bin/claude", mode: "hooks" }]);
});
...
test("shimSpecsFor keeps an unresolved backend's spec entry (realPath: null) rather than dropping it", () => {
  ...
  const specs = shimSpecsFor([CLAUDE_CANDIDATE], () => null, { wrapperReportingEnabled: false });
  expect(specs).toEqual([{ id: "claude", binary: "claude", realPath: null, mode: "hooks" }]);
});

test("shimSpecsFor omits a 'none' backend entirely, regardless of resolvability", () => {
  const specs = shimSpecsFor([OPENCODE_NONE_CANDIDATE], () => "/usr/local/bin/opencode", { wrapperReportingEnabled: true });
...
test("shimSpecsFor: wrapper-vs-hooks distinction — a 'wrapper' backend is included only when wrapperReportingEnabled", () => {
  ...
  expect(shimSpecsFor([WRAPPER_CANDIDATE], resolve, { wrapperReportingEnabled: false })).toEqual([]);
  expect(shimSpecsFor([WRAPPER_CANDIDATE], resolve, { wrapperReportingEnabled: true })).toEqual([
    { id: "opencode", binary: "opencode", realPath: "/usr/local/bin/opencode", mode: "wrapper" },
  ]);
});
...
test("shimSpecsFor handles a mixed list: resolved hooks + resolved wrapper + unresolved wrapper + none, in order", () => {
  ...
  expect(specs).toEqual([
    { id: "claude", binary: "claude", realPath: "/usr/local/bin/claude", mode: "hooks" },
    { id: "goose", binary: "goose", realPath: "/usr/local/bin/goose", mode: "wrapper" },
...
test("serializeShimSpecs round-trips through the ASCII US/RS delimiters", () => {
  ...
  expect(wire).toBe(
    ["claude", "claude", "/usr/local/bin/claude", "hooks"].join(SHIM_FIELD_SEP) +
      SHIM_RECORD_SEP +
  ...
  const records = wire.split(SHIM_RECORD_SEP).map((r) => r.split(SHIM_FIELD_SEP));
...
test("buildPtyEnv sets BISMUTH_SHIM_SPECS from shimSpecs when present, omits it when absent/empty", () => {
  const specs: ShimSpec[] = [{ id: "claude", binary: "claude", realPath: "/usr/local/bin/claude", mode: "hooks" }];
  const withSpecs = buildPtyEnv({ ...ENV_BASE, realClaude: null, shimSpecs: specs });
  ...
  const emptySpecs = buildPtyEnv({ ...ENV_BASE, realClaude: null, shimSpecs: [] });
...
test("buildPtyEnv never sets BISMUTH_SHIM_SPECS when the shim is unavailable, even with specs given", () => {
  const specs: ShimSpec[] = [{ id: "claude", binary: "claude", realPath: "/usr/local/bin/claude", mode: "hooks" }];
  const env = buildPtyEnv({ ...ENV_BASE, shimAvailable: false, realClaude: null, shimSpecs: specs });
...
test("buildPtyEnv prepends wrapperShimDir to PATH only when shimSpecs contains a resolved wrapper entry", () => {
  const wrapperSpec: ShimSpec = { id: "opencode", binary: "opencode", realPath: "/usr/local/bin/opencode", mode: "wrapper" };
  ...
  const hooksOnly = buildPtyEnv({
    ...ENV_BASE,
    realClaude: null,
    shimSpecs: [{ id: "claude", binary: "claude", realPath: "/usr/local/bin/claude", mode: "hooks" }],
    wrapperShimDir: "/tmp/bismuth-agent-shim-xyz",
  });
  ...
  const unresolvedWrapper = buildPtyEnv({
    ...ENV_BASE,
    realClaude: null,
    shimSpecs: [{ id: "opencode", binary: "opencode", realPath: null, mode: "wrapper" }],
    wrapperShimDir: "/tmp/bismuth-agent-shim-xyz",
  });
  ...
  expect(both.PATH).toBe("/tmp/bismuth-agent-shim-xyz:/repo/relay/shim:/usr/bin");
});
...
const SHIM_ZDOTDIR2 = join(import.meta.dir, "..", "..", "relay", "shim", "zdotdir");
...
test.if(HAS_ZSH2 && existsSync(SHIM_ZDOTDIR2))(
  ...
  async () => {
    ...
    writeFileSync(join(wrapDir, "wrap.ts"), `console.log("WRAP_ARGV:" + Bun.argv.slice(2).join("|"));\n`);

    const specs: ShimSpec[] = [
      { id: "claude", binary: "claude", realPath: claudeStub, mode: "hooks" },
      { id: "goose", binary: "goose", realPath: gooseStub, mode: "wrapper" },
...
test.if(HAS_ZSH2 && existsSync(SHIM_ZDOTDIR2))(
  ...
  async () => {
    ...
    const specs: ShimSpec[] = [{ id: "goose", binary: "goose", realPath: null, mode: "hooks" }];

    const proc = Bun.spawn(["/bin/zsh", ...loginShellArgs(), "-i", "-c", "goose hi"], {
      env: {
        HOME: home,
        ZDOTDIR: SHIM_ZDOTDIR2,
...
const AGENT_SHIM_SCRIPT_PATH = join(import.meta.dir, "..", "..", "relay", "shim", "agent-shim");
...
test.if(existsSync(AGENT_SHIM_SCRIPT_PATH))(
  ...
  async () => {
    ...
    const specs: ShimSpec[] = [{ id: "goose", binary: "goose", realPath: gooseStub, mode: "wrapper" }];
