...
function readRows(path: string): Array<Record<string, unknown>> {
    const name = path.split('/').pop() ?? path
    return parseBaseFile(readFileSync(join(vault, path), 'utf8'), {
        name,
        path,
    }).rows.map(r => r.note)
}
    ...
    install() {
        globalThis.fetch = (async (input: any, init: any) => {
            ...
            if (method === 'GET' && !id) {
                ...
                return json({
                    items: evs.map(({ _v, ...e }) => e),
                    nextSyncToken: `v${this.version}`,
                })
            }
            if (method === 'GET' && id) {
            ...
            if (method === 'PATCH' && id) {
                ...
                if (ifMatch && cur && ifMatch !== cur.etag)
                    return new Response('pre', { status: 412 })
                const ev = {
                    ...cur,
...
test('discovery lists each sync-enabled base with its own calendarId (flat + nested keys)', async () => {
    ...
    expect(
        targets.sort((a, b) => a.basePath.localeCompare(b.basePath)),
    ).toEqual([
...
test('discovery honors the legacy global mapping as a migration fallback', async () => {
    ...
    expect(targets).toEqual([
        { basePath: 'Legacy.md', calendarId: 'legacy-cal' },
    ])
})
