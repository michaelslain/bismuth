...
test('timed event → naive local date + HH:MM taken verbatim from the dateTime string', () => {
    const m = fromGoogle({
        ...
        description: 'daily',
        start: {
            dateTime: '2026-06-24T09:30:00-04:00',
            timeZone: 'America/New_York',
        },
        end: {
            dateTime: '2026-06-24T10:00:00-04:00',
            timeZone: 'America/New_York',
        },
    })
    expect(m).toEqual({
...
test('recurring master is mapped with a parsed recurrence (anchored to its start)', () => {
    const m = fromGoogle({
        ...
        end: { dateTime: '2026-06-24T10:00:00-04:00' },
    })
    expect(m?.date).toBe('2026-06-24')
    expect(m?.startTime).toBe('09:00')
    expect(m?.recurrence).toEqual({
        type: 'weekly',
        daysOfWeek: [3],
...
test('a modified exception instance (recurringEventId) is skipped (null)', () => {
    expect(
        fromGoogle({
            ...
            start: { dateTime: '2026-06-25T09:00:00-04:00' },
        }),
    ).toBeNull()
})
...
test('overnight timed event (end on a later calendar day) is skipped', () => {
    expect(
        fromGoogle({
            ...base,
            summary: 'Night',
            start: { dateTime: '2026-06-24T23:00:00-04:00' },
            end: { dateTime: '2026-06-25T01:00:00-04:00' },
        }),
    ).toBeNull()
...
test('missing summary falls back to a placeholder title', () => {
    expect(fromGoogle({ ...base, start: { date: '2026-06-24' } })?.title).toBe(
        '(no title)',
    )
})
...
test('toGoogle: timed event uses dateTime + timeZone; end defaults to start when missing', () => {
    expect(
        toGoogle(
            { title: 'X', date: '2026-06-24', startTime: '09:30' },
            'America/New_York',
        ),
    ).toEqual({
        summary: 'X',
...
test('toGoogle: all-day event uses date with an EXCLUSIVE next-day end', () => {
    expect(
        toGoogle({ title: 'Holiday', date: '2026-07-04' }, 'America/New_York'),
    ).toEqual({
        summary: 'Holiday',
        location: '',
        description: '',
...
test('signature changes iff a synced field changes', () => {
    ...
    expect(
        signature({ title: 'X', date: '2026-06-24', startTime: '09:00' }),
    ).toBe(a)
    expect(
        signature({ title: 'X', date: '2026-06-24', startTime: '10:00' }),
    ).not.toBe(a)
})

test('buildNote mirrors the calendar-base column set with the given id', () => {
    const note = buildNote('bid-1', {
        title: 'X',
        date: '2026-06-24',
        startTime: '08:00',
        endTime: '09:00',
