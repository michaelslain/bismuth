...
test('per-base keys drive the config; default is off/primary', () => {
    expect(resolveGcalConfig(view({}), 'Cal.md')).toEqual({
        enabled: false,
        calendarId: 'primary',
    })
    expect(
    ...
    expect(
        resolveGcalConfig(view({ googleCalendarSync: true }), 'Cal.md'),
    ).toEqual({ enabled: true, calendarId: 'primary' })
})

test('a blank/whitespace per-base id falls back to primary', () => {
    expect(
        resolveGcalConfig(
            view({ googleCalendarSync: true, googleCalendarId: '   ' }),
            'Cal.md',
        ).calendarId,
    ).toBe('primary')
})
...
test('migration: the legacy GLOBAL mapping is honored for the base it named', () => {
    ...
    expect(resolveGcalConfig(view({}), 'Cal.md', legacy)).toEqual({
        enabled: true,
        calendarId: 'legacy-cal',
    })
    // A DIFFERENT base does NOT inherit the legacy mapping.
    expect(resolveGcalConfig(view({}), 'Other.md', legacy)).toEqual({
        enabled: false,
        calendarId: 'primary',
    })
})
...
test('per-base keys override the legacy mapping (migration sticks after re-toggle)', () => {
    ...
    expect(
        resolveGcalConfig(view({ googleCalendarSync: false }), 'Cal.md', legacy)
            .enabled,
    ).toBe(false)
    // A per-base id overrides the legacy calendarId.
    expect(
        resolveGcalConfig(
            view({ googleCalendarSync: true, googleCalendarId: 'new-cal' }),
            'Cal.md',
            legacy,
        ),
...
test('undefined view (no calendar view yet) resolves to off/primary', () => {
    expect(resolveGcalConfig(undefined, 'Cal.md')).toEqual({
        enabled: false,
        calendarId: 'primary',
    })
})
