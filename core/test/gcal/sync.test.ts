...
function readRows(): Array<Record<string, unknown>> {
    return parseBaseFile(readFileSync(join(vault, 'cal.md'), 'utf8'), {
        name: 'cal',
        path: 'cal.md',
    }).rows.map(r => r.note)
}
...
class FakeGoogle {
    ...
    touch(id: string, changes: Record<string, any>) {
        ...
        Object.assign(cur, changes, {
            etag: `etag${this.etagN++}`,
            updated: changes.updated ?? nextUpdated(),
            _v: ++this.version,
        })
        return cur
    }
    install() {
        globalThis.fetch = (async (input: any, init: any) => {
            ...
            if (method === 'POST' && !id) {
                ...
                if (body.id && this.events.has(body.id))
...
const runBase = (
    basePath: string,
    policy: 'lastWriteWins' | 'googleWins' | 'bismuthWins' = 'lastWriteWins',
) =>
    syncEvents({
        vault,
        basePath,
...
test('remote recurrence change is applied locally and not reverted/re-pushed', async () => {
    ...
    const r = await run()
    expect(r.pulledUpdate).toBe(1)
    expect(JSON.parse(String(readRows()[0].recurrence)).daysOfWeek).toEqual([
        1, 3,
    ]) // applied locally
