...
import {
    ...
    recurrenceSignature,
    firstOccurrence,
    type BismuthRecurrence,
} from '../../src/gcal/recurrence'
...
test('buildRRule emits the expected RRULE per frequency', () => {
    ...
    ).toEqual(['RRULE:FREQ=WEEKLY;INTERVAL=2;BYDAY=FR'])
    expect(
        buildRRule(
            { type: 'monthly', startDate: '2026-06-24', seriesId: SID },
            true,
        ),
...
test('buildRRule encodes UNTIL as a DATE for all-day and a UTC DATE-TIME for timed', () => {
    ...
    ).toEqual(['RRULE:FREQ=DAILY;UNTIL=20260704'])
    expect(
        buildRRule(
            {
                type: 'daily',
...
test('buildRRule UNTIL for a timed series is the local end-of-day in UTC (not a naive Z)', () => {
    ...
    expect(buildRRule(rec, false, 'America/Los_Angeles')).toEqual([
        'RRULE:FREQ=WEEKLY;BYDAY=WE;UNTIL=20260705T065959Z',
    ])
    // No / UTC timezone → the historical naive form.
    expect(buildRRule(rec, false, 'UTC')).toEqual([
        'RRULE:FREQ=WEEKLY;BYDAY=WE;UNTIL=20260704T235959Z',
    ])
    // All-day UNTIL stays date-only and tz-independent.
    expect(buildRRule(rec, true, 'America/Los_Angeles')).toEqual([
        'RRULE:FREQ=WEEKLY;BYDAY=WE;UNTIL=20260704',
    ])
})
...
test('parseRRule reads UNTIL from a UTC DATE-TIME back into a date', () => {
    const rec = parseRRule(
        ['RRULE:FREQ=WEEKLY;BYDAY=MO;UNTIL=20260704T235959Z'],
        '2026-06-24',
        SID,
    )
    expect(rec).toEqual({
...
test('parseRRule returns null for unsupported rules', () => {
    ...
    expect(
        parseRRule(['RRULE:FREQ=DAILY;COUNT=10'], '2026-06-24', SID),
    ).toBeNull()
    expect(
        parseRRule(['RRULE:FREQ=WEEKLY;INTERVAL=3'], '2026-06-24', SID),
    ).toBeNull()
    expect(
        parseRRule(
    ...
    expect(
        parseRRule(['RRULE:FREQ=DAILY', 'EXDATE:20260625'], '2026-06-24', SID),
    ).toBeNull()
...
test('toGoogle anchors a malformed (off-day start) recurring event on the first valid weekday', async () => {
    ...
    expect((body.start as { dateTime: string }).dateTime).toBe(
        '2026-06-24T09:30:00',
    ) // the Wed, not the Tue
...
test('recurrenceSignature ignores seriesId and BYDAY order, but reflects content', () => {
    const a = recurrenceSignature({
        type: 'weekly',
        daysOfWeek: [3, 1],
        startDate: '2026-06-24',
        seriesId: 'x',
    })
    const b = recurrenceSignature({
        type: 'weekly',
        daysOfWeek: [1, 3],
        startDate: '2026-06-24',
        seriesId: 'y',
