...
views:
  - type: table
    name: V
`)
    expect(base.declaredProperties).toEqual([
        'status',
        'priority',
        'description',
    ])
    expect(base.properties?.status).toEqual({
...
views:
  - type: table
    name: V
`)
    expect(base.properties?.price?.type).toEqual({
        kind: 'number',
        number: 'currency',
        unit: 'USD',
    })
...
views:
  - type: table
    name: V
`)
    expect(base.properties?.stage?.type).toEqual({
        kind: 'select',
        options: ['todo', 'doing', 'done'],
    })
    expect(base.properties?.labels?.type).toEqual({
        kind: 'multiselect',
        options: ['a', 'b'],
    })
})
...
test('#99: formula carries its expr', () => {
    ...
    expect(base.properties?.ppu?.type).toEqual({
        kind: 'formula',
        expr: 'price / qty',
    })
})
