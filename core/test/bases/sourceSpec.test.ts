...
test('string with inline where', () => {
    expect(normalizeSource('notes where folder == "Keep"', {})).toEqual({
        kind: 'notes',
        where: 'folder == "Keep"',
    })
})
...
test('inline where on the string beats top-level where', () => {
    expect(normalizeSource('tasks where done', { where: 'not done' })).toEqual({
        kind: 'tasks',
        where: 'done',
    })
})
...
test('object form passes through (and prunes undefined)', () => {
    expect(normalizeSource({ kind: 'tasks', from: '[[Keep]]' }, {})).toEqual({
        kind: 'tasks',
        from: '[[Keep]]',
    })
})
...
test('base ref string with top-level ref', () => {
    expect(normalizeSource('base', { ref: '[[X]]' })).toEqual({
        kind: 'base',
        ref: '[[X]]',
    })
})
...
test("nested-array 'from' (unquoted [[X]]) is coerced to a wikilink string", () => {
    expect(normalizeSource('tasks', { from: [['Keep']] })).toEqual({
        kind: 'tasks',
        from: '[[Keep]]',
    })
})
test("nested-array 'ref' (unquoted [[X]]) is coerced", () => {
    expect(normalizeSource('base', { ref: [['My Base']] })).toEqual({
        kind: 'base',
        ref: '[[My Base]]',
    })
})
