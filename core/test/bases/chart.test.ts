...
describe('buildChartData', () => {
    test('sums a numeric value per day and sorts chronologically', () => {
        ...
        const d = buildChartData(
            rows,
            view({ x: 'date', y: 'glasses', aggregate: 'sum', bin: 'day' }),
        )
    ...
    test('average aggregate', () => {
        const rows = [
            row({ date: '2026-05-01', w: 170 }),
            row({ date: '2026-05-01', w: 172 }),
        ]
        const d = buildChartData(
            rows,
            view({ x: 'date', y: 'w', aggregate: 'avg' }),
        )
    ...
    test('month binning groups days into months', () => {
        const rows = [
            row({ date: '2026-05-01', g: 1 }),
            row({ date: '2026-05-28', g: 2 }),
            row({ date: '2026-06-03', g: 4 }),
        ]
        const d = buildChartData(
            rows,
            view({ x: 'date', y: 'g', aggregate: 'sum', bin: 'month' }),
        )
    ...
    test('does not auto-pick a boolean column as the value (uses count)', () => {
        const rows = [
            row({ date: '2026-05-01', done: true }),
            row({ date: '2026-05-01', done: false }),
        ]
        const d = buildChartData(rows, view({}))
    ...
    test("count mode labels as 'count' even with an explicit y", () => {
        const rows = [
            row({ date: '2026-05-01', glasses: 5 }),
            row({ date: '2026-05-01', glasses: 3 }),
        ]
        const d = buildChartData(
            rows,
            view({ x: 'date', y: 'glasses', aggregate: 'count' }),
        )
        expect(d.valueLabel).toBe('count')
        expect(d.points[0].value).toBe(2)
    })
    ...
    test('min and max aggregates', () => {
        const rows = [
            row({ date: '2026-05-01', w: 3 }),
            row({ date: '2026-05-01', w: 9 }),
            row({ date: '2026-05-01', w: 5 }),
        ]
        expect(
            buildChartData(rows, view({ x: 'date', y: 'w', aggregate: 'min' }))
                .points[0].value,
        ).toBe(3)
        expect(
            buildChartData(rows, view({ x: 'date', y: 'w', aggregate: 'max' }))
                .points[0].value,
        ).toBe(9)
    })
    ...
    test('week binning groups days into ISO weeks', () => {
        ...
        const d = buildChartData(
            rows,
            view({ x: 'date', y: 'g', aggregate: 'sum', bin: 'week' }),
        )
