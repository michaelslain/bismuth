...
test('declaredPropertyKeys returns bare names in declaration order, empty when undeclared', () => {
    expect(declaredPropertyKeys(declared)).toEqual([
        'status',
        'priority',
        'done',
        'description',
...
test('parseBasePropertyType: undefined when no type; malformed present-type → text', () => {
    ...
    expect(parseBasePropertyType({ type: 'CHECKBOX' })).toEqual({
        kind: 'boolean',
    }) // case-insensitive alias
})

test('propertyType looks up by exact, bare, and note.-prefixed name', () => {
    expect(propertyType(typed, 'priority')).toEqual({
        kind: 'number',
        number: 'currency',
        unit: 'USD',
    })
    expect(propertyType(typed, 'note.priority')).toEqual({
        kind: 'number',
        number: 'currency',
        unit: 'USD',
    })
...
test('toSchemaType projects each kind onto a schema PropertyType', () => {
    ...
    expect(toSchemaType({ kind: 'select', options: ['a', 'b'] })).toEqual({
        kind: 'enum',
        values: ['a', 'b'],
    })
    expect(toSchemaType({ kind: 'select' })).toBe('string') // no options → free string
    expect(toSchemaType({ kind: 'multiselect', options: ['a', 'b'] })).toEqual({
        kind: 'list',
        item: { kind: 'enum', values: ['a', 'b'] },
    })
    expect(toSchemaType({ kind: 'list' })).toEqual({
        kind: 'list',
        item: 'string',
    })
})
...
test('validatePropertyValue: null when valid, a diagnostic when not', () => {
    ...
    expect(validatePropertyValue({ kind: 'number' }, 'nope')?.severity).toBe(
        'error',
    )
    expect(validatePropertyValue({ kind: 'boolean' }, true)).toBeNull()
    expect(validatePropertyValue({ kind: 'date' }, '2026-07-13')).toBeNull()
    expect(
        validatePropertyValue({ kind: 'date' }, '2026-02-30')?.severity,
    ).toBe('error')
    expect(
        validatePropertyValue({ kind: 'select', options: ['a', 'b'] }, 'a'),
    ).toBeNull()
    expect(
        validatePropertyValue({ kind: 'select', options: ['a', 'b'] }, 'z')
            ?.severity,
    ).toBe('error')
...
test("coercePropertyValue coerces to the kind's runtime shape, tolerant of junk", () => {
    ...
    expect(coercePropertyValue({ kind: 'multiselect' }, 'a, b, c')).toEqual([
        'a',
        'b',
        'c',
    ])
...
test('coercePropertyValue passes an already-array-shaped multiselect / string-shaped select straight through', () => {
    expect(
        coercePropertyValue(
            { kind: 'select', options: ['low', 'medium', 'high'] },
            'medium',
        ),
    ).toBe('medium')
    expect(
        coercePropertyValue(
            { kind: 'multiselect', options: ['bug', 'feature'] },
            ['bug', 'feature'],
        ),
    ).toEqual(['bug', 'feature'])
})
