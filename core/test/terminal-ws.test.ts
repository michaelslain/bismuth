...
function collect(
    ...
    return new Promise((resolve, reject) => {
        let acc = ''
        const t = setTimeout(
            () => reject(new Error(`timeout, got: ${JSON.stringify(acc)}`)),
            timeoutMs,
        )
        ws.onmessage = ev => {
...
t(
    'GET /terminal upgrades to ws and echoes stdin via the PTY',
    async () => {
...
t(
    '0x01 resize frame propagates to the PTY',
    async () => {
        const { vault, memory } = await makeSampleVault()
        const server = createServer({ vault, memory, port: 0 })
        const base = `ws://localhost:${server.port}`
...
t(
    'closing the websocket kills the session after the grace period',
    async () => {
        const { vault, memory } = await makeSampleVault()
        const server = createServer({ vault, memory, port: 0 })
        const base = `ws://localhost:${server.port}`
...
async function openWsTerm(base: string, termId: string): Promise<WebSocket> {
    const ws = new WebSocket(
        `${base}/terminal?cols=80&rows=24&termId=${encodeURIComponent(termId)}`,
    )
    ws.binaryType = 'arraybuffer'
    await new Promise<void>((resolve, reject) => {
        ws.onopen = () => resolve()
...
t(
    'reconnecting with the same termId reattaches to the live shell',
    async () => {
...
t(
    'a clean close (1000) kills immediately; an abnormal close waits the grace window',
    async () => {
...
t(
    'shell exit closes the websocket with code 1000 (so the client closes the tab, not respawns)',
    async () => {
        const { vault, memory } = await makeSampleVault()
        const server = createServer({ vault, memory, port: 0 })
        const base = `ws://localhost:${server.port}`
        try {
            ...
            const code = await new Promise<number>((resolve, reject) => {
                const t = setTimeout(
                    () => reject(new Error('shell did not exit')),
                    5000,
                )
                ws.onclose = e => {
                    clearTimeout(t)
