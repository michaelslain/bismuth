...
test('migrateDaemonState copies a legacy claude-bot brain into the vault, COPY-ONLY + machine-gated', () => {
    ...
    expect(
        readFileSync(join(vaultA, '.daemon', 'memory', 'note.md'), 'utf8'),
    ).toBe('old memory')
    ...
    expect(readFileSync(join(home, '.claude-bot-migrated'), 'utf8')).toBe(
        vaultA,
    )
...
test("migrateDaemonState merges per-file — a pre-created empty .daemon/memory + a seeded cron don't block it", () => {
    ...
    expect(readFileSync(join(vault, '.daemon', 'memory', 'a.md'), 'utf8')).toBe(
        'note a',
    )
    expect(readFileSync(join(vault, '.daemon', 'memory', 'b.md'), 'utf8')).toBe(
        'note b',
    )
    // The user's other legacy cron is brought over...
    expect(existsSync(join(vault, '.daemon', 'crons', 'book-quotes.md'))).toBe(
        true,
    )
    // ...but the already-present (seeded) default is NOT clobbered.
    expect(
        readFileSync(join(vault, '.daemon', 'crons', 'dream.md'), 'utf8'),
    ).toBe('SEEDED dream')
})
...
function readVaultPaths(home: string): string[] {
    return JSON.parse(
        readFileSync(join(home, 'vaults.json'), 'utf8'),
    ) as string[]
}
...
test('vaults.json elements stay plain strings — an older installed daemon binary can still read it', () => {
    ...
    expect(
        written.filter((r): r is string => typeof r === 'string').sort(),
    ).toEqual([vaultA, vaultB].sort())
})
...
test("setCronEnabled flips the cron's enabled frontmatter (claude-bot-readable) both ways", () => {
    ...
    expect(
        daemonSnapshot(home).crons.find(c => c.name === 'dream')?.enabled,
    ).toBe(false)
    // The schedule (and body) survive the edit.
    expect(
        daemonSnapshot(home).crons.find(c => c.name === 'dream')?.schedule,
    ).toBe('0 * * * *')
...
test('setProcessEnabled flips frontmatter AND drops a reconcile trigger named by basename', () => {
    ...
    expect(frontmatterText(home, 'processes', 'engage-loop')).toMatch(
        /^enabled: false$/m,
    )
    ...
    expect(
        daemonSnapshot(home).processes.find(p => p.name === 'engage-loop')
            ?.enabled,
    ).toBe(false)
})
...
test('resolves by frontmatter `name` when it differs from the filename, but keys the trigger by FILENAME', () => {
    ...
    expect(existsSync(join(home, 'crons', '.triggers', 'Pretty Name'))).toBe(
        false,
    )
})
...
test('registerVaultRoot keeps throwaway (temp) vaults out of a persistent registry', () => {
    ...
    try {
        ...
        writeFileSync(
            ...
            JSON.stringify([tempVault, '/nope/never-existed']),
        )
        const realVault = join(
            import.meta.dir,
            `.vaultroot-real-${process.pid}`,
        )
...
test('registerVaultRoot migrates a legacy {path,lastSeenISO} vaults.json back to strings + sidecar', () => {
    ...
    writeFileSync(
        join(home, 'vaults.json'),
        JSON.stringify([
            { path: other, lastSeenISO: stamp },
            { path: opened, lastSeenISO: daysAgo(1) },
        ]),
    )
...
test('registerVaultRoot keeps every entry when there is NO stamp history at all (fresh/wiped sidecar)', () => {
    ...
    expect(Number.isNaN(Date.parse(readVaultsSeen(home)![disabled]))).toBe(
        false,
    )
})
...
test('registerVaultRoot keeps every entry when the sidecar is CORRUPT', () => {
    ...
    expect(() =>
        registerVaultRoot(realHome('corrupt-seen-trigger'), home),
    ).not.toThrow()
    expect(readVaultPaths(home)).toContain(disabled)
...
test('isEphemeralVaultRoot: OS temp + .claude/jobs scratch are throwaway; real paths are not', () => {
    ...
    expect(
        isEphemeralVaultRoot('/Users/u/.claude/jobs/fe1a462d/tmp/verify-vault'),
    ).toBe(true)
    ...
    expect(
        isEphemeralVaultRoot('/Users/u/dev/bismuth/.claude/worktrees/wt/vault'),
    ).toBe(false)
    expect(
        isEphemeralVaultRoot('/Users/u/Documents/library of alexandria'),
    ).toBe(false)
...
test('registerVaultRoot refuses ephemeral roots (temp dir, agent scratch) but registers a real one', () => {
    ...
    const scratch = join(
        realHome('ephemeral-refuse-jobs'),
        '.claude',
        'jobs',
        'fe1a462d',
        'tmp',
...
test("pruning never changes vaults.json's element shape — still a plain array of strings", () => {
    ...
    writeFileSync(
        join(home, 'vaults.json'),
        JSON.stringify([alive, deleted, stray]),
    )
    ...
    expect(written.filter((r): r is string => typeof r === 'string')).toContain(
        alive,
    )
})
...
function makeVaultWithSessionIds(body: string | null): string {
    ...
    if (body !== null)
        writeFileSync(join(vault, '.daemon', 'session-ids'), body)
    return vault
}
...
test('readDaemonSessionIds: a session the daemon never minted is NOT a member', () => {
    const vault = makeVaultWithSessionIds('s1\ns2\n')
    expect(readDaemonSessionIds(vault).has('a-chat-the-user-started')).toBe(
        false,
    )
})
