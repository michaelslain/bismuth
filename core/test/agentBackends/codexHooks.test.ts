...
describe('buildCodexHooksJson (pure)', () => {
    ...
    test('SessionStart matches every Codex session source (startup/resume/clear/compact)', () => {
        expect(json.hooks.SessionStart[0].matcher).toBe(
            'startup|resume|clear|compact',
        )
    })

    test('every command invokes the given script path with the right event argv', () => {
        expect(json.hooks.SessionStart[0].hooks[0].command).toContain(
            'session-start',
        )
        expect(json.hooks.SubagentStart[0].hooks[0].command).toContain(
            'subagent-start',
        )
        expect(json.hooks.SubagentStop[0].hooks[0].command).toContain(
            'subagent-stop',
        )
        ...
        for (const event of Object.keys(json.hooks)) {
            expect(json.hooks[event][0].hooks[0].command).toContain(
                '/vault/.codex/bismuth-relay-hook.ts',
            )
        }
    })
})
...
describe('upsertCodexHooksJson (pure)', () => {
    test('creates a fresh hooks.json when none exists', () => {
        const { text, warning } = upsertCodexHooksJson(
            null,
            '/v/.codex/bismuth-relay-hook.ts',
        )
    ...
    test("preserves an unrelated top-level key and another event's entries", () => {
        ...
        const { text } = upsertCodexHooksJson(
            existing,
            '/v/.codex/bismuth-relay-hook.ts',
        )
        const parsed = JSON.parse(text)
    ...
    test("preserves a user's OWN entry under one of Bismuth's four event names", () => {
        ...
        const parsed = JSON.parse(text)
        const commands = parsed.hooks.SessionStart.flatMap((e: any) =>
            e.hooks.map((h: any) => h.command),
        )
        expect(commands).toContain('echo my-own-hook')
        expect(
            commands.some((c: string) => c.includes(CODEX_HOOK_SCRIPT_NAME)),
        ).toBe(true)
    })

    test("regenerating twice does not duplicate Bismuth's own entry", () => {
        const first = upsertCodexHooksJson(
            null,
            '/v/.codex/bismuth-relay-hook.ts',
        ).text
        const second = upsertCodexHooksJson(
            first,
            '/v/.codex/bismuth-relay-hook.ts',
        ).text
...
describe('writeCodexHooksFiles (effectful, tmp dir)', () => {
    ...
    test('never throws even against an unwritable path', () => {
        expect(() =>
            writeCodexHooksFiles('/nonexistent-\0-vault'),
        ).not.toThrow()
    })
})
