...
describe('agent backend catalog <-> CHAT_BACKENDS parity', () => {
    test('CHAT_BACKENDS implements exactly the ids BACKEND_IDS declares — no missing, no orphaned entries', () => {
        ...
        expect(Object.keys(CHAT_BACKENDS).sort()).toEqual(
            [...BACKEND_IDS].sort(),
        )
    })
    ...
    function disagreements(
        ...
        for (const id of BACKEND_IDS as readonly BackendId[]) {
            ...
            if (!backend) {
                out.push(
                    `${id}: no CHAT_BACKENDS entry at all (see the id-parity test)`,
                )
                continue
            }
            ...
            if (claims !== has) {
                out.push(
                    `${id}: capabilities.${flag}=${claims} but ${verb} is ${has ? 'IMPLEMENTED' : 'NOT implemented'}`,
                )
            }
        }
        return out
