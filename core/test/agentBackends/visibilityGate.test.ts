...
import { resolveVisibilityGate, enforcesFor } from "../../src/agentBackends/visibilityGate";
...
describe("resolveVisibilityGate", () => {
  ...
  test("claude is allowed for a restricted vault — it enforces natively", async () => {
    expect((await resolveVisibilityGate("claude", "chat", vaultWithHidden())).allowed).toBe(true);
  });

  test("a backend that cannot enforce is REFUSED for a restricted vault", async () => {
    const v = await resolveVisibilityGate("cline", "chat", vaultWithHidden());
  ...
  test("every non-enforcing backend is refused — none of the seven may start ungated", async () => {
    ...
    for (const id of ["codex", "cline", "gemini", "goose", "openclaw", "claude-code-acp", "codex-acp"]) {
      const v = await resolveVisibilityGate(id, "chat", root);
      expect(v.allowed).toBe(false);
    }
  });
  ...
  test("an unknown backend id REFUSES rather than failing open", async () => {
    ...
    const v = await resolveVisibilityGate("not-a-backend", "chat", vaultWithHidden());
    expect(v.allowed).toBe(false);
  });
  ...
  test("chat-only restricts the DAEMON channel but not chat", async () => {
    ...
    expect((await resolveVisibilityGate("openclaw", "chat", root)).allowed).toBe(true);
    expect((await resolveVisibilityGate("openclaw", "daemon", root)).allowed).toBe(false);
  });
});
