...
test("patchJsonMcpServerEnv sets only the target entry's env, preserving unrelated keys + other servers", () => {
  const before = JSON.stringify(
    {
      ...
      mcpServers: {
        ...
        bismuth: { command: "/old/bismuth-mcp", args: [], disabled: false },
      },
    },
    null,
    2,
  );
  const { text } = patchJsonMcpServerEnv(before, ["mcpServers"], "bismuth", { BISMUTH_CLI: "/x" });
  ...
  expect(parsed.mcpServers.bismuth).toEqual({ command: "/old/bismuth-mcp", args: [], disabled: false, env: { BISMUTH_CLI: "/x" } });
});

test("patchJsonMcpServerEnv is a no-op signal (null text) when the target entry doesn't exist yet or JSON is unparseable", () => {
  expect(patchJsonMcpServerEnv(JSON.stringify({ mcpServers: {} }), ["mcpServers"], "bismuth", {}).text).toBeNull();
  expect(patchJsonMcpServerEnv("{not valid json", ["mcpServers"], "bismuth", {}).text).toBeNull();
});

test("removeJsonMcpServer removes only OUR entry, leaving a foreign one + unrelated keys untouched", () => {
  const isOurs = (e: unknown) => (e as any)?.command === "/home/.bismuth/bin/bismuth-mcp";
  const before = JSON.stringify({
    unrelated: 1,
    mcpServers: { other: { command: "npx" }, bismuth: { command: "/home/.bismuth/bin/bismuth-mcp" } },
  });
  ...
  const foreignBefore = JSON.stringify({ mcpServers: { bismuth: { command: "/some/other/tool" } } });
  const foreign = removeJsonMcpServer(foreignBefore, ["mcpServers"], "bismuth", isOurs);
...
test("upsertJsonMcpServer creates a fresh entry (and container) when absent, preserving unrelated keys", () => {
  const before = JSON.stringify({ unrelated: { a: 1 }, mcp: { other: { command: "npx" } } });
  const isOurs = (e: unknown) => (e as any)?.command?.startsWith?.("/home/.bismuth");
  const result = upsertJsonMcpServer(before, ["mcp"], "bismuth", { type: "stdio", command: "/home/.bismuth/bin/bismuth-mcp", args: [], env: {} }, isOurs);
...
test("upsertJsonMcpServer refuses to clobber a foreign entry", () => {
  const isOurs = (e: unknown) => (e as any)?.command?.startsWith?.("/home/.bismuth");
  const before = JSON.stringify({ mcp: { bismuth: { command: "/somewhere/else" } } });
  const result = upsertJsonMcpServer(before, ["mcp"], "bismuth", { command: "/home/.bismuth/bin/bismuth-mcp" }, isOurs);
...
const gooseIsOurs = (e: unknown) => (e as any)?.transport?.command?.startsWith?.("/home/.bismuth");
...
test("upsertYamlExtension appends a new entry, preserving unrelated top-level keys, comments, and other extensions", () => {
  ...
  const result = upsertYamlExtension(
    before,
    "bismuth",
    { name: "bismuth", enabled: true, transport: { type: "stdio", command: "/home/.bismuth/bin/bismuth-mcp", args: [] }, env: {} },
    gooseIsOurs,
  );
...
test("upsertYamlExtension creates the extensions: list fresh when absent (including a null/missing file)", () => {
  const result = upsertYamlExtension(null, "bismuth", { name: "bismuth", enabled: true }, gooseIsOurs);
...
test("upsertYamlExtension refuses to clobber a foreign extension entry", () => {
  ...
  const result = upsertYamlExtension(before, "bismuth", { name: "bismuth" }, gooseIsOurs);
  expect(result.text).toBeNull();
  expect(result.warning).toMatch(/wasn't created by Bismuth/);
});
...
test("removeYamlExtension removes only OUR entry, leaving a foreign one + siblings untouched", () => {
  ...
  const foreignBefore = "extensions:\n  - name: bismuth\n    transport:\n      command: /somewhere/else\n";
...
function makeIO(opts: { which?: (bin: string) => string | null; run?: RegistrarIO["run"] } = {}): RegistrarIO {
  return {
...
test("MCP_REGISTRARS (the real, default-IO singleton list) includes every CLI this build knows", () => {
  ...
  expect(ids).toEqual(["amp", "cline", "codex", "copilot", "crush", "droid", "gemini", "goose", "openclaw", "qwen"].sort());
});
...
test("OpenClaw writes under mcp.servers.bismuth, never top-level mcpServers", async () => {
  ...
  defaultRegistrarIO.writeFile(configPath, JSON.stringify({ mcp: { servers: { bismuth: { command: specFor(home).mcpBin } } } }));
...
const jsonForeignCases: Array<{
  ...
  { make: createClineRegistrar, configRelPath: [".cline", "data", "settings", "cline_mcp_settings.json"], keyPath: ["mcpServers"] },
  { make: createOpenClawRegistrar, configRelPath: [".openclaw", "openclaw.json"], keyPath: ["mcp", "servers"] },
  { make: createGeminiRegistrar, configRelPath: [".gemini", "settings.json"], keyPath: ["mcpServers"] },
  { make: createQwenRegistrar, configRelPath: [".qwen", "settings.json"], keyPath: ["mcpServers"] },
  { make: createCopilotRegistrar, configRelPath: [".copilot", "mcp-config.json"], keyPath: ["mcpServers"] },
  { make: createAmpRegistrar, configRelPath: [".config", "amp", "settings.json"], keyPath: ["amp.mcpServers"] },
  { make: createDroidRegistrar, configRelPath: [".factory", "mcp.json"], keyPath: ["mcpServers"] },
];
...
test("Crush: registers by writing mcp.bismuth directly (no CLI add subcommand exists), idempotently", async () => {
  ...
  expect(JSON.parse(afterFirst).mcp.bismuth).toEqual({ type: "stdio", command: spec.mcpBin, args: [], env: expect.any(Object) });
...
test("Crush: a foreign mcp.bismuth entry survives register() untouched", async () => {
  const configPath = join(home, ".config", "crush", "crush.json");
  const before = JSON.stringify({ mcp: { bismuth: { command: "/foreign" }, other: { command: "npx" } } });
  defaultRegistrarIO.writeFile(configPath, before);
...
test("Goose: registers by writing extensions[] directly (no non-interactive add exists), idempotently, preserving other keys", async () => {
  ...
  defaultRegistrarIO.writeFile(configPath, "provider: anthropic\nmodel: claude\nextensions:\n  - name: other\n    enabled: true\n");
  const io = makeIO();
...
test("unregister() is a no-op (never spawns the CLI) when we never registered in the first place", async () => {
  for (const factory of [createCodexRegistrar, createOpenClawRegistrar, createGeminiRegistrar]) {
    runCalls = [];
...
test("Codex: register()/unregister() round-trip is ledger-gated (TOML itself is never read or written)", async () => {
  ...
  const addCall = runCalls.find((c) => c.args[0] === "mcp" && c.args[1] === "add");
  ...
  runCalls = [];
  await registrar.unregister();
  const removeCall = runCalls.find((c) => c.args[0] === "mcp" && c.args[1] === "remove");
...
test("Cline: register() patches only the env block onto the entry the CLI (simulated) just created", async () => {
  ...
  const io = makeIO({
    run: async (bin, args) => {
      ...
      defaultRegistrarIO.writeFile(configPath, JSON.stringify({ mcpServers: { bismuth: { command: specFor(home).mcpBin, args: [] } } }));
      return { code: 0, stdout: "", stderr: "" };
    },
  });
  ...
  expect(written.mcpServers.bismuth.env).toEqual({ BISMUTH_CLI: spec.cliBin, BISMUTH_DOCS_DIR: spec.docsDir });
  const addCall = runCalls[0];
  expect(addCall.args).toEqual(["mcp", "add", "bismuth", "--transport", "stdio", "--yes", "--", spec.mcpBin]);
...
test("Gemini's add call carries -e env flags directly; Qwen's does not (two-step patch instead)", async () => {
  ...
  expect(geminiArgs).toEqual(["mcp", "add", "bismuth", spec.mcpBin, "-e", `BISMUTH_CLI=${spec.cliBin}`, "-e", `BISMUTH_DOCS_DIR=${spec.docsDir}`, "--scope", "user"]);
  ...
  const qwenIo = makeIO({
    run: async (bin, args) => {
      runCalls.push({ bin, args });
      defaultRegistrarIO.writeFile(qwenConfigPath, JSON.stringify({ mcpServers: { bismuth: { command: spec.mcpBin } } }));
      return { code: 0, stdout: "", stderr: "" };
    },
  });
  ...
  expect(qwenArgs).toEqual(["mcp", "add", "bismuth", spec.mcpBin, "--scope", "user"]);
  const qwenWritten = JSON.parse(readFileSync(qwenConfigPath, "utf8"));
  expect(qwenWritten.mcpServers.bismuth.env).toEqual({ BISMUTH_CLI: spec.cliBin, BISMUTH_DOCS_DIR: spec.docsDir });
});
...
test("Copilot's add call: --env repeated, then -- <mcpBin>", async () => {
  ...
  expect(runCalls[0].args).toEqual(["mcp", "add", "bismuth", "--env", `BISMUTH_CLI=${spec.cliBin}`, "--env", `BISMUTH_DOCS_DIR=${spec.docsDir}`, "--", spec.mcpBin]);
});
...
test("Amp's add call: --env repeated, then -- <mcpBin>, against the LITERAL dotted key amp.mcpServers", async () => {
  ...
  defaultRegistrarIO.writeFile(configPath, JSON.stringify({ "amp.mcpServers": { bismuth: { command: spec.mcpBin } } }));
  expect(await registrar.isRegistered()).toBe(true);
  const raw = JSON.parse(readFileSync(configPath, "utf8"));
...
test("Droid's add call: the command is a single positional string, not a `-- cmd` split", async () => {
  ...
  expect(runCalls[0].args).toEqual(["mcp", "add", "bismuth", spec.mcpBin, "--env", `BISMUTH_CLI=${spec.cliBin}`, "--env", `BISMUTH_DOCS_DIR=${spec.docsDir}`]);
