...
describe("buildSeatbeltProfile", () => {
  test("permissive-except header, one subpath deny per path", () => {
    const profile = buildSeatbeltProfile(["/vault/secret.md", "/vault/.git"]);
    expect(profile).toContain("(version 1)");
    expect(profile).toContain("(allow default)");
    expect(profile).toContain('(deny file-read* (subpath "/vault/.git"))');
    expect(profile).toContain('(deny file-read* (subpath "/vault/secret.md"))');
  });
  ...
  test("deduplicates and sorts — same logical set is byte-identical regardless of input order", () => {
    const a = buildSeatbeltProfile(["/vault/b.md", "/vault/a.md", "/vault/b.md"]);
    const b = buildSeatbeltProfile(["/vault/a.md", "/vault/b.md"]);
    expect(a).toBe(b);
  });

  test("escapes double quotes and backslashes in a path", () => {
    const profile = buildSeatbeltProfile(['/vault/weird "quoted" \\ name.md']);
    expect(profile).toContain('(deny file-read* (subpath "/vault/weird \\"quoted\\" \\\\ name.md"))');
  });

  test("handles a path with spaces and non-ASCII characters verbatim (no extra escaping needed)", () => {
    const profile = buildSeatbeltProfile(["/vault/naïve folder/secret été.md"]);
    expect(profile).toContain('(subpath "/vault/naïve folder/secret été.md")');
  });
});

describe("wrapArgv", () => {
  test("returns argv UNCHANGED when profilePath is null", () => {
    expect(wrapArgv(["opencode", "run", "hi"], null)).toEqual(["opencode", "run", "hi"]);
  });

  test("prefixes sandbox-exec -f <profile> when a profile path is given", () => {
    expect(wrapArgv(["opencode", "run", "hi"], "/vault/.daemon/tmp/visibility-abc123.sb")).toEqual([
  ...
  test("honors a custom sandbox-exec path (test seam)", () => {
    expect(wrapArgv(["x"], "/p.sb", "/custom/sandbox-exec")).toEqual(["/custom/sandbox-exec", "-f", "/p.sb", "x"]);
  });
...
describe("checkSandboxWrapperAvailability / sandboxWrapperAvailable", () => {
  test("P1: unavailable on a non-darwin platform, even with sandbox-exec present", () => {
    const r = checkSandboxWrapperAvailability({ platform: "linux", sandboxExecPath: __filename });
    expect(r).toEqual({ available: false, reason: "unsupported-platform" });
    expect(sandboxWrapperAvailable({ platform: "linux", sandboxExecPath: __filename })).toBe(false);
  });

  test("P2: unavailable when the backend self-sandboxes, even on darwin with sandbox-exec present", () => {
    const r = checkSandboxWrapperAvailability({ platform: "darwin", sandboxExecPath: __filename, selfSandboxes: true });
    expect(r).toEqual({ available: false, reason: "backend-self-sandboxes" });
  });

  test("P1: unavailable when sandbox-exec is missing on darwin", () => {
    const r = checkSandboxWrapperAvailability({ platform: "darwin", sandboxExecPath: "/does/not/exist/sandbox-exec" });
  ...
  test("available: darwin + sandbox-exec present + not self-sandboxing", () => {
    const r = checkSandboxWrapperAvailability({ platform: "darwin", sandboxExecPath: __filename, selfSandboxes: false });
    expect(r).toEqual({ available: true });
    expect(sandboxWrapperAvailable({ platform: "darwin", sandboxExecPath: __filename })).toBe(true);
  });
...
describe("describeSandboxWrapperUnavailable", () => {
  test("names a mechanism only for the specific reason given — never overclaims", () => {
    expect(describeSandboxWrapperUnavailable("unsupported-platform")).toMatch(/macOS/);
    expect(describeSandboxWrapperUnavailable("sandbox-exec-missing")).toMatch(/sandbox-exec/);
    expect(describeSandboxWrapperUnavailable("backend-self-sandboxes")).toMatch(/own OS sandbox/);
  });
});

describe("isSandboxApplyFailure", () => {
  test("true only for exit 71, false for null/undefined/other codes", () => {
    expect(isSandboxApplyFailure(SANDBOX_APPLY_FAILURE_EXIT_CODE)).toBe(true);
...
describe("materializeSandboxProfile", () => {
  ...
  test("writes a 0600 profile file under <vault>/.daemon/tmp and returns its path", async () => {
    const vault = mkdtempSync(join(tmpdir(), "bismuth-sandboxwrapper-"));
    const path = await materializeSandboxProfile(vault, ["/vault/secret.md"]);
    expect(path).not.toBeNull();
    expect(path as string).toMatch(/\.daemon\/tmp\/visibility-[0-9a-f]{16}\.sb$/);
    const content = readFileSync(path as string, "utf-8");
    expect(content).toContain('(deny file-read* (subpath "/vault/secret.md"))');
  ...
  test("content-addressed: the SAME deny set reuses the same file path across calls", async () => {
    const vault = mkdtempSync(join(tmpdir(), "bismuth-sandboxwrapper-"));
    const a = await materializeSandboxProfile(vault, ["/vault/secret.md", "/vault/other.md"]);
    const b = await materializeSandboxProfile(vault, ["/vault/other.md", "/vault/secret.md"]); // different order
    expect(a).toBe(b);
  });
