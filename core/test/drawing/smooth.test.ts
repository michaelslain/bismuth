...
test('endpoints are kept exact and the curve is densified', () => {
    const pts = [
        0, 0, 255, 30, 24, 255, 12, 48, 255, 48, 66, 255, 24, 90, 255, 60, 108,
        255,
    ]
...
test('a jittery path becomes far less kinky (smooth, not geometric)', () => {
    ...
    for (let i = 0; i <= 120; i++)
        pts.push(
            i * 6 + rnd() * 8,
            250 + Math.sin(i / 12) * 90 + rnd() * 8,
            255,
        )

    const before = maxTurn(pts),
        after = maxTurn(smoothStrokePoints(pts))
...
test('a near-straight stroke stays straight (gesture preserved, no overshoot)', () => {
    ...
    for (let i = 0; i + 2 < out.length; i += 3)
        expect(Math.abs(out[i] - out[i + 1])).toBeLessThan(1.5)
})

test('coincident / duplicate points produce no NaN', () => {
    const pts = [
        10, 10, 255, 10, 10, 255, 10, 10, 255, 40, 40, 255, 40, 40, 255, 80, 20,
        255,
    ]
    const out = smoothStrokePoints(pts)
...
test('strokes with fewer than 3 points are returned unchanged', () => {
    ...
    expect(smoothStrokePoints([0, 0, 255, 10, 10, 255])).toEqual([
        0, 0, 255, 10, 10, 255,
    ])
})
