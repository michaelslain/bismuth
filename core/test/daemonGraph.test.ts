...
beforeAll(() => {
    ...
    writeFileSync(
        join(home, 'processes', 'my-proc.md'),
        `---\ncommand: /bin/true\n---\n\nproc\n`,
    )
})
...
test("#51: daemonSnapshot reads a file-change cron's on/watch, and defaults every other cron to on:schedule/watch:null", () => {
    ...
    expect(by['on-edit']).toMatchObject({
        schedule: '',
        on: 'file-change',
        watch: 'inbox.md',
    })
...
test('daemonSnapshot merges schedule, enabled, last-fired, and running state', () => {
    ...
    expect(by['failed-cron'].lastFired).toEqual({
        timestamp: RECENT,
        result: 'failed',
    })
...
test('buildDaemonGraph: one hub + a node per cron/process, all edges from the hub, NO you-node', () => {
    ...
    expect(success.daemon).toMatchObject({
        enabled: true,
        running: false,
        lastResult: 'success',
    })
    ...
    expect(running.daemon).toMatchObject({
        enabled: true,
        running: true,
        lastResult: null,
        lastFiredMs: null,
...
test("buildDaemonGraph: a 'skipped' incremental-cron result surfaces its `detail` verbatim as lastResult", () => {
    ...
    expect(cron.daemon?.lastResult).toBe(
        'skipped: no changes since 2026-07-20T10:00:00Z',
    )
})
...
test('daemonGraph(home) end-to-end: reads the fixture and builds the graph', () => {
    ...
    expect(
        g.edges.every(
            e => e.from === DAEMON_NODE_ID && e.kind === 'supervises',
        ),
    ).toBe(true)
    ...
    expect(onEdit.daemon).toMatchObject({
        on: 'file-change',
        watch: 'inbox.md',
    })
    const success = g.nodes.find(n => n.id === 'cron:success-cron')!
