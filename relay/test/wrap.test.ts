...
test("wrap.ts reports session start/end, forwards SIGINT to the child, and relays its exact exit code", async () => {
  ...
  const proc = Bun.spawn(["bun", "run", WRAP_TS, "goose", stub], {
    env: { ...process.env, CLAUDE_TERMINAL_ID: "test-tab-1", CLAUDE_RELAY_URL: relay.url },
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  expect(startReq.body).toMatchObject({ terminalId: "test-tab-1", backend: "goose" });
  ...
  await waitFor(() => relay.requests.some((r) => r.path === "/relay/session/end"));
  const endReq = relay.requests.find((r) => r.path === "/relay/session/end")!;
  expect((endReq.body as { sessionId?: string }).sessionId).toBe((startReq.body as { sessionId?: string }).sessionId);
...
test("wrap.ts relays a normal (non-signal) exit code and still reports start/end", async () => {
  const dir = tmp();
  const stub = writeStub(dir, "stub-agent", `#!/bin/bash\necho CHILD_RAN\nexit 5\n`);
  const relay = startMockRelay();

  const proc = Bun.spawn(["bun", "run", WRAP_TS, "opencode", stub, "--some-flag"], {
    env: { ...process.env, CLAUDE_TERMINAL_ID: "test-tab-2", CLAUDE_RELAY_URL: relay.url },
    stdout: "pipe",
    stderr: "pipe",
  });
  ...
  expect(relay.requests.map((r) => r.path)).toEqual(["/relay/session", "/relay/session/end"]);
  expect(relay.requests[0].body).toMatchObject({ backend: "opencode", terminalId: "test-tab-2" });
...
test("wrap.ts NEVER reports for backendId 'claude' (guard), even with a terminal id set, but still runs it transparently", async () => {
  const dir = tmp();
  const stub = writeStub(dir, "stub-claude", `#!/bin/bash\necho CHILD_RAN_CLAUDE\nexit 3\n`);
  const relay = startMockRelay();

  const proc = Bun.spawn(["bun", "run", WRAP_TS, "claude", stub], {
    env: { ...process.env, CLAUDE_TERMINAL_ID: "test-tab-3", CLAUDE_RELAY_URL: relay.url },
    stdout: "pipe",
    stderr: "pipe",
  });
  const out = await new Response(proc.stdout).text();
...
test("wrap.ts reports nothing when not launched from a Bismuth terminal tab (no CLAUDE_TERMINAL_ID), but still runs the binary", async () => {
  ...
  const env: Record<string, string | undefined> = { ...process.env, CLAUDE_RELAY_URL: relay.url };
  delete env.CLAUDE_TERMINAL_ID;

  const proc = Bun.spawn(["bun", "run", WRAP_TS, "goose", stub], { env, stdout: "pipe", stderr: "pipe" });
  const out = await new Response(proc.stdout).text();
  const code = await proc.exited;
